...
- キーワード自体は英語のままで構いません (日本語本文との混在 OK)。PR 本文の
  冒頭または末尾の独立した行に置くのが確実です。コードブロック・インライン
  コード (バッククォート囲み)・引用 (`>`) の中に入れるとパースされません。
- **逆に言うと、PR 本文で「このキーワードは入れていません」と説明したいときは
  バッククォートで囲んでください。** 地の文にそのまま書くと、否定文でも
  キーワードとして拾われます (Issue #252 / D126。実際に Issue #247 が
  誤ってクローズされました)。
- **⚠️ コミットメッセージにも同じ規則が効きます。そしてそこでは
  バッククォートは効きません。** GitHub 本体はマージ先ブランチに載った
  コミットメッセージを解析しますが、その解析は Markdown のコードスパンを
  解釈しないため、囲んでも素通りします。**キーワードの字面を説明のために
  書きたいときは、コミットメッセージでは番号と隣接させないでください**
  (例: 「キーワード + 番号」のように分けて書く)。Issue #253 / D128 —
  #252 の対策を入れたコミット自身がこれを踏み、同じ Issue が同じ日に
  2 度目のクローズをされました。
...
- **本リポジトリの PR は基本的に `.github/workflows/auto-merge.yml` が
  ...
  で明示的に閉じます (Issue #168、docs/decisions.md D83)。**ただしこの前提は
  現在失効しています** — auto-merge は `AUTO_MERGE_TOKEN` (PAT) でマージする
  ようになっており (#194 / D91)、PAT による操作は後続処理を誘発するため
  **GitHub 標準の自動クローズが復活しています** (Issue #253 / D128)。
  現在は本体と代行の両方が動いており、**本体のほうが守備範囲が広い**
  (コミットメッセージも読む) 状態です —
