...
///
/// **なぜ必要か** (D93): メモリ予算の既定は D90 の時点では 700 MiB と
/// いう**絶対値**で、搭載 RAM を一切見ていなかった。4 GiB 機でも 64 GiB
/// 機でも同じ値になるため、前者では予算として機能せず、後者では実質 OFF
/// になる (D93「なぜ裸の比率では成立しないか」)。
///
/// **唯一の呼び出し元は `config::Config::from_env_and_args` である。**
/// そこで読んだ値が `config::resolve_suspension` →
/// `browser::suspension::memory_budget_for_ram` に渡り、既定の予算が
/// `clamp(700 MiB, RAM / 16, 2048 MiB)` になる (D114)。予算の式そのものは
/// 純粋な `suspension` 側にあり、環境を読むのはこの関数だけ — 探索と判断
/// を分ける形は D20 の約束そのままである。
///
