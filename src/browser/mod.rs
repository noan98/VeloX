...
//! `history`, `bookmarks`, `benchmark`, `automation`, `suspension` and
//! `session` hold no filesystem or UI dependency. `persistence` and
//! `perf_log` are the exceptions — thin, deliberately "dumb" IO layers
//! (JSON files for `persistence`; stderr/a file for `perf_log`'s
//! [`metrics::PerfRecord`] lines); see their module doc comments.
