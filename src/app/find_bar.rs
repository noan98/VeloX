...
/// search.
pub(super) fn open_find_bar(window: &BrowserWindow, window_id: WindowId, state: &mut AppState) {
...
pub(super) fn close_find_bar(window: &BrowserWindow, window_id: WindowId, state: &mut AppState) {
...
pub(super) fn step_find(
    ...
    show_find_result(window, tab_id, session.total(), active);
}

/// `window_id` のタブ `tab_id` で DOM 検索が終わり `total` 件見つかった
/// (`UserEvent::FindMatchesUpdated`)。
///
/// A session for a *different* tab (the find bar moved on, or closed, while
/// this DOM search was still running) means this result is stale — see that
/// variant's doc comment. Scoped to `window_id`'s own find session
/// (`Windows::find_mut`) — a result meant for one window's find bar can never
/// update another window's, even one whose active tab happens to share this
/// `tab_id` value (Issue #29/D68).
pub(super) fn apply_find_matches(
    window: &BrowserWindow,
    window_id: WindowId,
    state: &mut AppState,
    tab_id: TabId,
        ...
        .filter(|session| session.tab_id() == tab_id)
    {
        ...
        show_find_result(window, tab_id, total, session.active());
    }
}

/// 件数表示 "N/M" を更新し、現在の一致 `active` があればハイライトする。
fn show_find_result(window: &BrowserWindow, tab_id: TabId, total: usize, active: Option<usize>) {
