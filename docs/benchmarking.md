...
| `trials` | 試行回数 | `10` |
| `compare_env` | A/B 比較の B 条件で追加する環境変数 (`KEY=VALUE`、複数はセミコロン区切り)。空欄なら A 条件だけを 1 回計測する。`VELOX_MEMORY_BUDGET_INPUT=resident` (D151 / D152) を含めると、休止スイープの表の「超過 / 要求」も B 側だけワーキングセットで出る | `VELOX_MEMORY_BUDGET_MB=0` |
