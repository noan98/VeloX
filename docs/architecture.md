...
- **Tab strip presentation (#11)**: `TabSummary` also carries `title` and
  ...
  Rust-side network call; see docs/decisions.md D21). The tab strip's CSS
...
- **Keyboard shortcuts (#11)**: Ctrl/Cmd+T/W/Shift+T/Tab/Shift+Tab/1-9 are
  ...
  focus instead. See docs/decisions.md D22 for the two channels' different
...
- **Reopen closed tab (#11)**: `browser::tabs::Tabs` keeps a small, capped,
  ...
  docs/decisions.md D23), fed by `Tabs::close` and drained by
...
- **`target="_blank"`/`window.open()` (#11)**: every content webview's
  ...
  VeloX tab — see docs/decisions.md D24 for the wry-0.56-source-confirmed
