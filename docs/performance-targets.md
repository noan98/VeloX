...
2. ~~**`tabs_hold_50` は未測定。**~~ → **測定済み。§31 を参照。** 50 タブ
   でも予算は守られ (527.9 MiB)、65 MiB/タブの線形は続く。頭打ちは無い。
...
4. ~~**休止による状態喪失のコストは未計測**~~ → **計測基盤ができた
   ...
   ~~**実測はまだこれから**~~ → **実測した (§31)。50 タブでは 48 タブ
   (96%)、20 タブでは 16 タブ (80%) が休止されていた。**
   復帰にかかる時間は別メトリクス `tab_resume_ms` の対象で
...
### 28.9 この節が残した宿題と、その後 (2026-09-11 追記)

§28.7 の残る課題のうち、**2 (`tabs_hold_50` が未測定) は §31 で埋まった** —
50 タブでも予算は守られ (527.9 MiB)、65 MiB/タブの線形も続く。4 (状態喪失の
コスト) は **`suspended_tab_count` の初の実測値** (50 タブで 48 タブ休止) が
出たところまでで、復帰時間は依然として未計測である。判断は D109 を参照。

§28.8 が 20 タブについて出した結論 (休止はプロセス単位で回収される) は、
§31 で 50 タブ (57 → 9.8 プロセス) まで広げても変わらなかった。
...
## 31. `tabs_hold_50` — 50 タブでの予算の効きと、同一機種での再現性 (Issue #197 / #211, 2026-09-11)

§28 (D97) は `tabs_hold_1/5/10/20` までを測り、**`tabs_hold_50` を未測定の
まま残した** (D97 Revisit condition (2))。「65 MiB/タブの線形が続くか、予算が
どこで頭打ちになるか」が open だった。

**D97 Revisit (1) (休止で WebView2 のプロセスが減るか) は、本節ではなく
§28.8 (run 34247895418) で既に決着している** — 20 タブで 27 → 11 プロセス。
本節はそれを 50 タブまで広げて確かめる位置づけであり、**新しい結論を出すのは
`tabs_hold_50` の側だけである。**

一方、`suspended_tab_count` (D105) は表に出す実装はできていたが
**実際の値は 1 度も取られていなかった** (§28.7 の残る課題 4)。本節がその初の
実測値になる。
...
| 項目 | 値 |
| --- | --- |
| run | [34627143113](https://github.com/noan98/VeloX/actions/runs/34627143113) |
| 対象 | `main` (`7032f65`) |
| シナリオ | `tabs_hold_20,tabs_hold_50` (`scenarios` 入力で同一 run に収める) |
| trials / repeats | 2 / 2 (A→B→A→B) |
| `compare_env` | `VELOX_MEMORY_BUDGET_MB=0` |
| `common_env` | **空** — 既定のメモリチェック周期 5 秒のまま (§28 と同条件) |
| ランナー | **AMD EPYC 7763 64-Core / 4 論理コア / RAM 15.99 GiB** |

**20 タブを同じ run に入れたのは、50 タブの値を読むための足場を同一 run 内に
用意するためである。** D96 決定 3 のとおり、run をまたいだ Windows の絶対値
比較は成立しない — §28 の 20 タブの値を「今回の 50 タブと並べて読む」ことは
できない。

### 31.2 結果

`rss_total_bytes` の平均 (MiB)、`rss_process_count`、`suspended_tab_count`。

| シナリオ | A (既定 700 MiB) | B (予算 OFF) | 削減 | 比 (B/A) | A プロセス数 | B プロセス数 | A 休止タブ数 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `tabs_hold_20` | **610.0 MiB** | 1596.3 MiB | **61.8%** | 2.617 | 11 | 27 | 16 |
| `tabs_hold_50` | **527.9 MiB** | 3534.9 MiB | **85.1%** | 6.697 | 9.8 | 57 | 48 |

B 側の休止タブ数はどちらも 0 (予算 OFF なので当然であり、**計測が意図どおり
A/B で分かれていることの確認**になる)。

### 31.3 事実 1: 50 タブでも予算は守られる — むしろ深く下へ着地する

`tabs_hold_50` の A は **527.9 MiB**、予算 700 MiB の **172.1 MiB (24.6%) 下**
である。頭打ちして予算を超えることは起きなかった。

**しかも 20 タブ (610.0 MiB) より小さい。** これは §28 で観測した「`tabs_hold_10`
の A (463.0 MiB) が `tabs_hold_5` の A (630.0 MiB) より小さい」のと同じ形であり、
**予算がタブ数ではなくメモリ量で効いていることの再確認**である。タブ数が増える
ほど休止できる対象が増えるため、回収はより深いところまで進む。

### 31.4 事実 2: プロセス単位の回収は 50 タブでも成り立つ (§28.8 の 50 タブへの拡張)

| シナリオ | B (予算 OFF) | A (既定) | 減少 | 出典 |
| --- | ---: | ---: | ---: | --- |
| `tabs_hold_20` | 27 プロセス | 11 プロセス | -59% | §28.8 と本節で一致 |
| `tabs_hold_50` | 57 プロセス | 9.8 プロセス | -83% | 本節 |

**候補 1「休止してもプロセスが解放されない」を否定したのは §28.8 であって、
本節ではない。** 本節が足すのは「それが 50 タブでも成り立つ」ことだけである。

その意味では**再現性の確認としての価値の方が大きい** — 20 タブの A=11 /
B=27 は §28.8 (別 run、同一機種) と**完全に一致した。**

50 タブの側で新しく分かるのは、プロセス構成の読み方が崩れないことである。
§28.8 事実 2 の「基本 7 + タブ 1 個につき 1 プロセス」に当てはめると、
B の 57 = 基本 7 + タブ 50 でぴったり合う。A の 9.8 は「基本 7 + 生存タブ
2〜3」に相当し、**50 タブでも生存タブは数個まで落ちる** (20 タブでの生存
タブ 4 と同程度)。

### 31.5 事実 3: 65 MiB/タブの線形は 50 タブまで続く
...
D97 Revisit (2) の問い「65 MiB/タブの線形が続くか」は **続く**が答えである。
50 タブでも逓減も頭打ちも起きない。§27.4 が「傾きは逓減する」と読んでいたのは
**測定条件 (`tabs_N`) が回収前の途中の値を見ていたため**という §28 の訂正が、
50 タブでも崩れないことの追加確認になる。

### 31.6 事実 4: 同一機種での run 間再現性 — 初のデータ点 (Issue #211 項目3)

今回のランナーは **AMD EPYC 7763 / 4 論理コア / 15.99 GiB** で、§28 の run
(34244194428) と CPU モデル・論理コア数・搭載 RAM が一致する。**`machine_key`
(D106) が一致する 2 本目の run が初めて手に入った。**

`tabs_hold_20` は §28.5 / §28.8 / 本節の 3 つの run で測られており、
**3 本とも同一機種である** (§28.8 の冒頭に「ランナーは §28.5 と同じ
AMD EPYC 7763」とある)。

| run | B (予算 OFF) | A (既定 700 MiB) |
| --- | ---: | ---: |
| 34244194428 (§28.5) | 1602.6 MiB | 616.8 MiB |
| 34247895418 (§28.8) | 1593.3 MiB | 615.4 MiB |
| 34627143113 (本節) | 1596.3 MiB | 610.0 MiB |
| **幅** | **9.3 MiB (0.58%)** | **6.8 MiB (1.11%)** |

**同一機種なら run をまたいでも 20 タブでは 1% 前後に収まる。** これは
Issue #211 項目3 (同一機種内の分散の実測) が求めているデータの最初の 3 点で
ある。

⚠️ **ただしこれを「同一機種なら比較してよい」の結論にしてはいけない。**

1. **3 点では分散と呼べない。** しかも 3 本とも `tabs_hold_20` という 1 つの
   シナリオの値で、試行数・ペア数も揃っていない (§28.5 は 3 試行、§28.8 は
   2 試行 1 ペア、本節は 2 試行 2 ペア)。閾値の根拠には全く足りない。
2. **この一致は B 側 (予算 OFF) のものである。** A 側は下記 31.7 のとおり
   50 タブで 8.3% 揺れており、**予算が効いている条件では再現性が明確に低い。**
   「同一機種なら安定」は条件によって成り立ったり成り立たなかったりする。
3. D106 Revisit (3) のとおり、`cpu_model` の文字列一致が物理的な同一性を
   保証するとは限らない (世代・ステッピング・マイクロコード)。

### 31.7 注意: A 側のばらつきは 50 タブで大きい

| シナリオ | A の各 run | ばらつき | B の各 run | ばらつき |
| --- | --- | ---: | --- | ---: |
| `tabs_hold_20` | 611.2 / 608.9 MiB | 0.4% | 1602.4 / 1590.3 MiB | 0.8% |
| `tabs_hold_50` | **505.9 / 549.8 MiB** | **8.3%** | 3541.6 / 3528.1 MiB | 0.4% |

**回収がどこまで進むかは実行ごとに揺れる。** A/B 差が 6.7 倍あるので
31.3〜31.5 の結論は揺るがないが、**A の絶対値を 1 MiB 単位で語ってはいけない。**
「50 タブで約 530 MiB」まで。

### 31.8 残る課題: 96% のタブを休止した結果である

`tabs_hold_50` の A は **50 タブ中 48 タブ (96%) が休止した状態**の値である
(20 タブでは 16/20 = 80%)。527.9 MiB という着地は、**背景タブをほぼ全部捨てた
対価として得られている。**

本 run では復帰コスト (`tab_resume_ms`) を測っていないため、**「48 タブを
休止した状態からユーザが 1 つ戻るのに何秒かかるか」は依然として未計測**
である (D97 Revisit (4) の後半)。数字の上では「50 タブ開いても 528 MiB」だが、
**その状態のブラウザが日常利用に耐えるかは、この数字だけでは言えない。**
これは #176 (Memory Budget Manager) が扱うべき設計課題 — 削減率ではなく
foreground UX を優先するという Phase 4 設計原則 2 — そのものである。

## 32. 休止したタブに戻るコスト (Issue #176 Stage 1, 2026-09-12)

§31 は「Windows で 50 タブ開いても 527.9 MiB」を実測し、同時にそれが
**50 タブ中 48 タブ (96%) を休止した結果**であることも記録した
(§31.8)。D97 Revisit condition (4) が求めていた 2 つのうち「何個休止した
か」はそこで埋まり、**「復帰にどれだけかかるか」**が残っていた。

本節はその答えである。計測には `tabs_hold_resume_N`
(`Scenario::TabCountMemoryResume`、D110) を使う — `mark` までは
`tabs_hold_N` と完全に同一で、その後に最長未使用のタブから順に 4 回
`switch` するシナリオである。

### 32.1 なぜこれを #176 の最初の一手にしたか

#176 が挙げる実装候補 — RAM 相対のメモリ予算 / suspend-resume
hysteresis / priority score / 段階的なタブ状態モデル — は**どれも「今の
休止は深すぎるのか」が分からないと閾値を決められない。** 復帰が十分速い
なら 96% の休止は問題ではなく、遅いなら予算そのものを見直す話になる。

先に実装へ進めば、Epic #73 の設計原則 8 (機能追加より計測を優先) と、
D46/D96 が繰り返し戒めてきた「推測で閾値を決める」を同時に破ることに
なる。**この節の結論は実際に予測を否定した** (32.4) ので、順序を守った
ことには実利があった。
...
| | run A | run B |
| --- | --- | --- |
| run | [34669849332](https://github.com/noan98/VeloX/actions/runs/34669849332) | [34670868774](https://github.com/noan98/VeloX/actions/runs/34670868774) |
| ページ | `minimal.html` (229 バイト) | `dom_heavy.html` (99 KB) |
| **ランナー** | **AMD EPYC 7763** / 4 論理コア / 15.99 GiB | **INTEL XEON PLATINUM 8573C** / 4 論理コア / 15.99 GiB |
| シナリオ | `tabs_hold_resume_20,tabs_hold_resume_50` | 同左 |
| trials / repeats | 2 / 2 | 2 / 2 |
| `compare_env` | `VELOX_MEMORY_BUDGET_MB=0` | 同左 |
| `common_env` | 空 (既定の 5 秒周期) | 同左 |

> ⚠️ **この 2 つの run は機種が違う。** D96 決定 3 のとおり、**両者の
> 絶対値を並べて比較してはならない。** 以下で「ページの重さによる違い」
> として書けるのは**同一 run 内で読める量 (比) だけ**であり、それも
> 32.6 の限界を伴う。ページを揃えた比較を run をまたがずに行う手段は
> この計測の時点では無かった (Issue #231 — **D115 で `pages` 入力として
> 入った**。以後の計測はこの制約を受けない)。

### 32.3 結果

いずれも A = 既定の予算 700 MiB。

#### `minimal.html` (run 34669849332、AMD EPYC 7763)

| メトリクス | `tabs_hold_resume_20` | `tabs_hold_resume_50` |
| --- | ---: | ---: |
| `tab_resume_ms` | **119.4** (121.3 / 117.5) | **124.6** (117.7 / 131.5) |
| `page_load_ms` | 29.4 | 34.5 |
| `rss_total_bytes` (A) | 792.0 MiB | 687.2 MiB |
| `rss_process_count` (A) | 14 | 11 |
| `suspended_tab_count` | 16 | 24 / 48 |

#### `dom_heavy.html` (run 34670868774、Intel Xeon 8573C)

| メトリクス | `tabs_hold_resume_20` | `tabs_hold_resume_50` |
| --- | ---: | ---: |
| `tab_resume_ms` | **111.9** (111.1 / 112.75) | **116.1** (113.15 / 118.95) |
| `page_load_ms` | 75.5 | 87.6 |
| `rss_total_bytes` (A) | 847.8 MiB | 648.2 MiB |
| `rss_total_bytes` (B、予算 OFF) | 2094.3 MiB | **4738.1 MiB** |
| `rss_process_count` (A / B) | 13 / 27 | 10 / 57 |
| `suspended_tab_count` | 17 | 49 |

B (予算 OFF) では休止が起きないため復帰も起きず、`tab_resume_ms` /
`page_load_ms` は**原理的に存在しない**。A/B 比較表ではこれらの行を
片側 `-` で出す (D111)。

### 32.4 事実 1: 復帰の支配項は **webview の作り直し**であって、ページの重さではない

`tab_resume_ms` は「webview を作り直して表示されるまで」で、その後の
ページ再読み込みは別イベント (`page_load`) である
(`metrics::TabLatencyKind::Resume` の定義)。**同一 run 内**で両者の比を
取ると:

| ページ | 20 タブ | 50 タブ |
| --- | ---: | ---: |
| `minimal.html` | 0.25 | 0.28 |
| `dom_heavy.html` | **0.67** | **0.75** |

**99 KB のページでも、再読み込みは webview の作り直しより速いままで
ある。** ページが重くなるほど比は 1 に近づくが、超えない。

**これは事前の予測を否定した。** D111 の背景は「実サイトでは
`page_load` の側が桁違いに重くなり、**支配項が入れ替わる可能性が高い**」
と書いていた (D110 Revisit (1) はその手前で「桁が違いうる」までしか言って
おらず、どちらが支配項になるかは主張していない — **踏み込んだのは
D111 である**)。根拠は開発環境 (Linux/WebKitGTK、GPU 無し) で
の観測で、そこでは **`dom_heavy` の比が 121** だった。

| 開発環境 (Linux/WebKitGTK、GPU 無し) | `tab_resume_ms` | `page_load_ms` | 比 |
| --- | ---: | ---: | ---: |
| `minimal.html` | 3.15 | 10.50 | 3.33 |
| `dom_heavy.html` | 2.95 | 355.80 | **121** |

**ページを重くすると比が 3.33 → 121 へ動いた**ことが「実サイトなら
さらに移る」という予測の根拠だった。Windows では同じ動きが 0.25 → 0.75
に留まる。

Windows/WebView2 では 0.67〜0.75 である。**エンジンも環境も違うので
どちらが速いという話にはならないが、「開発環境の観測を Windows の予測
として使った」ことが誤りだったことは言える。** D46 が「参考環境ですら
ない場所の数字を実力値として扱わない」と戒めてきたのと同じ誤りを、
今回は「予測」という形でやりかけた。

### 32.5 事実 2: メモリ予算はページが重いほど効く (同一 run 内)

`dom_heavy.html` の run で、A (既定 700 MiB) と B (予算 OFF) を比べる:

| シナリオ | A | B | 削減 |
| --- | ---: | ---: | ---: |
| `tabs_hold_resume_20` | 847.8 MiB | 2094.3 MiB | 59.5% |
| `tabs_hold_resume_50` | **648.2 MiB** | **4738.1 MiB** | **86.3%** |

**重いページを 50 個開くと、休止が無ければ 4.7 GiB に達する。** 予算 ON
の着地点は 648.2 MiB で、§31 の `minimal.html` (527.9 MiB) と同様に
**ページの重さによらず予算の下に収まっている** (予算はメモリ量で効く
ので、重いページほど早く予算に達し、同じだけ休止する — 休止数も 49 で
`minimal` の 48 とほぼ同じ)。

### 32.6 限界

1. **ページ比較が run をまたいでいる。** 32.4 の比の比較は、2 つの
   メトリクスが CPU 速度に同じ比率で効くことを暗に仮定している。これは
   検証していない。同一 run 内で複数ページを測る手段が要る
   (**Issue #231**)。
   → **その手段は D115 で入った** (`perf-windows.yml` の `pages` 入力)。
   **本節の数値は取り直していない** — 記録は当時の条件のまま残す。
   ページ差を結論にしたいときは、`pages` で測り直すこと。
2. **`dom_heavy.html` は実サイトではない。** ネットワーク遅延・
   サードパーティスクリプト・画像デコードを含まない。99 KB の DOM が
   重いページの代理として妥当かは別の問題である。
3. **サンプルは 8 点** (4 ラウンド × 2 試行)。
4. **状態喪失そのものは数値化していない。** スクロール位置・フォーム
   入力・未保存の JS 状態は復帰しても戻らず、`tab_resume_ms` にも
   `page_load_ms` にも現れない (D105 の「まだ分からないこと」)。
5. `minimal.html` の run では `suspended_tab_count` が 24 / 48 と割れた。
   §31.7 で記録した「A 側は 50 タブで揺れる」と同じ現象である。

### 32.7 #176 への含意

1. **休止の対価を下げたいなら、減らすべきは再読み込みの重さではなく
   休止の発生回数である** (32.4)。VeloX 側で削れるのは後者だけなので、
   これは #176 の実装項目の性格を決める。
2. **メモリ予算の価値は重いページほど大きい** (32.5)。予算を緩める方向
   の変更は、この効きを捨てることになる。
3. したがって **RAM 相対の予算** (16 GiB 機で 700 MiB 固定の妥当性) は
   「復帰が遅いから緩める」ではなく「**搭載 RAM に余裕がある機では
   休止の回数そのものを減らせる**」という形で検討するのが筋である。

## 33. RAM 相対の既定予算 (D114) の実測検証 (Issue #176 / D93 案 C, 2026-09-12)

D114 で既定のメモリ予算を `clamp(700 MiB, 搭載 RAM / 16, 2048 MiB)` に
した。**式が実際に効いていること**を Windows 実機で確認する。

### 33.1 実行条件

| | |
| --- | --- |
| run | [34700469298](https://github.com/noan98/VeloX/actions/runs/34700469298) |
| ブランチ | `claude/phase-4-progress-yspbfv` (PR #234、D114 の実装を含む) |
| ページ | `minimal.html` (229 バイト) |
| **ランナー** | **AMD EPYC 7763** / 4 論理コア / **RAM 15.99 GiB** |
| シナリオ | `tabs_hold_resume_20,tabs_hold_resume_50` |
| trials / repeats | 10 / 2 (**50 タブ側は途中で打ち切り、下記参照**) |
| A (既定) | **新しい RAM 相対の既定 = 15.99 GiB / 16 ≒ 1023 MiB** |
| B (`compare_env`) | `VELOX_MEMORY_BUDGET_MB=700` (= D90 の固定値) |
| `common_env` | 空 (既定の 5 秒周期) |

**A/B は同一 run・同一ビルド・同一ランナー内**なので、D96 が禁じる
「run をまたいだ絶対値の比較」には当たらない。

> ⚠️ **この run は 30 分の `timeout-minutes` に当たって打ち切られた。**
> `trials: 10` は `tabs_hold_resume_N` には過大である (§32 が完走できた
> のは `trials: 2` だったため)。打ち切りは集計ステップより前では**なく**、
> Job Summary と artifact は出力済みなので、下の値は有効な計測結果で
> ある。ただし **`tabs_hold_resume_50` は A/B とも 1 反復しか終わって
> いない** (20 タブ側は 2 反復)。50 タブの行は**データ点 1 つ**として
> 読むこと。
...
| メトリクス | A (1023 MiB) | B (700 MiB) | 差 (B-A) | 比 (B/A) |
| --- | ---: | ---: | ---: | ---: |
| **`suspended_tab_count`** | **12** | **16** | **+4** | **1.333** |
| `rss_total_bytes` | 1072238592 (1022.6 MiB) | 786143232 (749.7 MiB) | -286095360 | 0.733 |
| `rss_process_count` | 18 | 14 | -4 | 0.771 |
| `tab_resume_ms` | 118.12 | 121.08 | 2.95 | 1.025 |
| `page_load_ms` | 29.3 | 32.75 | 3.45 | 1.118 |

#### `tabs_hold_resume_50` (各 1 反復のみ)

| メトリクス | A (1023 MiB) | B (700 MiB) | 差 (B-A) | 比 (B/A) |
| --- | ---: | ---: | ---: | ---: |
| **`suspended_tab_count`** | **44** | **48** | **+4** | **1.091** |
| `rss_total_bytes` | 976660480 (931.4 MiB) | 705431552 (672.8 MiB) | -271228928 | 0.722 |
| `rss_process_count` | 15 | 11 | -4 | 0.733 |
| `tab_resume_ms` | 131.05 | 122.65 | -8.4 | 0.936 |
| `page_load_ms` | 40.95 | 38.3 | -2.65 | 0.935 |

### 33.3 事実 1: 予算を上げた分だけ休止が減る — 主指標が動いた

D112 決定4 が確定させたとおり、**VeloX にできるのは休止の回数を減らす
ことだけ**である。したがって D114 の効果はそこに出なければならない。

**20 タブ・50 タブのどちらでも、休止したタブが 4 つ減った** (20 タブでは
16 → 12 で **25% 減**)。`rss_process_count` も両方でちょうど 4 つ多く、
「4 つ多くのタブが webview を保ったまま生き残った」ことと整合する。

### 33.4 事実 2: 減り方が §31.5 の 65 MiB/タブと一致する

予算の差は 1023 - 700 = **323 MiB**。§31.5 の約 65 MiB/タブで割ると
**約 5 タブ分**である。

実測の `rss_total_bytes` の差は 273.0 MiB (20 タブ) / 258.6 MiB
(50 タブ)。65 MiB × 4 タブ = 260 MiB とほぼ一致し、**休止が 4 つ減った
こととメモリ差の両方が同じ 1 つの係数で説明できる。** 式が意図した
とおりに動いていることの、単なる「数字が変わった」より強い証拠である。

### 33.5 事実 3: 予算は守られている — 上げても青天井にはならない

A の `rss_total_bytes` は 1022.6 MiB (20 タブ) / 931.4 MiB (50 タブ) で、
**予算 1023 MiB のすぐ下に着地している。** B も 749.7 / 672.8 MiB で
予算 700 MiB の近傍である。**予算を上げた分だけメモリが増え、それ以上は
増えない** — 安全網としての性質 (D114 決定3) は保たれている。

なお 20 タブの A が 1022.6 MiB とほぼ予算ちょうどなのに対し 50 タブでは
931.4 MiB と下振れするのは、§31.3 が既に記録した「50 タブでは予算より
深く下へ着地する」傾向 (`ESTIMATED_BYTES_PER_TAB` 単位でまとめて休止
するため) と同じである。

### 33.6 事実 4: 復帰コストは変わらない — 予測どおり

`tab_resume_ms` は A/B で 118.1 / 121.1 (20 タブ)、131.1 / 122.7
(50 タブ)。**予算の値によらず 118〜131 ms の範囲**であり、D112 決定1
の「復帰の支配項は webview の作り直しであり、タブ数にもページの重さにも
依らない」に予算という 3 つ目の軸を加えても成立した。

**これは D114 の前提そのものである。** 1 回あたりのコストが下げられない
以上、回数を減らすしかない — 事実 1 がその回数を実際に減らしている。

### 33.7 限界

1. **搭載 RAM は 15.99 GiB の 1 点のみである。** `windows-latest` は
   機種が変わっても RAM は 15.99 GiB のままで、D93 §25.4 の「搭載 RAM を
   変えられるマシンが 1 台も無い」は解消していない。**したがって本節は
   「16 GiB 機で式が効くこと」の検証であって、比率 1/16 そのものの妥当性
   の検証ではない** (D114 Revisit (1))。
2. **`tabs_hold_resume_50` はデータ点 1 つである** (33.1 の打ち切り)。
   20 タブ側の 2 反復と同じ向き・同じ大きさ (どちらも 4 タブ) の結果で
   あることが唯一の傍証である。
3. **ページは `minimal.html` の 1 種類のみ。** D112 §32.5 は「予算は
   ページが重いほど効く」を示しており、重いページでは休止の減り方は
   これより大きくなると**予想される**が、測っていない。予想を実測として
   扱わない (D46 / D112 Revisit)。
   → **測った (§35)。予想は外れた。** D115 の `pages` で同じ A/B を
   ページ違いで並べたところ、休止の減少は `minimal.html` で 4 タブ
   (-25%)、`dom_heavy.html` で **1 タブ (-6%)** — 重いページほど
   **効かない**。理由は飽和で、重いページでは 700 MiB 側が 19 個中
   17 個を休止してもなお予算に届いておらず、予算を足しても使う余地が
   無い (§35.4、D117)。
   なお §34 も同一 run 内でページ差を測っているが、あちらの A/B は
   「既定と予算 OFF」であって本節の「新既定 1023 MiB と従来の 700 MiB」
   とは**問いが違う** — その違いは実際に結論を分けた (D117 決定1)。
4. `page_load_ms` の差 (20 タブで +3.45ms、50 タブで -2.65ms) は向きが
   逆で、**本節からは何も読み取らない。** 予算とページ読み込みを結ぶ
   機序が無く、反復数も足りない。

## 34. `pages` の検証と、ページ差の初の同一 run 内計測 (Issue #231 / D115, 2026-09-12)

D115 で入れた `pages` 入力 (複数ページを 1 run 内で計測) の動作確認。
**目的は機能の検証であり、本格的な計測ではない** (下記 34.4)。ただし
検証のために回した A/B が、**ページ差を同一 run 内で読める初のデータ**に
なったので、読めることだけを記録する。
...
| | |
| --- | --- |
| run | [34703501485](https://github.com/noan98/VeloX/actions/runs/34703501485) |
| ブランチ | `claude/phase-4-progress-yspbfv` (PR #235) |
| **ページ** | **`minimal.html` (229 バイト) と `dom_heavy.html` (99 KB) — 同一 run 内** |
| シナリオ | `tabs_hold_10,tabs_hold_20` |
| trials / repeats | **2 / 1** (検証目的の最小構成) |
| A (既定) | RAM 相対の既定予算 (D114) |
| B (`compare_env`) | `VELOX_MEMORY_BUDGET_MB=0` (予算 OFF) |

**4 つの値 (2 ページ × 2 シナリオ) がすべて同一ジョブ・同一ビルド・
同一ランナーで採られている。** D96 決定 3 が禁じる run 間比較には
当たらない — それがこの入力を足した理由である。

### 34.2 機能の検証結果

- タブ数スケーリング表が **`— minimal` と `— dom_heavy` の 2 つに
  分かれて**出力された (D115 決定2)。
- 表の行はシナリオ名のみ (`tabs_hold_10` / `tabs_hold_20`) で、
  ページ接尾辞は見出し側にある。
- 並び順は **(ページの入力順, タブ数)**。`minimal` が先、各表の中は
  10 → 20。
- 結果ファイルは 2 ページ分が衝突せずに揃った (artifact に 14 ファイル)。
- ジョブ時間は約 7 分 (計測ステップ)。30 分の `timeout-minutes` に対して
  この構成なら余裕がある。

従来経路 (1 ページ) は `pull_request` の自己テストが既定入力で走り、
ファイル名も挙動も変わっていないことを確認した。

### 34.3 結果

| ページ | シナリオ | A (既定) MiB | B (予算 OFF) MiB | 差 (MiB) | 比 (B/A) | A 休止タブ数 | B 休止タブ数 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `minimal.html` | `tabs_hold_10` | 932.4 | 972.7 | 40.3 | 1.043 | **0** | 0 |
| `minimal.html` | `tabs_hold_20` | 861.1 | 1596.2 | 735.1 | 1.854 | 12 | 0 |
| `dom_heavy.html` | `tabs_hold_10` | 854.7 | 1192.5 | 337.8 | 1.395 | **4** | 0 |
| `dom_heavy.html` | `tabs_hold_20` | 712.0 | 2066.7 | 1354.7 | 2.903 | 16 | 0 |

### 34.4 事実 1: 予算はページが重いほど効く — §32.5 の同一 run 内での確認

§32.5 は「メモリ予算はページが重いほど効く」を示していたが、**その比較は
機種の違う 2 つの run にまたがっていた** (§32.6 限界 1)。同じ傾向が、
**同一 run 内で再現した**:

- 比 (B/A) は 10 タブで 1.043 → 1.395、20 タブで 1.854 → 2.903。
  **どちらのタブ数でも、重いページの方が予算の効き目が大きい。**
- 10 タブでは `minimal.html` は**予算に一度も届かない** (休止 0)。同じ
  10 タブでも `dom_heavy.html` は届き、4 タブ休止している。**「何タブで
  予算が発動するか」はページの重さで変わる。**
- 20 タブでは休止が 12 → 16 に増える。

§32.5 を run をまたがずに確認できたのはこれが初めてである。

### 34.5 限界

1. **trials 2 / repeats 1 の最小構成である。** A/B ペアが 1 組しか無く、
   §31.6 が測ったような run 内のばらつきを分離できない。**傾向の向きは
   読めるが、値を引用してはならない。**
2. **これは §33.7 限界 3 の解消ではない。** §33 が比べたのは「RAM 相対の
   新既定 (1023 MiB) と従来の固定値 (700 MiB)」であり、本節が比べたのは
   「既定と予算 OFF」である。**問いが違う。** 「新既定の優位がページの
   重さでどう変わるか」は依然として測っていない。
3. `dom_heavy.html` は実サイトではない (§32.6 限界 2 と同じ)。
4. A 側の `rss_total_bytes` がタブ数の増加に対して**下がって**いる
   (932.4 → 861.1 / 854.7 → 712.0)。予算が効いて着地点が下がるためで、
   §31.3 が記録した「50 タブでは予算より深く下へ着地する」と同じ現象で
   ある。**タブを増やすとメモリが減る、と読んではならない。**

## 35. RAM 相対の予算はページが重いほど「効かない」 (Issue #176 / D114, 2026-09-12)

§33.7 限界 3 が残していた問い — **「新しい既定 (RAM 相対) の優位は、ページ
が重いとどう変わるか」** — を、D115 の `pages` で同一 run 内に収めて測った。

**予測は外れた。** §33.7 は §32.5 の「予算はページが重いほど効く」から
「重いページでは新既定の効きも大きいはず」と予想していたが、**実測は逆**
だった。
...
| | |
| --- | --- |
| run | [34715143524](https://github.com/noan98/VeloX/actions/runs/34715143524) |
| ページ | **`minimal.html` と `dom_heavy.html` — 同一 run 内** (D115) |
| シナリオ | `tabs_hold_resume_20` |
| trials / repeats | 2 / 2 |
| A (既定) | RAM 相対の新既定 ≒ **1023 MiB** (D114、ランナーは 15.99 GiB) |
| B (`compare_env`) | `VELOX_MEMORY_BUDGET_MB=700` (= D90 の固定値) |

**§33 と同じ A/B を、ページだけ変えて並べたもの**である。§34 の A/B
(既定 vs 予算 OFF) とは**問いが違う**ので混同しないこと。

### 35.2 結果

| ページ | メトリクス | A (1023 MiB) | B (700 MiB) | 差 (B-A) |
| --- | --- | ---: | ---: | ---: |
| `minimal.html` | **`suspended_tab_count`** | **12** | **16** | **+4** |
| | `rss_total_bytes` | 1074840576 (1025.0 MiB) | 781726720 (745.5 MiB) | -279.5 MiB |
| | `rss_process_count` | 18 | 13 | -4 |
| | `tab_resume_ms` | 116.3 | 111.8 | -4.5 |
| `dom_heavy.html` | **`suspended_tab_count`** | **16** | **17** | **+1** |
| | `rss_total_bytes` | 997355520 (951.2 MiB) | 903777280 (861.9 MiB) | -89.2 MiB |
| | `rss_process_count` | 14 | 12 | -1 |
| | `tab_resume_ms` | 113.7 | 112.0 | -1.7 |

> **百分率は本節では出さない。** 「A は B より 25% 少ない」(分母 B) と
> 「B は A より 35% 多い」(分母 A) はどちらも同じ 12 対 16 を指すが、
> 並べると矛盾しているように見える。本文書が答えたいのは**「新しい既定に
> 変えると休止が何割減るか」**なので、以降の百分率は**すべて B (従来の
> 700 MiB) を分母とする** — `minimal.html` で -25%、`dom_heavy.html` で
> -6% である。`velox-bench` の A/B 表が出す「比 (B/A)」(1.354 / 1.062) は
> 分母が A なので、そのままでは別の数字になる。

### 35.3 事実 1: 効き目はページが重いほど**小さい**

休止タブ数の減少は `minimal.html` で **4 タブ (従来比 -25%)**、
`dom_heavy.html` で **1 タブ (同 -6%)**。**予算を 323 MiB 増やして買える
ものが、重いページでは 1/4 になる。**

### 35.4 事実 2: 理由は「重いページでは既にほぼ全部休止している」

`dom_heavy.html` の B を見ると、**19 個の背景タブのうち 17 個を休止しても
なお 861.9 MiB** で、予算 700 MiB に届いていない。A も 16 個休止して
951.2 MiB で、予算 1023 MiB を下回ってはいるが余裕は小さい。

つまり**ポリシーが既に飽和している。** 休止できる背景タブがほとんど残って
いないところに予算を足しても、使える余地が無い。D93 が「500 MiB 以下では
応答が飽和して `max_live_tabs=1` 相当に退化する」と記録した現象と同じ形で
あり、**そこへ到達する経路が「予算を小さくする」ではなく「ページを重く
する」に変わっただけ**である。

`minimal.html` では逆に、A も B も予算のすぐ近く (1025.0 / 745.5 MiB) に
着地できている。両条件とも「予算で止まっている」ので、予算の差 323 MiB が
そのまま効き、§31.5 の約 65 MiB/タブで割った 5 タブ弱 (実測 4 タブ) の差に
なる。

### 35.5 事実 3: 復帰コストはここでも変わらない

`tab_resume_ms` は 4 条件すべてで 111.8〜116.3 ms。D112 決定1 の「復帰の
支配項は webview の作り直しであり、タブ数にもページの重さにも依らない」は、
**予算の大小という軸を足しても、ページを変えても**成立している。

### 35.6 #176 への含意

**RAM 相対の予算は、助けが最も要らない場面で最もよく効く。**

- 軽いページ = メモリ圧が小さい場面 → 休止 25% 減
- 重いページ = メモリ圧が大きい場面 → 休止 6% 減

D114 は「どのマシンでも予算を小さくしない」という制約の下で正しく動いて
おり (§33)、**この結果は D114 を否定しない。** 否定されたのは「これで
重いページの問題も軽くなるだろう」という**期待の方**である。

重いページでの状態喪失を減らしたいなら、予算の値を動かすだけでは届かない。
1 タブあたりの実コストを下げるか (Stage 4 の cache/process policy)、休止の
対象選択を賢くするか (Stage 3 の tab priority)、休止そのものの代わりに
段階的な回収を入れるか (Stage 2 の memory pressure) — **いずれも #176 の
Stage 2 以降がやろうとしていることである。** Stage 1 の結論として、
「予算の調整で到達できる範囲はここまで」という上限が見えた。

### 35.7 限界

1. **1 つの run・1 機種・2 反復。** §31.6 が測った run 内のばらつきと
   同程度の差ではないが (4 タブ vs 1 タブ)、値そのものを引用するには薄い。
2. **20 タブの 1 点のみ。** 50 タブでは `minimal.html` 側も飽和に近づく
   はずで (§31.3 は 50 タブで 44〜48 タブ休止)、そこでは両ページの差が
   縮む可能性がある。**つまり 35.3 は「20 タブでの話」である。**
3. `dom_heavy.html` は実サイトではない (§32.6 限界 2 と同じ)。
4. **搭載 RAM は 15.99 GiB の 1 点のまま** (§33.7 限界 1)。比率 1/16 の
   妥当性は依然として未検証である。

## 36. VeloX 自身のメモリは約 25 MiB — フットプリントの 96〜99% はエンジンである (Issue #176 Stage 1 / D118, 2026-09-12)

Stage 1 の「Rust heap と WebView/engine 由来メモリを明確に分離して**評価**」
に対する実測。D118 で入れた `rss_browser_bytes` / `rss_engine_bytes` の
初計測でもある。
...
| | |
| --- | --- |
| run | [34718717835](https://github.com/noan98/VeloX/actions/runs/34718717835) |
| ページ | `minimal.html` と `dom_heavy.html` — 同一 run 内 (D115) |
| シナリオ | `tabs_hold_20` |
| trials / repeats | 2 / 2 |
| A (既定) | RAM 相対の既定予算 (D114) |
...
| ページ | 条件 | `rss_browser_bytes` | `rss_engine_bytes` | `rss_total_bytes` | browser の割合 |
| --- | --- | ---: | ---: | ---: | ---: |
| `minimal.html` | A (予算 ON) | **25.0 MiB** | 835.2 MiB | 860.2 MiB | **2.9%** |
| | B (予算 OFF) | **26.4 MiB** | 1573.9 MiB | 1601.1 MiB | **1.7%** |
| `dom_heavy.html` | A (予算 ON) | **26.6 MiB** | 689.9 MiB | 716.9 MiB | **3.7%** |
| | B (予算 OFF) | **24.1 MiB** | 2047.6 MiB | 2071.6 MiB | **1.2%** |

> ⚠️ **3 列目は「1 列目 + 2 列目」ではない。** `rss_total_bytes` は独立に
> 計測された値で、足し算の結果ではない。生バイトで突き合わせると
> `minimal A` は完全に一致するが、残り 3 行は **-73 KiB 〜 +787 KiB**
> ずれる。
>
> **丸めのせいではない** (丸めなら符号が負になることも 0.75 MiB ずれる
> こともない)。**各メトリクスの中央値が別々のサンプルから来るから**で
> ある: `velox-bench` はメトリクスごとに独立に中央値を取り、A/B 表は
> それを run 間で平均する。`browser_rss_bytes` の中央値を出したサンプルと
> `engine_rss_bytes` の中央値を出したサンプルは、同じとは限らない。
>
> 起動内訳表が「**差は区間ごとの中央値ではない** (各中央値は別々の試行から
> 来うる)」と注意しているのと**同じ現象**である。
>
> **1 サンプル内では分割は厳密に一致する** — それは実装側の不変条件として
> テストで固定してある (`the_browser_engine_split_always_adds_up_to_the_total`)。
> ずれるのは集計の段階だけであり、**この表の行を足し算の検算に使っては
> ならない。**

### 36.3 事実 1: VeloX 自身は 24〜27 MiB で、**何を変えても動かない**

4 条件すべてで 24.1〜26.6 MiB — **幅 2.5 MiB** に収まっている。この 4 条件は
ページの重さ (229 バイト vs 99 KB) も予算の有無も違い、エンジン側は
689.9〜2047.6 MiB と **3 倍**動いているのに、VeloX 自身は動かない。

**条件間の差は読んではならない。** `rss_browser_bytes` の A/B 差は
+1.5 MiB (minimal) / -2.5 MiB (dom_heavy) で、**どちらも有意差の下限
5 MiB (`MetricKey::min_significant_delta`) を下回る。** 符号が逆なのも
ノイズであることの傍証である。

### 36.4 事実 2: 予算が効いているのは **engine 側だけ**である

D118 決定3 が立てた予想 — 「休止はエンジンのプロセスを落とす操作なので、
予算が効いていれば `rss_engine_bytes` が動き `rss_browser_bytes` はほぼ
動かないはず」— がそのまま成立した。
...
**内訳を出したことで初めて「総量が動いた理由」が確かめられる形になった。**
`rss_total_bytes` だけでは、予算が効いたのか VeloX 側で何かが起きたのかを
区別できない。

### 36.5 事実 3: **VeloX の Rust 側を最適化してもメモリ目標には届かない**

フットプリントに占める VeloX 自身の割合は **1.2〜3.7%** である。仮に
VeloX の Rust 側のメモリを**ゼロにしても**、総量は 1.2〜3.7% しか減らない。

D48 が Linux/WebKitGTK で出した「メモリ超過の主因はエンジン差ではなく
VeloX 自身の webview 構成」という結論とは**別のこと**を言っている点に
注意すること。D48 は「どの webview をいくつ持つか」の話で、ここでの
「VeloX 自身」は**root プロセスの RSS** — Rust のヒープと `tao` の
ウィンドウである。両者は矛盾しない: **webview の数は VeloX が決めるが、
その webview が食うメモリはエンジン側に計上される。**

したがって T2 (Chromium 比 +10% 以内) へ向けて動かせるレバーは、
**エンジンのプロセスをいくつ・どれだけ生かしておくか**しかない。これは
D112 決定4 (減らせるのは休止の回数だけ) / D117 決定4 (予算の調整で届く
範囲には上限がある) と同じ方向を、別の角度から裏づけている。

### 36.6 限界

1. **2 反復・1 run・1 機種**である。ただし本節の主張 (browser 側が
   24〜27 MiB に収まる) は 4 条件すべてで成立しており、条件間の差を
   主張していないので、少ないサンプル数の影響を受けにくい。
2. **`rss_browser_bytes` は「Rust が確保した量」ではない** (D118 決定3)。
   `tao` のウィンドウやエンジンがプロセス内に置くものを含む。したがって
   36.5 は「Rust のアロケーションを削っても無駄」という主張であって、
   「Rust が 25 MiB 使っている」という主張ではない。
3. **ロール別 (WebProcess / Network / GPU) の内訳は無い** (D118 決定2)。
   `rss_engine_bytes` の 690〜2048 MiB がどのプロセスに分かれているかは
   Windows では分からない。
4. 20 タブの 1 点のみ。
5. **集計後の内訳は足し算の検算に使えない** (36.2 の注意書き)。1 サンプル内では厳密に一致するが、メトリクスごとに独立に中央値を
   取るため、集計後の `browser` + `engine` は `total` と一致しない。

## 37. `TrySuspend` はメモリを返さない — 代わりに復帰が 19 倍速くなる (Issue #243, 2026-09-13)

D120 決定3 が「使えると効くは別」として測っていなかった数字が出た。
**答えは、期待していた方向とは正反対だった。**

- run: [34766286144](https://github.com/noan98/VeloX/actions/runs/34766286144)
- A = 既定 (`SuspendMechanism::Discard` — webview を drop する D9 の方式)
- B = `VELOX_SUSPEND_MECHANISM=freeze` (`ICoreWebView2_3::TrySuspend`)
- シナリオ: `tabs_hold_20`, `tabs_hold_resume_20` を同一 run 内で (D115)
- 6 試行 × A/B 2 ペア、`minimal.html`
- ランナー: AMD EPYC 7763 / 4 論理コア / RAM 15.99 GiB
  → 既定のメモリ予算は `clamp(700, 15.99 GiB/16, 2048)` = **1023 MiB** (D114)

### 37.1 メモリ — `freeze` は `discard` の 1.9 倍使う

`tabs_hold_20`:

| メトリクス | A (discard) | B (freeze) | 差 | 比 (B/A) |
| --- | ---: | ---: | ---: | ---: |
| `rss_total_bytes` | 859.6 MiB | **1622.6 MiB** | **+763.0 MiB** | **1.888** |
| `rss_engine_bytes` | 833.0 MiB | 1595.3 MiB | +762.4 MiB | 1.915 |
| `rss_browser_bytes` | 27.4 MiB | 27.3 MiB | −0.1 MiB | 0.996 |
| `rss_process_count` | 15 | **27** | +12 | 1.8 |
| `suspended_tab_count` | 12 | **19** | +7 | 1.583 |

`tabs_hold_resume_20`:

| メトリクス | A (discard) | B (freeze) | 差 | 比 (B/A) |
| --- | ---: | ---: | ---: | ---: |
| `rss_total_bytes` | 1024.8 MiB | 1627.6 MiB | +602.7 MiB | 1.588 |
| `rss_engine_bytes` | 998.6 MiB | 1600.6 MiB | +602.0 MiB | 1.603 |
| `rss_browser_bytes` | 26.6 MiB | 27.3 MiB | +0.6 MiB | 1.024 |
| `rss_process_count` | 18 | 27 | +9 | 1.543 |
| `suspended_tab_count` | 12 | 19 | +7 | 1.583 |
| **`tab_resume_ms`** | **117.5** | **6.25** | **−111.25** | **0.053** |

**`rss_process_count` が答えを説明している。** 20 タブで A は 15 プロセス、
B は 27 プロセス。`TrySuspend` は名前のとおり**休止であって終了ではない** —
レンダラのプロセスは生きたまま残る。プロセスが残ればその常駐も残るので、
**メモリはほとんど返ってこない。**

B の 27 プロセス / 1622 MiB は、§28 / §31 が測った「休止なしの 20 タブ」
(約 65 MiB/タブ + 固定分) とほぼ一致する。つまり **`freeze` で休止した
20 タブは、休止していない 20 タブとほぼ同じだけメモリを使っている。**

### 37.2 休止タブ数が増えているのは、成功ではなく暴走である

`suspended_tab_count` は B のほうが多い (12 → 19)。一見「よく効いている」
ように読めるが逆である。

予算は 1023 MiB。A は 12 タブ休止した時点で 859 MiB まで下がり、予算を
満たして**止まる**。B はいくら休止させてもメモリが下がらないので、
**20 タブ中 19 タブを休止させてなお予算に届かず、止まらない。**
D117 が「重いページでは予算 700 MiB 側が 19 個中 17 個を休止してもなお
飽和する」と書いた現象と同じ形だが、こちらは**ページの重さではなく
機構そのものが原因**なので、予算をいくら上げても解けない。

つまり `freeze` を既定にすると、メモリ管理の観点では
**「ほぼ全部のタブを休止扱いにしながら、メモリは何も返らない」**
という最悪の状態になる。

### 37.3 復帰は 19 倍速い — ここだけは期待どおりだった

`tab_resume_ms` は **117.5 ms → 6.25 ms (0.053 倍)**。

D112 / §32 は「休止 1 回の対価は約 110〜130 ms で、タブ数にもページの
重さにも予算の大小にも依らない。webview の作り直しが支配項」と結論した。
今回の 6.25 ms はその結論を**裏側から裏づけている** — 作り直しをやめれば
対価は消える。D112 が測っていたのは確かに「webview の作り直し」であって、
それ以外ではなかった。

**D112 の「対価は下げられない」は、機構としては誤りだった。**
下げる方法は存在する。ただし §37.1 が示すとおり、その代金は
**メモリ 763 MiB** である。

### 37.4 VeloX 自身のメモリは、やはり動かない

`rss_browser_bytes` は A / B で 27.4 MiB と 27.3 MiB (差 −0.1 MiB、0.4%)。
休止の機構をまるごと入れ替えても VeloX 自身のプロセスは動かない。
D118 / §36 の「フットプリントに占める VeloX 自身は 1.2〜3.7% で、
ページの重さにも予算にも動かない」に、**休止機構も動かさない**が加わった。

### 37.5 この計測の限界

1. **`freeze` の失敗は 1 件も無かった。** 診断ステップの stderr に
   `freeze しました` は並び、`freeze に失敗` は 0 行。つまり上の数字は
   「`TrySuspend` が断ったせいで効かなかった」ものではなく、
   **成功した `TrySuspend` の実力値**である。
2. **20 タブ 1 点のみ。** 5 / 50 タブでの挙動は測っていない。ただし
   原因が「プロセスが残る」である以上、タブ数を変えて向きが変わるとは
   考えにくい。
3. **`minimal.html` のみ。** 重いページでは `TrySuspend` が返す量が
   増える可能性は残る (ページ内のヒープが大きいほど trim の余地がある)。
   ただし §37.1 の差は 763 MiB で、ページ差 (§35) の桁を大きく超える。
4. **時間を置いた後の再計測はしていない。** WebView2 が休止後に遅延して
   メモリを返す可能性は否定できない。`tabs_hold_*` は
   `MEMORY_HOLD_SETTLE_MS` (12 秒) 待ってから mark を打つので、
   **12 秒では返らない**ことまでは言える。

## 38. `MemoryUsageTargetLevel(LOW)` はフットプリントを 56〜60% 返す (Issue #242, 2026-09-14)

D121 が `TrySuspend` を退けた翌日に、**同じ Stage 2 の probe が見つけた
もう一方の API を測ったら、正反対の結果が出た。**

- run: [34814713139](https://github.com/noan98/VeloX/actions/runs/34814713139)
- A = 既定のまま / B = `VELOX_BACKGROUND_MEMORY_TARGET=low`
- **A/B 共通: `VELOX_MEMORY_BUDGET_MB=0`** (理由は §38.1)
- シナリオ: `tabs_hold_20`, `tabs_hold_resume_20` を同一 run 内で (D115)
- 6 試行 × A/B 2 ペア、`minimal.html`
- ランナー: AMD EPYC 9V74 / 4 論理コア / RAM 15.99 GiB

### 38.1 なぜ予算を切って測ったか

既定の予算のままだと 20 タブのうち 12 個が休止し (§37 の A 条件)、起きて
いる背景タブは 7 個しか残らない。**それではこのノブの効果が休止の効果に
埋もれる。** 予算を切れば背景タブ 19 個すべてが起きたまま hint を受ける
ので、「この API が何かするのか」という門番の問いにまっすぐ答えられる。

`suspended_tab_count` が A / B とも **0** であることが、狙いどおり
分離できたことを示している — 下表の差は**休止とは一切関係がない。**

### 38.2 メモリ — 半分以下になる

`tabs_hold_20`:

| メトリクス | A (既定) | B (low) | 差 | 比 (B/A) |
| --- | ---: | ---: | ---: | ---: |
| `rss_total_bytes` | 1596.6 MiB | **700.7 MiB** | **−895.9 MiB** | **0.439** |
| `rss_engine_bytes` | 1570.9 MiB | 675.1 MiB | −895.9 MiB | 0.430 |
| `rss_browser_bytes` | 27.2 MiB | 24.9 MiB | −2.3 MiB | 0.915 |
| `rss_process_count` | 27 | **27** | **0** | **1.0** |
| `suspended_tab_count` | 0 | 0 | 0 | — |

`tabs_hold_resume_20`:

| メトリクス | A (既定) | B (low) | 差 | 比 (B/A) |
| --- | ---: | ---: | ---: | ---: |
| `rss_total_bytes` | 1603.5 MiB | **653.0 MiB** | **−950.5 MiB** | **0.407** |
| `rss_engine_bytes` | 1577.0 MiB | 627.3 MiB | −949.6 MiB | 0.398 |
| `rss_browser_bytes` | 27.1 MiB | 25.6 MiB | −1.5 MiB | 0.943 |
| `rss_process_count` | 27 | 27 | 0 | 1.0 |
| `suspended_tab_count` | 0 | 0 | 0 | — |

**タブを 1 つも休止させずに、フットプリントが 56〜60% 減った。**
20 タブすべてが起きたまま、触れば即座に使える状態でこれである。

### 38.3 `rss_process_count` が動かないことが、D121 の説明を否定している
...
D121 決定1 はこう書いた —「原因ははっきりしている。`rss_process_count`
が 15 → 27。`TrySuspend` は休止であって終了ではないので、レンダラの
プロセスが生きたまま残る。**プロセスが残れば常駐も残る。**」

**最後の一文が誤りだった。** §38 はプロセス数を 1 つも減らさずに常駐を
半分以下にしている。プロセスが残ることと常駐が残ることは別である。

D121 の**結論** (`Freeze` を採らない) は実測値そのものに立っているので
揺るがない。誤っていたのは**なぜそうなるかの説明**で、正しくはこうなる:
`TrySuspend` は「止めろ」と言うだけでレンダラのヒープを縮めさせない。
`MemoryUsageTargetLevel` は「縮めてよい」と言い、エンジンはそれに従う。
**効くかどうかを分けたのはプロセスの生死ではなく、要求の中身だった。**

### 38.4 切替コストは、この計測では差を結論にできない

`tabs_hold_resume_20` の `tab_switch_ms` (予算 OFF なので休止は起きず、
背景タブへの切替はすべて `tab_switch`):

| | 各 run の median | 平均 | p95 平均 |
| --- | --- | ---: | ---: |
| A (既定) | 5.55, 6.05 | 5.80 ms | 23.80 ms |
| B (low) | 6.10, 6.75 | 6.42 ms | 24.00 ms |

差は +0.62 ms (1.108 倍) だが、**A 同士のばらつきが 0.50 ms あり、A/B 差
と同じ桁である。** 本文書が毎回書いている条件 (「A 同士のばらつきが A/B 差
と同程度なら、その差は結論にしてはならない」) にそのまま当てはまる。

言えるのは **「切替コストは、測れる範囲では変わっていない。変わっていた
としても 1 ms 未満」** までである。896 MiB の削減に対して 6 ms 前後の
切替なので、どちらに転んでも判断は変わらない。

`cpu_percent` は A が 9.29 / 9.31、B が 9.30 / 9.30 で**一致**している。
hint に CPU の対価は無い。

### 38.5 この計測の限界

1. **予算 OFF は既定の設定ではない。** §38.2 は「このノブ単体の実力」で
   あって「利用者が経験する差」ではない。既定 (予算 ON) では休止と
   組み合わさるので、別に測る必要がある。
2. **`discard` との直接比較になっていない。** §37 の A (discard、既定予算)
   は 859.6 MiB だったが、**あれは別の run・別の機種であり、D96 が禁じて
   いる比較**である。「LOW だけで休止より小さくなるのでは」という魅力的な
   読みは、同一 run で測るまで仮説に留める。
3. **20 タブ 1 点・`minimal.html` のみ。** ただし §38.2 の差は 896 MiB で、
   ページ差 (§35) の桁を大きく超える。
4. **エンジンが何を縮めたかは分からない。** JS ヒープか、描画バッファか、
   キャッシュか。返ってきた量は測れたが、**何を失ったのか**は測れていない。
   §38.4 が切替コストを見たのはその代理でしかない。

## 39. hint を入れると休止が発火しなくなる — そして休止するより軽い (Issue #242, 2026-09-14)

D122 決定3 が要求した追試 2 件。**2 本とも同じ答えを出し、しかも
想定していなかった形で出た。**

| | 追試 | A | B | run |
| --- | --- | --- | --- | --- |
| 1 | 既定の予算 ON で hint を A/B | 既定のまま | 既定 + `low` | [34818739432](https://github.com/noan98/VeloX/actions/runs/34818739432) |
| 2 | hint 単体 と 休止単体 の直接比較 | 既定のまま (休止あり) | `low` + 予算 OFF (休止なし) | [34816805759](https://github.com/noan98/VeloX/actions/runs/34816805759) |

いずれも 6 試行 × A/B 2 ペア、`tabs_hold_20` / `tabs_hold_resume_20` を
同一 run 内で (D115)、`minimal.html`。

### 39.1 追試1 — 予算 ON にしたのに、B では休止が一度も起きなかった

| シナリオ | A (今日の既定) | B (既定 + `low`) | 比 |
| --- | ---: | ---: | ---: |
| `tabs_hold_20` | 861.5 MiB / 15 proc / **12 休止** | **700.8 MiB** / 27 proc / **0 休止** | **0.814** |
| `tabs_hold_resume_20` | 1008.0 MiB / 17 proc / 12 休止 / `tab_resume_ms` **115.25** | **652.4 MiB** / 27 proc / **0 休止** / 復帰は発生しない | **0.647** |

**`suspended_tab_count` が B で 0 になっている。** 予算は両側とも ON
(この機体で 1023 MiB、D114) なのに、である。

理由は単純で、**hint が効きすぎて予算に届かないから**である。B の
着地は 700.8 MiB で、予算 1023 MiB を大きく下回る。policy は
「予算内なので何もしなくてよい」と判断し、**休止を一度も発火させない。**

つまり追試1 は「hint と休止を併用したときの値」を測るつもりで組んだが、
**併用という状態が成立しなかった。** これは設計ミスではなく結果である。

### 39.2 追試2 — 「hint だけ」は「休止だけ」より軽い

追試1 の B で休止が起きなかった以上、「hint 単体 vs 休止単体」を
明示的に組んだ追試2 の意味が大きくなる。

| シナリオ | A (休止 12 個・hint なし) | B (hint のみ・休止 0) | 比 |
| --- | ---: | ---: | ---: |
| `tabs_hold_20` | 865.0 MiB / 15 proc | **701.6 MiB** / 27 proc | **0.811** |
| `tabs_hold_resume_20` | 1000.1 MiB / 17 proc / `tab_resume_ms` **98.78** | **650.9 MiB** / 27 proc / 復帰なし | **0.651** |

**タブを 1 つも休止させずに、休止したときより 19〜35% 軽い。**
そのうえ復帰コスト (A は 98.78 ms を 12 回払っている) も、
状態喪失 (D105) も、一切払っていない。

### 39.3 差は結論にしてよい大きさである

本文書が毎回課している条件 (「A 同士のばらつきが A/B 差と同程度なら
結論にしてはならない」) を確認する。

| 追試 / シナリオ | A 同士のばらつき | A/B 差 |
| --- | ---: | ---: |
| 2 / `tabs_hold_20` | 0.18% | **18.9%** |
| 2 / `tabs_hold_resume_20` | 0.68% | **34.9%** |
| 1 / `tabs_hold_20` | 0.84% | **18.6%** |
| 1 / `tabs_hold_resume_20` | 2.80% | **35.3%** |

**桁が違う。** §37 の `freeze` や §35 のページ差のように解釈に注意が要る
数字ではない。

### 39.4 hint を入れたときの着地値は、3 run で再現している

| 計測 | 予算 | B の `rss_total_bytes` |
| --- | --- | ---: |
| §38 | OFF | 700.7 MiB |
| 追試2 | OFF | 701.6 MiB |
| 追試1 | **ON** | 700.8 MiB |

3 本とも **700 MiB 前後**に着地している。D96 は「run をまたいだ絶対値を
比較してはならない」と定めているが、**これは比較して結論を出している
のではなく、独立した 3 本が同じ値を出したという再現性の報告**である。
逆に A 側は 861.5 / 865.0 / (§38 は条件が違う) と、機体差ぶんの揺れが
見える。

### 39.5 何が変わって、何が変わらないか

| | A (今日の既定) | B (`low`) |
| --- | --- | --- |
| 20 タブでのフットプリント | 861〜1008 MiB | **652〜701 MiB** |
| 休止されるタブ | 12 個 | **0 個** |
| 復帰コスト | 98.78〜115.25 ms × 12 回 | **発生しない** |
| ページの状態 (D105) | 12 タブぶん失われる | **失われない** |
| プロセス数 | 15〜17 | 27 |
| VeloX 自身 (`rss_browser_bytes`) | 27.1〜27.3 MiB | 27.7 MiB (誤差) |

プロセス数だけが増えている。休止しないので当然で、**そのうえで総量は
減っている** — D122 決定2 が訂正したとおり、プロセスの数と常駐量は
別の話である。

### 39.6 この計測の限界

1. **20 タブ 1 点のみ。** 50 タブでは hint だけでは予算に届かず、休止が
   発火する可能性がある。**そのとき予算が安全網として残っていることが
   重要になる** (D123 決定2)。
2. **`minimal.html` のみ。** 重いページではページあたりの常駐が大きく、
   §39.1 の「予算に届かない」が成立しなくなりうる。
3. **何を失ったかは依然として測れていない** (D122 決定4 / §38.5 限界4)。
   `tab_switch_ms` に有意差が無かった (§38.4) ことは「失ったものが無い」
   の証明ではない。
4. **メディア再生中のタブを試していない。** hint が再生を阻害しないかは
   未確認である。

## 40. 予算が効き始める負荷では、hint は「メモリを減らす」のではなく「同じ予算で生きたままのタブを増やす」 (Issue #247, 2026-09-14)

D123 決定5 / Issue #247 が残した「測っていないこと」のうち、50 タブ・
重いページ・背景 CPU の 3 本。**既定が `Low` になったので、ここから
A = `low` (新既定) / B = `normal` (旧挙動) であり、§38・§39 とは A/B が
逆向きである。**

| 計測 | run |
| --- | --- |
| 50 タブ / `minimal.html` | [34840182818](https://github.com/noan98/VeloX/actions/runs/34840182818) |
| 背景 CPU (`busy.html`) | [34842006009](https://github.com/noan98/VeloX/actions/runs/34842006009) |
| 20 タブ / `dom_heavy.html` | [34844830778](https://github.com/noan98/VeloX/actions/runs/34844830778) |

### 40.1 総量の列を単独で読んではいけない

まず表をそのまま並べる。**このまま読むと結論を間違える。**

| 条件 | A (`low`) 総量 | B (`normal`) 総量 | 比 (B/A) |
| --- | ---: | ---: | ---: |
| 20 タブ / minimal (§39 追試1) | 652.4 MiB | 1008.0 MiB | 1.545 |
| 20 タブ / dom_heavy | **1008.0 MiB** | **722.2 MiB** | **0.716** |
| 50 タブ / minimal | **1014.8 MiB** | **774.2 MiB** | **0.763** |

**負荷を上げると符号が逆転している。** 軽い条件では `low` が小さく、
重い条件では `normal` のほうが小さい。

「重い条件では旧挙動のほうが優れている」と読みたくなるが、**誤りである。**
2 つの条件は**違う数のタブを生かしている**ので、総量を直接比べる意味が無い。

### 40.2 生きているタブ数で割ると、向きは一つしかない

| 条件 | | 総量 | プロセス | 休止 | **生きタブ** | **エンジン / 生きタブ** |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| 20 / minimal | A (`low`) | 652.4 MiB | 27 | 0 | **20** | **31.3 MiB** |
| | B (`normal`) | 1008.0 MiB | 17 | 12 | 8 | 122.9 MiB |
| 20 / dom_heavy | A (`low`) | 1008.0 MiB | 23 | 4 | **16** | **61.4 MiB** |
| | B (`normal`) | 722.2 MiB | 11 | 16 | 4 | 173.8 MiB |
| 50 / minimal | A (`low`) | 1014.8 MiB | 42 | 16 | **34** | **29.0 MiB** |
| | B (`normal`) | 774.2 MiB | 13 | 44 | 6 | 124.3 MiB |

1 タブあたりでは **2.8〜4.3 倍**効率的で、**3 条件すべてで `low` が上**。
§38 / §39 の 0.4 倍前後という数字と同じことを言っている。

そして生きたままのタブ数は:

| 条件 | `low` | `normal` | 倍率 |
| --- | ---: | ---: | ---: |
| 20 / minimal | 20 | 8 | **2.5 倍** |
| 20 / dom_heavy | 16 | 4 | **4.0 倍** |
| 50 / minimal | 34 | 6 | **5.7 倍** |

**負荷が重いほど差が開く。** `normal` は 1 タブが高いのですぐ予算に
ぶつかり、大量に休止させる。`low` は 1 タブが安いので、同じ予算で
何倍も抱えられる。

### 40.3 予算はどの条件でも守られている — policy は設計どおり

| 条件 | A (`low`) の着地 | 予算 (1023 MiB) に対して |
| --- | ---: | ---: |
| 20 / minimal | 652.4 MiB | 64% |
| 20 / dom_heavy | 1008.0 MiB | **98%** |
| 50 / minimal | 1014.8 MiB | **99%** |

重い条件では**予算のすぐ下に張り付いている**。これは「予算いっぱいまで
使い、超えた分だけ休止する」という policy の設計どおりの挙動で、
`suspended_tab_count` が 4 / 16 と出ているのは**安全網が正しく働いた印**
である。

**D123 決定2 の予想 (「50 タブや重いページでは hint だけでは届かない。
そのとき休止が安全網として要る」) は、そのとおりだった。** 予算を切らずに
残した判断は正しかったことになる。

### 40.4 背景 CPU — メモリには効くが、肝心の問いには答えられていない

`background_cpu` シナリオ (`busy.html`、Issue #64):

| メトリクス | A (`low`) | B (`normal`) | 比 |
| --- | ---: | ---: | ---: |
| `rss_total_bytes` | 385.4 MiB | 438.3 MiB | 1.137 |
| `rss_process_count` | 9 | 9 | 1.0 |
| `cpu_percent` median | 0.00, 0.00 | 0.39, 0.78 | — |
| `cpu_percent` p95 | 1.56 | 1.56 | 1.0 |

メモリは 2 タブ程度の軽い条件でも 13.7% 効いている。

**`cpu_percent` は「低い」方向に出ているが、結論にしてはならない。**

- 絶対値が**どの条件でも 1 コアの 1% 未満**で、分布が 0 に張り付いている。
  中央値 0.00 は「半数以上のサンプルが 0」というだけで、
  「ページが動いていない」のか「サンプラが idle を多く拾った」のかを
  **区別できない。**
- **`p95` は 4 本とも 1.56 で一致している。** ピーク時の仕事量は変わって
  いないことを示唆する。

したがって **Issue #247 項目1 の核心 (背景タブの JS タイマーが止まって
いないか) は未解決のままである。**

`busy.html` は rAF / `setInterval` / CSS アニメーションのカウンタを持ち、
`visibilitychange` のたびに `<title>` へ書き出す作りになっている。しかし
**perf 出力に title が載らない**ため、そのカウンタを読めていない。読むには
harness 側の変更が要る。

**この節を「`Low` に CPU の対価は無い」と要約してはならない。** 言えるのは
「背景 CPU は増えていない」までで、**何を失ったのかは D122 決定4 から
一歩も進んでいない。**

### 40.5 この計測の限界

1. **項目1 が未解決** (§40.4)。カウンタを読む手段が要る。
2. **`dom_heavy.html` 1 種類のみ。** 「重いページ」の代表として十分かは
   分からない。
3. **50 タブは 4 試行 × 2 ペア**と、他より試行数が少ない (1 試行が長い
   ため)。A 同士のばらつきは 0.55% で A/B 差より 1 桁以上小さいので
   読みに影響はしないが、記録しておく。
4. **「生きタブが多いほうが良い」は測定ではなく判断である** (§40.2)。
   34 タブが即座に使える 1014.8 MiB と、6 タブの 774.2 MiB のどちらを
   選ぶかは製品判断で、D105 (状態喪失) と D112 (復帰 100 ms) を踏まえて
   前者を採っている。**数字がそう言っているのではない。**

## 41. 撤回 — beacon を数えたつもりで、診断ステップの単発起動を数えていた (Issue #247, 2026-09-14)

**この節の初版には結果の表があった。撤回する。** 測ったのは
`background_cpu` の背景タブではなく、**その前段で velox を 8 秒だけ
単独起動する診断ステップ**だった。以下は何をどう間違えたかの記録である。

### 41.1 測り方の意図 — ページ自身に報告させる

`busy.html` (Issue #64) は rAF / `setInterval` のカウンタを `<title>` に
書き出すが、**それを VeloX から読むには計測対象のタブで JS を評価する
必要がある。** `LOW` を伝えた背景タブで JS を走らせれば、測ろうとして
いる当のものを乱しうる (D122 決定4 が `IsSuspended` について書いたのと
同じ危険)。

そこで同ページの `?beacon=1` を使う。`report()` のたびに loopback へ
`beacon?state=..&f=..&t=..` を 1 本投げるので、**ブラウザに触れずに
数えられる。** Issue #64 がこのオプションを置いた理由がまさにこれで、
ページのコメントはこう書いている —「CPU 使用量は『少ない』ことしか
示せず、0 との区別がつかない」。**§40.4 で踏んだのはその通りの壁だった。**

**この設計自体は妥当である。** 壊れていたのは配線のほうだった。

### 41.2 何が繋がっていなかったか

`beacon` 入力は `bench_url` ステップの **`url` 出力にだけ**クエリを
付けていた:

```powershell
"url=http://127.0.0.1:8731/$($pageList[0])$beaconQuery" >> $env:GITHUB_OUTPUT
```

ところが実計測のループは、その `url` を使わない。`pages` 出力から
ページ名を取って **URL を組み直していた**:

```powershell
$url = if ($pg) { "http://127.0.0.1:8731/$pg" } else { $env:BENCH_URL }
```

**固定ページを配信するとき `$pg` は必ず非空**なので、`$env:BENCH_URL`
(= beacon 付き) の分岐には**一度も入らない。**

`url` 出力を実際に使っていた唯一の場所は、**「Diagnose VeloX launch」
ステップ**だった。velox を `--homepage` 付きで起動し、8 秒待って
プロセス一覧を撮るだけの診断で、`mark` / `hidden` / `idle` の自動操作
スクリプトとは何の関係もない。

### 41.3 数字が合っていたことが、そのまま証拠だった

初版は「80 本前後出るはずが 13 本しか届かない」を**未説明**として
残していた。その 13 本の内訳は `hidden` が 1、`visible` が 12 である。

**13 = 1 + 12** — つまり届いた beacon は**全部が**診断ステップ由来で、
`background_cpu` の 8 トライアル (baseline×4 + compare×4) からの寄与は
**ゼロだった。**

「総数が合わない」という違和感は正しかった。**合わない量が『別のものを
数えていた』ことをそのまま指していたのに、サーバ側の取りこぼしを疑って
そちらを潰しに行った。** 単一スレッド版でも 300/300 通ったところで
「サーバは原因ではない」とまでは書いたが、**では何なのかを詰めずに
仮説 (`visibilityState`) へ飛んだ。**

### 41.4 「3 run で同一」は同一で当たり前だった

初版は 3 run (`low`/`normal` 混在・両腕 `low`・両腕 `normal`) で beacon が
一致したことを、**`LOW` が背景タブの挙動を変えていない証拠**として
読んだ。

診断ステップは `compare_env` による A/B の切り替えより**前**に走る。
つまり 3 run とも**まったく同じ条件**を測っていた。一致するのは当然で、
**`low` と `normal` の違いはそもそも一度も試されていない。**

**「条件を変えたのに値が動かない」ときは、条件が届いていない可能性を
先に潰すこと。** ここでの教訓はそれに尽きる。

### 41.5 修正

- 実計測ループの URL 組み立てにも `$beaconQuery` を適用し、
  `Run velox-bench` ステップに `INPUT_BEACON` を渡す
- **診断ステップは beacon の付かない `url_plain` 出力を使う。**
  診断の 8 秒分が計測と同じカウンタに混ざるのを断つ
- `beacon` 入力をシェルへ直接展開せず環境変数経由にする
  (他の入力と揃える。script injection 対策)
- `.github/scripts/test_perf_windows_inputs.py` に配線の検査を 5 件追加。
  **修正前の workflow に対して 5 件とも落ちることを確認した**
...
同じ性質の失敗が §26 系列でも 2 回起きており (サマリの正規表現)、
`test_perf_windows_inputs.py` はもともとそのために置かれたファイルである。
**「Windows でしか動かない PowerShell の中身を、Linux 上で突き合わせる」**
という既存の枠にこの検査も入れた。

### 41.7 再計測は未了

**修正後の計測はまだ行っていない。** したがって
「`LOW` が背景タブの挙動を変えるか」は、§40.4 の時点と同じく**未回答**
である。D124 決定5 の歯止め (「解くまでは『対価が無い』と言わない」) は
そのまま効いている。

### 41.8 この計測に残る限界 (再計測後も残るもの)

1. **前景タブと背景タブが同じ `state` バケットに入る。** 両方が
   `visible` を通るため。idle タブは `f=0&t=0` で送るので値である程度は
   分かるが、設計として分離できていない。
2. **`compare_env` に空文字列を渡しても単独条件にならない。**
   `workflow_dispatch` の入力は、空文字列を渡すと宣言時の既定値
   (`VELOX_MEMORY_BUDGET_MB=0`) で埋められる。単独条件の計測は今の
   入力設計ではできない。
3. **ページは `fetch` の失敗を握り潰す。** 届かなかった beacon と、
   そもそも投げられなかった beacon は区別できない。`advanced_*` が 0 で
   あることを、それだけで「止まっている」と読んではならない。

## 42. 背景タブは `LOW` を受けても JS を止めていない — ただし `low`/`normal` の分離はまだできていない (Issue #247, 2026-09-14)

§41 で撤回した計測の、配線を直したうえでのやり直し。
[run 34857453812](https://github.com/noan98/VeloX/actions/runs/34857453812)
(`background_cpu` / `busy.html` / A = `low` 既定 / B = `normal` /
repeats=4 = 8 トライアル / beacon=true)。

### 42.1 まず、経路が本当に繋がった

| | §41 (壊れていた) | §42 (修正後) |
| --- | ---: | ---: |
| 届いた beacon | **13 本** | **2,614 本** |

結果 JSON の `url` も `http://127.0.0.1:8731/busy.html?beacon=1` になっており、
**計測タブ自身が投げている**ことを確認できる。§41 ではここが素の URL だった。

### 42.2 結果

```
hidden:  count=805   frames 100 -> 95 (+-5)  / ticks 156 -> 162 (+6)
visible: count=1809  frames 0 -> 0 (+0)      / ticks 0 -> 0 (+0)
```

### 42.3 言えること — 背景タブのタイマーは止まっていない

`report()` は `setInterval` で 500ms ごとに呼ばれる。**その報告が
`state=hidden` として 805 本届いている。**

したがって:

1. **`document.visibilityState` は背景タブに伝わっている。** ページは自分が
   隠れていることを認識して `hidden` と申告している。§41.5 が立てた仮説
   (`set_visible(false)` が伝わっていない) は**否定された** → #251 を閉じた。
2. **背景タブで `setInterval` は動き続けている。** 止まっていれば `report()`
   自体が呼ばれず、beacon は 1 本も来ない。

### 42.4 言えないこと — `advanced_*` は読んではならない

**`hidden` の `advanced_frames` が -5 と負になっている。** これは異常値では
なく、**集計の設計がこの計測に合っていない**ことの表れである。

`serve.py` は `state` ごとに「最初の beacon の値」と「最後の beacon の値」を
持つが、**1 run で velox は 8 回起動し、そのたびにページのカウンタは 0 から
やり直す。** 別々の起動の first と last を引き算しても意味がない。

**`advanced_frames` / `advanced_ticks` を「カウンタが進んだ量」として
読んではならない。** 読めるのは `count` (何本届いたか) だけである。

### 42.5 言えないこと — `low` と `normal` を分離できていない

**`serve.py` は job 全体で 1 つのカウンタしか持たない。** A 4 回と B 4 回の
beacon が同じバケットに積み上がっている。

したがって §42.3 は「**どちらかの条件で**背景タブの JS が動いていた」しか
言っておらず、**`low` の背景タブが `normal` と違う振る舞いをするかは、
やはり測れていない。**

D124 決定5 の問い (`LOW` が何を縮めたのか) も未回答のままである。

### 42.6 メモリの A/B は、この run が最もきれいに採れている

| メトリクス | A (`low`) | B (`normal`) | 比 (B/A) |
| --- | ---: | ---: | ---: |
| `rss_total_bytes` | **414.1 MB** | 466.3 MB | **1.126** |
| `rss_engine_bytes` | 387.2 MB | 439.7 MB | 1.136 |
| `rss_browser_bytes` | 27.5 MB | 27.1 MB | 0.985 |
| `rss_process_count` | **9** | **9** | 1 |
| `suspended_tab_count` | **0** | **0** | — |

**プロセス数が同じで、どちらも休止が発火していない。** D124 決定3 が
「`suspended_tab_count` が A/B で違う表では `rss_total_bytes` の比較は
成立しない」と書いた条件に、この表は**当たらない。**

つまり **`low` は 12 タブの `busy.html` で常駐を 52 MB (11%) 減らしている**
— 生きタブ数を変えずに。§40 の表がプロセス数と休止数の違いに汚染されて
いたのに対し、ここは素直に読める。

各 run の median のばらつき (A: 413.4/414.4/413.8/414.9 MB、
B: 465.8/467.0/467.1/465.2 MB) は **1 MB 未満**で、A/B 差 52 MB より
2 桁小さい。§26.4 の「A 同士のばらつきが A/B 差と同程度なら結論にするな」
という条件も満たしている。

### 42.7 beacon を有効にすると計測時間が桁で変わる

| | ベンチステップの所要時間 |
| --- | ---: |
| beacon なし (repeats=2、[34851494354](https://github.com/noan98/VeloX/actions/runs/34851494354)) | 約 2 分 |
| **beacon あり (repeats=4)** | **約 16 分** |

トライアル数は 2 倍だが、時間は 8 倍である。12 タブが 500ms ごとに loopback
へ HTTP を投げるので、2〜4 コアのランナーでは無視できない負荷になる。

**beacon を有効にした run の時間系メトリクスを、beacon 無しの系列と並べて
読んではならない。** §42.6 のメモリ値は A/B とも同じ負荷の下で採っているので
比較できるが、`page_load_ms` のような値を §21〜§34 に並べるのは誤りである。

### 42.8 次にやること

`serve.py` に条件の区切りを入れないと #247 は進まない。必要なのは
**A/B の腕ごとにカウンタを分けること** — 例えば `/beacon-reset` を足して
workflow が腕の切り替え時に叩き、腕ごとに集計を読む。あわせて起動ごとの
リセットも要る (§42.4)。

## 43. `LOW` は背景タブの実行を何も変えない — 腕ごとに分離して確定 (Issue #247, 2026-09-14)

§42.5 が「分離できていない」と残した比較を、`/beacon-reset` を入れて取り直した。
[run 34863935972](https://github.com/noan98/VeloX/actions/runs/34863935972)
(`background_cpu` / `busy.html` / A = `low` 既定 / B = `normal` / repeats=2)。

### 43.1 結果 — 腕ごとの背景タブ

| 腕 | `count` | `instances` | `advanced_frames` | `advanced_ticks` |
| --- | ---: | ---: | ---: | ---: |
| A (`low`) #1 | 102 | 12 | **0** | 107 |
| B (`normal`) #1 | 104 | 13 | **0** | 109 |
| A (`low`) #2 | 101 | 11 | **0** | 103 |
| B (`normal`) #2 | 102 | 12 | **0** | 113 |

前景タブ (`visible`) 側は 4 本とも `advanced_frames` が 899〜966 で、
rAF が回っていることが確認できる。

### 43.2 `instances` が集計の正しさを示している

**11〜13 は背景タブの数 (12) と一致する。** §42.4 の壊れ方 (state ごとに
first/last だけを持ち、12 個のタブと 8 回の起動を混ぜて引き算していた) が
直ったことが、この列から読める。`advanced_*` も負になっていない。

### 43.3 `LOW` と `normal` に差は無い

**A 同士のばらつきが A/B 差と同程度である。**

| | A の 2 本 | B の 2 本 |
| --- | --- | --- |
| `count` | 102, 101 | 104, 102 |
| `advanced_ticks` | 107, 103 | 109, 113 |

`count` の A-A 差は 1、A-B 差は 2〜3。`advanced_ticks` の A-A 差は 4、
A-B 差は 2〜10。**§26.4 の判定条件 (「A 同士のばらつきが A/B 差と同程度なら
結論にしてはならない」) にそのまま当たる。**

**したがって `MemoryUsageTargetLevel(LOW)` は、背景タブの JS 実行を
何も変えていない。** 腕を分離したうえでの、直接の観測である。

### 43.4 rAF はもともと止まっている — `LOW` とは無関係に

**`advanced_frames` が両腕とも 0 である。** 背景タブで
`requestAnimationFrame` は 1 回も呼ばれていない。

これは `LOW` の効果ではない。**`normal` でも 0 だからである。**
隠れたタブに描画フレームを渡さないのはエンジン自身の既定の抑制で、
Issue #64 が `busy.html` にこのカウンタを置いたのは、まさにこれを
外側から見るためだった。

一方 `advanced_ticks` は 103〜113 進んでいる。**タイマーは動き、
描画は止まる** — 背景タブの抑制がこの形であることが、初めて数字で出た。

### 43.5 メモリ比が別機種で再現した

| run | ランナー | `rss_total_bytes` 比 (B/A) |
| --- | --- | ---: |
| [34857453812](https://github.com/noan98/VeloX/actions/runs/34857453812) (§42.6) | Intel Xeon 6973P-C | 1.126 |
| **34863935972 (本節)** | **AMD EPYC 7763** | **1.125** |

**機種が違う run どうしの絶対値は比較できない** (D96) が、**同一 run 内の
A/B 比は比較できる。** 2 つの run で 1.126 と 1.125 が独立に出たことは、
§42.6 の値が機種に依らないことを示す。

どちらの run も `rss_process_count` が両腕 9 で一致し、
`suspended_tab_count` が両腕 0 である (D124 決定3 の「比較してはいけない
条件」に当たらない)。

### 43.6 それでも D124 決定5 の本体は解けていない

答えたのは**派生した問い**である —「`LOW` は背景タブの実行を奪うか」。
答えは **No** で、これは直接測れた。

**本体の問い (エンジンが何を縮めたのか — JS ヒープか、描画バッファか、
キャッシュか) には、beacon は原理的に答えられない。** beacon が運べるのは
ページから見える値だけで、エンジンの内訳は見えない。

### 43.7 次の一手 — `performance.memory`

**手段が無いわけではない。** WebView2 は Chromium なので、ページから
`performance.memory.usedJSHeapSize` が読める。これを beacon に載せれば、
**JS ヒープが縮んだかどうかだけは直接測れる。**

3 つの候補 (JS ヒープ / 描画バッファ / キャッシュ) のうち 1 つを潰せる。
残り 2 つはページから見えないので、D118 決定2 が「Windows ではロール別に
割れない」と書いた壁に戻る。
...
## 44. `LOW` が縮めたのは JS ヒープではない — 桁が 3 倍足りない (Issue #247, 2026-09-14)

D124 決定5 の 3 候補 (JS ヒープ / 描画バッファ / キャッシュ) のうち、**ページから
手の届く 1 つ**を測った。
[run 34867663331](https://github.com/noan98/VeloX/actions/runs/34867663331)
(`background_cpu` / `busy.html` / A = `low` 既定 / B = `normal` / repeats=2)。

### 44.1 `performance.memory` は WebView2 で読めた

`heap_count` が `count` と一致している (101〜104 / 229〜230)。**すべての
beacon が JS ヒープを運んできた。** Chromium の API がそのまま使える。

### 44.2 結果 — 背景タブの JS ヒープ

| 腕 | `heap_mean` | `heap_max` |
| --- | ---: | ---: |
| A (`low`) #1 | 1.336 MB | 1.508 MB |
| A (`low`) #2 | 1.318 MB | 1.530 MB |
| B (`normal`) #1 | 1.259 MB | 1.416 MB |
| B (`normal`) #2 | 1.404 MB | 1.548 MB |

| | 平均 | 自身のばらつき |
| --- | ---: | ---: |
| A (`low`) | 1.327 MB | 0.018 MB |
| B (`normal`) | 1.332 MB | **0.145 MB** |

**B 自身のばらつきが A/B 平均差の 29 倍ある。** §26.4 の判定条件にそのまま
当たり、**差は無い。**

### 44.3 桁が足りない — こちらのほうが強い根拠である

`heap_mean` は 1 タブあたり約 1.33 MB。背景タブは 12 個なので、
**全部合わせても約 16 MB** である。

一方、A/B の `rss_total_bytes` 差は **49.4 MB**。

**背景タブの JS ヒープを全部ゼロにしても、差の 32% にしか届かない。**

§44.2 の「差が無い」は統計的な判断だが、こちらは**桁の話**である。
仮に §44.2 が測り損なっていたとしても、**JS ヒープは 49.4 MB を説明
できない。** 2 つの独立した根拠が同じ結論を指している。

### 44.4 メモリ比が 3 回目の再現

| run | ランナー | 比 (B/A) |
| --- | --- | ---: |
| 34857453812 (§42.6) | Intel Xeon 6973P-C | 1.126 |
| 34863935972 (§43.5) | AMD EPYC 7763 | 1.125 |
| **34867663331 (本節)** | **AMD EPYC 7763** | **1.127** |

3 run とも `rss_process_count` 9 で一致、`suspended_tab_count` 0。
**`low` が 11〜13% 減らすことは、もはや疑う余地がない。**

### 44.5 背景タブの実行も再現した

`advanced_frames` は 4 本とも **0**、`advanced_ticks` は 109〜121。
§43.1 (0 / 103〜113) と同じ形である。**rAF は止まり、タイマーは動く** —
これも両腕で同じ。

### 44.6 残ったのは 2 つ、どちらも壁の向こう
...
残り 2 つを測るには Windows の WebView2 のロール別内訳が要るが、
D118 決定2 が「全ロールが `msedgewebview2.exe` で、ロールはコマンド
ラインにしか現れない。取るには WMI (依存追加) か PEB 読み出し
(`unsafe`) が要る」と結論している。

**ページから手の届く範囲は測り切った。**

### 44.7 仮説として残すもの — 描画バッファ

§44.5 のとおり、背景タブでは**両腕とも** rAF が止まっている。描いて
いないのに、エンジンは最後に合成した面を持ち続けうる。`LOW` がそれを
手放しているなら辻褄は合う。

**ただしこれは仮説であって、測っていない。** §41 で「壊れた計測から出た
数字を根拠に仮説を立て、別 Issue に切り出した」失敗をしているので、
ここでは Issue にしない。D118 決定2 の Revisit condition が入口である。

## 45. 同一 run 内のばらつきは ±2% — 機種を跨いだ ±23% とは桁が違う (Issue #211, 2026-09-14)

D132 (週次計測の取り込み) の PR で `perf-windows.yml` の自己テストが走り、
**週次 schedule と同じ既定条件** (`cold_startup` / `compare_env` =
`VELOX_MEMORY_BUDGET_MB=0` / repeats 2) の実測が 1 run ぶん採れた。
run [34876499643](https://github.com/noan98/VeloX/actions/runs/34876499643)。

### 45.1 この条件では A/B に差が出ない — そしてそれが項目3 には好都合である

| メトリクス | A (既定) | B (`VELOX_MEMORY_BUDGET_MB=0`) | 比 (B/A) |
| --- | ---: | ---: | ---: |
| `rss_total_bytes` | 367.6 MB | 368.0 MB | **1.001** |
| `startup_window_created_ms` | 624.02 | 624.2 | **1.000** |
| `rss_process_count` | 8 | 8 | 1 |

**`cold_startup` はタブを 1 枚しか開かないので、メモリ予算に到達しない。**
したがって B 腕は「予算を変えた条件」ではなく、**実質的に 2 本目の A 腕**
として振る舞う。

⚠️ **この 1.001 を §42.6〜§44 の 1.126 と並べてはならない。** あちらは
タブを積んだシナリオでの `low` / `normal` の比であり、シナリオも条件も
別物である。**別の現象の数値を同じ系列として読む**のは §21 / §40 が
繰り返し警告してきた誤読そのものである。

### 45.2 得られた数字 — 同一 run・同一機種でのばらつき

B 腕が 2 本目の A 腕として振る舞うということは、**1 run で 4 本の
ほぼ同条件のサンプルが採れている**ということである。

| `startup_window_created_ms` の 4 サンプル | 幅 |
| --- | ---: |
| 611.25 / 636.8 / 616.15 / 632.25 | **±2.1%** |

D96 が測った**機種を跨いだ**幅と並べると:

| 比較の単位 | 幅 |
| --- | ---: |
| 同一 run・同一機種 (本節) | **±2.1%** |
| run を跨ぐ (機種が変わる、D96 / §26.4) | 573〜922ms = **±23%** |

**桁が 1 つ違う。** D96 が「run ごとの絶対値を比較してはならない」と
定めた根拠が、同一 run 内の幅と並べることで初めて定量的に見える。

### 45.3 これは項目3 の答えではない

Issue #211 項目3 が要るのは「**機種を揃えたうえで run を跨いだ**ばらつき」
であり、本節が出したのは「**同一 run 内の**ばらつき」である。両者の間に
あるもの (同じ機種を引いた別の run での差) はまだ測れていない — それが
D132 で貯め始めるデータである。

本節が言えるのは**下限**だけである: 同一機種の run 間のばらつきは、
少なくとも ±2.1% より小さくはならない。ゲート化の閾値を決めるとき、
この値より狭い閾値を置くことはできない。

### 45.4 偶然、同一機種の 2 run 目が採れた — 項目3 の最初の 1 点

同じ PR の 2 つ目の push で CI が回り直し、**同じ既定条件の計測がもう 1 run
走った。そして引いた機種が同じだった。**

| run | 機種 | 論理コア | RAM |
| --- | --- | ---: | ---: |
| [34876499643](https://github.com/noan98/VeloX/actions/runs/34876499643) | AMD EPYC 7763 64-Core | 4 | 15.99 GiB |
| [34877390649](https://github.com/noan98/VeloX/actions/runs/34877390649) | **AMD EPYC 7763 64-Core** | **4** | **15.99 GiB** |

`derive_machine_key` の入力 (os / cpu_model / cpu_count / RAM の GiB 丸め)
がすべて一致するので、**この 2 run は同じ `machine_key` になる** — D106 の
定義上「比較してよい」組である。

#### `startup_window_created_ms` の全サンプル

| run | 4 サンプル (A×2, B×2) | 平均 | run 内の幅 |
| --- | --- | ---: | ---: |
| 34876499643 | 611.25 / 636.8 / 616.15 / 632.25 | 624.11 | **4.1%** |
| 34877390649 | 606.9 / 608.15 / 614 / 612.9 | 610.49 | **1.2%** |

**run 平均の差は 13.6ms = 2.2%。**

#### 結論: この差は差として読めない

§26.4 の判断規則 (「**A 同士のばらつきが A/B 差と同程度なら、その差は結論に
してはならない**」) を run 間に当てはめると:

- run 平均の差 **2.2%** < run 34876499643 の run 内の幅 **4.1%**
- 2 つの run のサンプル範囲は**重なっている** (611.25 < 614)

したがって **run 間の差はノイズと区別がつかない。**

#### ⚠️ これは「同一機種なら比較してよい」の証明ではない

n=2 である。**2 回引いて 2 回とも差が出なかった、というだけ**であり、
D96 Revisit condition (2) を閉じるには全く足りない。言えるのは:

- 同一機種の run 間の差の**オーダー**が、run 内のばらつきと同程度だった
- **run 内のばらつき自体が run ごとに 3.5 倍違う** (1.2% と 4.1%)。閾値は
  「run 内のばらつきの代表値」ではなく**その上振れ**を見て決める必要がある

また `machine_key` が一致しても**同じ物理マシンとは限らない** — D106
Revisit condition (3) が「`cpu_model` の文字列だけでは粒度が粗すぎる
可能性」を既に挙げている。

**D132 で週次の蓄積が始まるので、この 1 点は「最初の 1 点」として扱う。**

## 46. 復帰後の「揺り戻し」— 戻ったタブは再休止されない。ただし 50 タブでは `mark` の時点で回収が終わっていなかった (Issue #279 / #176 Stage 3, 2026-09-16)

D110 Revisit condition (3) が残していた「復帰後の揺り戻し (戻したタブが
次のメモリ判定で再び休止される) を測っていない」を、`tabs_hold_bounce_N`
(`Scenario::TabCountMemoryBounce`、D145) と 3 つの導出メトリクス
(`tab_resuspend_count` / `tab_resuspend_revisited_count` /
`tab_resuspend_delay_ms`) で測った。**3 本の run で、予想していなかった
形の答えが出た。**

| # | 目的 | シナリオ | A | B | 末尾の待ち | run | 機種 |
| --- | --- | --- | --- | --- | ---: | --- | --- |
| 1 | 揺り戻しの有無 | `tabs_hold_bounce_20,_50` | 既定 | `VELOX_MEMORY_BUDGET_MB=0` | 12 秒 | [35123564647](https://github.com/noan98/VeloX/actions/runs/35123564647) | AMD EPYC 7763 |
| 2 | 窓の延長 + グループ回収の切り分け | `tabs_hold_bounce_50` | 既定 | `VELOX_MAX_TABS_PER_PROCESS=1` | 22 秒 | [35125998701](https://github.com/noan98/VeloX/actions/runs/35125998701) | AMD EPYC 9V45 |
| 3 | **対照** (復帰を一切しない) | `tabs_hold_50` | 既定 | `VELOX_MEMORY_BUDGET_MB=0` | (`mark` 後 8 秒) | [35127353640](https://github.com/noan98/VeloX/actions/runs/35127353640) | AMD EPYC 9V45 |

いずれも `minimal.html`、trials 2 × repeats 2、4 論理コア / 15.99 GiB
(予算は D114 の RAM 相対で 1023 MiB)、`common_env` 空 (既定の 5 秒周期)。
run 1 と run 2/3 は機種が違うので、**絶対値を run をまたいで比べては
ならない** (D96 決定 3)。以下は原則として同一 run 内の事実に限る。

### 46.1 20 タブでは何も起きない — §39 の再確認

run 1 の `tabs_hold_bounce_20` は A/B とも `suspended_tab_count` 0、
`tab_resuspend_count` 0、27 プロセス、685.6 / 682.8 MiB。§39.1 のとおり
`LOW` で予算に届かないので、休止も復帰も揺り戻しも発生しない。
**Windows の既定設定では、揺り戻しを測れるタブ数は 50 だけ**である。

### 46.2 50 タブ — 戻ったタブ自身は一度も再休止されなかった

3 本の run の A 腕 (既定) をまとめる。

| run | 窓 | `suspended_tab_count` (trial あたり) | `tab_resuspend_count` (`mark` 後) | `tab_resuspend_revisited_count` | `tab_resume_ms` |
| --- | ---: | ---: | ---: | ---: | ---: |
| 1 | 12 秒 | 24 / 24 | **12 / 12** | **0 / 0** | 126.0 / 140.9 |
| 2 | 22 秒 | 28〜32 / 28 | **16〜20 / 16** | **0 / 0** | 90.0 / 87.5 |
| 3 (対照、復帰なし) | 8 秒 | 12 / 16 | **4 / 4** | 0 / 0 | — |

**`tab_resuspend_revisited_count` は全 run・全 trial で 0** (run 1〜2 の
A/B 4 腕 × 2 trial × 4 復帰 = 32 回の復帰のうち、戻ったタブがその後に
再休止された例は無い)。利用者が戻ったタブは最も新しく使ったタブになり、
最長未使用から取る `reclaim_order` はそれを最後まで残す。**「戻したタブが
すぐまた消える」という意味の揺り戻しは、今の設計では起きていない。**

### 46.3 しかし `mark` 後の休止は復帰への反応ではなかった — 対照が示したこと

run 1 の `tab_resuspend_count` 12 (4 回の復帰に対して 3 倍) を最初は
揺り戻しと読んだ。**run 3 の対照がそれを否定した。** `tabs_hold_50` は
`mark` の後に何もせず 8 秒待つだけだが、その 8 秒に **4 件の `tab_suspend`
が入っている**。復帰が無くても休止は続いていた。

`tab_resuspend_delay_ms` の分布も同じことを言っている。

| run / 腕 | サンプル数 | min | median | max | stddev |
| --- | ---: | ---: | ---: | ---: | ---: |
| 1 / A | 24 (12 × 2) | 1.70 秒 | 6.75 秒 | 11.79 秒 | 4.17 秒 |
| 2 / A (repeat 1) | 36 (18 + 20 — 注) | 1.72 秒 | 11.75 秒 | 21.78 秒 | 6.67 秒 |
| 2 / A (repeat 2) | 32 (16 × 2) | 1.74 秒 | 9.27 秒 | 16.79 秒 | 5.68 秒 |
| 2 / B (`MAX_TABS_PER_PROCESS=1`) | 31 / 30 | 1.74 秒 | 11.77 秒 | 21.79 秒 | 6.96 秒 |

(注: run 2 / A repeat 1 の 2 trial は 16 件と 20 件。他はすべて trial 間で一致)

run 1 の 24 サンプルは「1.7 / 6.7 / 11.7 秒に 8 件ずつ」と置くと平均
6.70 秒・標準偏差 4.17 秒になり、実測 (6.74 / 4.17) と一致する。
run 2 は同じ位相で 16.7 / 21.7 秒まで続く。つまり **最後の復帰の 1.7 秒後
から 5 秒周期で、1 回につき 4 タブずつ**休止が進み、**22 秒の窓の端
(21.8 秒) でもまだ止まっていない**。復帰の回数 (4) にも、復帰から何秒
経ったかにも依っていない — 周期に依っている。

対照 (run 3) の `mark` 後 8 秒に 4 件 = 1 スイープ分、というのもこの
「5 秒に 4 タブ」と整合する。

**したがって、50 タブの `tabs_hold_bounce_50` で `mark` 後に見えた
12〜20 件の休止は、復帰への反応ではなく「タブを開き終えてから続いている
回収」である。** 復帰が上乗せした分があるとしても、この 3 run の集計値
からは分離できない (窓の中の休止数は復帰の有無に依らず周期 × 4 で決まって
いる)。

### 46.4 副産物: §27.5 の前提は Windows の 50 タブでは成立していない

`tabs_hold_N` は「12 秒待てば既定周期の判定が 2 回以上走り、回収が
起きるなら起き切る」(§27.5 / D97) を前提に `mark` を打つ。**その前提は
50 タブ + `LOW` では崩れている。** 対照の A 腕は `mark` 後の 8 秒にも
4 件休止しており、`suspended_tab_count` が trial 間で 12 と 16 に割れて
いるのは (§32 事実 3 で「回収がどこまで進むかは実行ごとに違う」と記録した
のと同じ) 収束途中の値を切り取っているからである。

これは D145 決定2 が `tab_resuspend_count` に持たせた副次的な役割
(「`tabs_hold_N` で 0 でなければ、§27.5 の前提が崩れている合図」) が
**最初の実測でそのまま発動した**形になる。§31 / §40 の `tabs_hold_50` の
着地値 (527.9 MiB / 1014.8 MiB) は「落ち着いた後」ではなく「開き終えて
20 秒後」の値として読み直す必要がある。**値そのものは間違っていない**
(同じ手順なら同じ時点の値が出る) が、「定常値」という説明が過大だった。

### 46.5 収束が遅い理由は、この集計からは決められない

「5 秒に 4 タブ」が何で決まっているかは 2 つの仮説がある。

1. **疑似プロセスグループの丸ごと回収。** Windows でもグループ id
   (最大 4 タブ) は振られ、`reclaim_order` はグループを空にする単位で
   取る (`process_group_of` の doc comment は「実プロセスに対応しないが
   無害」と書いている)。→ **run 2 の B 腕 (`VELOX_MAX_TABS_PER_PROCESS=1`、
   グループを 1 タブにする) でも 15〜16 件 / 4 スイープ ≒ 4 タブずつ**
   だった。**この仮説は否定された。**
2. **1 タブあたりの見込み解放量 (`ESTIMATED_BYTES_PER_TAB` = 64 MiB、
   D54 の Linux 実測) が Windows + `LOW` の実態 (§40.2: 生きタブ 1 つ
   あたり 29 MiB) より大きく、要求が小さく出て収束に回数が要る。**
   集計値とは矛盾しないが、「なぜ毎回ちょうど 4 か」までは説明できない。
   確かめるには**スイープごとの `rss` サンプルと `tab_suspend` の時系列**
   (perf ログの生の JSON Lines) が要るが、perf-windows の artifact には
   集計後の JSON しか残っていない。

D46 に従い、ここでは決めない。**時系列を artifact に残す**のが次の
計測基盤の一手になる。

### 46.6 この計測の限界

1. **復帰が上乗せした休止を分離できていない** (46.3)。分離するには、
   復帰の前に回収が本当に止まった状態を作る必要がある — `mark` までの
   待ちを伸ばすか、予算が飽和しない中間のタブ数 (20 と 50 の間) を使うか。
   どちらも `tabs_hold_N` との地続き性 (D110 決定3) を壊さずには足せない
   ので、別シナリオとして設計し直す話になる。
2. **`minimal.html` のみ。** 重いページでは 20 タブでも予算に届く
   (§40.2 の `dom_heavy` で 4 休止) ので、そちらなら「収束済みの状態から
   戻す」が作れるかもしれない。未計測。
3. **B 腕 (`VELOX_MAX_TABS_PER_PROCESS=1`) の副作用。** run 2 で B は
   `mark` 前の休止が 8〜9 件 (A は 12) と少なく、総量が 130 MiB 多い。
   グループ丸ごと回収の「取りすぎ」が Windows でも実際に効いていた
   ということだが、これは本節の問いではないので数字だけ残す。
4. **20 タブでは測れない** (46.1)。Windows の既定設定で揺り戻しが
   起こり得るのは予算が発火する負荷だけである。

## 47. 休止スイープの時系列 — 「5 秒に 4 タブ」は収束の途中ではなく定常状態。1 スイープで返した分が次の判定までに戻る (Issue #176 Stage 3 / D148, 2026-09-21)

§46.5 が「時系列を artifact に残すのが次の一手」と書いた問いを、
D147 (`velox-bench run --keep-logs`) と D148 (`perf_log_timeline.py`、
Job Summary の「休止スイープの時系列」) で初めて読んだ。**問いの前提
(収束が遅い理由を探す) のほうが崩れた。**

| 項目 | 値 |
| --- | --- |
| run | [35562270535](https://github.com/noan98/VeloX/actions/runs/35562270535) (PR #283 のブランチで `workflow_dispatch`) |
...
表の 1 行は判定 1 回 (`tab_suspend` の 200 ms 以内の連続)。「要求」は
直前の `rss` から `tabs_to_free` と同じ式で出した値で、その `rss` が
判定の何 ms 前に採れたかを「古さ」として併記している (D148 決定1・2)。

### 47.1 A は毎回ちょうど 4 タブ — 要求とは無関係で、疑似プロセスグループの上限そのもの

A の 4 試行・30 スイープは**すべて 4 タブ**だった。`rss` が判定の直前に
採れていた行 (古さ ≤ 73 ms) に限ると:

| 試行 | 直前 rss (MiB) | 超過 (MiB) | 要求 | 実際 |
| --- | ---: | ---: | ---: | ---: |
| baseline-1 trial 1 | 1070.6 | 46.9 | 1 | **4** |
| baseline-1 trial 2 | 1136.1 | 112.5 | 2 | **4** |
| baseline-2 trial 1 | 1132.3 | 108.6 | 2 | **4** |
| baseline-2 trial 2 | 1133.3 | 109.6 | 2 | **4** |

4 は `VELOX_MAX_TABS_PER_PROCESS` の既定値で、`reclaim_order` が
グループを丸ごと取る (`suspension.rs`) 結果である。**§46.5 が「B 腕でも
≒4/スイープだったから否定」とした仮説 1 は、A の「4」の説明としては
正しかった。** §46 は 4 スイープ分の合計 15〜16 件を見ていたが、
スイープ単位では B は 3 である (47.2)。合計が近かったのは、A が
要求 1〜2 に対し 4 取り、B が要求 3 に対し 3 取っていたためで、
別々の理由で似た数になっていた。

### 47.2 B は要求どおりに取れている — `tabs_to_free` は設計どおり

B (1 タブ = 1 グループ) の 32 スイープは 3 タブが 26 回、4 が 4 回、
2 が 2 回。直前の `rss` が新鮮な行 (古さ 20〜40 ms) は 4 試行とも
**1172〜1176 MiB → 超過 約 150 MiB → 要求 3 → 実際 3** で一致した。
B の表で ⚠️ (実際 > 要求) が付いた行はすべて `rss` が 2〜5 秒古い行で、
超過の証拠にはならない (47.11)。

### 47.3 1 スイープで返るのは見込みの 25〜40%

直前と直後の `rss` の差を休止数で割る:

| 腕 | 差 (MiB) | 休止 | 1 タブあたり (MiB) |
| --- | ---: | ---: | ---: |
| A | −104 / −99 | 4 | 25〜26 |
| B | −76 / −41 / −41 / −45 | 3 | 14〜25 |

見込みの 64 MiB (D54 の Linux 実測) に対して **14〜26 MiB**。§40.2 の
「`LOW` 下の生きタブ 1 つ 29 MiB」と整合する。**仮説 2 の前提 (見込みが
Windows + `LOW` では過大) は成立している** — ただし、それが収束の遅さの
理由かどうかは 47.4 で否定される。

### 47.4 本題: 次の判定までに RSS が戻る — 定常状態であって収束途中ではない

復帰ラウンド (`MEMORY_RESUME_ROUNDS` = 4、各 500 ms) は `mark` の
2 秒後までに終わるので、`+3.3 s` 以降のスイープ間に復帰は無い。その
区間で、スイープ直後の `rss` と次のスイープ直前の `rss` を並べる:

| 腕 / 試行 | 直後 → 次の直前 (MiB) | 5 秒での増加 |
| --- | --- | ---: |
| A baseline-1 trial 1 | 971.4 → (判定は >1024 を見た) → 1041.6 | 約 +70 |
| A baseline-1 trial 2 | 1032.3 → 1049.1 | +17 |
| B compare-1 trial 1 | 1095.9 → 1173.2 | +77 |
| B compare-1 trial 2 | 1133.3 → 1174.3 | +41 |
| B compare-2 trial 1 | 1132.4 → 1174.2 | +42 |
| B compare-2 trial 2 | 1131.5 → 1174.0 | +43 |

**B は 1 サイクルで約 41 MiB 返し、約 41 MiB 戻る** — 純増減ゼロの
定常状態で、`rss` は 1130〜1190 MiB の帯から出ない (1186 → 1158 → 1172
→ 1174 → 1133 → 1174 → 1152)。A は返しが約 100 MiB・戻りが約 70 MiB で、
予算 1024 MiB のまわりを ±50 MiB で往復する (1070 → 971 → 1041 → 1020)。

したがって §46.4 の「5 秒に 4 タブ」は**収束の途中ではなく定常状態**で
ある。§46.5 の 2 つの仮説はどちらも「収束が遅い理由」を探していたが、
そもそも収束に向かっていない。見込み解放量を 25 MiB に直して要求を
2〜3 倍にしても、戻りが返しに比例するなら定常状態の水位が下がるだけで、
5 秒ごとに休止し続けることは変わらない。**D46 に従い、
`ESTIMATED_BYTES_PER_TAB` の OS 別化 (D145 Revisit (1) 後半) は戻りの
正体が分かるまで行わない。**

何が戻っているかはこの表からは決められない。候補は (a) 残った生きタブ
...
既にあるので、表にスイープ直前→直後の列を足した (PR #284)。次の run で
新しい記録なしに (a)〜(c) をある程度切り分けられる。

### 47.5 内訳を足して読み直す — `Discard` はプロセスを 1 タブ 1 つ終わらせ、browser は動かず、戻るのは残った engine 側 (run 35563243278)

表に `process_count` と browser / engine の内訳 (スイープ直前→直後) を
足して、同じ条件・同じ機種 (AMD EPYC 7763 / 15.99 GiB) で回し直した
(run [35563243278](https://github.com/noan98/VeloX/actions/runs/35563243278)、
PR #284)。47.1〜47.4 はすべて再現した (A は毎回 4、B は要求 3 に対して
3、返した分が 5 秒で戻る)。

1. **`Discard` はプロセスを 1 タブ 1 つ終わらせている。** 復帰の無い
   区間 (`+13 s` 以降) で、A は 4 休止ごとにプロセスが 41→37、37→33、
   B は 3 休止ごとに 46→43、47→44。50 タブでプロセス 48〜55 なので、
   このランナーでは 1 タブ 1 レンダラプロセスであり、47.4 の候補 (b)
   「プロセス共有で `Discard` してもレンダラが残る」は否定される。
2. **browser プロセスは動かない。** `browser_rss_bytes` は全試行・全行で
   26〜30 MiB (±0.5 MiB)。候補 (c) のうち browser 側は否定。GPU /
   ユーティリティは engine 側に含まれるので未分離。
3. **4 プロセス終わらせながら engine が増える。** A の `+13.3 s` の
   スイープで engine は 997→1065 (+68)、1004→1023 (+19)、950→1020 (+70)、
   1004→1019 (+15) MiB、いずれもプロセス 41→37。消えた 4 プロセスを
   1 つ約 25 MiB とすると −100 MiB 相当なので、**残った 37 プロセスが
   その 5 秒で 100〜170 MiB 増えている**。B も同じ形 (46→43 で +41、
   45→41 で +71)。
4. `+3.3 s` の行 (復帰 4 回の直後) では、B の 1 試行でプロセスが 52→44
   と 8 つ減りながら RSS が +56 MiB 増えた。復帰で作り直した 4 つの
   webview (1 つ 30〜40 MiB) が乗ったと読める — 復帰コストがメモリの側
   からも見えた形。

「残ったレンダラのどれが、なぜ膨らむか」は総量では決められない。残る
候補は (a) `LOW` ヒントで縮んだ背景タブが時間とともに戻る、(a') 復帰で
作り直した (まだ `LOW` が掛かっていない) タブが膨らむ、(c') GPU /
ユーティリティプロセス。perf サンプラが 5 秒周期なので、増え方が
「スイープ直後に一気に」か「5 秒かけて漸増」かも見えていない。次は
perf の RSS サンプル間隔を workflow から指定できるようにして (
`velox-bench run --rss-interval-ms` は既にあり、明示すれば D50 の自動
調整に勝つ) 1 秒周期で読む。漸増なら (a)/(a')、段なら別の何かであり、
(a) と (a') は復帰を一切しない `tabs_hold_50` を同じ窓で比べれば分かれる
(§46 run 3 の形)。

### 47.6 1 秒周期で採り直す — 「戻り」は定常的な漏れではなく、復帰で作り直した webview が 15〜20 秒かけて育つ過渡 (run 35564048530)

PR #284 で足した `rss_interval_ms` 入力を 1000 にして同じ条件で回した
(run [35564048530](https://github.com/noan98/VeloX/actions/runs/35564048530)、
機種は AMD EPYC 9V74 / 15.99 GiB で 47.1〜47.5 の 7763 とは違うため、
絶対値は並べない。読むのは形である)。

1. **全行が新鮮になった。** 「古さ」は全 60 行で 810〜983 ms。perf
   サンプラは判定の約 0.1 秒後に刻む (どちらも起動時刻基準の周期で位相が
   揃う) ので、直前の値は常に 0.9 秒前になる。「要求」列が全行で読め、
   結果は 47.1 / 47.2 と同じ: A は要求 1〜3 に対して常に 4、B は要求 3
   に対して 3 (要求 2 に対して 3、要求 3 に対して 4 の行が数行)。
2. **戻りは 5 秒窓ごとに見ると山になっている。** 復帰 (mark 直後 2 秒
   以内) より後の各窓で、スイープ直後の rss → 次のスイープ直前の rss の差:

   | 窓 (mark 基準) | A の 3 試行 (MiB) | B の 4 試行 (MiB) |
   | --- | --- | --- |
   | +4 → +8 | −8 / −7 / −29 | +13 / +14 / +13 / +13 |
   | +9 → +13 | +27 / +76 / +27 | +41 / +42 / +42 / +37 |
   | +14 → +18 | **+90 / +38 / +90** | **+88 / +90 / +89 / +40** |
   | +19 → +23 | −6 | −5 / −6 / −6 / −5 |

   B の 4 試行は小数点まで揃い、ノイズではなく**時刻に固定された決定的な
   動き**である。漸増ではなく復帰の約 6〜16 秒後に集中する山で、**+18 秒
   を過ぎると止まる** (最後の窓は全試行で −5〜−6)。合計は B で約 140 MiB、
   復帰した 4 タブで割ると **1 タブ約 35 MiB** — 47.5 の 4 で読んだ
   「復帰で作り直した webview 1 つ 30〜40 MiB」と一致する。
3. **mark の前にも同じ形がある。** タブを開き終えるのは mark の 12 秒前
   だが、−11.7 → −6.7 の窓で +10〜+24 MiB 増えており、開いたばかりの
   webview も同じように育っていると読める。

**47.4 の「定常状態」は言い過ぎだった。** 正しくは: (1) webview は作られ
てから 15〜20 秒かけて約 35 MiB 育つ (`minimal.html` でも)、復帰も新規
タブも同じ。(2) メモリ判定は 5 秒ごとに今の値を見るので、この育ちを
あとから 3〜4 回に分けて発見し、そのたびに背景タブを 3〜4 個 `Discard`
する。(3) 22 秒の窓はこの過渡がちょうど収まるまでの長さで、B の最後の
窓が −5 なのは収束し始めたということ (窓を伸ばせば止まるはずだが未確認)。

したがって「5 秒に 4 タブ」は、復帰した 4 タブの分のメモリ (約 140 MiB)
を育ち終わるまで払い続けている姿であり、予算としては筋の通った動きで
バグではない。ただし**復帰 1 回につき背景タブ 4〜6 個が消える**のは
D105 の状態喪失の観点では重く、「若い webview はこれから約 35 MiB 育つ」
ことを判定が織り込めば (例: 作成後 20 秒以内の webview 1 つにつき見込み
+35 MiB を先に足す) 1 回のスイープで済ませて後続の 3 回を省ける。これは
設計案であってまだ何も決めていない (D46)。

未確認: **予算を切った腕 (`VELOX_MEMORY_BUDGET_MB=0`) で同じ山が出るか**
(出れば「webview が育つ」が予算と無関係に確定する)。スイープの表は休止の
行しか出さないので、休止の無いログの rss の推移を読む見方 (mark 基準
5 秒刻み、`perf_log_timeline.py --rss-track`) を足してから回す。

### 47.7 予算 OFF の腕でも同じ山が出る — 休止も webview の作り直しも無いのに、復帰ラウンドの後 15 秒で約 +300 MiB (run 35601333889)

`--rss-track` (mark 基準 5 秒刻みの rss の推移。休止の無いログでも読める)
で、B = `VELOX_MEMORY_BUDGET_MB=0`、`rss_interval_ms=1000`、機種は 47.1〜
47.5 と同じ AMD EPYC 7763 / 15.99 GiB で回した
(run [35601333889](https://github.com/noan98/VeloX/actions/runs/35601333889)、
PR #286)。

B (予算 OFF、休止ゼロ、プロセス 57 のまま) の 5 秒窓ごとの rss の差:

| 窓 (mark 基準) | 試行 1 | 試行 2 | 試行 3 | 試行 4 |
| --- | ---: | ---: | ---: | ---: |
| +0 → +5 | −58 | −74 | −8 | −58 |
| +5 → +10 | **+112** | **+101** | **+69** | **+119** |
| +10 → +15 | **+128** | **+115** | **+120** | **+120** |
| +15 → +20 | **+89** | **+81** | **+89** | **+89** |

(単位 MiB。+25 の行は `quit` の途中のサンプルで読めないため、PR #286 で
刻みの時刻にサンプルが追いついている行だけを出すよう直した。)

**プロセス数は 57 のまま変わらず、browser は 26〜29 MiB で不動。** engine
側だけが +5〜+20 で約 +300〜330 MiB 増え、4 試行が小数点まで揃う。+20 の
時点でまだ +89/5 秒で増えており、**止まったことは確認できていない**
(窓が 22 秒で終わる)。

**47.6 の読みを訂正する。** B では予算 OFF なので休止が 1 つも無く、
`resume` コマンドは webview を作り直していない (休止していないタブへの
`resume` はそのタブを表示するだけ)。それでも同じ山が出た以上、原因は
作り直しではなく**タブを表示 (アクティブ化) したこと**である:

1. mark 直後の 4 回の「復帰」は、B ではただの 4 回のタブ切り替え。
2. その 5〜20 秒後に engine が約 300 MiB 増える (切り替え 1 回あたり
   約 75 MiB。線形かは未確認)。
3. A (予算 ON) では同じ増え方を 16 タブの `Discard` (1 つ約 25 MiB =
   約 400 MiB) で相殺し、rss はほぼ横ばい (同 run の A: +0 1082 → +20
   1086 MiB、休止 16)。**§46 以来追ってきた「5 秒に 4 タブ」は、タブを
   切り替えたことによる増分を背景タブから徴収し続けている姿**である。
4. mark の前 (タブを 1 つずつ開いてはアクティブにする区間、47.6 の 3)
   に同じ形が見えたのも、これで説明がつく。

何が増えているかはプロセス単位では見えていない。候補は、表示された
タブのレンダラが確保するコンポジタ / ラスタのメモリ (隠れていた webview
は持っていない) と GPU プロセス。どちらも engine 側に含まれる。

次に要るもの: (1) 窓を伸ばして止まるのか戻るのかを見る —
`MEMORY_BOUNCE_SETTLE_MS` (22 秒、D145) は定数なので、計測用に上書き
できる入り口 (自動操作スクリプトの生成側に環境変数 1 つ) を足す (Rust
側、別 PR)。(2) 復帰ラウンド数 (`MEMORY_RESUME_ROUNDS` = 4) を 1 / 2 / 4
で振って「1 回あたり約 75 MiB」かどうかを見る (同じ入り口)。(3) プロセス
ごとの rss を perf ログに書く (1〜2 で形が決まってから)。設計への含意は
D46 に従いまだ決めない。

### 47.8 60 秒窓と「切り替えなし」の対照 — 増分はタブ操作と無関係で、レンダラが作られてからの経過時間に固定された 2 段の階段 (run 35605075352 / 35606650730)

PR #287 (D149) の計測用の上書きで 2 本回した。どちらも `tabs_hold_bounce_50`、
A = 既定 / B = `VELOX_MEMORY_BUDGET_MB=0`、`rss_interval_ms=1000`、
trials 2 × repeats 2。機種は run 35605075352 が AMD EPYC 9V45、
run 35606650730 が 9V74 (どちらも 4 論理コア / 15.99 GiB)。

| run | 上書き | 意味 |
| --- | --- | --- |
| [35605075352](https://github.com/noan98/VeloX/actions/runs/35605075352) | `bounce_settle_ms=60000` (ラウンドは既定の 4 回) | 止まるのか・戻るのか |
| [35606650730](https://github.com/noan98/VeloX/actions/runs/35606650730) | `bounce_settle_ms=60000` + `resume_rounds=0` | **切り替えを一切しない対照** |

B (予算 OFF、休止ゼロ、プロセス 57 のまま) の 5 秒窓ごとの rss の差 (MiB):

| 窓 (mark 基準) | 切り替え 4 回 (4 試行) | 切り替えなし (4 試行) |
| --- | --- | --- |
| +5 → +10 | +113 / +113 / +113 / +113 | +115 / +115 / +104 / +114 |
| +10 → +15 | +128 / +128 / +128 / +129 | +124 / +124 / +113 / +123 |
| +15 → +20 | +90 / +89 / +89 / +90 | +81 / +82 / +75 / +82 |
| +20 → +35 | 0 (完全に横ばい) | 0 (完全に横ばい) |
| +35 → +40 | +47 / +62 / +47 / +47 | +47 / +48 / +2 / +50 |
| +40 → +45 | +51 / +44 / +51 / +53 | +52 / +52 / +5 / +49 |
| +45 → +50 | +51 / +49 / +51 / +52 | +52 / +51 / +7 / +51 |
| +50 → +60 | ≈ 0 | ≈ 0 |

(切り替えなしの試行 3 だけは開始時点の rss が 1412 MiB と他より 170 MiB
高く、第 2 の山がほぼ無い。第 2 の山を先に済ませていたように見えるが、
1 試行なので保留。)

1. **切り替えは関係ない。** `resume_rounds=0` でも、4 回切り替えた run と
   同じ時刻に同じ大きさの山が出る。47.6 の「復帰で作り直した webview が
   育つ」も 47.7 の「タブを表示したことが原因」も撤回する。
2. **止まる。ただし 2 段。** 第 1 の山 (+5〜+20、約 +320 MiB) → 15 秒の
   横ばい → 第 2 の山 (+35〜+50、約 +150 MiB) → 横ばい。60 秒では戻らない。
3. **時刻はプロセスの起動基準で固定。** mark は毎回起動の 27.2 秒後なので、
   第 1 の山は起動 32〜47 秒、第 2 は 67〜77 秒。タブは起動 0.5〜15 秒の間
   に 0.3 秒間隔で開いているので、**レンダラ 1 つが作られてから約 30 秒後
   と約 65 秒後にワーキングセットが段階的に増え、それが 15 秒の開き幅の分
   だけ滲んで山に見える**と読むと辻褄が合う (仮説。確かめるにはプロセス
   ごとの系列が要る)。1 レンダラあたり第 1 の山 320 ÷ 49 ≈ 6.5 MiB、第 2
   の山 150 ÷ 49 ≈ 3 MiB。
4. **予算 ON の腕では、この 2 段の山を背景タブの `Discard` で相殺している
   のが「揺り戻し」の正体。** 切り替えなしの A 腕でも mark 後に 4 タブ ×
   4〜5 回休止しており (+5 / +15 / +20 / +25、第 2 の山に対して +55 でもう
   1 回)、§46 以来「復帰後の再休止」として追っていたものは、復帰とは無関係
   に起きる**開いたばかりのレンダラの成長分の徴収**だった。

含意: (1) **`tabs_hold_N` の「開き終えてから 12 秒待って mark」(D110) は
Windows では短すぎる。** 最後のタブを開いてから約 75 秒経つまでワーキング
セットは段階的に増え続けるので、それより前の値は定常値ではない (§46.4 /
D145 Revisit (3) の理由と必要な長さ)。(2) 判定が見ている量は
`WorkingSetSize` (ワーキングセット全体、共有ページ込み、`metrics.rs`) で、
この増分が実際の確保量 (コミット) の増加なのか、GC などがページを触って
ワーキングセットに乗っただけなのかは分からない。`PROCESS_MEMORY_COUNTERS`
の `PagefileUsage` (私的コミット) を `rss` レコードに並べれば切り分けられる。
(3) `ESTIMATED_BYTES_PER_TAB` の OS 別化 (D145 Revisit (1) 後半) は引き続き
見送り — 分かったのは判定の入力側 (測っている量が起動後 75 秒まで動く) の
問題で、見込み解放量の問題ではない。

次: `rss` レコードに私的コミットを足す (Windows のみ、D42 の追記規則、
→ D150) → 山がコミットかワーキングセットかで、`tabs_hold_N` の settle を
伸ばすか、判定が「作成後 75 秒以内のレンダラ」を別扱いするか、予算の入力を
私的コミットに替えるかを D46 に従って数字で決める。

### 47.9 私的コミットを並べる — 2 段の山はどちらも確保量の増加ではなく、コミット済みのページがワーキングセットに乗っただけ (run 35610806143、D150)

PR #288 (D150) で `rss` レコードに `PagefileUsage` (私的コミット) を足し、
47.8 の「切り替えなし」と同じ条件 (`tabs_hold_bounce_50`、A = 既定 /
B = `VELOX_MEMORY_BUDGET_MB=0`、`rss_interval_ms=1000`、
`bounce_settle_ms=60000`、`resume_rounds=0`) で回した。機種は AMD EPYC
7763 (4 論理コア / 15.99 GiB)。`trials` を既定の 10 のまま起動したため
[run 35610806143](https://github.com/noan98/VeloX/actions/runs/35610806143)
...
9 試行の代表値。試行 5 だけ第 2 の山が +30→+35 に 1 段で +151 / +22
と早く来た以外は全試行同じ):

| 窓 (mark 基準) | rss (WorkingSet) の差 | 私的コミットの差 |
| --- | ---: | ---: |
| +0 → +5 | −5〜−8 | **−50** |
| +5 → +10 | **+114** | −2 |
| +10 → +15 | **+123** | −2 |
...
絶対値: mark の時点で rss 1242 MiB に対し私的コミット 1512 MiB (engine
側 1504 MiB)。**コミットの方がワーキングセットより 270 MiB 大きい** —
確保してあるが常駐していないページがそれだけある。

1. **第 1 の山 (+320 MiB) は確保量の増加ではない。** 同じ 15 秒で私的
   コミットはむしろ 50 MiB 減って、その後は横ばい (−2 / 5 秒)。増えた
   ワーキングセットは、既にコミット済みだったページ (270 MiB の差分の
   中) か、共有 / ファイル裏付きのページ (コミットに数えない) が触られて
   常駐になった分。
2. **第 2 の山 (+150 MiB) もほぼ同じ。** コミットは +16 MiB (5 MiB × 3
   窓) しか動いていない。約 1 割だけが新規確保で、残りはページを触った
   分。
3. **A (予算 ON) の `Discard` は 4 タブで私的コミットを約 95〜100 MiB
   返す** (1 タブ約 24 MiB。ワーキングセットの戻りは同じ 4 タブで
   約 87 MiB、47.3 の値と同じ)。10 試行すべてで、+5 / +15 / +20 / +25 の
   4 スイープ (計 16 タブ) がコミットを 1231 → 787 MiB まで落とし、その
   後 +50 でワーキングセットだけが +50 MiB 増える (コミット +5) と、
   試行によっては +55 で 5 回目のスイープが来る。

含意: **判定が徴収しているのは、確保量が増えたわけではないページの
常駐化である。** `WorkingSetSize` を予算と比べる限り、レンダラが起動後
30〜75 秒でページを触る (GC / JIT / コンポジタの初期化と読める) たびに
背景タブを落とし続けることになり、47.4 の「5 秒に 4 タブ」と §46 の
「揺り戻し」はその帰結だった。逆に私的コミットは (a) タブを開き終えた
時点で既に定常で、(b) `Discard` に 1 タブ約 24 MiB で確実に応じ、
(c) 起動後の階段を持たない — 予算の入力として求めていた性質を全部
持っている。

次: D150 Revisit (1) の後半に従い、**Windows の判定の入力を私的コミット
(`total_private_bytes`) に替える**検討に進む。あわせて Linux の
`RssAnon:` を同じフィールドに入れて対称にし、`ESTIMATED_BYTES_PER_TAB`
(64 MiB) は実測 (Windows で 1 タブ約 24 MiB) との差が大きいので、入力を
替えた後の A/B で見直す (D145 Revisit (1) 後半、D46)。`tabs_hold_N` の
settle (12 秒) は、私的コミットを見る限り伸ばす必要がない。

### 47.10 判定の入力を私的コミットにした A/B — B は開き終える前に収束し、mark 後の休止はゼロ、休止は 28 → 20 タブ (run 35618824317、D151)

PR #289 (D151) の `VELOX_MEMORY_BUDGET_INPUT` で、A = 既定 (`resident`、
ワーキングセット) / B = `private` (私的コミット) を同じ条件で回した:
`tabs_hold_bounce_50`、`trials=4` × repeats 2、`rss_interval_ms=1000`、
`bounce_settle_ms=60000`、`resume_rounds=0` (切り替えなしの 60 秒保持)、
[run 35618824317](https://github.com/noan98/VeloX/actions/runs/35618824317)、
機種 Intel Xeon Platinum 8573C (4 論理コア / 15.99 GiB、予算 1024 MiB)。
今回は予算が両腕で ON なので、休止スイープの表 (「超過 / 要求」は腕ごとの
入力で計算、D151 決定2) がそのまま比較になる。

| | A (`resident`、8 試行) | B (`private`、8 試行) |
| --- | --- | --- |
| mark 前のスイープ | 3 回 (−11.7 / −6.7 / −1.7)、4 タブずつ = 12 | 4 回 (−16.7 / −11.7 / −6.7 / −1.7)、4 / **8** / 4 / 4 = 20 |
| **mark 後のスイープ** | **4 回** (+3.3 / +13.3 / +18.3 / +23.3、4 タブずつ = 16)、試行 1-1 は +48.4 にもう 1 回 | **0 回** (6 試行)、+3.3 に 1 回 (2 試行) |
| 休止の合計 | 28 タブ (8 試行とも) | 20 タブ (6 試行) / 24 タブ (2 試行) |
| mark 後のプロセス数 | 45 → 29 (減り続ける) | 37 のまま |
| 私的コミット (+0 → +60) | 1222 → 775 MiB | 1024 → 971 MiB (予算 1024 の下で横ばい) |
| ワーキングセット (+0 → +60) | 1082 → 1013 MiB (±50 で往復) | 964 → 1225 MiB (2 段の階段、休止なし) |

1. **B は mark の前 (タブを開いている最中) に収束する。** 4 回目のスイープ
   (−1.7) の直後に私的コミットは 1017〜1020 MiB で予算の下に入り、以降
   60 秒間、判定は何も命じない。A は mark 後も 4〜5 回休止し続け、§46 以来
   の「5 秒に 4 タブ」がそのまま出ている。
2. **B では 2 段の階段 (47.8 / 47.9) がそのまま見えるが、休止に変換
   されない。** +10〜+20 で +122 / +83、+40〜+50 で +37 / +51 とワーキング
   セットは増えるが、私的コミットは −3 / +5 しか動かず、予算の判定はそれ
   を見ていない。**「揺り戻し」は消えた。**
3. **休止は少なくて済む。** B は 20 タブ (2 試行で 24) で予算に収まった。
   A は 28 タブ落としてもワーキングセットが階段で戻り続け、+48 でまだ
   落としている。B の 1 スイープ目 (−16.7、4 タブ) は超過 0〜18 MiB での
   休止で、直前の `rss` サンプル (1 秒古い) が予算の下でも判定は上を見て
   いた — 見込み解放量 64 MiB に対して要求 1 → 実際 4 (グループ丸ごと)。
   2 スイープ目 (−11.7) は要求 6 に対して 8 (2 グループ)。ここでも
   「実際 ≥ 要求」で、`ESTIMATED_BYTES_PER_TAB` (64 MiB) が実測の 1 タブ
   約 24 MiB (47.9) より大きいことは収束を遅らせていない — グループ単位の
   丸め (47.1) が吸収している。
4. **B の方がワーキングセットは大きい** (+60 で 1225 vs 1013 MiB)。ただし
   その差は「確保していないのに常駐しているページ」で、私的コミットは
   B の方が 200 MiB 大きいだけ (971 vs 775) — A はその 200 MiB を 8 タブ
   分の状態喪失 (D105) と復帰コスト (D112) で買っている。

含意: D151 Revisit (1) の条件 (B の休止が初回のスイープで収束し、その後の
揺り戻しが消える) は満たされた。**Windows の既定を `private` に替える**
(D152、別 PR)。`memory_budget_for_ram` (RAM / 16) は据え置く — B は同じ
予算 1024 MiB で 50 タブを 20 休止で収めており、予算を私的コミット基準で
引き直す必要は今回の数字からは出てこない。`ESTIMATED_BYTES_PER_TAB` も
据え置く (上記 3)。

### 47.11 この計測の限界

1. **perf の RSS サンプラが約 5 秒周期** (D50 の自動調整。このシナリオは
   約 80 秒なので目標サンプル数から逆算すると粗くなる) で、メモリ判定
   (既定 5 秒) と位相が揃っている。そのため**判定の直前に採れている行は
   試行ごとに 1 行だけ**で、他の行の「要求」は 1〜5 秒古い値から出ている。
   ⚠️ の有無はその行では読めない。D148 Revisit (2) (判定が見た
   `total_bytes` と要求数を perf レコードに書く) が必要になった。
   `common_env` で `VELOX_PERF_RSS_INTERVAL_MS` を詰める手は、
   `velox-bench run` が自動調整値で上書きするため使えない。
2. **`minimal.html` のみ・1 run** (機種 1 つ)。47.4 の「戻る」量が
   ページや機種でどう変わるかは未計測。
3. 47.3 の 1 タブあたりの値は直前・直後の `rss` の差で、その 5 秒間の
   戻り (47.4) を含んでいる。実際に返った量はこれより大きい可能性がある
   (返した直後に測れていない)。
