...
## 41. `LOW` は背景タブの挙動を変えていない — ただし背景タブが何をしているかは依然として不明 (Issue #247, 2026-09-14)

D124 決定5 が「`cpu_percent` は指標として粗すぎた」と残した問いに、
**ブラウザの外側から**答えようとした記録。§40.4 の続きである。

### 41.1 測り方 — ページ自身に報告させる
...
集計は `scripts/bench/serve.py` が行う (perf-windows の `beacon` 入力が
真のときだけ使う。既定の計測環境を変えないため)。

### 41.2 結果 — 3 run で同一

`background_cpu` / `busy.html` / 4 試行 × 1 ペア。

| run | 条件 | `hidden` | `visible` |
| --- | --- | --- | --- |
| [34851494354](https://github.com/noan98/VeloX/actions/runs/34851494354) | `low` と `normal` 混在 | count=1, f 355 (+0), t 552 (+0) | count=12, f 0→328 (+328), t 0→510 (+510) |
| [34852383792](https://github.com/noan98/VeloX/actions/runs/34852383792) | **両腕 `low`** | count=1, f 312 (+0), t 486 (+0) | count=12, f 0→308 (+308), t 0→480 (+480) |
| [34854245476](https://github.com/noan98/VeloX/actions/runs/34854245476) | **両腕 `normal`** | count=1, f 339 (+0), t 527 (+0) | count=12, f 0→322 (+322), t 0→501 (+501) |

**`count` は 3 run すべてで `hidden`=1 / `visible`=12。** `f` / `t` の
値は 312 / 339 / 355 と 13% ほど散らばるが、これは機種差の範囲で
(D96)、`low` (312) と `normal` (339) の差もその中に収まっている。

**つまり `LOW` は、外側から見える背景タブの挙動を何も変えていない。**

### 41.3 言えること / 言えないこと

**言えること**: `LOW` を伝えても伝えなくても beacon の来方は同じ。
**背景タブの JS 実行について、`LOW` が何かを止めているという証拠は
無い。** D123 が既定を変えた根拠のうち「タブは起きたまま」に対する
特定の疑い (hint がタイマーを止めているのではないか) は、この計測の
範囲では否定された。

**言えないこと**: 背景タブが実際に何をしているか。理由は次項。

### 41.4 beacon の総数が説明できていない

`background_cpu` は 1 run で 8 回 velox を起動する (4 試行 × A/B)。
カウンタから逆算するとページは 1 回あたり約 5 秒動いており
(f≈320 は 60fps で 5.3 秒、t≈500 は 10ms 間隔で 5.0 秒)、`report()` は
500ms ごとなので **1 起動あたり約 10 本、合計 80 本前後**の beacon が
出るはずである。**実際に届いたのは 13 本。**

この差を説明できていない。**説明できないまま「タイマーが止まった」と
読んではならない** — 届かなかった beacon と、そもそも投げられなかった
beacon は、この計測では区別がつかない (ページは `fetch` の失敗を
`.catch(() => {})` で握り潰す)。

**サーバ側の取りこぼしは潰した。** 40 並列 × 300 本を投げる負荷試験で、
スレッド化した `serve.py` は 300/300 数えた。念のため**変更前の単一
スレッド・backlog 5 の実装でも同じ試験を通したところ、やはり 300/300
だった** — つまりサーバは原因ではない。スレッド化は取りこぼしが
「タイマーが止まった」ように見える失敗を起こりにくくするための保険で
あって、**本件の説明にはならない。**

### 41.5 最有力の仮説 — `visibilityState` が `set_visible` に追随していない

`hidden` の beacon が 3 run とも **1 本だけ**なのが手がかりである。
8 回の起動それぞれで背景化のたびに `visibilitychange` が発火するなら、
8 本前後あってよい。

**VeloX の `WebView::set_visible(false)` が `document.visibilityState` を
`hidden` にしていない**と考えると辻褄が合う。ネイティブのコントロールを
隠すことと、ページから見た可視性は別物である。その場合:

- 背景タブはずっと `state=visible` で報告し続ける
- `visible` バケットの +308〜328 frames / +480〜510 ticks は、
  **背景に回った後も動き続けたぶんを含んでいる**かもしれない
- `hidden` の 1 本は、何かの拍子に一度だけ可視性が伝わった回のもの

**これが事実なら `busy.html` の前提が VeloX では成立していない。**
同ページは「隠れたタブでは rAF が呼ばれなくなる / タイマーの間隔が
伸びる / CSS アニメーションが描画されない」という 3 つの抑制に反応
するよう作られているが (Issue #64)、**可視性が伝わっていなければ
どれも起きない。**

未確認である。確かめるには `visibilityState` を直接観測する必要があり、
それは本節が避けた「タブで JS を評価する」に戻ってしまう。

### 41.6 この計測の限界

1. **§41.4 の未説明が最大の限界。** 総数が合わないので、`advanced_*` が
   0 であることを「止まっている」と読めない。
2. **前景タブと背景タブが同じ `state` バケットに入る。** 両方が
   `visible` を通るため。idle タブは `f=0&t=0` で送るので値である程度は
   分かるが、設計として分離できていない。
3. **`compare_env` に空文字列を渡しても単独条件にならない。**
   `workflow_dispatch` の入力は、空文字列を渡すと宣言時の既定値
   (`VELOX_MEMORY_BUDGET_MB=0`) で埋められる。§41.2 の 2 本目・3 本目が
   「両腕とも同じ hint」という形になっているのはそのためで、単独条件の
   計測は今の入力設計ではできない。
4. **D124 決定5 の問い (`LOW` が何を縮めたのか) は解けていない。**
   解けたのは「タイマーを止めているのではないか」という**一つの疑い**
   だけである。
