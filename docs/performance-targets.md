...
### 28.8 この節が残した宿題と、その後 (2026-09-11 追記)

§28 は `tabs_hold_50` を測っておらず、プロセス数・休止タブ数の値も取れて
いなかった (D97 Revisit condition (1)/(2))。**どちらも §31 で埋まった** —
50 タブでも予算は守られ (527.9 MiB)、休止は WebView2 のプロセスまで回収して
いる (57 → 9.8 プロセス)。判断は D109 を参照。
...
どこで頭打ちになるか」が open だった。同時に D97 Revisit (1) の
「休止で WebView2 のプロセスが実際に減るか」も、値を表に出す実装
(D105) はできていたが **実際の値はまだ 1 度も取られていない**状態だった。
...
### 31.4 事実 2: 休止は WebView2 のプロセスまで回収している (D97 Revisit (1) への回答)

| シナリオ | B (予算 OFF) | A (既定) | 減少 |
| --- | ---: | ---: | ---: |
| `tabs_hold_20` | 27 プロセス | 11 プロセス | -59% |
| `tabs_hold_50` | 57 プロセス | 9.8 プロセス | -83% |

`suspend_tab` は `tab.webview.take()` で webview を drop するだけなので、
**プロセスまで消えるかどうかは RSS の減少だけでは区別がつかない** (D97 が
プロセス数を表に出す実装を入れた理由)。今回の値は、**プロセス単位で回収
できている**ことを示す。

これにより §27.4 事実 3 が挙げた 3 候補のうち、**候補 1「休止してもプロセスが
解放されない」は否定される。** §28 は候補 3 (休止が追いつかない) が主因である
ことを示したが、候補 1 は「支持されない」までしか言えていなかった。候補 1 は
これで閉じてよい。
...
| `tabs_hold_20` の B (予算 OFF) | 値 |
| --- | ---: |
| §28 (run 34244194428) | 1602.6 MiB |
| 本節 (run 34627143113) | 1596.3 MiB |
| 差 | **-6.3 MiB (-0.39%)** |

**同一機種なら run をまたいでも 0.4% で一致した。** これは Issue #211 項目3
(同一機種内の分散の実測) が求めているデータの 1 点目である。

⚠️ **ただしこれを「同一機種なら比較してよい」の結論にしてはいけない。**

1. **2 点では分散と呼べない。** 閾値の根拠には全く足りない。
