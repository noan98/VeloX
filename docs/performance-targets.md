...
#### 付随 (未解決): `page_load` が §21.2 から 38 倍離れている

2 つの run の `page_load_ms` 中央値は **2077ms / 2085ms** で、互いには
非常に安定しているが、同じ固定ページを使った §21.2 の **54.10ms**、および
§24.5 の run 2〜4 から逆算される約 100ms (`first_load - window_created`)
とは桁が違う。**A と B の差は 1〜5ms しかない**ので A/B 比較の妥当性には
影響しないが、`first_load` が 2664 / 2720ms と大きいのはすべてこの
`page_load` に引きずられたものである (起動区間 `window_created` までは
573 / 624ms に収まっている)。

**最有力の仮説は計測側 — この PR が入れた変更である。** 初回実行の失敗を
受けて固定ページ配信ステップに
`-RedirectStandardOutput`/`-RedirectStandardError` を足したが、
`python -m http.server` は**シングルスレッドで、リクエストごとに stderr へ
ログを 1 行書く**。書き込み先がファイルになったことでその I/O が応答を
ブロックしている可能性がある。上の 2 つの run はどちらもリダイレクトを
入れた後のものであり、54ms を出した §21.2 は入れる前である。

**切り分け**: リダイレクトを外して測り直す (この節を書いた次のコミットで
外した)。`page_load` が 100ms 未満に戻れば計測側の問題として確定し、
戻らなければ VeloX 側かランナー側の変化として別途追う。**どちらに転んでも
事実 1〜3 の結論は変わらない** — A/B は同一 run 内の比較であり、
`page_load` は A と B で同じだけ影響を受けているためである。
