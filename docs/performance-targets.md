...
の数え上げは手動のワンオフ計測だったが、D104 で `velox-bench` の
...
3. **休止による状態喪失のコストは測っていない。** 660.9 MiB という数字は
   ...
   レコードを数える別の計測 (§25.3 の Windows 版) が要る。**→ D104 で
...
4. ~~**休止による状態喪失のコストは未計測**~~ → **計測基盤ができた
   (D104)。** `MetricKey::SuspendedTabCount` (`suspended_tab_count`) を
