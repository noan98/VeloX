...
#### 付随: `page_load` が §21.2 から大きく乖離した

本 run の `page_load_ms` 中央値は約 **2077ms** で、同じ固定ページを使った
§21.2 の 54.10ms から 38 倍離れている。**A と B で 1.03ms しか違わない**ので
A/B 比較の妥当性には影響しないが、原因は分かっていない。**固定ページ・
固定ビルドでも run 間でこれだけ動く**という、事実 3 の別の現れである。
`first_load` が 2664ms と大きいのもこの `page_load` に引きずられたもので、
起動区間 (`window_created` まで) は 573ms に収まっている。
