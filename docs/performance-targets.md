...
> §24 に追加した。本節が「700 MiB という 1 点で何が起きるか」を記録して
> いるのに対し、§24 は「予算を動かすと何が起きるか」を記録している。
...
## 24. Memory Budget Manager — Stage 1 の現状計測 (Issue #176, 2026-09-07)

**設計判断は `docs/decisions.md` D92 を参照。** ここでは Issue #176 の
...
### 24.1 ベースラインの再現 (既定 ON = メモリ予算 700 MiB)
...
この表から読み取れる 3 つの事実 (解釈と設計への含意は D92):
...
2. **予算 500 MiB 以下では応答が飽和する。** 300/400/500 の 3 点は
   ...
   > 24.3 の休止イベント数がそれを裏付けている: 10 タブで予算 300 は
...
### 24.3 低い予算は「暴走」しない — 休止イベント数の実測

24.2 の事実 2 から「下限を割る予算ではサンプルのたびに sweep が走り続けて
...
残りはすべて 24.2 の事実 1 の下限とアクティブタブが占めている。

> **この 3 本の実行は 24.1/24.2 と PSS を比較できない。**
> `VELOX_PERF_METRICS=1` は `spawn_rss_sampler` (既定 5 秒間隔の `/proc`
> 走査) を追加で動かすため、被測定側の条件が変わる。この表から使うのは
> **イベント数と時刻だけ**で、メモリの値は一切使っていない。

### 24.4 搭載 RAM 相対にしたときに何が起きるか (このマシンの 1 点から)

現在の既定 700 MiB は、この測定環境の搭載 RAM 15.70 GiB に対して
**4.35%** にあたる (700 / 16075.2 MiB)。この 1 点を「比率」として他の
マシンに当てはめたときの値を、24.2 の応答曲線に重ねると:

| 想定マシンの RAM | 4.35% の予算 | 24.2 から予想される挙動 |
| ---: | ---: | --- |
| 4 GiB | 178 MiB | 下限 (約 397 MiB) を大きく割る → 24.2 事実 2 の飽和域。背景タブを常に全部休止 |
| 8 GiB | 357 MiB | 同上 (下限を割る) |
| **15.70 GiB (本環境)** | **700 MiB** | 実測どおり: 10 タブ 464.1 / 20 タブ 619.3 MiB |
| 32 GiB | 1427 MiB | 24.2 の 1400 MiB 行に相当: 10 タブでは一度も発動せず 982.8 MiB、20 タブで 1163.3 MiB |
| 64 GiB | 2854 MiB | 20 タブ (休止 OFF で 1626.5 MiB) でも発動しない → 実質 OFF |

**この表は「4.35% を採用すべき」という主張ではない。** 24.2 の予算軸の
...
### 24.5 Windows について、既存データから言えること / 言えないこと

D90 Revisit condition (1) は「Windows 実機/CI での 700 MiB の実効性は
未計測」としているが、**§21.2 に D90 が使っていない Windows の 1 点が
既に存在する。**
...
| 24.1 (本セッション) | このコンテナ / WebKitGTK / RAM 15.70 GiB | 1 | PSS | **396.7 MiB** (プロセス数 4) |
...
ときに Windows の RSS がどう伸びるかは一度も測られていない。24.1 で
...
実測するのが唯一の解決手段である (D92 の子 Issue 案 A)。
