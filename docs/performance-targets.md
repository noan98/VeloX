...
3. **休止による状態喪失のコストは測っていない。** 660.9 MiB という数字は
   ...
   レコードを数える別の計測 (§25.3 の Windows 版) が要る。
...
1. ~~**`rss_process_count` を A/B 表に出していない。**~~ → **対応済み。**
   A/B 比較表のキーに `rss_process_count` を追加し、タブ数スケーリング表にも
   「A プロセス数 / B プロセス数」の列を足した。**次の計測から**「休止で
   WebView2 のプロセスが実際に減っているか」が読める。`suspend_tab` は
   `tab.webview.take()` で webview を drop するだけなので、プロセスまで
   消えるかは RSS の減少だけでは区別がつかない — 減っていればプロセス単位で
   回収できており、減っていなければ #176 の設計課題になる。
   **§28.5 の run (34244194428) では取れていない**ので、値そのものは
   次の run を待つ。
...
3. **Chromium との比較が Windows では取れていない** (T2 の目標
   ...
   は Linux 専用である。
4. **休止による状態喪失のコストは未計測** — 616.8 MiB が「背景タブを
   何個休止した結果か」、復帰にどれだけかかるかは測っていない。
