...
| T3 | `startup_toolbar_ready_ms` を短縮する | 528.4ms | **300ms 以下** | 内部内訳で window_created (224ms) からツールバー ready まで 300ms かかっている。ここは VeloX 自身のコードであり、エンジン差ではない = **確実に手が出せる** — この仮説は #59 の実測で覆った。§9 参照 |
...
`docs/decisions.md` D42 を参照）。そのため **本 Issue では有効な最適化を
...
いことは `docs/decisions.md` D42 に記載した実験（フル版 vs 最小版の比較）で
確認済み。
...
〔minimal〕）。本 Issue では実行パスを変更していないため、この差はセッション
間のノイズ（このマシンは他セッションと共有されており、並行するビルドの有無
で ±10〜20% 程度変動する）であり、コード変更による回帰ではない。**T1
（Chromium 比 -20% 以内を維持）は引き続き達成**とみなす — 優位そのものは
明確に保たれている。
...
（`docs/decisions.md` D42 参照）。したがって本 Issue のコードには含めていない。
