...
> **その後の進捗 (Issue #176 Stage 1、D92)**: 上記 (1) (Windows での
> 700 MiB の実効性) と (2) (搭載 RAM に対する相対化) は **D92 で調査した
...
> D92 は「実装せず、正しい式の形と必要な追加計測を確定させる」に留めた。
> 詳細は D92 と `docs/performance-targets.md` §24。
...
## D92: Memory Budget Manager (Issue #176) Stage 1 — 計測の結果、本体コードは変更しない。RAM 相対の予算は「裸の比率」では成立しないことが分かった
...
`docs/performance-targets.md` §24。
...
| #187/#189 (D90) | `process_map` を二段階化、既定ポーリング間隔 2 秒 → 5 秒 | `tab_scaling.py` の `--stabilize-secs` が間隔から自動算出 (15 秒) される。§24 の計測はすべてこの既定で採った |
| #186 (D90) | 複数ウィンドウでの予算をラウンドロビンで修正 | §24 の計測はすべて単一ウィンドウなので影響しない。ただし設計上の制約としては残る (後述) |

### 計測から確定した 4 つの制約

数値は `docs/performance-targets.md` §24。**すべてこのコンテナ 1 台・
同一セッション・Linux/WebKitGTK での実測である** (Epic #57 ルール 5)。

1. **休止では届かない下限が約 397 MiB ある** (§24.2 事実 1)。1 タブ時の
...
2. **予算 500 MiB 以下では応答が飽和する** (§24.2 事実 2)。300/400/500 の
   ...
   1 個違うことで説明がつく (§24.2 事実 2 の注)。**予算をこれより細かく
   刻んでも意味が無い**ので、RAM 相対の式が生む値もこの粒度で丸めて
   考えればよい。
3. **飽和域でも churn は起きない** (§24.3)。10 タブ・40 秒保持で
   ...
   懸念した種類の害そのものである。
4. **予算は階段状に効き、着地点は予算を大きく下回る** (§24.2 事実 3)。
...
10 タブの時点で既に背景タブ 9 個中 8 個を休止している (§24.3)。**「もっと
...
| 比率 | 「ブラウザが搭載 RAM の何割を取ってよいか」という製品判断 | **測れない。** 搭載 RAM を変えられるマシンが 1 台も無い (§24.4)。4.35% は現行既定を逆算しただけで、根拠ではない |
...
§24.1 で測った Linux の RSS/PSS 比 (VeloX で 1.68〜1.80 倍) を Windows に
...
| E | **1 タブ時の toolbar 用 `WebProcess` の削減**。制約 1 の下限そのものを下げる。§12/§24 の両方が「T2 の残りはここ」と指している | なし | `cost:high` / `benefit:5` |
