...
### #187: `memory_check_interval` の既定見直し (2 秒 → 10 秒)

**対象**: Issue #187。上記「軽量ケースのポーリングコスト」で PR #185 が
「実用上無視できる」と結論した判断を、**誤りと判定して覆す**。判断根拠は
定性的な感覚ではなく、以下の追加実測に基づく。

**smaps_rollup 読み取りが支配的コストであることの確認**: `metrics::
sample_process_tree_rss` (Linux) が呼ぶ `process_map()` は **VeloX 自身の
プロセスツリーだけでなく `/proc` に見えるマシン上の全プロセス**を毎回
列挙し、各プロセスの `status`/`smaps_rollup`/`stat` を読む
(`build_sample` が事後にツリーへ絞り込む)。この「フルスキャン 1 回」の
コストを、Rust 実装と同じ手順を踏む外部プローブで直接計測した (VeloX を
1 タブで起動した状態、20 試行の中央値、このコンテナ、プロセス総数 88・
うち `smaps_rollup` が読めたもの 21):
...
**`smaps_rollup` はアクセスできた 21/87 プロセス分だけで全体の 86% を
占める** — 1 プロセスあたりのコストが `status`/`stat` (定数個のフィールド
を読むだけ) とは桁違いに高いことを裏付けており、Issue #187 が立てた
「カーネルのページテーブル走査が支配的」という見立てを実測で確認した。

**間隔別のアイドル CPU** (`scripts/profile/cpu_usage.py`、idle 60 秒窓、
`--settle-secs` 6〜8 秒、既定バイナリ [700 MiB 予算・当時 2 秒間隔] に
`VELOX_MEMORY_CHECK_INTERVAL_MS` だけ変えて計測。参考として §23 の
30 秒窓の数値も残しているため、本節は窓を伸ばして再計測した値):

| 間隔 | 1 タブ CPU% | 3 タブ CPU% |
| ---: | ---: | ---: |
| 2000ms (旧既定) | 1.2% (0.70〜0.72 秒/60 秒、2 試行) | 1.4% (0.86 秒/60 秒) |
| 5000ms | 0.5% (0.31 秒/60 秒) | 0.6% (0.38 秒/60 秒) |
| **10000ms (新既定)** | **0.3% (0.16〜0.19 秒/60 秒、2 試行)** | **0.4% (0.25 秒/60 秒)** |
| 30000ms | 0.2% (0.11 秒/60 秒) | 0.2% (0.11 秒/60 秒) |
| (参考) OFF (`VELOX_MEMORY_BUDGET_MB=0`) | 0.1% | 0.2% |

コストは間隔にほぼ反比例する (2000→5000ms で -58%、モデル上の理論値は
-60%、2000→10000ms で -75%、理論値 -80%) が、**間隔を伸ばすほど固定
オーバーヘッド (スレッドの起床・イベント送出など、フルスキャンとは別の
コスト) の割合が相対的に増え、単純な反比例からは徐々にずれる** — 30 秒
時点で OFF 時の床 (0.1〜0.2%) にほぼ到達しており、それ以上間隔を伸ばして
も CPU 面の追加の得は乏しい (10000→30000ms の差はわずか 0.1〜0.2pt)。

**採用した値: 10 秒。** 2000ms から 5000ms・10000ms までは大きく効くが、
10000ms から 30000ms への追加の伸びは限界効用が小さい一方、休止の検知が
最大 30 秒遅れるのは体感上の遅さにつながりうる。10 秒は CPU 削減の大部分
(1 タブで 1.2% → 0.3%、OFF 時の床 0.1% にかなり近づく) を得つつ、検知遅延
を実用的な範囲に収める妥協点と判断した。

**メモリ予算による休止は遅延に厳しい処理ではない、という前提の確認**:
メモリ予算シグナルの目的は「タブを多く開いてメモリを圧迫した状態を長時間
放置しない」ことであり (D56/D90 冒頭の背景を参照)、検知が数秒〜十数秒
遅れても失うのは「回収が開始されるまでの時間」だけで、正しさには影響
しない。休止自体は依然として即座に (1 sweep で) 実行される。

**10/20 タブでの削減効果が維持されるかの再確認** (`tab_scaling.py`、
`minimal.html`、3 試行の中央値、既定バイナリ [700 MiB・10 秒間隔] を
env 上書きなしで使用):

| stabilize 秒数 | 10 タブ PSS | 20 タブ PSS | 状態 |
| ---: | ---: | ---: | --- |
| 3 秒 (既定値、旧 2 秒間隔での §23 計測と同じ待ち時間) | 980.4 MiB | 1668.9 MiB | **未収束** (休止前とほぼ同じ — 979.7/1655.7 MiB) |
| 15 秒 | 464.1 MiB | 615.1 MiB | 収束済み |
| 40 秒 | 463.4 MiB | 615.3 MiB | 収束済み (15 秒と同じ) |

**最終到達点は変わらない** (10 秒間隔でも 15 秒待てば 10/20 タブとも
§23 の 2 秒間隔の値 [464.4/617.7 MiB] と誤差範囲で一致) が、**収束に
かかる時間は正直に見て伸びた** — 2 秒間隔では tab_scaling.py の既定
stabilize (3 秒) だけで既に収束していたのに対し、10 秒間隔では 3 秒
待った時点ではまだ 1 回もサンプルが取れておらず休止が始まってすらいない
(3 秒 vs 15 秒、約 5 倍)。これは間隔を 10 秒にしたことの直接の帰結であり
隠さず記録する。実際の利用ではタブを開き続けている間は問題にならず、
「タブを開くのをやめてから最大 10 秒程度で休止が始まる」という遅延が
生じるだけである。

**回帰ゲート** (`cold_startup`、baseline=旧既定 [2 秒間隔] バイナリ、
candidate=新既定 [10 秒間隔] バイナリ ×2、各 8 試行、`--warn-pct 20
--fail-pct 60`): **総合判定 OK** (`startup_toolbar_ready_ms` などは
-2.8%〜-6.6% と、むしろ間隔を伸ばした分わずかに改善方向)。

**結論**: `SuspensionPolicy::DEFAULT_MEMORY_CHECK_INTERVAL` を 2 秒から
**10 秒**に変更した (`browser::suspension::SuspensionPolicy::
DEFAULT_MEMORY_CHECK_INTERVAL`、`browser::settings::
DEFAULT_MEMORY_CHECK_INTERVAL_MS` を同期)。`VELOX_MEMORY_CHECK_INTERVAL_MS`
で引き続き上書き可能。
...
おく (上記実測の「`tab_switch_20` は比較不能になった」参照)。(5) **(#187)**
30 秒間隔は CPU 面ではほぼ床 (OFF 時と同水準) に到達するが、本 Issue では
検知遅延とのバランスを優先して 10 秒を採用し、それ以上は伸ばさなかった —
実機のバッテリー計測 (powertop 等) で 10 秒間隔の効果がなお不十分と分かっ
た場合に再検討する。(6) **(#187)** 間隔別のアイドル CPU 計測はすべて
このコンテナ 1 台のセッション内で行った (Windows での同種のオーバーヘッド
[Toolhelp32 スナップショット + `GetProcessMemoryInfo` の全プロセス走査、
D88] は未計測のまま) — Windows 実機での計測は上記 (1) の Windows 実機計測
と合わせて行うのが効率的。
