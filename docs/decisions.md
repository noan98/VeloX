...
(1) **数値は出たが、T2 は判定できていない** (§29.8)。run 34365218545 で
VeloX [88.9, 370.0] MiB / Edge [173.3, 692.5] MiB を得たが、区間が重なるため
`inconclusive`。上界を締めた版で測り直すまで Stage 1 (#176) は閉じられない。
**締めても重なるなら、それが答えである** — その場合は「現状の手段では Windows で
T2 を判定できない」ことを結論として記録し、別の手段を検討する。
...
前提で読むこと。**
(3) **的中した。** §29.8 で実際に区間が広すぎ (VeloX 4.2 倍幅 / Edge 4.0 倍幅、
working set の 3/4 以上が共有ページ)、`inconclusive` になった。対策として上界を
`ShareCount` で締めた (決定1 の改訂)。**下界 (`private_bytes`) を締める手段は
依然として無い** — 共有ページを何プロセスが使っているかは分かっても、その中で
自プロセスの取り分の下限は決まらないため。締めた上界でも重なるなら、そこが
この手法の限界である。
