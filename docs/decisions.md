...
(1) **`page_load_ms` が §21.2 の 54.10ms に対し本 run では約 2077ms** と
38 倍離れた理由は分かっていない (§26.4 付随)。A/B 双方で同じ値なので本節の
結論には影響しないが、**Windows の page load を論じる前に原因を掴む必要が
ある。** 次に Windows で測るときは、この値が再現するか (どの帯に落ちるか)
を最初に見ること。
(2) 決定 3 は「機種が違えば絶対値も違う」という観測に基づくが、**同一機種の
run を複数回そろえて分散を採ったわけではない。** 機種を揃えても run 間で
どれだけ動くかは未計測であり、「同一機種なら比較してよい」とまでは言えない。
(3) 本節の A/B は `cold_startup` (1 タブ・`minimal.html`) のみである。
メモリ予算の効果はタブ数を増やして初めて出るので、**#197 (Windows での
700 MiB の実効性) はこの結果では何も解決していない** — 同じ A/B モードを
`tabs_10` / `tabs_20` で回すのが次の一手になる。
