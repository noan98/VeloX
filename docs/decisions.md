...
(1) **対応済み (表示のみ)。** `rss_process_count` を A/B 比較表のキーに
追加し、タブ数スケーリング表にも「A プロセス数 / B プロセス数」の列を
足した。ただし **§28.5 の run ではこの列が無かったため、値はまだ無い** —
「休止で WebView2 のプロセスが実際に減るか」の答えは次の計測を待つ。
減っていればプロセス単位で回収できており、減っていなければ webview を
drop してもプロセスが残っているということで、#176 の設計課題になる。
