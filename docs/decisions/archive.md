...
## D125: `LOW` は背景タブの挙動を変えていない (Issue #247) — ただし「何を失ったのか」は解けておらず、既定は据え置く

**対象**: Issue #247 / D124 決定5 が残した「`LOW` が背景タブから何を
奪ったのか」。**実測は `docs/performance-targets.md` §41。**

### 決定1: `BackgroundMemoryTarget::Low` の既定 (D123 決定1) を維持する

3 run で `low` と `normal` の beacon が同一だった (§41.2)。**背景タブの
JS 実行を `LOW` が止めているという証拠は無い。** D123 を覆す材料は
出ていない。

### 決定2: 解けたのは「一つの疑い」であって、問いそのものではない

D124 決定5 の問いは「エンジンが JS ヒープを縮めたのか、描画バッファを
捨てたのか、キャッシュを落としたのか」だった。**今回それには答えて
いない。**

答えたのは派生した疑いの一つ —「hint が背景タブのタイマーを止めている
のではないか」— だけで、そこは否定された。

**この節を「`LOW` に対価は無いと分かった」と要約してはならない。**
D124 決定5 が置いた歯止め (「解くまでは『対価が無い』と言わない」) は
**まだ効いている。**

### 決定3: 計測経路は作れたが、実験としては未完である

§41.4。`background_cpu` は 1 run で 80 本前後の beacon を出すはずが、
**13 本しか届いていない。** 説明できていない。

**届かなかった beacon と、そもそも投げられなかった beacon を、この
計測は区別できない** (ページは `fetch` の失敗を握り潰す)。したがって
`advanced_*` が 0 であることを「止まっている」と読んではならない。

サーバ側の取りこぼしは潰した — 40 並列 × 300 本の負荷試験で
スレッド化版は 300/300 数え、**変更前の単一スレッド版でも 300/300
だった。** つまりサーバは原因ではない。**スレッド化は保険であって、
本件の説明ではない。**

### 決定4: 最有力の仮説は「`visibilityState` が `set_visible` に追随していない」

§41.5。`hidden` の beacon が 3 run とも 1 本だけであることの説明として
最も素直である。ネイティブのコントロールを隠すことと、ページから見た
可視性は別物である。

**事実なら影響は #247 より広い。** `busy.html` は「隠れたタブでは
rAF が止まる / タイマーが伸びる / アニメーションが描画されない」という
3 つの抑制に反応するよう作られているが (Issue #64)、可視性が伝わって
いなければ**どれも起きない**。Issue #64 が測った背景 CPU の系列
(§13 前後) の読み方にも関わる。

未確認である。確かめるには `visibilityState` を直接読む必要があり、
それは本計測が避けた「タブで JS を評価する」に戻ってしまう。
**Issue #251 として切り出した。**

### 決定5: 単独条件の計測は、今の入力設計ではできない

§41.6 限界3。`workflow_dispatch` の入力に空文字列を渡すと、GitHub は
**宣言時の既定値で埋める**。`compare_env` は「空欄 = 比較しない」という
意味を持たせた入力だが、`workflow_dispatch` から空欄を指定する手段が
無い。

そのため §41.2 の 2 本目・3 本目は「両腕とも同じ hint」という形にして
条件を揃えた。**回避はできたが、入力設計としては壊れている。**
`compare_env` の既定値を空文字列にするか、`no_compare` のような明示的な
入力を足すかは、次にこの入力で困ったときに決める。

### Revisit condition

(1) 決定4 の仮説が確かめられたら、`busy.html` を使う計測すべて
(Issue #64 の系列を含む) の読み方を見直す。
(2) 決定3 の未説明が解けたら、§41.2 の `advanced_*` を改めて読む。
解けるまでこの列から結論を出さない。
(3) D124 決定5 の本体 (何を縮めたのか) は #247 に残る。**既定を
変えたまま未検証である状態は変わっていない。**
