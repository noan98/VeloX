...
## D109: `tabs_hold_50` の実測で D97 の宿題 2 つを閉じる (Issue #197) — 予算は 50 タブでも守られ、休止はプロセスまで回収する。残るのは「深さ」の妥当性

**対象**: D97 Revisit condition (1) (休止で WebView2 のプロセスが減るか) と
(2) (`tabs_hold_50` が未測定)。実測は
`docs/performance-targets.md` §31、run 34627143113。

### 決定1: 候補 1「休止してもプロセスが解放されない」を閉じる

§27.4 事実 3 は、20 タブで予算超過に見えた原因の候補を 3 つ挙げた。§28 は
候補 3 (休止が追いつかない) が主因であることを示したが、候補 1 については
「支持されない」までしか言えていなかった — **プロセス数を測っていなかった
ため、RSS の減少がプロセス回収によるものか、プロセスは残ったまま中身だけ
減ったのかを区別できなかった。**

今回の実測で、`tabs_hold_50` の `rss_process_count` は **B 57 → A 9.8
(-83%)**、`tabs_hold_20` で **27 → 11 (-59%)** だった。プロセス単位で回収
できている。候補 1 は閉じる。

これは `suspend_tab` の実装 (`tab.webview.take()` で drop するだけ) から
**コードだけでは決められなかった**点である。D97 が「構造的な説明が、その
まま観測される差の原因とは限らない」と書いたのと同じ理由で、実測を待つのが
正しかった。
...
### 決定4: 同一機種の再現性は「1 点目」として記録するに留める

今回のランナーは §28 と同じ AMD EPYC 7763 / 4 コア / 15.99 GiB で、
`machine_key` (D106) が一致する 2 本目の run が初めて得られた。
`tabs_hold_20` の B は **1602.6 → 1596.3 MiB (-0.39%)** で一致した。

**これを「同一機種なら比較してよい」(D96 Revisit condition (2)) の結論には
しない。** 理由は 3 つ:

1. **2 点では分散ではない。** Issue #211 項目3 が求める閾値の根拠に足りない。
2. **一致したのは B 側だけである。** A 側 (予算が効いている条件) は
