...
### #186: 複数ウィンドウでメモリ予算が機能しない不具合の修正 — サンプルの割り当てをラウンドロビン化

**対象**: Issue #186。PR #185 のレビューで Codex (chatgpt-codex-connector) が
指摘し、コードを読んで実在を確認した不具合。`app::sweep_tabs` が
`state.pending_memory_sample.take()` でメモリサンプルを取り出して空に
する一方、イベントループはウィンドウを `state.windows.ids()` の固定順
(ウィンドウの作成順) で 1 つずつ sweep していた。結果、**そのパスの
サンプルは常に最初のウィンドウが消費する**。最初のウィンドウに休止候補
が無ければ (例: 1 タブだけ = アクティブタブは休止対象外) `suspension::
plan` は空を返し、**サンプルはそのまま捨てられる**。2 つ目以降の
ウィンドウがどれだけメモリを食う背景タブを抱えていても、受け取るのは
常に `None` で、**メモリ理由では永久に休止されない**。

この挙動自体は #63/D56 の時点からコードのコメントに明記されていた
(「a budget shared across every open window is left to a follow-up」)。
しかし #184 (D90) でメモリ予算が**唯一の既定 ON シグナル**になったため、
複数ウィンドウ (#29/D68 で実装済みの通常機能) を開いた瞬間に既定機能が
黙って効かなくなる、という話に格上げされた。

#### 再現テストで実在を確認

まず再現テストを書き、修正前のコードで実際に失敗することを確認した
(`tests/integration.rs::memory_budget_signal_reaches_a_second_windows_
background_tabs`)。ウィンドウ 1 はホームタブだけ (常にアクティブ = 休止
候補なし)、`new_window` で開いたウィンドウ 2 に背景タブを 2 つ持たせ、
`VELOX_MEMORY_BUDGET_MB=1`・`VELOX_MEMORY_CHECK_INTERVAL_MS=100` で確実に
予算超過させる。**修正前のコードではこのテストは失敗した** (`tab_suspend`
イベントが 0 件 — 100ms ごとにサンプルが取れているにもかかわらず、
ウィンドウ 2 の背景タブは一度も休止されない)。修正後は成功する。

#### 対処方針: ラウンドロビン (案 2) を採用

Issue が挙げた 3 案のうち、**案 2 (サンプルを消費するウィンドウを
ラウンドロビンする) を採用した**。

- **案 1 (全ウィンドウに同じサンプルを適用) は採用しなかった**: 1 回の
  予算超過で全ウィンドウが同時に休止対象になり、D56 が記録した鋸歯状の
  過剰回収がウィンドウ数倍に増幅されるリスクがある。実測で定量化する
  以前に、実装の単純さで飛びつくべきではないという Issue 本文の指摘
  どおり、構造的なリスクが明白だったため見送った。
- **案 3 (ウィンドウ横断で回収計画を立てる、Codex の提案) は最も正しいが、
  見送った**: `suspension::plan` は 1 ウィンドウの候補リストを受け取る
  設計であり (D68 の「各ウィンドウの独立した候補プールを 1 つの純粋関数
  `plan` で処理する」という既存の形)、複数ウィンドウの候補を横断して
  1 つの計画を立てるには `plan` のシグネチャ自体を変えるか、呼び出し側で
  複数ウィンドウの候補をマージする新しい層が要る。**根本修正としては
  将来検討する価値があるが、本 Issue のスコープ (「既定 ON にした機能が
  通常機能である複数ウィンドウで黙って動かない」を直す) に対しては
  過大**と判断した — D56 が最初に確立した「1 ウィンドウの独立したポリシー
  評価」という設計 (D68) を壊さずに直せる案 2 で十分だった。
- **案 2 の性質 — 過剰回収が構造的に起きない**: `sweep_tabs` の呼び出し元
  (event loop) は 1 パスにつき**厳密に 1 つのウィンドウにだけ**
  `Some(sample)` を渡し、残りには `None` を渡す。これは実装上の偶然の
  結果ではなく、新設した純粋関数 `choose_memory_sample_window(ids: &[WindowId],
  next: Option<WindowId>) -> (Option<WindowId>, Option<WindowId>)` の
  戻り値の型そのものが「1 つの `Option<WindowId>`」であることから
  構造的に保証される — このヘルパーだけがどのウィンドウにサンプルを渡す
  か決め、event loop 側は残りのウィンドウすべてに機械的に `None` を渡す
  だけ。ウィンドウ/webview に依存しない純粋関数のため、ラウンドロビンの
  公平性そのものをユニットテストできる (D20 の「`browser::`/ポリシー
  ロジックは可能な限り純粋関数として書く」という既存の方針と同じ形):
  - 1 ウィンドウでは常にそのウィンドウが選ばれる (旧実装と挙動が完全に
    一致することの保証)
  - 3 ウィンドウで 7 回連続呼び出すと、順序どおりに 2 周と少し分の
    ラウンドロビンになる (どのウィンドウも飢餓しないことの保証)
  - `next` が指す前回選ばれたウィンドウが (その後閉じられて) 存在しなく
    ても、先頭にフォールバックして必ず 1 つ選ぶ (自己修復)
  - 1〜5 ウィンドウのどのケースでも、1 回の呼び出しで選ばれるのは常に
    ちょうど 1 つ (過剰回収の構造的な不可能性そのもの)

#### 過剰回収が起きていないことの実測確認

ユニットテストによる構造的な保証に加え、Epic #57 ルール 1 (ユニット
レベルだけで判断しない) に従い、実バイナリでも確認した
(`tests/integration.rs::memory_budget_signal_never_suspends_more_than_
one_windows_tabs_per_sample`)。ウィンドウ 1・2 双方に背景タブを 2 つずつ
(計 4 つ、すべて休止対象) 持たせ、`VELOX_MEMORY_BUDGET_MB=1`
(即座に大幅超過) の下で `tab_suspend` (reason=memory) イベントの
タイムスタンプを 50ms 以内のクラスタにまとめ、**どのクラスタも 2 件
(1 ウィンドウ分) を超えないこと**を確認した — `tab_suspend` perf
イベントに `window_id` が無いため window 別の直接集計はできないが、
「同一パスで休止されたものはタイムスタンプが近接する」という性質を
使って間接的に検証した。テストは green (最終的に両ウィンドウの背景タブ
は複数パスに分かれて休止されるが、1 回のサンプルで両方同時に落ちる
ことは無い)。

#### 実装

- `AppState::next_memory_sample_window: Option<WindowId>` を新設 — 次の
  フレッシュなサンプルをどのウィンドウの sweep に渡すか (D90 上記参照)。
  `None` は「まだ決めていない」(既定でウィンドウ一覧の先頭)。
- `choose_memory_sample_window` (純粋関数、`src/app.rs`) — 上記のとおり。
- `sweep_tabs` のシグネチャを変更: `state.pending_memory_sample.take()`
  を内部で行うのをやめ、`memory: Option<MemorySample>` を引数として
  受け取るようにした (呼び出し元が「このウィンドウに渡すサンプルは
  これ」を決めてから渡す)。
- `run` のイベントループ: パスごとに 1 回だけ `state.pending_memory_
  sample.take()` し、`choose_memory_sample_window` でどのウィンドウに
  渡すか決めてからループへ入る (旧実装は各ウィンドウの `sweep_tabs`
  呼び出しの中で `take()` していたため、実質的に「最初に呼ばれた
  ウィンドウが勝つ」レース状態だった)。
- 単一ウィンドウの挙動は一切変わらない (`choose_memory_sample_window`
  はウィンドウが 1 つだけならそのウィンドウを常に返すため) — 既存の
  単一ウィンドウの統合テスト (`live_tab_cap_suspends_background_tabs_
  and_switching_back_resumes_them`、`memory_budget_signal_suspends_a_
  background_tab_end_to_end`) は変更なしでそのまま通る。

#### 回帰ゲート

`cold_startup`/`tab_create`/`tab_switch` (baseline=修正前バイナリ、
candidate=修正後バイナリ ×2、各 8 試行、`--warn-pct 20 --fail-pct 60`):
いずれも**総合判定 OK**。`tab_switch` の一部の候補測定に少数の
`tab_resume_ms` が混ざったが (`tab_switch_ms` 自体は baseline と同じ
0.90ms)、これは既定 ON のメモリ予算が単一ウィンドウのシナリオでも
時折発火する既存の特性であり (§23 の `tab_switch_20` の知見と同種)、
本 Issue の変更が原因ではない。
...
効率的。
(8) **(#186)** 案 3 (ウィンドウ横断の回収計画) は、複数ウィンドウを多数開く
利用が実際に増えた場合に改めて検討する価値がある — `suspension::plan`
を複数ウィンドウの候補を受け取れる形に拡張するか、呼び出し側で候補を
マージする層を追加する設計が必要になる。(9) **(#186)** ラウンドロビンは
「公平だが遅い」トレードオフを持つ: N 個のウィンドウが同時に予算超過して
いる場合、最後のウィンドウが最初のサンプルを受け取るまでに最大 N 回分の
メモリサンプル間隔 (`memory_check_interval`、既定値は別 Issue で見直され
うる — 本 Issue の変更対象ではない) がかかる。ウィンドウを多数 (5 個以上)
同時に開いて全部メモリを圧迫させる利用が実際に問題になった場合は、案 3
か、あるいは 1 パスで複数ウィンドウにサンプルを配る折衷案 (例: 上位 K 個)
を再検討する。

> **その後の進捗 (Issue #176 Stage 1、D93)**: 上記 (1) (Windows での
> 700 MiB の実効性) と (2) (搭載 RAM に対する相対化) は **D93 で調査した
> が、どちらも未解決のまま残っている。** (1) については
> `docs/performance-targets.md` §21.2 に既存の Windows の 1 点 (1 タブの
> RSS 367.5 MiB) があり、少なくとも 1 タブ時点では「700 MiB が早すぎる」
> とは言えないことが分かったが、タブ数に対する傾きは依然として未計測。
> (2) については、**搭載 RAM に比率を掛ける裸の式では 4〜8 GiB のマシンで
> 予算が VeloX 自身の下限を割り機能しなくなる**ことが実測から分かり、
> D93 は「実装せず、正しい式の形と必要な追加計測を確定させる」に留めた。
> 詳細は D93 と `docs/performance-targets.md` §25。
...
### 検証: フォールバックは一気通貫で機能する (Issue #194 完了、2026-09-14 追記)

**上記の「残る未検証項目」は解消済みである。** Issue #194 の完了条件を
すべて満たしたので、その記録をここに残す (同 Issue の最後の完了条件が
「確認結果を D91 に追記する」だった)。

#### 原因は「トークンが失効していた」でも「上限」でもなかった

`CLAUDE_CODE_OAUTH_TOKEN` の**値そのものに改行が入っていた** (110 文字中
107 文字目)。HTTP ヘッダ値に改行は入れられないため、Claude Code は
**ネットワークに出る前にローカルで弾いていた。**

```
Invalid Authorization header value from CLAUDE_CODE_OAUTH_TOKEN:
it contains a line break at character 107 (110 characters on 2 lines).
```

当時立てていた仮説は「トークン失効」と「アカウント側の利用上限」の 2 つで、
**どちらも外れだった。** 両方とも「値は正しいが受理されない」という形で、
**「値そのものが不正」という第三の形を見落としていた。** `duration_ms: 82`
(ネットワーク往復が無い) / `modelUsage: {}` (モデルを 1 回も呼んでいない)
という観測は、最初からこの形を指していた。

#### 一気通貫の実地確認

| # | 段階 | 初回 (PR #216、2026-09-09) | 再確認 (PR #257・#258、2026-09-14) |
| --- | --- | --- | --- |
| 1 | Codex が利用上限に到達 | ✅ | ✅ |
| 2 | auto-merge が `@claude` 依頼を自動投稿 | ✅ | ✅ |
| 3 | `claude.yml` が起動しレビューを投稿 | ✅ | ✅ |
| 4 | ゲートが Claude のレビューを受理 | ✅ | ✅ |
| 5 | **auto-merge が実際にマージ** | — | ✅ |

再確認時 (PR #258) の auto-merge ログ:

```
##[warning]PR #258: Codex の利用上限到達を検知し、この PR は Codex に一度も
レビューされていないため、head SHA `d6b1c3c` への再レビュー要件を Claude の
レビューで代替しました
  問題なし (未解決スレッド 0 件 / CHANGES_REQUESTED 0 件 / ... / 猶予期間経過済み)
all green: マージします (merge)
merged: PR #258
```

**`automerge-without-codex` ラベルは使っていない。** 純粋にフォールバック
経路だけでマージが成立している。

#### レビューの中身も実質的だった

形式的に動いただけではない。PR #216 では `docs/performance-targets.md`
§3.1 の定義表が古いままであることを指摘され、それは**実際の見落とし**
だった (3 か所直して 4 か所目を見落としていた)。PR #258 でも
`git status --porcelain` の rename 行を 1 パスとして検査している穴を
指摘され、これも実在するバグだった。

#### `CLAUDE_REVIEWER_LOGINS` / `AUTO_MERGE_TOKEN`

- ログイン名は **`claude[bot]`** (id 209825114) を PR #198 で実測し、
  `auto-merge.yml` の `env` に設定済み。推測で決め打ちしていない。
- `AUTO_MERGE_TOKEN` は設定済みで、依頼コメントの投稿とマージの両方に
  使われている。
- ⚠️ **「OAuth 設定後なら `GITHUB_TOKEN` でも反応するか」は未検証のまま
  である。** PAT が動いている以上、これを確かめるには一度 PAT を外して
  空打ちのリスクを負う必要があり、確かめる価値より失うもののほうが
  大きいと判断した。**D91 の「PAT が要る」という記述は、否定的観測
  (PR #193) に基づく安全側の設計として維持する。**

## D133: Codex が沈黙しても永久ブロックしない (Issue #207) — 緩和の根拠が「特定の文言」だけだと、文言が来なければ待ち続ける

**対象**: Issue #207。実装は `.github/scripts/check_review_gate.py`。

### 事象

D91 が入れた緩和はいずれも **Codex が `reached your codex usage limits`
という特定の文言を返したこと**を前提にしていた。ところが Codex は
「沈黙する」「別の文言で断る」という返し方もする。その場合は緩和が成立
せず、**待ち時間に上限が無いため、その head SHA は二度とマージできない。**

実測 (2026-09-08、PR #202 / #204):
...
上限メッセージは**マーカーより前**なので厳密判定は成立せず (01:42:55 <
01:48:06)、マーカー後の返答は上限の文言を含まない。結果、**CI 全緑の PR が
2 件同時に、人手でラベルを付けるまでマージ不能になった。**

### 決定1: 依頼からの経過時間で緩和する (既定 60 分)

起点は**依頼コメントの `createdAt`** とする (head の push 観測時刻ではない)。
測りたいのは「リクエストは届いたが返事が無い」であって「head が古い」では
ないため。既定 60 分は、実測で Codex の応答が数十秒〜数分であることから、
通常のレビュー所要時間を跨がない値として選んだ。`0` で無効化できる。

### 決定2: タイムアウトは Claude フォールバックにだけ合流させる

D91 の緩和には 2 つの経路がある。**A (この PR の過去のレビューで代替) には
決して合流させない。**

A は「**別の commit へのレビューで現在の head を通す**」という最も強い
緩和であり、その根拠は「Codex は応答した。ただし上限だった」という積極的な
観測である。**沈黙を根拠に A を許すと、Codex が死んでいる間は過去の
レビューだけで何でも通る。** 実装上も `request_timed_out` は
`not limit_strict` のときしか立たないので、A に入ることは構造的にない。

### 決定3: フォールバック未設定なら緩めない

`claude_logins` が空のときはタイムアウト緩和を働かせない。緩めてしまうと
**レビューを誰も行わないまま Codex 要件だけが消える** —「沈黙していれば
通る」という、レビューゲートとして最悪の抜け方になる。この場合は従来どおり
`automerge-without-codex` ラベルによる人手の判断に委ねる。

これは `limit_recent` (D91 の B) が同じ条件を課しているのと同じ考え方で
ある。

### 決定4: 緩和の理由を文言で区別する

「上限だった」と「返事が来なかった」では、**次に取るべき行動が違う**
(前者は待てば回復する。後者は Codex 側を調べる必要がある)。Job Summary に
出る文言を出し分け、タイムアウト由来の場合は `Issue #207` を明示する。

### 副次的に直したこと — docstring が実装と食い違っていた

`check_review_gate.py` のモジュール docstring は、Claude の判定シグナルを
「`latestReviews[].commit.oid` が head SHA と一致するレビュー 1 つだけ」と
書いていた。**しかし Issue #216 の修正 (完了見出しを持つコメントも受理) が
入った時点でこれは嘘になっていた。** 読んだ人は「コメントは数えない」と
思い込む。実装に合わせて書き直した。

同じ docstring の「`@claude` メンションが実際に応答を得られるかは未検証」も
失効していた (Issue #194、上記 D91 の検証節)。

### Revisit condition

(1) 60 分という既定値は実測に基づく見積もりであって、タイムアウト緩和が
実際に発火した事例はまだ無い。発火したらその所要時間を見て調整する。
(2) Codex が上限の文言を変えた場合、厳密判定は死ぬがタイムアウトが拾うので
**永久ブロックにはならない**。ただし緩和まで 60 分待つことになるので、
文言の変更に気付いたら `_USAGE_LIMIT_PHRASE` を更新すること。
(3) タイムアウト由来の緩和が常態化するなら、それは Codex 連携そのものが
壊れているということである。`CODEX_RESPONSE_TIMEOUT_MINUTES` を縮める前に、
Codex を必須要件から外すかどうかを判断すること。
...
**実測結果 (Windows、本命)**: 詳細は `docs/performance-targets.md` §24.4。
本 workflow 変更を含む PR に対する `pull_request` トリガーの自動実行
(run 34178072263、`windows-latest`、`cold_startup` 10 試行) で取得できた。
`window_created` 922.35ms の内訳 (累積中央値どうしの差) は「VeloX 自身の
Rust セットアップ **0.55ms (0.06%)**」「最初の webview の生成
**672.15ms (72.9%)**」「tao のイベントループ 4.50ms」「tao のウィンドウ生成
58.90ms」「**2 個目の webview 186.25ms**」。

**Linux から外挿していたら間違えていた点が 1 つある** (Epic #57 ルール 5 の
実例): Linux では 2 個目の webview が 1.60ms しかかからず「エンジンの初回
コストは 1 個目に集中する」と読めるが、**Windows では 2 個目にも 186ms
残る**。WebView2 と WebKitGTK のプロセスモデルの違いがここに出ている。

なお §21 (run 34127310212) の `startup_window_created_ms` 中央値 644.05ms に
対して本 run は 922.35ms で約 1.43 倍ある。**本 Issue の計装が原因である可能性は
対照実験で否定した** — 計装の入っていない `main` (`c38016c`) を同じ条件で
測って 885.65ms であり、計装のある 922.35 / 905.80ms と同じ帯にある
(§24.5、run 34178981601)。**真の原因は特定できていない**が、09-07 13:26 から
09-08 02:12 の間に `main` へ入った実行時変更 (最有力は #185 = 自動タブ休止の
デフォルト ON) かランナー環境の変化のいずれかである。§24.4 が答えているのは
「どの区間に割れるか」という比率の問いであって、Windows の起動時間の代表値では
ない。

**この Issue では最適化を一切行っていない。** そして Windows の実測が出た
いま、**それが正しい結論であることが確認できた**: VeloX が唯一直接短縮できる
`event_loop` → `pre_window_setup` 区間は Linux 0.07% / Windows 0.06% で、
ここを速くしても起動時間は動かない。**「計測を追加し、最適化はしない」**で
決着させる — #59 (D43) / #60 / #64 / #66 / #69 と同じである。実行パスは
変えていないので、`velox-bench` の before/after で既存メトリクスが動かないのも
当然の結果になる。次に見るべき候補は「2 個目の webview の 186ms」だが、これも
WebView2 側のコストであり、着手するなら**まず区間の分布 (§24.2 の手順を
Windows で実行) を取ってから**別 Issue として起票する。
...
1. ~~Windows の実測 (§24.4) で `event_loop` → `pre_window_setup` 区間が
   無視できない大きさだった場合~~ — **§24.4 で否定された** (0.55ms /
   0.06%)。VeloX が唯一直接短縮できる区間はここだが、最適化対象としての
   価値が無いことが Linux・Windows の両方で確認できた。**将来 VeloX 自身の
   起動時処理を増やしたとき**(起動時に何かをディスクから読む機能を足した
   ときなど) に、この区間が伸びていないかを見る回帰の観測点として使う。
2. `native_window` → `toolbar_webview` (エンジンの初回初期化) — **§24.4 で
   支配的であることが確認された** (672.15ms / 72.9%)。Epic #57 ルール 3 の
   ブラックボックスであり、WebView2 の初期化を前倒し・並列化する手段が
   あるかどうかは wry/tao の API 次第。D43 のフォローアップと同じく、
   新規依存を避け `unsafe` を使わないという本リポジトリの制約の中で
   実現できるかを先に検討すること。
3. **`toolbar_webview` → `window_created` (2 個目の webview) が Windows で
   186.25ms かかる** — Linux の 1.60ms と質的に違い、§24.4 で初めて見えた。
   ただし着手前に §24.2 の手順を Windows 上で実行して**区間の分布 (p95 /
   min / max) を取ること**。現状あるのは累積中央値どうしの差 1 点だけで、
   ばらつきが分かっていない。
4. §21 (644.05ms) と §24.4 (922.35ms) の約 1.43 倍の開き — **本 Issue の
   スコープ外として別 Issue に分離した**。§24.5 に 4 つの計測点と切り分けの
   手順をまとめてある。本 Issue の計装が原因でないことだけは対照実験で確定
   しているので、**#182 の内訳 (比率) の結論はこの未解明点に影響されない**。
   ただし **Windows の起動時間の代表値を論じる前には片付ける必要がある。**

## D93: Memory Budget Manager (Issue #176) Stage 1 — 計測の結果、本体コードは変更しない。RAM 相対の予算は「裸の比率」では成立しないことが分かった

**対象**: Issue #176 (Memory Budget Manager / Adaptive Memory Management、
Epic #57 配下) の Stage 1 (現状計測・Memory Budget 設計)。**D9 / D56 / D90
はいずれも書き換えず、本節から参照する。** 実測値は
`docs/performance-targets.md` §25。

### 決定

**Stage 1 では `src/` に一切変更を加えない。** Epic #57 のルール 1
(ベンチマーク無しに最適化しない) に従い、まず現状を測った。その結果、
Issue 起票時に想定していた「700 MiB という絶対値を搭載 RAM 相対に置き換える」
という打ち手が、**そのままの形では成立しない**ことが分かったためである
(下記「なぜ裸の比率では成立しないか」)。正しい式の形を決めるには
Windows のタブ数スケーリング実測 (未取得) が要る。**「測った結果、
手を入れないのが正解だった」という Epic #57 が明示的に認めている結論である。**

### 起票時点からの前提の変化

Issue #176 は D56 (Adaptive Tab Suspension、既定すべて無効) の世界を前提に
起票されているが、その後 3 つの変更が入っている。**Stage 1 の計画はこの
差分を織り込んで立て直した。**

| 変更 | 内容 | Stage 1 への影響 |
| --- | --- | --- |
| #184 (D90) | タブ休止を既定 ON に。既定シグナルはメモリ予算 700 MiB のみ | 「予算をどう決めるか」が全ユーザに影響する設定になった。Stage 1 の計測対象そのもの |
| #187/#189 (D90) | `process_map` を二段階化、既定ポーリング間隔 2 秒 → 5 秒 | `tab_scaling.py` の `--stabilize-secs` が間隔から自動算出 (15 秒) される。§25 の計測はすべてこの既定で採った |
| #186 (D90) | 複数ウィンドウでの予算をラウンドロビンで修正 | §25 の計測はすべて単一ウィンドウなので影響しない。ただし設計上の制約としては残る (後述) |

### 計測から確定した 4 つの制約

数値は `docs/performance-targets.md` §25。**すべてこのコンテナ 1 台・
同一セッション・Linux/WebKitGTK での実測である** (Epic #57 ルール 5)。

1. **休止では届かない下限が約 397 MiB ある** (§25.2 事実 1)。1 タブ時の
   PSS は予算 0 / 300 / 700 のいずれでも 396.4〜396.8 MiB で動かない。
   `docs/memory-analysis.md` §2.1 のとおり内訳は toolbar 用と content 用の
   `WebKitWebProcess` ×2 が支配的で、**どちらも休止の対象外**である
   (toolbar はタブではなく、content 側の最後の 1 つはアクティブタブ)。
2. **予算 500 MiB 以下では応答が飽和する** (§25.2 事実 2)。300/400/500 の
   3 点は 5/10/20 タブのいずれでも散らばりが 14 MiB 以下で互いに区別が
   つかない。**下限より低い予算は、値を変えてもほとんど挙動が変わらない。**
   併せて分かったこととして、**予算の実効的な分解能はおよそ 1 タブ
   (`ESTIMATED_BYTES_PER_TAB` = 64 MiB) 刻み**である — 10 タブでの
   500/700 の差 13.8 MiB も 20 タブでの 67.5 MiB も、生き残る背景タブが
   1 個違うことで説明がつく (§25.2 事実 2 の注)。**予算をこれより細かく
   刻んでも意味が無い**ので、RAM 相対の式が生む値もこの粒度で丸めて
   考えればよい。
3. **飽和域でも churn は起きない** (§25.3)。10 タブ・40 秒保持で
   `tab_suspend` は予算 300 で 9 件 / 500 で 8 件 / 700 で 8 件、いずれも
   起動後約 5.2 秒に 2〜3ms のバースト 1 回だけで、**同じタブの再休止は
   0 件**。休止済みタブは `plan` の候補に入らないためで、低すぎる予算は
   「CPU を食い潰す」のではなく **`max_live_tabs=1` 相当に退化する** —
   コストはタブ切替のたびの再読み込み (状態喪失) であり、D9 が最初に
   懸念した種類の害そのものである。
4. **予算は階段状に効き、着地点は予算を大きく下回る** (§25.2 事実 3)。
   10 タブは休止 OFF で 980.4 MiB なので、予算 1000 では一度も発動せず
   980.6 MiB、予算 700 では発動して 464.1 MiB — **予算 300 MiB の差が
   PSS 516 MiB の差になる。** プロセスグループ単位で回収する D56 の設計
   上、常にオーバーシュートする。

**付随して分かった、T2 にとってより重要な事実**: 現在の既定 700 MiB は、
10 タブの時点で既に背景タブ 9 個中 8 個を休止している (§25.3)。**「もっと
積極的に休止する」方向に残っている余地は最大でも数十 MiB しかない。**
T2 (Chromium 比 +10% 以内) の残りは、休止ポリシーの調整ではなく制約 1 の
下限 (特に toolbar 用 `WebProcess`) を削ることでしか埋まらない。これは
§12 が既に「残りは (1) 1 タブ時の toolbar 用 `WebProcess` (+45%)」と
書いていたことの、既定 ON 後の再確認である。

### なぜ「裸の比率」では成立しないか

現在の 700 MiB は本環境の搭載 RAM 15.70 GiB の **4.35%** にあたる。この
比率をそのまま他のマシンに当てはめると (§25.4):

- **4 GiB → 178 MiB / 8 GiB → 357 MiB** — どちらも制約 1 の下限
  (約 397 MiB) を割る。制約 2 により**予算として機能せず**、制約 3 により
  **常に背景タブを全部休止する**状態に固定される。
- **64 GiB → 2854 MiB** — 20 タブを休止 OFF で開いても 1626.5 MiB なので
  一度も発動しない。**実質 OFF** になる。

8 GiB は仮定の値ではない: **§21.1 の Windows CI ランナーの搭載 RAM が
まさに 7.99 GiB** であり、そこでの 1 タブ RSS 実測は 367.5 MiB (§21.2) で、
4.35% がそのランナーに与える 356 MiB を既に上回っている。**現実に存在
するマシンで式が破綻する。**

したがって式は最低限
`budget = clamp(下限, 比率 × 搭載RAM, 上限)` の形でなければならない。
そして**3 つのパラメータはそれぞれ別の根拠を要求する**:

| パラメータ | 何で決まるか | 現状 |
| --- | --- | --- |
| 下限 | VeloX 自身の約 397 MiB + 最低限生かす背景タブ分。制約 2 より 500 MiB を下回る値にはほとんど意味が無い | **本環境の 1 点のみ実測済み。** マシン・OS・ページによって動く値であり、定数で持つのが妥当かも未検証 |
| 比率 | 「ブラウザが搭載 RAM の何割を取ってよいか」という製品判断 | **測れない。** 搭載 RAM を変えられるマシンが 1 台も無い (§25.4)。4.35% は現行既定を逆算しただけで、根拠ではない |
| 上限 | 大容量機で実質 OFF にしてよいか (=タブを開くだけ増え続けてよいか) | 未検討 |

**とくに「下限」が定数でよいのかは疑わしい。** 制約 1 の約 397 MiB は
「このマシンで `minimal.html` を 1 枚開いたとき」の値であり、実サイトでは
確実に大きくなる (D56 Revisit condition (2) の「実サイトでの推奨値」と
同じ未解決事項)。**起動直後の実測値を下限として使う (予算を「観測された
下限 + ヘッドルーム」として組み立てる) 設計**は Memory Budget Manager
らしい打ち手だが、本 Issue では一切検証していない — 子 Issue 案 D。

なお「下限 + タブ N 個分」という形は、**`max_live_tabs` を別の単位で
書き直したものに近い。** D56/D90 が `max_live_tabs` を既定にしなかった
理由 (プロセスグループ単位の回収なので上限を 1 超えただけで最大 4 タブ分の
状態を失う / 推奨値が実サイトで未決) は、この形を採るならそのまま
引き継がれる。**設計時にこの等価性を意識せずに進めると、D56 が明示的に
見送った判断を裏口から入れてしまう。**

### Windows について

D90 Revisit condition (1) の「Windows 実機/CI での 700 MiB の実効性は
未計測」に対し、**§21.2 の既存データが部分的な答えを既に持っている**
ことを Stage 1 で確認した。D90 は §21 の存在自体には触れているが
「メモリ予算シグナルを対象にしていない」として使わなかった — しかし
そこに載っている `rss_total_bytes` は、**Windows でメモリ予算と突き合わ
される値そのもの**である: `cold_startup` (1 タブ) の
`rss_total_bytes` 中央値 385,329,152 B = **367.5 MiB**、プロセス数 8、
RAM 7.99 GiB のランナー。**700 MiB の予算に対して約 332 MiB の余裕が
ある** — D90 が「最重要の検討事項」とした「Windows では RSS の二重計上で
700 MiB が極端に早く発動する」という懸念は、**少なくとも 1 タブ時点では
成立していない。**

**ただしこれは切片が分かっただけで、傾きは依然として未知である。**
タブ数を増やしたとき Windows の RSS がどう伸びるかは一度も測られていない。
§25.1 で測った Linux の RSS/PSS 比 (VeloX で 1.68〜1.80 倍) を Windows に
持ち込む根拠は無い (Epic #57 ルール 5)。同一セッションの Chromium が
20 タブで 5.95 倍だったことは、Chromium 系プロセスモデルの WebView2 で
比がもっと速く開きうる**可能性への注意**にはなるが、**予測として使っては
ならない。** 解決手段は `perf-windows.yml` にタブ数スケーリングを足して
実測することだけである (子 Issue 案 A)。

### 子 Issue の分割案

**この節は案であり、Issue はまだ作成していない** — 本セッションからは
GitHub API に到達できなかった (`403 GitHub access is not enabled for this
session`)。作成時は CLAUDE.md の必須ルールどおり `cost:` と `benefit:`
を必ず付けること。

| # | 内容 | 依存 | ラベル案 |
| --- | --- | --- | --- |
| A | **Windows でのタブ数スケーリング実測**。`perf-windows.yml` に `tab_scaling.py` 相当 (1/5/10/20 タブの `rss_total_bytes`) を追加し、Windows の RSS の傾きを採る。D90 Revisit (1) と (7) をまとめて解消できる | なし | `cost:medium` / `benefit:4` |
| B | **搭載 RAM の検出**。`browser::metrics` に `installed_ram_bytes()` を追加 (Linux: `/proc/meminfo` の `MemTotal`、Windows: `GlobalMemoryStatusEx` — `windows` クレートの `Win32_System_SystemInformation` feature 追加が要る、macOS: 未対応でフォールバック)。単体で完結し、テストしやすい | なし | `cost:low` / `benefit:3` |
| C | **RAM 相対のメモリ予算の導入**。`clamp(下限, 比率 × RAM, 上限)` の 3 パラメータを決めて `SuspensionPolicy::default()` に入れる。既存の設定ファイルに保存済みの `memory_budget_mb` (絶対値) との整合・移行も含む | A, B | `cost:medium` / `benefit:4` |
| D | **起動直後の実測値を下限に使う自己調整**。予算を「観測された下限 + ヘッドルーム」として組み立てる案の検証。`max_live_tabs` との等価性 (上記) の整理を含む | C | `cost:high` / `benefit:3` |
| E | **1 タブ時の toolbar 用 `WebProcess` の削減**。制約 1 の下限そのものを下げる。§12/§25 の両方が「T2 の残りはここ」と指している | なし | `cost:high` / `benefit:5` |

**着手順としては A → B → C。** A が無いまま C を実装すると、Windows での
挙動を確かめずに全 Windows ユーザの既定を変えることになり、CLAUDE.md の
「Windows を最優先」に反する。

### Revisit condition

(1) **本節の計測はすべて Linux/WebKitGTK・`minimal.html`・このコンテナ
1 台である。** 制約 1 の下限 (約 397 MiB) も制約 2 の飽和点 (≈500 MiB) も
実サイトでは確実に動く — D56 Revisit condition (2) が未解決のままである
以上、これらの数値を「定数として正しい」と扱ってはならない。
(2) **子 Issue 案 A が済むまで、RAM 相対の式は実装しない** (上記の理由)。
(3) §25.4 の「想定マシンの RAM」表は、25.2 の予算軸の実測に比率を掛けた
**外挿であり実測ではない。** 搭載 RAM の異なるマシンが手に入った時点で
測り直すこと — とくに「その RAM のマシンにとってその予算が体感として
妥当か」は本環境では原理的に測れていない。
(4) §25.3 の休止イベント計測は各予算 1 回ずつの実行であり、試行を重ねて
いない。結論 (再休止 0 件) は明快だが、`tab_suspend` の件数そのもの
(8 対 9) を有意差として扱うには試行数が足りない。
(5) 複数ウィンドウでは #186 のラウンドロビン (D90 の (8)(9)) が効くため、
§25 の単一ウィンドウの応答曲線がそのまま当てはまらない。RAM 相対の予算を
入れるときは、予算がウィンドウ単位ではなくプロセスツリー単位である
(サンプルは 1 つ) 点との整合を確認すること。
...
## D96: Windows の起動「回帰」(Issue #208) は回帰ではなかった — 同一 run 内 A/B で #185 を否定し、`windows-latest` が run ごとに別スペックのマシンを割り当てることを確認した

**対象**: Issue #208。**`src/` に変更を加えていない。** Epic #57 の
ルール 1 (推測で直さない) に従い、まず切り分けの実験だけを行った。実測値は
`docs/performance-targets.md` §26。

### 決定 1: 切り分けは「コミットの二分探索」ではなく「同一 run 内の A/B」で行う

Issue #208 が最初に挙げた手順は、`beba7b7` (#185 の 1 つ前) と `a6aa703`
(#185) にブランチを立てて `perf-windows.yml` を複数回まわす二分探索だった。
**採らなかった。** これは run をまたぐ比較であり、ランナー個体差・ランナー
イメージや WebView2 Runtime の更新・run 1 が外れ値である可能性が、どれも
分離できないまま残るためである — §24.5 が結論を出せずにいたのは、まさに
これらが交絡していたからで、同じ交絡を抱えた点を増やしても確度は上がらない。

代わりに `perf-windows.yml` に、**同一ジョブ・同一ビルド・同一ランナーの中で
「既定 (A)」と「指定した環境変数を設定した状態 (B)」を交互に計測する**
モードを入れた (入力 `compare_env` / `repeats`、既定は
`VELOX_MEMORY_BUDGET_MB=0` を 2 ペア)。上記の交絡はすべて A と B で共通なので
相殺され、差が出れば「その環境変数が起動時間を変えた」以外の説明が残らない。
実験用ブランチを候補コミットに立てる必要も無くなる。

`VELOX_MEMORY_BUDGET_MB=0` が B 条件として成立するのは、
`config::resolve_suspension` の `overridable` が「明示的な `0` は既定が
`Some` でも必ず `None`」と決めているためで (D90 が既定 ON にするときに
用意した逃げ道そのもの)、かつ `settings.json` は起動時に自動生成されない
(`app::run` の `None => config.to_settings()` は in-memory に留まる) ため、
クリーンな CI ランナーで `apply_settings` に上書きされる心配も無い。

### 決定 2: #185 は起動時間の原因ではない (実測)

§26.4 の A/B で `startup_window_created_ms` の差は **-5.10ms (-0.9%)**、
**A 同士のばらつき 11.30ms の内側**だった。全メトリクスで比は 0.991〜1.018。
26.2 に書いた読み方の規則をそのまま適用して、**差は無い**と読む。
**§24.5 の最有力仮説は否定された。** #185 (D90) を戻す・遅延させるといった
対応は不要であり、10 タブ -52.6% / 20 タブ -62.7% のメモリ削減 (§23) を
起動時間と引き換えにするトレードオフ判断も**発生しない**。

コード上も整合する: `spawn_memory_pressure_sampler` は `sleep(interval)` を
**先に**実行する (既定 5 秒) ため、起動区間 (~570ms) にサンプリングは
一度も重ならない。

### 決定 3: `windows-latest` の run 間で絶対値を比較しない

§26.4 事実 2 のとおり、`windows-latest` は run ごとにハード構成の異なる
マシンを割り当てる — §21.1 は **AMD EPYC 9V74 / 2 論理コア / 7.99 GiB**、
本 run は **Intel Xeon Platinum 8573C / 4 論理コア / 15.99 GiB** で、OS
ビルドと WebView2 Runtime は一致している。**§24.5 が疑った「イメージの
更新」ではなく、ハードウェアそのものが違う。**

そして `startup_window_created_ms` の絶対値は 644.05ms (run 1) /
885〜922ms (run 2/3/4) に続く**3 つ目の帯 573.45ms** に落ちた。したがって
**「644 → 885ms の回帰」は回帰ではなく、機種差を見ていた可能性が高い。**

これは Epic #57 ルール 5 (OS ごとに結果を分ける) の Windows 版として
**「run ごとに分ける」**が要ることを意味する。運用ルールとして:

- **別 run の Windows の数値を並べて「速くなった / 遅くなった」と言わない。**
- Windows で条件を比較したいときは、**同一 run 内の A/B で測る。**
- 結果を残すときは CPU モデル・論理コア数・搭載 RAM を必ず併記する。本 PR
  では A/B 比較表のすぐ上にこの 3 つを出すようにした (`results/
  environment-info.md` にも従来どおり保存される)。

### 副産物: 固定 wait の除去

初回実行 (run 34225449751) は、固定ページを配信する
`python -m http.server` を `Start-Sleep -Seconds 2` で待つ既存ステップが
2 秒では間に合わず落ちた。D84/D85 が統合テストから固定 wait を追い出したのと
同じ失敗なので、**最大 30 秒・0.5 秒間隔で実際に取得できることを確認する**
形に変え、サーバの stdout/stderr を `diag/` に残すようにした。

### Revisit condition

(1) **解決済み。** `page_load_ms` が §21.2 の 54.10ms に対し 2077 / 2085ms
と 38 倍離れていた原因は、**本 PR が初回失敗の対応で足した固定ページ配信
サーバの stdout/stderr リダイレクト**だった。外した 3 回目の run で
**23.22 / 22.42ms** に戻っている (§26.4 付随)。`python -m http.server` は
シングルスレッドでリクエストごとに stderr へログを書くため、書き込み先が
ファイルになると応答がブロックされる。**計測環境に足した診断用の仕組みが
計測対象を歪めた**例であり、しかも失敗ではなくそれらしい数字が出るため、
既知の値 (§21.2) と突き合わせなければ気付けなかった。診断用の仕掛けを
足したら既知の値と一度突き合わせること。なお 3 回目の A/B でも事実 1 は
再現しており (差 -0.20ms)、**#185 の否定は 3 回連続で確認できている。**
(2) 決定 3 は「機種が違えば絶対値も違う」という観測に基づくが、**同一機種の
run を複数回そろえて分散を採ったわけではない。** 機種を揃えても run 間で
どれだけ動くかは未計測であり、「同一機種なら比較してよい」とまでは言えない。
(3) **着手済み。** 本節の A/B は `cold_startup` (1 タブ) のみだったが、
同じ A/B モードを `tabs_10` で回した結果が §27 にある。**Windows でも
D90 のメモリ予算は機能する** (10 タブの RSS が予算 OFF の 889.9 MiB に対し
既定で 660.9 MiB、-25.7%)。ただし `tabs_N` はプロセスの生存が約 6 秒しか
なく、休止判定のサンプラ周期 (既定 5 秒) と噛み合わないため、
`VELOX_MEMORY_CHECK_INTERVAL_MS=500` を A/B 共通で設定して初めて測れた
(§27.1/§27.2)。**「既定設定のままの Windows ユーザが 10 タブ開いたときの
値」はまだ測れていない。**
(4) Windows の回帰を継続的に見る仕組み (定期計測・機種別集計) は入れて
いない。Issue #208 の残タスクとしてそこだけが残る。

---

## D97: Windows のメモリ計測は「回収が終わるまで待つ」シナリオで行う (Issue #197) — `tabs_N` の値で製品の挙動を論じてはいけない

**対象**: Issue #197。`src/` への変更は `Scenario::TabCountMemoryHold`
(`tabs_hold_N`) の追加のみで、**ブラウザ本体の挙動は変えていない。**
実測値は `docs/performance-targets.md` §28。

### 決定

**Windows のメモリを論じるときは `tabs_hold_N` を使い、`tabs_N` の値を
根拠にしない。**

`tabs_N` はタブを待ちなしで連続に開き、`MEMORY_STABILIZE_MS` (3 秒) 待って
`quit` する。プロセスの生存は約 6 秒で、休止判定を行う
`app::spawn_memory_pressure_sampler` は `sleep(interval)` を**先に**実行
するため、既定 5 秒周期では判定が 0〜1 回しか走らない。**つまり `tabs_N`
が測っているのは「タブが開き切る前、回収が終わる前」の途中の値である。**

`tabs_hold_N` は (1) タブを開く間隔を空け、(2) 開き終えてから 12 秒待ち、
(3) そこで `mark` を打ってから 8 秒の窓を測る。`benchmark::
aggregate_trials` は最後の `measure_start` 以降しか集計しないので、
**得られる値は落ち着いた後の定常値**になる。

### この決定が覆した 3 つの記述

いずれも §27.3 / §27.4 で `tabs_N` を使って出した結論である。

| 記述 | 実際 (§28.5) |
| --- | --- |
| 「素の傾きは逓減する (66.7 → 51.7 → 27.4 MiB/タブ)」 | **約 65 MiB/タブで線形** (64.9 / 64.2 / 65.3)。逓減は「まだロードされていない分」が増えて見えた錯覚 |
| 「20 タブでは予算 700 MiB を 16.6% 超過する (815.9 MiB)」 | **616.8 MiB で予算内。** 超過は回収が終わる前に測っていたため |
| 「Linux は予算を下回りすぎ、Windows は届かない」 | **待ち時間を揃えるとほぼ同じ着地点** (Linux 464.1 MiB / Windows 463.0 MiB、10 タブ) |

**とくに 3 番目は、危うく「OS によるプロセスモデルの差」という構造的な
結論に飛びつくところだった。** `with_related_content_view` が Linux/BSD で
しか働かず、Windows ではプロセスグループに実体がないのは事実だが、
**着地点においてその差は現れなかった。** コードから読める構造的な違いが、
そのまま観測される差の原因とは限らない。

### 併せて確定したこと

**VeloX の自動タブ休止は Windows で正しく機能している。** 20 タブで
1602.6 MiB → 616.8 MiB (**-61.5%**)、10 タブで 950.0 → 463.0 MiB
(**-51.3%**)。§27.4 が -30% と読んでいたのは、B (予算 OFF) 側も途中の値
だったためである。

また `tabs_hold_10` の A (463.0 MiB) が `tabs_hold_5` の A (630.0 MiB)
**より小さい**。5 タブでは予算 700 に届かないので発動せず、10 タブで初めて
発動して 5 タブ時より下に落ちる — **予算がタブ数ではなくメモリ量で効いて
いることの直接の観測である。**

### Revisit condition

(1) **解決済み (§28.8)。** `rss_process_count` を A/B 比較表に足して
測り直した結果、**20 タブで 27 → 11、16 プロセスが実際に消えていた。**
「webview を drop してもプロセスが残るのではないか」という懸念は否定され、
**#176 の設計課題としてこの問題は存在しない。** RSS 比 2.589 とプロセス数比
2.455 がほぼ一致しており、メモリ削減はプロセス削減にほぼ比例している。
併せて「タブ 1 個 = WebView2 プロセス 1 個 + 基本プロセス 7 個」という構成
が数字で確認でき、予算 ON の 20 タブは**生存タブ 4 個**に落ち着いていた
(Linux の `max_live_tabs=4` 相当という §25 の記述と同じ数だが、一致の理由は
説明できていない)。
...
(1) **解決済み — 原因は仮説 A でも B でもなく、シークレットの値に混入した
改行だった (下記「原因」節)。**
(2) **解決済み。** トークン再登録後、agent モードのプローブ
(`claude-token-check.yml`) が `TOKEN_OK` を返しただけでなく、**tag モードでも
実際に動いた。** PR #216 で Codex が利用上限に達し、auto-merge が設計どおり
`@claude` レビュー依頼を自動投稿 → `claude.yml` が起動 → Claude が差分を読んで
**実質的なレビューを返した** (run 34370056438、2026-09-09)。

しかも**その指摘は正しかった** — `docs/performance-targets.md` §3.1 の定義表と
数式が、上界を `ShareCount` で締めた後 (`5cacd80`) も `rss_bytes` のままである
ことを見つけた。私は print / JSON の `memory_metric` / Job Summary の 3 か所を
直したときに、**メトリクスの一次定義である §3.1 を 4 か所目として見落として
いた。** レビューはその見落としを的確に突いている。

**つまり D91 決定10 のフォールバック経路は、依頼の自動投稿から実際のレビューまで
一気通貫で機能する。** Codex の利用上限で PR が永久にブロックされる状態
(Issue #207 が指摘した問題) は解消された。
...
### 原因 — `CLAUDE_CODE_OAUTH_TOKEN` の値に改行が入っていた (run 34337763164)

`claude-token-check.yml` を main で実行し、`show_full_output: true` で
現れたエラー本文がこれである。

```
Invalid auth token · Fix external auth token · Invalid Authorization header
value from CLAUDE_CODE_OAUTH_TOKEN: it contains a line break at character 107
(110 characters on 2 lines).
```

**シークレットの値そのものが 2 行になっていた。** 110 文字中、107 文字目に
改行が入っている。HTTP ヘッダの値に改行は入れられないため、Claude Code は
**ネットワークに出る前にローカルで弾いていた**。仮説 A (失効) でも仮説 B
(利用上限) でもない、**第三の原因**である。
...
| 観測 | 説明 |
| --- | --- |
| `duration_ms: 82` | ネットワーク往復が無い。ヘッダ組み立ての時点で失敗している |
| `modelUsage: {}` / `total_cost_usd: 0` | モデルを 1 回も呼んでいない |
| init (`model: claude-sonnet-5`) までは成功 | 初期化はトークンを使わない |
| 再試行しても毎回同じ | 一時的な事象ではなく、値が壊れている |

**この症状は「時間をおいて再試行する」では絶対に直らない。** 仮説 B を
前提に待つ運用にしていたら、いつまでも解決しなかった。

### 対処

**GitHub の Actions secret `CLAUDE_CODE_OAUTH_TOKEN` を、改行を含まない値で
登録し直す** (`Settings > Secrets and variables > Actions`)。`claude
setup-token` の出力をコピーする際、折り返し表示ごと選択すると改行が混入
しうる。貼り付けたあと**末尾で Enter を押さない**こと。シークレットは
セッション側からは設定も確認もできないため、これはオーナーの手作業になる。

再登録後は `claude-token-check.yml` を `workflow_dispatch` で実行して
`TOKEN_OK` が返ることを確認する。**PR にコメントを投稿しないので、何度
試しても副作用が無い。**

### 教訓

**「トークンが登録されている」ことと「トークンが使える」ことは別である。**
D98 の前半で「マスクされていること自体が登録済みの証拠だ」と書いたのは
正しかったが、そこで止まってしまい、**値の中身が壊れている可能性を仮説に
挙げられなかった**。仮説 A (失効) と仮説 B (利用上限) はどちらも「値は
正しいが受理されない」という形をしており、「値そのものが不正」という
第三の形を見落としていた。

そして**その第三の形は、伏せられていたエラー本文にはっきり書かれていた**。
`show_full_output: false` という既定値ひとつのために、1 文字ずつ位置まで
教えてくれる診断メッセージが読めず、複数の PR が手動マージに追い込まれて
いた。**原因を推測する前に、まずログを全部見えるようにするべきだった。**

### 追記 (2026-09-09): `show_full_output` は既定 (`false`) に戻した

`claude.yml` の `show_full_output: true` に添えた「本リポジトリはプライベート
なので、ログを読めるのはオーナーだけ」という但し書きは、リポジトリの public
化により**失効した** (D102 決定2)。上の診断は既に完了しているため、
`claude.yml` は既定へ戻し、再診断が必要なときは
`claude-token-check.yml` のプローブ (`workflow_dispatch`、`show_full_output`
は `true` のまま) を先に使う運用にした。

**上の教訓「まずログを全部見えるようにするべきだった」は取り下げない。**
診断中に一時的にログを開けるのは正しい判断だった。誤りだったのは、
**目的を果たしたあとも設定を残し、しかもその根拠 (private だから安全) を
検証しないまま前提に置き続けたこと**である。
...
`QueryWorkingSet` は working set の各ページについて「共有か (`Shared`)」と
「何プロセスで共有されているか (`ShareCount`)」を返すので、1 回の呼び出しで
すべて得られる:

    private_bytes  <=  真の PSS  <=  pss_upper_bytes  <=  rss_bytes

**ここで `ShareCount` の扱いが D88 と決定的に違う。** D88 が退けたのは
`1/ShareCount` を**近似値**として使うことだった。**上界として使えば、飽和は
破綻しない:**
...
**いずれの場合も n >= c**。よって各ページの寄与は `page_size / n <= page_size / c`
であり、`private + Σ(page_size / c)` は常に真の PSS 以上になる。**飽和は上界を
緩めるだけで、上界であること自体を壊さない。** 飽和ページが 1 枚も無ければ等号が
成立して PSS そのものになる。

`rss_bytes` (Working Set 合計) も正しい上界だが、「c = 1 と置いた」のと同じで
最も緩い。§29.8 の実測では 4 倍ほど緩く、**そのままでは T2 を判定できなかった**
(区間が重なって `inconclusive` になった)。この経験が上界を締める動機である。

ページ単位の計算は `working_set_pages` として **FFI の外に切り出し**、飽和した
場合を含めてユニットテストで固定した — 検証の無い箇所で落ちた §29.7 の教訓による。
...
### 続き: D101 で Windows 用の目標を定義した

この決定の結論 (Windows では T2 を判定できない) を受けて、**Windows で判定できる
目標 T2-W を D101 で定義した** — 「区間比較で `met` を出す」。上下界はどちらも
厳密な量なので PSS 無しで判定でき、しかも現在は 4.8% 不足で未達である。
**Private Working Set で定義し直す案は却下した** — VeloX は既に比較相手の 51.6% で、
目標として機能しない (何もしなくても「達成」になる) ため。

### Revisit condition

(1) **結論が出た: Windows では現状の手段で T2 を判定できない** (§29.10)。
上界を締めた run 34367395559 で VeloX [88.9, 198.7] MiB / Edge [172.4, 311.7] MiB。
上界は 46〜55% 縮んだが、**なお 4.8% 足りずに `inconclusive`** のままだった
(`met` には VeloX 上界 <= Edge 下界 × 1.10 = 189.6 が必要で、実測は 198.7)。

**これは測定の失敗ではなく、Windows が PSS 相当を提供していないことから来る
原理的な限界である。** `ShareCount` は 7 で飽和する一方 Edge は 15 プロセスあり、
飽和したページを最大 2 倍多く見積もっている。これを解消するには同じ物理ページの
重複を検出する必要があるが、**`QueryWorkingSet` が返すのは仮想ページ番号で、
物理ページの同一性が分からない。** 突き合わせる手段がない以上、この幅が限界である。

したがって **Stage 1 (Issue #176) を「T2 達成」で閉じることはできない。**
判定できるようにするには ETW/WPT、非公開 API (`SystemSuperfetchInformation`)、
あるいは **Windows 向けに T2 の定義自体を見直す** (Windows で厳密に測れる量、
例えば Private Working Set で目標を定義し直す) のいずれかが要る。
**測れない量で目標を決めていること自体が問題**とも言える。
(2) ⚠️ **`proctree.py` の Windows 実装 (ctypes による FFI) が返す値の妥当性は
未確認。** run 34364659960 で FFI 自体は動き、ブラウザの起動・`load` 検知・
メモリ採取まで到達した (その後の表示で落ちた。§29.7) が、**採れた数値が正しいか
は誰も見ていない。** 動いたことと正しいことは別である。 開発環境は Linux コンテナで Windows 実機が無い (D61/D88 と同じ制約)。
...
前提で読むこと。**
(3) **的中し、対策しても足りなかった。** §29.8 で区間が広すぎ (VeloX 4.2 倍幅 /
Edge 4.0 倍幅) `inconclusive` になったため、上界を `ShareCount` で締めた
(決定1 の改訂)。§29.10 で幅は 2.2 倍 / 1.8 倍まで縮んだが、**判定には届かなかった。**
下界 (`private_bytes`) を締める手段は依然として無い — 共有ページを何プロセスが
使っているかは分かっても、その中で自プロセスの取り分の下限は決まらないため。
**上下どちらもこれ以上締められない**というのが (1) の結論の根拠である。
(4) `scripts/profile/pss_sampler.py` はまだ独自実装のまま。Linux 専用ツールで
今回の変更に巻き込む必要が無かったため触っていない。

## D100: auto-merge の待ち時間を縮める (#188/#194/#216) — Claude のコメントレビューを受理し、レビュー返答で即再評価し、Codex の無駄な往復を省く

**Scope**: PR #210 以降、1 つの PR がマージされるまでに毎回十数分〜数十分かかって
いた。#216 でその内訳を実測したところ、**待ちの大半は実作業ではなく段の切り替え
待ち**であり、しかも**一部の経路は原理的に完走できなかった**ことが分かった。

### 実測した内訳 (PR #216 head `4a2e852`)

| 時刻 (UTC) | 出来事 | 待ちの正体 |
| --- | --- | --- |
| 15:30 | push | — |
| 15:35 | CI 完了 → auto-merge 起動 | 猶予期間 (15:45 まで) 未経過でスキップ |
| 15:51 | 手動 dispatch → `@codex review` 投稿 | **次の起動要因が無く滞留** |
| 15:52 | Codex が上限を返答 | **何も起動しない** |

CI 自体は 5 分。1 回のマージに auto-merge の実行が 3 回要るのに、**自動で起こるのは
最初の 1 回だけ**だった。

### 決定1 (F): Claude の「完了コメント」をレビューとして受理する

**これが無いと Claude フォールバックは永久に完走しない。**
`anthropics/claude-code-action` は**正式な PR レビューを作れない** — 実行時の
プロンプトに `What You CANNOT Do: - Submit formal GitHub PR reviews` と明記されて
おり、実際 PR #216 のレビューオブジェクトは 0 件だった。一方
`_claude_reviewed_head_sha` はレビューオブジェクトだけを見ていた。つまり
**レビューは実際に行われ、的確な指摘まで出ていたのに、ゲートが受理できずに
止まっていた** (#216 で実測)。

D91 の当該 docstring は「Claude がレビューオブジェクトを作らない実装だった場合は
緩和が成立しなくなるが、その場合は安全側に倒れるだけ」と書いていた。**予期は
正しかったが、実際には『安全側』が『永久に手動マージ』を意味していた。**

Issue #203 が「コメントを数えるのはやめる」と決めた理由は正当である — 起動直後の
進捗コメント (「working…」) を数えると、**レビューが 1 文字も書かれていない時点で
要件が緩和されてしまう。** しかし**進捗と完了は本文で区別できる**:

    **Claude finished @noan98's task in 2m 15s** —— [View job](...)

完了時にのみこの見出しが付く (`_CLAUDE_COMPLETION_RE`)。失敗時は
「encountered an error」で、この見出しは無い。したがって**完了見出しを含む
コメントだけを数える**なら #203 の懸念は再現しない。見出しの文言が action 側で
変わったら一致しなくなるが、そのときの挙動は「マージを見送る」= 安全側である。

### 決定2 (A): レビューの返答で auto-merge を再評価する

`on:` に `issue_comment: [created]` と `pull_request_review: [submitted, dismissed]`
を追加した。Codex の上限メッセージも Claude のレビュー完了もコメントとして届くので、
**十数分の滞留が数秒になる。**

なお `schedule: */10` は D91 で「最悪遅延を縮めるため」に入れたものだが、
**実測ではほとんど発火していない** (GitHub が負荷に応じて間引く)。頼りにできるのは
今回追加したイベントと `workflow_dispatch` である。

> ⚠️ **セキュリティ**: `contents: write` とマージ権限を持つ workflow をコメントで
> 起動できるようにするのは、一般には権限昇格の経路になりうる。許容できると判断した
> 根拠は 2 つ。(1) 本リポジトリはプライベートで、コメントできるのは共同作業者だけ
> (**公開リポジトリならこの判断は成立しない**)。(2) **このジョブはコメント本文を
> 判定に一切使わない** — 起動後は `gh pr list` から取り直して API から一から判定
> する。コメントでできるのは「判定をやり直させる」ことだけで、判定結果は変えられない。

> 🚫 **上の根拠 (1) は失効した (2026-09-09)。** リポジトリが public へ変更され、
> **コメントできるのは共同作業者だけではなくなった** — 「公開リポジトリなら成立
> しない」と自分で書いた条件に、その 2 日後に自分で入った。根拠 (2) は現在も有効
> だが、それだけでは「外部ユーザが書き込み権限付き job を起動できる」状態は正当化
> できない。ジョブ `if` で投稿者を絞る対応を **D102** で入れた。**D100 の決定2 (A)
> を採用するなら、D102 の投稿者絞り込みとセットでなければならない。**

### 決定3 (B): Codex の上限が分かっているなら往復を省く

従来は「`@codex review` を投げる → 上限が返る → 次の実行で Claude に依頼」と
1 往復していた。Codex が上限に達している間、この往復は確実に無駄になる。直近
`CODEX_USAGE_LIMIT_LOOKBACK_HOURS` (既定 6 時間) 以内にこの PR で上限を観測して
いれば、最初から Claude に依頼する。

**安全性を緩めないための制約を 2 つ付けた。**

1. **Claude フォールバックが設定されているときだけ有効。** 未設定なら行き先が
   無く、Codex への再依頼を止める副作用だけが残る。
2. **「過去の Codex レビューでの代替」には使わない。** 別 commit へのレビューで
   現在の head を通すのは最も強い緩和なので、従来どおり「この head SHA への依頼
   より後に上限が来た」という厳密な検知 (`limit_strict`) のときだけ許す。B が
   導入した緩い検知は Claude に**新しくレビューさせる**経路にのみ使う。

窓を設ける理由: Codex の上限は時間で回復する。窓が無いと回復後も永久に Codex を
使わなくなる。`codexUsageLimitLookbackHours: 0` で B を無効化して従来の挙動に
戻せる (テストで固定)。

### 検証

既存 103 件のテストを**1 件も変更せずに**通した — 上記の制約により、既存の安全性は
一切緩んでいない。新規 11 件を追加 (合計 114 件)。加えて**変異テスト**で、以下を
壊すと実際にテストが落ちることを確認した:

- 完了見出しの判定を外す (進捗コメントでも通る) → 4 件失敗
- B の「Claude 設定時のみ」条件を外す → 2 件失敗
- 過去 Codex レビュー代替を `limit_recent` でも許す → 1 件失敗

### Revisit condition

(1) **ブランチ保護は別問題として残る。** F/A/B はいずれも「ゲートの判定が出るまで」
を速くするもので、その先の `the base branch policy prohibits the merge`
(承認レビュー必須) は解消しない。**リポジトリ設定の変更が要る**ため、この決定の
範囲外とした。auto-merge のゲートが実質的に同じ役目を果たしているので、
「Require approvals」との二重化を見直す価値はある。
(2) **完了見出しの文言は action の実装依存。** 変われば受理されなくなる (安全側)。
Claude のレビューが付いているのにゲートが待ち続けている場合は、まずこの文言を疑う。
(3) ~~`issue_comment` トリガの空振り実行がどれだけ増えるかは未計測。1 回 15 秒程度の
ジョブなので問題にならない見込みだが、Actions の使用量が気になるなら
`types` を絞るか、bot 以外のコメントを弾く条件を足す。~~
→ **D102 で解消。** ただし動機は使用量ではなくセキュリティだった (public 化)。
「性能上の任意の最適化」として書いていた対応が、前提が変わったことで**必須の
防御**になった、という点も含めて D102 を参照のこと。

## D101: Windows 用のメモリ目標 T2-W を定義する (Issue #197) — 「区間比較で met を出す」。Private Working Set への置き換えは**しない**

**Scope**: D99 Revisit condition (1) で「Windows では現状の手段で T2 を判定できない」
と結論した。T2 は PSS で定義されており、Windows に PSS は無い (D88)。CLAUDE.md が
Windows を最優先と定める以上、**最優先 OS で評価できない目標を Stage 1 (#176) の
完了条件に据えることはできない。** Windows で判定できる目標を定義する。

### 決定: T2-W = `compare_bounds` が `met` を返すこと

    VeloX のメモリ上界  <=  比較相手のメモリ下界 × 1.10

上下界はどちらも厳密な量 (§3.1、D99 決定1) なので、この判定は**真の PSS がどこに
あっても +10% 以内である**ことを意味する。現在は VeloX 上界 198.7 に対し
Edge 下界 × 1.10 = 189.6 で、**4.8% 不足の未達** (§29.10)。

### 却下した案: Private Working Set で定義し直す

一見もっともらしい (Windows で厳密に測れる) が、**目標として機能しない。**
実測は VeloX 88.9 / Edge 172.4 MiB で、**VeloX は既に相手の 51.6%**。
「Private Working Set で +10% 以内」は**既に大差で達成済み**であり、

- **改善を 1 ミリも駆動しない** — 何もしなくても「達成」になる
- **測定を都合よく変える**行為そのもの — この Issue で繰り返し戒めてきたこと

Private Working Set は共有ページを一切含まないので、そもそもこれを
「メモリ使用量」と呼べない。**Private Working Set とプロセス数は回帰ガードの
補助指標として記録するに留め、目標には据えない。**

**この却下判断がこの決定の中心である。** 「Windows で測れる量で定義し直す」という
依頼に対して素直に応じると、**製品が既に通っている指標を目標に据えて『達成』と
宣言する**結果になっていた。目標は、現に未達で、かつ改善が数字に現れるものでなければ
意味がない。

### 達成手段に課す制約

T2-W は 2 通りで達成しうる。**(a) VeloX が実際にメモリを減らす** (正当)、
**(b) 測定の区間が締まる** (**厳密な上下界を保つ改善に限り**正当)。

§29.8 → §29.10 で (b) を一度行っている (`ShareCount` による上界の締め直し、
上界が 46% 縮んだ)。**近似で幅を詰めて `met` を出すのは禁止** — 測れていないものを
測れたことにする行為であり、§27.4 の誤結論・D97 の構造的決めつけと同じ誤りになる。
区間を締める変更には、**なぜ厳密な上界/下界であり続けるのかの証明を必ず添える**
(D99 決定1 が `ShareCount` の飽和について行ったように)。

**(b) だけで到達した場合はそう明記する。** 「測り方が良くなって判定できるように
なった」と「VeloX が省メモリになった」は別の主張である。

### T2 との関係

**T2-W は T2 の十分条件である** (真の PSS ≤ 上界なので、T2-W を満たせば T2 も
満たす)。ただし**上界で比較する分だけ厳しい**主張なので、T2-W が未達でも T2 相当は
満たしている可能性がある。逆に **T2-W 達成をもって「Linux の T2 も達成」とは
言えない** — OS をまたいだ数値比較は成立しない (Epic #57 絶対ルール5、D88)。

**⚠️ この「十分条件」は比較相手が同じものを指しているときにだけ成り立つ。**
不等式 (真の PSS ≤ 上界) の導出自体は正しいが、「T2 を満たした」と言えるかは
*T2 の言う「Chromium」と T2-W が実際に測っている相手が同じか* に依存する。
現在の実測はすべて Edge が相手で、**Chrome の数値は 1 度も取っていない** —
この前提は未検証である (下記 Revisit condition (3)、`docs/performance-targets.md`
§30.5 の警告ブロック)。**T2-W 達成時点で書けるのは「Edge に対して +10% 以内」
までである。**
...
(1) **T2 と T2-W はどちらも未達なので、Stage 1 (#176) を「メモリ目標達成」で
閉じることはできない。** ただし T2-W は 4.8% しか離れていない。
(2) **上界を締める余地は §29.10 の「なぜこれ以上締められないのか」で塞がっている** —
`QueryWorkingSet` は仮想ページ番号しか返さず、物理ページの同一性が分からないため
共有の重複を除けない。したがって (b) の経路で 4.8% を埋めるには、ETW など別の
取得手段が要る。**現実的には (a) 実際のメモリ削減の方が近い。**
(3) **比較相手を Edge に固定してよいかは未検討。** §29.3 のとおり Edge は VeloX と
同じエンジン (WebView2) なのでエンジン差が相殺され、VeloX 自身のオーバーヘッドだけが
残る — 目標としてはこちらが厳しく、意味がある。ただし T2 の原文は「Chromium 比」で
あり、Chrome と比べた場合の数字は未取得。

**この未検討事項は「T2-W は T2 の十分条件」という言い方に直接効く。** 不等式
(真の PSS ≤ 上界) の導出自体は正しいが、「T2 を満たした」と言えるかは *T2 の言う
Chromium と実際に測っている相手が同じか* に依存する。この留保が
`docs/performance-targets.md` §30.5 側の断定的な書きぶりから抜け落ちていたため、
同 §30.5 に警告ブロックを足した (PR #217 への Claude のレビュー指摘による)。
**T2-W 達成時点で書けるのは「Edge に対して +10% 以内」までである。**

## D102: リポジトリの public 化 (2026-09-09) で失効した前提を洗い出し、コメント起動 workflow に投稿者の絞り込みを入れる

**Scope**: 2026-09-09、**CI (GitHub Actions) の利用上限を緩めるためにリポジトリを
public へ変更した**。可視性は「設定項目が 1 つ変わった」だけに見えるが、**これまでの
設計判断のうち「プライベートだから安全」と書いていたものが一斉に失効する。**
D100 の決定2 (A) は導入からわずか 2 日で、**自分で書いた「公開リポジトリなら
この判断は成立しない」という但し書きの側に落ちた。**

### 何が変わったのか

| | private | public (現在) |
| --- | --- | --- |
| Issue / PR にコメントできる人 | 共同作業者のみ | **GitHub の任意のユーザ** |
| PR にレビューを提出できる人 | 共同作業者のみ | **GitHub の任意のユーザ** |
| Actions の実行ログを読める人 | 共同作業者のみ | **誰でも** |
| PR を作れる人 | 共同作業者のみ | 共同作業者のみ (`pull_request_creation_policy: collaborators_only`) |
| GitHub 標準の auto-merge | 使えない | **使える** |

PR 作成が共同作業者に限られている点は重要で、**fork からの `pull_request` トリガ
という最大の攻撃面は存在しない**。失効したのは「コメント」「レビュー」「ログ」の
3 つである。

### 決定1: コメント / レビューで起動する workflow は、投稿者を `if` で絞る

`auto-merge.yml` の `auto-merge` ジョブは `contents: write` / `pull-requests: write`
/ `issues: write` と `AUTO_MERGE_TOKEN` (repo 権限の PAT) を持ち、`issue_comment` と
`pull_request_review` で起動する。**public では任意のユーザがこれを起動できていた。**

ジョブ `if` に次の条件を足した。

- `issue_comment` は **PR に付いたコメントに限る** (`github.event.issue.pull_request.url != ''`)。
  素の Issue へのコメントでは必ず空振りするので、そもそも起動させない。
- **投稿者が信頼できる場合のみ。** `author_association` が
  OWNER / MEMBER / COLLABORATOR、または**ログイン名がレビューボット
  (`chatgpt-codex-connector[bot]` / `claude[bot]`) と完全一致**。
- `pull_request_review` も同条件で絞る。

**なぜログイン名の一致を別に見るのか**: ボットの `author_association` は
NONE / CONTRIBUTOR になりうるので、association だけで絞ると **D100 の決定2 が
狙った「レビューボットの返答で即再評価する」経路ごと塞がってしまう。**
逆にログイン名の一致は偽装できない — `claude[bot]` は `[` `]` を含み、**GitHub の
ユーザ名として作成できない**。D91 で Codex から受けた P2 指摘 (前方一致だと別名
アカウントがすり抜ける) と同じ理由で、判定は完全一致で行う。

**この `if` は多層防御であって、ゲートの安全性そのものではない。** ジョブ本体は
コメント本文を判定に一切使わず、起動後に API から取り直して一から判定する
(D100 決定2 の根拠 (2)、現在も有効)。レビュー主体の照合は
`check_review_gate.py` の `_is_login_in` が `__typename == "Bot"` まで見て行う
(Issue #194)。`if` が担うのは「**外部ユーザに書き込み権限付き job を起動させない**」
ことである。

### 決定2: `claude.yml` の `show_full_output` を既定 (`false`) に戻す

D98 でこれを `true` にしたときの但し書きは「本リポジトリはプライベートなので、
ログを読めるのはオーナーだけ」だった。**この前提は失効した。**

もっとも、`true` にした本来の目的 (`@claude` が 82ms で `is_error` を返す原因を
読む) は**既に達成済み**である — 原因はトークンの値に混入した改行で、再登録後に
PR #216 で tag モードの完走まで実測した (D98)。**目的を果たした診断用の設定が、
根拠を失ったまま残っていた**というのが実態で、public 化はそれを見つける契機に
なったにすぎない。

`claude-token-check.yml` のプローブは `true` のまま残す。起動が
`workflow_dispatch` (オーナーのみ) に限られ、プロンプトが固定文言で
リポジトリの内容を読ませないため。再診断が必要になったらまずこちらを使う。

### 決定3: D55 の「private だから自前 auto-merge」という説明を書き換える

D55 の導入理由は「プライベートリポジトリでは GitHub 標準の auto-merge が使えない」
だった。public になった今、**標準 auto-merge は使える。**

それでも自前実装を残す。**標準 auto-merge にはレビューゲートが無いからである** —
Codex / Claude のレビュー必須化、未レビュー時の自動リクエスト、猶予期間
(Issue #188 / D91) はこの仕組みの本体であり、標準機能では代替できない。

ただし**残す理由が変わった**ことは記録しておく。「private だから」で残っている、と
誤解したまま将来この仕組みを触ると、判断を誤る。`auto-merge.yml` 冒頭と CLAUDE.md
の説明も同様に書き換えた。

### 検証: `if` の式は main に入るまで一度も評価されない

`issue_comment` で起動する workflow は**常にデフォルトブランチの定義**が使われる
(GitHub の仕様。`claude-token-check.yml` の但し書きに既出)。つまり **`if` の式は、
手元でも PR の CI でも一度も実行されない。** これは docs/performance-targets.md §29.9 (Windows CI で 4 回続けて
踏んだ「Linux で検知できたはずの失敗」) とまったく同じ構図である。

そこで `.github/scripts/test_auto_merge_trigger_guard.py` を追加した。
`auto-merge.yml` から `if` の式を**実際に読み出して**、Actions 式言語のごく一部
(`&&` / `||` / `!` / `!=` / `contains()` / `fromJSON()` / コンテキスト参照) を
実装したミニ評価器で評価する。17 件。
...
| ケース | 期待 |
| --- | --- |
| `workflow_run` / `schedule` / `workflow_dispatch` | 起動する (従来どおり) |
| `pull_request` | 起動しない (dry-run ジョブ専用) |
| OWNER / MEMBER / COLLABORATOR のコメント | 起動する |
| **外部ユーザ (NONE / CONTRIBUTOR) のコメント** | **起動しない** |
| `claude[bot]` / `chatgpt-codex-connector[bot]` (association NONE) | 起動する |
| **`claude` / `claude-bot` / `Claude[bot]` という名の一般ユーザ** | **起動しない** |
| auto-merge 自身のマーカー入りコメント | 起動しない |
| 素の Issue へのコメント | 起動しない |
| 外部ユーザのレビュー | 起動しない |

**変異テストで負の対照を取った**: `if` を変更前の式に戻すと 17 件中 **9 件が失敗**
する。テストが実際にこの防御を見張っていることを確認済み。

ミニ評価器そのものにもテストを付けた (4 件)。特に **Actions の比較は型が違うと
数値へキャストする**ため `null` と `''` は等しく、`github.event.issue.pull_request.url != ''`
が「Issue なら false」になる — 素の Python の比較では再現できないので、ここを
取り違えると防御が効いているつもりで効かない。

### 副産物: `.github/scripts` のテストは CI で一度も走っていなかった

このテストを置く場所を探して気付いたが、`test_check_review_gate.py` (103 件) も
`test_extract_closing_issues.py` も **CI のどこからも実行されていなかった**
(`ci.yml` は Rust だけを見ている)。D91 で「単体テストがある」と書いた 103 件は、
**人間が手元で走らせたときだけ**動いていた。

`auto-merge.yml` の `dry-run-review-gate` ジョブ (この workflow とスクリプトを
変更する PR でのみ起動) に、Python の単体テストと shell 側テストを回すステップを
足した。`pull_request.paths` も `.github/scripts/**` に広げ、テストだけを変更した
PR でも走るようにした。

### Revisit condition

(1) **他にも失効した前提が残っていないか。** 今回は「プライベート」で全文検索して
`claude.yml` / `CLAUDE.md` / `auto-merge.yml` の 3 箇所を見つけた (`docs/` の
「プライベートモード」は VeloX の機能名で無関係)。**可視性以外にも、リポジトリ設定に
依存した判断は他にもありうる** — ブランチ保護の必須承認数を 0 にした件 (D100
Revisit (1)) も同種である。設定を変えるときは、それに依存した判断を検索する習慣を
付ける。
(2) **`CLAUDE_REVIEWER_LOGINS` と `if` のボット一覧が二重管理になっている。**
`auto-merge.yml` の `env` と `if` の `fromJSON` 配列に同じログイン名が書かれている。
`if` は `env` を参照できない (GitHub の仕様) ため現状は避けられないが、片方だけ
変更すると**ゲートは通るのにジョブが起動しない**という分かりにくい壊れ方をする。
ボットを増減するときは必ず両方を直すこと。
(3) **`show_full_output: false` に戻したことで、`@claude` が再び壊れたときの
診断が一段面倒になる。** まず `claude-token-check.yml` のプローブを使う。それで
切り分かない場合に限り、一時的に `true` へ戻す。
(4) 標準 auto-merge との併用 (レビューゲートを満たしたら標準 auto-merge を有効に
するだけにして、マージ自体は GitHub に任せる) は未検討。`AUTO_MERGE_TOKEN` を
持ち回らずに済む可能性がある。

## D103: 猶予期間の満了はイベントを生まない (Issue #219) — 待つのをやめるのではなく、auto-merge が自分で待ち直す

**Scope**: D100 の決定2 (A) で「レビュー返答で即再評価する」を入れ、段の切り替え待ちは
数秒になった。しかし **1 区間だけ取り残されていた** — 猶予期間 (`GRACE_PERIOD_MINUTES`、
既定15分) の満了である。

### 何が起きていたか (PR #218 で実測、UTC)

| 時刻 | 出来事 | 判定 |
| --- | --- | --- |
| 16:49 | push | — |
| 16:50 | CI 完了 → 起動 | 猶予期間未経過 (17:04 まで) |
| 16:55 | Codex 上限コメント → 起動 | 同上 |
| 16:59 | Claude レビュー完了 → 起動 | 同上 |
| **17:04** | **猶予期間が満了** | **何も起きない** |

**auto-merge の起動要因はすべて「誰かが何かをしたとき」である。** 猶予期間の満了は
誰も何もしないので、イベントが無い。レビュー条件が 16:59 の時点で完全に揃っていても、
残り 5 分の時計待ちの間に起動要因が尽きれば、その PR は次に誰かが何かするまで止まる。

### `schedule` は受け皿になっていなかった — 数字で確定した

D91 は `schedule: */10` を「最悪遅延を縮めるため」に入れ、後に「実測ではほとんど
発火していない」と追記していた。Issue #219 でこれを**数えた**。

**cron は 1 日 144 回。実際の発火は全期間で 36 回 (約6%)。**

| 直近の発火間隔 (UTC) | |
| --- | --- |
| 09-09 01:35 → 06:41 | 5時間06分 |
| 09-09 06:41 → 11:58 | 5時間17分 |
| 09-09 11:58 → 16:32 | 4時間34分 |
| 09-08 15:11 → 18:48 | 3時間37分 |
| 09-08 18:48 → 21:30 | 2時間42分 |

**「最後の数分の待ち」は、実際には 2〜5 時間の放置だった。** D91 の定性的な観察が、
定量的には想像よりずっと悪かったことになる。`schedule` を安全網として当てにする
設計判断は、この数字を見たうえで書き直すべきである。

### 却下した案: 猶予期間を短くする / 無くす

**最も安易で、最も間違っている。** 猶予期間は D91 が「レビューボットがまだ投稿して
いない場合の保険」として置いたもので、待ち時間が長いことが問題なのではなく、
**満了に気付けないこと**が問題である。症状 (遅い) に対して原因 (通知が無い) と
無関係な変数 (待ち時間) をいじるのは、§29 で繰り返し戒めてきた「測定を都合よく
変える」のと同じ形をしている。

### 却下した案: `schedule` の間隔を詰める

上の実測どおり、`*/10` ですら 6% しか発火していない。**間隔を詰めても発火率が
上がる保証は無い** (GitHub は負荷に応じて間引く)。効かない対策を入れて「対処した」
と記録するのが一番害が大きい。

### 決定: 判定側が「あと何秒か」を返し、auto-merge が同じ run の中で待ち直す

`check_review_gate.py` の返り値に 2 つ足した。

| フィールド | 意味 |
| --- | --- |
| `grace_remaining_seconds` | 猶予期間の残り秒数 (切り上げ)。猶予期間が理由に挙がったときのみ非 `None` |
| `blocked_only_by_grace` | **ブロック要因が猶予期間だけ**か |

`auto-merge.yml` は全 PR を評価したあと、`blocked_only_by_grace` が真の PR の
**最短の残り時間**だけ待ち、**全 PR をもう一度評価する**。

**`blocked_only_by_grace` が要である。** 猶予期間以外の理由 (未解決スレッド・
レビュー待ち) が 1 つでも残っているなら待ってはいけない — それらは時計が進んでも
解決せず、誰かが行動したときにイベントで起動するのが正しい。待てば runner を
占有したうえで同じ結論に戻ってくるだけになる。

**待ちは高々 1 回、上限は猶予期間 + 1 分**、job にも `timeout-minutes: 30` を置いた。

### なぜ「待って再評価」が安全か

待ち明けには `gh pr list` から**取り直して一から評価する**。待つ前の判定結果は
一切持ち越さない。したがって:

- 待っている間に push があれば head SHA が変わり、`--match-head-commit` が弾く
- チェックが増減していれば再評価で拾う
- 待っている間に別の PR が条件を満たせば、同じ再評価で一緒に処理される

`concurrency: group: auto-merge` を最大 16 分ほど占有するが、**その間に起動した
run が待たされても失うものは無い** — 待ち明けの再評価が全 PR を見るためである。

### 実装で踏んだ罠: 秒→分→秒の往復で 1 秒ずれる

残り時間を `(猶予分 - 経過分) * 60` で計算すると、二進浮動小数の誤差が乗る。
ちょうど 226 秒のケースが `226.00000000000006` になり、切り上げで **227** になった。
秒のまま引き算するよう直し、**切り上げ 227 / 切り捨て 226 で結果が割れる値**を
テストに固定した (`test_remaining_seconds_rounds_up`)。切り上げるのは、切り捨てると
待ち明けにまだ猶予期間内で二度手間になるためである。

### 決定4: `issue_comment` は `edited` も購読する — Claude の完了は編集で届く

**この PR 自身が、これを入れていなかったせいで 38 分以上止まった** (2026-09-09 実測)。

| 時刻 (UTC) | 出来事 | auto-merge |
| --- | --- | --- |
| 21:55 | auto-merge が `@claude` を依頼 | — |
| 21:56 | 「Claude Code is working…」が **`created`** | 起動 → 「応答を待っています」で見送り |
| 21:59 | **レビュー完了** が **`edited`** | **起動しない** |
| 22:37 | CI 全緑・猶予期間満了済みのまま | まだ停止中 |

`claude-code-action` は**進捗コメントを 1 つ立てて、それを編集して完了させる**。
つまり**レビュー完了は `created` ではなく `edited` として届く。** `types: [created]`
だけを購読していると、拾えるのは「working…」の瞬間だけで、そのときゲートが
「応答待ち」と判定するのは当然である。そして本当のレビューが載っても**二度と
起動しない。**

**`@codex review` 経路では表面化しなかった。** Codex は毎回**新規コメント**を
投稿するためである。これは D100 決定1 で足した **Claude フォールバック経路に
固有の穴**だった。

`types: [created, edited]` にした。編集のたびに起動するので空振りは増えるが
(1 レビューあたり 3〜4 回、1 回 15 秒程度)、auto-merge 自身の依頼コメントは
ジョブ `if` のマーカー判定で弾かれるため自己ループにはならない。

> **これは決定1〜3 と同じ形の誤りである。** 「状態は変わったのに、購読して
> いるイベントが発生しない」。猶予期間の満了 (決定1〜3) は*誰も何もしない*
> ケース、こちらは*誰かが何かをしたのに種類が違う*ケース。**`if` をいくら
> 正しく書いても、そもそも購読していなければ起動しない** — `if` の 1 段
> 外側にも同じ注意が要る。

`test_auto_merge_trigger_guard.py` に `TriggerDeclarationTest` を足し、
`edited` を外すとテストが落ちることを確認済み。

### 副産物: この `run:` ブロックにはテストが 1 件も無かった

`auto-merge.yml` の `run:` に直書きされたマージ判定ループは、**main にマージして
実際に auto-merge が起動するまで一度も実行されない。** `check_review_gate.py` に
100 件超の単体テストがあるのに、**それを呼ぶ側は完全に無検査だった。**
D102 の「`if` の式は main に入るまで評価されない」とまったく同じ構図である。

`test_auto_merge_step.sh` を追加した。`auto-merge.yml` から `run:` の中身を
取り出し、`gh` / `sleep` / `review_gate_decision.sh` をスタブに差し替えた
サンドボックスで実際に走らせる (6 件)。

| ケース | 期待 |
| --- | --- |
| 猶予期間だけが残っている | 残り+15秒 待ち、再評価してマージ (ゲート呼び出し 2 回) |
| **猶予期間 + それ以外**が残っている | **待たない** (残り秒数が分かっていても) |
| ブロック要因なし | 待たず 1 パスでマージ |
| 残り時間が異常に大きい | 960 秒 (猶予15分+1分) で頭打ち |
| 待っても満了しない | 待ち直しは 1 回まで |
| 複数 PR | 最短に合わせて起き、全 PR を再評価 |

**変異テストで負の対照を取った** — 待ち直しの廃止 / `blocked_only_by_grace` の無視 /
境界の余裕の削除 / 最短ではなく最長を採る / 上限の撤廃、の 5 つすべてで
テストが落ちることを確認した。Python 側も 2 つ (唯一の理由判定、切り上げ) で確認済み。

### Revisit condition

(1) **`schedule` を安全網として当てにしている箇所が他にもある。** 6% という発火率は
auto-merge 以外にも影響しうる。`schedule` を持つ workflow を見直す価値がある。
(2) **待ち直しは 1 回まで。** 猶予期間より長い遅延要因が重なると、依然として次の
イベント待ちになる。実運用で 2 回目が必要になる頻度は未計測。
(3) **`concurrency` の占有時間が最大 16 分に伸びた。** 現状は PR 数が少なく問題ない
が、同時に多数の PR が動くようになったら待ち直しを別 job に分ける必要がある。
(4) `test_auto_merge_step.sh` は `run:` ブロックを**文字列として**取り出して実行する。
ステップを増やす・分割するとテスト側の取り出しが壊れる (その場合は明示的に失敗する
ようにしてある)。
(5) **決定4 は「レビューボットがコメントをどう更新するか」に依存している。**
`claude-code-action` が将来「完了時に新規コメントを投稿する」実装に変われば
`edited` は不要になり、逆に別のボットが別の更新方法を採ればまた拾えなくなる。
**購読しているイベントの種類は、ボットの実装を観測して決めるしかない** —
推測で決めてはいけない (今回も実測で分かった)。
(6) `edited` を足したことで空振り実行が増える。実測して問題になるようなら、
`if` に「本文に完了見出しを含む編集だけ通す」条件を足す余地はある。ただし
...
## D106: Performance Dashboard に「機種」という比較単位を足し、`perf-windows.yml` を週次スケジュール実行にする (Issue #211 項目2/項目4) — 機種不明のエントリは安全側に倒して連結しない、ゲート化はまだしない

**対象**: Issue #211 の「やること」項目2 (機種ごとの比較単位) と項目4
(定期計測)。項目1 (`BenchmarkResult.environment` への `cpu_model` 等の
追加、Rust 側) は別 PR が同時に実装しており、本決定はそのスキーマを
**契約として前提にする** (`os`/`cpu_model`/`cpu_count`/
`total_memory_bytes`/`os_version`/`webview_runtime`、すべて省略可能)。
項目3 (同一機種内での分散の実測) は実機を複数回回す必要があるため未着手
のまま — 本決定のゲート化見送りの主因でもある。

### 背景: D96 が残した宿題

D96 (Issue #208) は `windows-latest` が **run ごとに別スペックのマシンを
割り当てる**ことを実測した (AMD EPYC 9V74 / Intel Xeon 8573C / Intel
Xeon 6973P-C / AMD EPYC 7763 の 4 機種、`startup_window_created_ms` は
573〜922ms に散らばる)。D96 の Revisit condition (4) は「Windows の回帰を
継続的に見る仕組み (定期計測・機種別集計) は入れていない」と明記して
おり、本決定はその宿題への回答である。

### 決定1: `session_id` に加えて `machine_key` を比較のもう一段の単位にする

D82 は「`session_id` が同じ隣接エントリ同士だけを比較する」という規則を
データ構造 (フィールドの有無) と UI (線・色) の両方で強制していた。
これは「呼び出し側が明示的に同一と指定した run」の粒度であり、**その run
がどの機種で計測されたかは別問題**である。将来、定期計測が「時系列として
蓄積する」ために複数回の run へ同じ `session_id` を使い回すようになった
場合 (項目4 がまさにその入口)、機種が違う run を同一系列としてつないで
しまう恐れがある。

そこで `scripts/dashboard/common.py` に `derive_machine_key(environment,
salt)` を追加した。`os`/`cpu_model`/`cpu_count`/`total_memory_bytes` から
決まる純粋関数で、`report.py` は **`session_id` と `machine_key` の両方が
一致する隣接エントリ同士だけ**を線でつなぎ、差分を計算し、同じ色を割り
当てる。どちらか一方でも変われば D82 と同じ扱い (点は表示するが線を
引かず、差分も出さない、色も変える) になる。`record.py` は記録のたびに
導出した機種を表示し、機種不明の場合は警告を出す (永続化するフィールドは
増やしていない — `machine_key` は読み出し時に `result.environment` から
毎回導出する、副作用の無い計算値)。

### 決定2: 「機種不明」は、他のどのエントリとも同一機種として扱わない (安全側)

**ここが本決定で最も設計判断を要した部分。** `cpu_model` が無いエントリ
(item1 以前に記録された `results/history/` の全既存エントリを含む) を
どう扱うか、3 案を検討した。

- **却下: 「機種不明」を単一の機種として扱う** (`os`/`cpu_count` だけで
  グルーピングする等)。D96 が実測した通り、同じ `windows-latest` でも
  論理コア数が 2 と 4 で割れる — `cpu_count` が同じだけでは同一機種の
  証拠にならない。「不明」同士を同一とみなすと、比較してはいけない
  組み合わせを比較することになり、D82/D96 の原則そのものを破る。
- **却下: `session_id` を salt にする** (同一セッション内の機種不明
  エントリ同士は同一機種とみなす)。一見自然に見える (同一 CI ジョブなら
  物理的に同一マシンのはず) が、**項目4 が想定する「複数回の run へ同じ
  `session_id` を使い回して時系列を作る」運用と正面から衝突する** —
  もし将来、週次実行のたびに固定の `session_id` (例:
  `"windows-scheduled"`) を使い回す実装になった場合、`session_id` を salt
  にすると、機種不明期間の run は全部「同一機種」に化けてしまう。まさに
  本決定が防ぎたい事故そのものを salt の選び方で再導入することになる
  ため却下した。
- **採用: 呼び出し側がそのエントリを一意に指す値を salt にする。**
  `HistoryEntry.machine_key` は `f"{path}:{line_no}"` (JSONL ファイル内で
  の物理的な行位置) を salt にする — ファイル内で行番号が重複することは
  無いため、**機種不明のエントリは常に他のどのエントリとも異なる
  `machine_key` になる。** 結果として「機種不明」の点は常に孤立点として
  描画され、線でつながることも差分が計算されることも無い。

これにより、item1 以前に記録された既存の `results/history/` の全エントリ
(`baseline-committed` 含む) は、**引き続き問題なく読み込める** (パースは
壊れない) が、**表示上は常に孤立点になる** — 事前の動作確認
(`docs/performance-dashboard.md` §8) 時点のデータで、以前は同一
`session_id` 内で連結・差分表示されていた組み合わせが、本決定の適用後は
連結されなくなる。これは意図した退行であり、バグではない。

### 決定3: `SCHEMA_VERSION` は上げない

`machine_key` は永続化フィールドではなく、読み出し時に
`result.environment` から都度導出する計算値 — ラッパー
(`schema_version`/`session_id`/`source`/...) 側のスキーマは 1 バイトも
変えていない。そのため `common.SCHEMA_VERSION` は `1` のまま据え置いた。
`test_common.py` の `test_v1_entry_without_environment_machine_fields_
still_parses` で、item1 以前の v1 エントリ (`cpu_model` 等のキー自体が
存在しない) が例外なく読め、「機種不明」として扱われることを固定した。

### 決定4: `perf-windows.yml` に週次 `schedule` を足すが、まだ fail させない

Issue #208 の切り分けが示した「run ごとに機種が変わる」性質のもとでは、
**手動実行だけでは機種ごとのデータがいつまで経っても十分な本数貯まらない
。** そこで `schedule: cron: "0 3 * * 1"` (UTC 月曜 03:00、週次) を足し、
既定条件 (`cold_startup`、`VELOX_MEMORY_BUDGET_MB=0` との A/B 比較) で
機械的に run を積み重ねる。

**ゲート化 (fail させる) はまだしない。** 理由は 2 つ:
1. Issue #211 項目3 (同一機種内での分散の実測) が未着手で、「同一機種内
   でどこからが有意な差か」の閾値を決める実測データが無い。D46/D96 が
   繰り返し指摘してきた「ノイズを回帰の証拠であるかのように見せる」誤り
   を、勘で閾値を決めて Windows の機種間・機種内ノイズに対して繰り返す
   ことになる。
2. 機種ごとの系列 (`machine_key`) が十分な本数貯まって初めて、「同一
   機種内で許容できるばらつき」を実測できる。今回の schedule 追加は、
   その実測データを貯めるための前段であって、ゲートの実装そのものでは
   ない。

頻度を週次にしたのは、Windows ランナーの実行コストの高さ (このファイル
冒頭のコメントが元から指摘している) と、機種のサンプル数を項目3 の着手
に間に合う速度で増やしたいという要求のバランスを取ったため。詳細な比較
検討は `.github/workflows/perf-windows.yml` 冒頭のコメントに記載した。

### 副産物: `schedule`/`pull_request` の入力補完漏れを直した

`inputs.*` は `workflow_dispatch` 以外のイベントでは常に空文字列になる
(GitHub の仕様)。既存コードは「`pull_request` (この workflow 自身の変更
PR での自己テスト) のときだけ既定値を補う」実装になっており、`schedule`
を追加するだけではこの判定から漏れて `compare_env` が空欄のまま単独計測
に落ちる — ジョブは失敗しないが、**定期計測に期待する「機種ごとの A/B
系列を貯める」目的を CI が緑のまま達成しない**という壊れ方をする
(D102/D103 が「`if` を通っても、そもそも購読/分岐していなければ動かない」
と繰り返し指摘してきたのと同じ形)。`$env:EVENT_NAME -eq "schedule"` を
判定に足して解消した。

### 未着手のまま残したこと (項目4 のスコープ調整)

**「機種ごとの系列として蓄積する」の後半 — schedule の実行結果を
`results/history/windows/` へ自動で取り込む導線は、本決定では作っていない
。** `perf-windows.yml` は従来通り結果 JSON を artifact として残すのみで、
`record.py` の呼び出しや `results/history/` への commit は行わない。
理由: D82 の Revisit condition (1) がすでに「CI からの自動記録には『誰が
commit を作るか』という運用設計が要る」と指摘しており、これは今回の
決定1〜4 (比較単位の追加とスケジュール自体の追加) とは独立した意思決定
(commit の作者・タイミング・競合時の扱い・fork/権限) を要する。無理に
繋いで運用設計を素通りさせるより、決まっていないことを決まっていないと
明示する方を選んだ。

### Revisit condition

(1) **`results/history/windows/` への自動取り込みは依然として手動。**
schedule が生成する結果 JSON を artifact から拾って `record.py` に渡す
運用 (人手、または別途設計する自動化) を今後決める必要がある。
(2) **ゲート化の閾値は項目3 の実測が揃ってから決める。** 週次 schedule で
機種ごとの系列がある程度貯まった時点で、同一機種内の分散を見て再検討
すること — このタスクを追跡する Issue が無ければ、その時点で新規に立てる。
(3) `machine_key` は `os`/`cpu_model`/`cpu_count`/`total_memory_bytes`
(GiB 丸め) から決まる。将来 CPU の同一モデル内でも世代/ステッピングが
性能に無視できない差を生むと分かった場合、`cpu_model` の文字列だけでは
粒度が粗すぎる可能性がある — その場合は item1 のスキーマ自体に追加の
識別子 (マイクロコード版数等) が要る。
...
## D109: `tabs_hold_50` の実測 (Issue #197) — 予算は 50 タブでも守られる。D97 Revisit (2) を閉じ、(1) の結論を 50 タブまで広げる。残るのは「深さ」の妥当性

**対象**: D97 Revisit condition (2) (`tabs_hold_50` が未測定) を閉じる。
**(1) (休止で WebView2 のプロセスが減るか) は §28.8 で既に解決済み**であり、
本決定はその結論を 50 タブまで広げて確認するに留まる。あわせて
`suspended_tab_count` (D105) の初の実測値を得た (§28.7 の残る課題 4 の前半)。
実測は `docs/performance-targets.md` §31、run 34627143113。

### 決定1: 候補 1 の否定 (§28.8) が 50 タブでも成り立つことを確認する

**候補 1「休止してもプロセスが解放されない」を否定したのは §28.8
(run 34247895418) であり、本決定ではない。** D97 Revisit condition (1) は
その時点で「解決済み」と記録されている (20 タブで 27 → 11 プロセス)。本決定
が足すのは、**それが 50 タブでも崩れない**ことの確認である
(`tabs_hold_50` で B 57 → A 9.8、-83%)。

この区別は重要である。D97 の決着を「まだ open だった」かのように書くと、
**決定履歴としての docs/decisions.md の価値が落ちる** — 後から読む人には、
どの run が何を決めたのかが分からなくなる。

確認としての価値はもう 1 つあり、`tabs_hold_20` の A=11 / B=27 は §28.8
(別 run、同一機種) と**完全に一致した**。プロセス数は run をまたいでも
再現する。50 タブの側では §28.8 事実 2 の「基本 7 + タブ 1 個につき 1
プロセス」がそのまま当てはまり (B 57 = 7 + 50)、A の 9.8 は生存タブ 2〜3 に
相当する。

### 決定2: Windows のメモリ予算 700 MiB は 50 タブでも上限として機能する

`tabs_hold_50` の A は **527.9 MiB** (予算の 24.6% 下)。頭打ちも超過も
起きず、**20 タブ (610.0 MiB) より小さい**。予算がタブ数ではなくメモリ量で
効いていることの再確認になる。素の値 (予算 OFF) の傾きは 20→50 タブで
**64.6 MiB/タブ**で、§28 の 64.9/64.2/65.3 と一致する — 線形は 50 タブまで
続く。

**したがって「Windows では OS 別の既定値が要るのではないか」という
Issue #197 の当初の懸念は、50 タブまで広げても否定される。** D90 の既定
700 MiB を変更する理由は見つかっていない。

### 決定3: ただし「守られている」を製品品質の結論にはしない

`tabs_hold_50` の A は **50 タブ中 48 タブ (96%) が休止した状態**の値で
ある。527.9 MiB は背景タブをほぼ全部捨てた対価として得られており、
**復帰コスト (`tab_resume_ms`) は本 run では測っていない。**

数字の上では「50 タブ開いても 528 MiB」だが、**その状態が日常利用に耐えるか
はこの数字だけでは言えない。** Phase 4 設計原則 2 (Memory を減らすだけで
なく Foreground UX を守る) に照らせば、削減率の大きさは達成ではなく
**#176 (Memory Budget Manager) が答えるべき問いの提示**である。休止の深さを
どこで止めるべきか — pinned/media/フォーム入力中の保護、priority score、
hysteresis — は #176 の設計そのものになる。

### 決定4: 同一機種の再現性は「最初の 3 点」として記録するに留める

`tabs_hold_20` は §28.5 / §28.8 / 本 run の 3 つで測られており、**3 本とも
AMD EPYC 7763 / 4 コア / 15.99 GiB** — `machine_key` (D106) が一致する。

| 指標 | 3 run の幅 |
| --- | ---: |
| B (予算 OFF) | 1602.6 / 1593.3 / 1596.3 MiB → **0.58%** |
| A (既定 700 MiB) | 616.8 / 615.4 / 610.0 MiB → **1.11%** |

**これを「同一機種なら比較してよい」(D96 Revisit condition (2)) の結論には
しない。** 理由は 3 つ:

1. **3 点では分散ではない。** しかも 3 本とも `tabs_hold_20` 1 シナリオの
   値で、試行数・ペア数も揃っていない。Issue #211 項目3 が求める閾値の
   根拠には足りない。
2. **20 タブで揃ったからといって、他の条件でも揃うとは限らない。** A 側は
   `tabs_hold_50` で 505.9 / 549.8 MiB と **8.3%** 揺れた。回収がどこまで
   進むかは実行ごとに違う。**「同一機種なら安定」は条件依存であり、
   メトリクスによらず成り立つ性質ではない。**
3. D106 Revisit (3) のとおり `cpu_model` の文字列一致が物理的同一性を保証
   しない。

D96/D46 が繰り返し戒めてきた「ノイズを結論の証拠に使う」の裏返しとして、
**都合のよい一致 1 点を再現性の証拠に使うこともしない。**

### Revisit condition

(1) **復帰コストが未計測。** 48 タブ休止状態からの `tab_resume_ms` を測る
まで、決定3 は「問いの提示」のままである。#176 の中で測る。
(2) **A 側の 8.3% のばらつきの原因は未解明。** 休止判定が走る回数の揺れか、
回収の進み方そのものの揺れかを切り分けていない。Windows のゲート化
(#211) で予算が効く条件のメトリクスを対象にするなら、先にここを潰す必要が
ある。
(3) **`tabs_hold_100` 以上は未測定。** 64.6 MiB/タブの線形がどこまで続くか、
予算が守れなくなる点があるかは分かっていない。ただし 50 タブでも 24.6% の
余裕があるため、優先度は低い。

## D110: #176 の最初の一手を「計測」にする (Issue #176 Stage 1) — `tabs_hold_resume_N` で休止の**対価**を測れるようにする

**対象**: Phase 4 Stage 1 (#176 Memory Budget Manager) の最初の増分。
実装は `Scenario::TabCountMemoryResume` (`tabs_hold_resume_N`)。

### 背景: §31 が数字を出したが、その数字の意味は確定していない

§31 (D109) は「Windows で 50 タブ開いても 527.9 MiB」を実測した。同時に
**その値が 50 タブ中 48 タブ (96%) を休止した結果である**ことも記録した。
`suspended_tab_count` (D105) が初めて値を出したことで、削減率の隣に
「何を捨てたか」が並ぶようになった。

**しかし「捨てた対価」はまだ測っていない。** D97 Revisit condition (4) は
2 つを求めていた: (a) 何個休止した結果か、(b) 復帰にどれだけかかるか。
(a) は §31 で埋まり、(b) が残っている。

### 決定1: #176 の最初の一手は、機能追加ではなく計測にする

#176 の本文が挙げる実装候補 — RAM 相対のメモリ予算、suspend/resume
hysteresis、priority score、Active/Recent/Background/Dormant/Suspended の
状態モデル — は**どれも「今の休止は深すぎるのか」が分からないと閾値を
決められない。** 復帰が十分速いなら 96% の休止は問題ではなく、遅いなら
予算そのものを見直す話になる。順序を逆にすると、Epic #73 の設計原則 8
(機能追加より計測を優先する) と、D46/D96 が繰り返し戒めてきた「推測で
閾値を決める」を同時に破ることになる。

そこで最初の PR は**シナリオを 1 つ増やすだけ**にした。製品の挙動は
一切変えていない (`velox` バイナリの動作に差は無く、変わるのは
`velox-bench` が生成する自動操作スクリプトの選択肢だけ)。

### 決定2: 既存の `tab_resume` シナリオでは答えにならない

`tab_resume` (Issue #63) は既にある。しかしそれは

- **手動で** `suspend <index>` して復帰させる (メモリ予算が関与しない)
- タブ数が少ない (`TAB_SWITCH_EXTRA_TABS` と同数)

という形であり、**測っているのは休止の「機構」であって、製品がユーザを
実際に置く「状況」ではない。** 50 タブ中 48 タブが自動で休止された状態
から 1 つ戻すコストは、この形では出てこない。

### 決定3: `mark` までの組み立ては `tabs_hold_N` と完全に同一にする

`tabs_hold_resume_N` は、開く間隔・12 秒の待ち・既定のメモリチェック周期
まで `tabs_hold_N` と同じで、違いは `mark` の**後ろ**だけである
(最長未使用のタブから順に `switch` する)。

**これは意図的な制約であり、テストで固定した**
(`tabs_hold_resume_sets_up_exactly_like_tabs_hold_then_switches_back` が
両シナリオの `mark` 前の列の一致を検査する)。理由: §31 の値と地続きに
読めることが、このシナリオの価値そのものだからである。準備が少しでも
違えば「同じ落ち着いた状態からの復帰」という前提が崩れ、得られた
`tab_resume_ms` を §31 の 527.9 MiB / 48 タブ休止と並べて論じられなく
なる。

`switch` を index 0 から昇順にするのも同じ理由である。休止は最長未使用
から順に行われる (`suspension::reclaim_order`) ので、この順に戻すのが
「実際に休止されていたタブ」を引き当てる確率が最も高い。

### 決定4: ラウンド数は 4 — 測定対象の状態を壊さないため

1 回戻すたびに休止タブが 1 つ減り、その webview が復活してメモリも増える。
`TAB_RESUME_REPEATS` のような大きな値にすると、**後半のラウンドは
「あまり休止されていない状態からの復帰」を測る**ことになり、シナリオの
意味が途中で変わる。50 タブ / 48 休止に対して 4 ラウンドなら、測り終えて
も 44 タブは休止のままである。サンプル数は試行 (`--trials`) をまたいで
積み上がるので、2 試行で 8 サンプルになる。

### 決定5: 予算が休止しなかったときは「値を出さない」

生きているタブへの `switch` は `tab_resume` ではなく `tab_switch` を生む
ため、予算が 1 つも休止していないタブ数 (Windows 実測では 1 / 5 タブ、
§31.3) では `tab_resume_ms` が 1 件も出ず、A/B 比較表にも行が現れない。

**これを欠陥として塞がない。** 塞ぐとすれば「生きているタブを手動で
suspend してから戻す」ことになるが、それは決定2 で退けた `tab_resume` に
戻るだけである。**行が無いこと自体が「予算は何も休止しなかった」という
観測**であり、`suspended_tab_count` の 0 と整合する。嘘の値を出すより
欠測を出す方を選ぶ (D105 が「0 件は欠損ではない」と決めたのと同じ方向で、
今回は逆に「該当が無ければ出さない」を選んでいる — どちらも
**観測できたものだけを書く**という同じ原則である)。

### Revisit condition

(1) **`minimal.html` での復帰コストである。** 実サイトの復帰は再読み込み
のネットワーク/パースを含むため桁が違いうる。「復帰は速い/遅い」を製品
方針として決めるには、実サイトでの測定が要る (§28.7 の残る課題 3 と同じ
制約)。
(2) **Linux 側では未実行。** シナリオ自体は OS 非依存だが、値は
Windows/WebView2 のものだけになる予定である (CLAUDE.md の OS 優先度)。
(3) **復帰後の「揺り戻し」を測っていない。** 1 つ戻すとメモリが増え、
次の判定でまた別のタブが休止されうる。この往復 (hysteresis が必要かどうか
の判断材料) は本シナリオの窓では捉えきれない — 測るなら窓をもっと長く
取るか、`tab_suspend` の時系列を見る別の仕組みが要る。

  **なお、この揺り戻しが実在することは動作確認の過程で観測された。**
  予算を人為的に下げた条件 (`VELOX_MEMORY_BUDGET_MB=300`、5 タブ、Linux)
  で 1 試行回したところ、背景タブは 4 つしか無いのに
  `suspended_tab_count` が **8** を返した — 4 回の復帰それぞれの後に
  再休止が起きている。**これは開発環境での動作確認であって性能値では
  ない** (D46 の言う「参考環境ですらない場所の数字」であり、予算値も
  実ユーザの設定ではない) が、hysteresis を #176 の検討項目に挙げた
  根拠が推測ではないことは言える。
(4) ラウンド数 4 は「状態を壊さない」側から決めた値で、サンプル数の
十分性からは決めていない。ばらつきが大きすぎて中央値が読めない場合は
試行数で増やす (ラウンド数ではなく) こと。

## D111: 計測ページを選べるようにし、結果に記録する (Issue #176) — 「どのページで測ったか」を結果から消さない

**対象**: #176 Stage 1 の 2 番目の増分。D110 (`tabs_hold_resume_N`) の実測
(§32 予定) が残した最大の限界 — **`minimal.html` での値である** — を潰す
ための足場。製品コードは変えていない。

### 背景: 復帰コストの結論が 1 つのページに依存している

D110 の実測は「復帰は約 120ms、再読み込みは約 30ms」だった。ここから
「96% 休止の対価は許容範囲」と読めるが、**その読みは `minimal.html`
(229 バイト) に依存している。** 実サイトでは再読み込みの側が桁違いに重く
なり、支配項が入れ替わる可能性が高い。

`scripts/bench/pages/` には `dom_heavy.html` (99 KB、DOM ノード多数) が
既にあり、`compare-windows.yml` は `page` 入力でそれを選べる。ところが
`perf-windows.yml` は `minimal.html` に固定されていた。

### 決定1: `perf-windows.yml` に `page` 入力を足す (`compare-windows.yml` と同じ形)

新しい仕組みではなく、**既にある選択肢を同じ形でもう一方の workflow にも
出す**だけにした。`url` 入力 (外部 URL) との関係は「`url` が指定されて
いれば `page` は無視される」で、これは `url` が「固定ページを配信しない」
経路であることから自動的に決まる。

`inputs.*` は `workflow_dispatch` 以外のイベントでは常に空文字列になる
(GitHub の仕様)。D106 がまさにこれを踏んで `schedule` の A/B 比較が
無言で単独計測に落ちていたので、既定値の補完を最初から入れてある。

### 決定2: 結果 JSON に `url` を持たせる — これが本体である

**`page` 入力だけを足すのは危険だった。** 結果 JSON (`BenchmarkResult`)
は `scenario` しか持たず、**どのページで測ったかを記録していない。**
`tabs_hold_resume_20` という同じ ID の結果が、`minimal.html` のものか
`dom_heavy.html` のものか区別できない。

これは D104 が機種情報について解決したのと**同型の問題**である
(「`environment-info.md` に別立てで保存しているだけで、機械的に突き合わせ
られない」)。記録済みの §21〜§31 はすべて `minimal.html` なので、重い
ページの結果がそれと同じ顔で並ぶと、D96 が禁じた「条件の違う数値を並べ
る」を、今度はページ差で犯すことになる。

そこで `BenchmarkResult.url: Option<String>` を追加し、`run` が `--url`
をそのまま記録する。`#[serde(default)]` により既存の結果ファイルは
`null` として読め、読めなくなるものは無い (テストで固定)。

`aggregate` サブコマンドは**常に `null`** にした。既に採り終えたログから
は「どのページで採られたか」を知る手段が無く、**推測で埋めるくらいなら
空のままにする**方が正しい (D105 が「0 件は欠損ではない」と決めたのと
同じく、観測できたものだけを書く)。

### 決定3: Job Summary にも計測ページを出す

ランナー情報 (D96 / PR #210) と同じ理由である。**表を読む人が最初に
必要とするのは「これは何と比べてよい数字か」**であり、機種とページは
どちらもその条件を成す。サマリには「記録済みの値はすべて `minimal.html`
のもの」という注意書きも一緒に出す。

### Revisit condition

(1) **ダッシュボードの比較単位にはまだ入れていない。** D106 は `session_id`
と `machine_key` の両方が一致する隣接エントリだけを連結するが、そこに
「ページ」は入っていない。`results/history/` への取り込みが手動である
(D106 Revisit (1)) 間は実害が出にくいため見送ったが、自動取り込みを作る
時点では**ページも比較単位に足す必要がある。**
(2) `page` 入力の選択肢は `scripts/bench/pages/` の 3 つを手で列挙して
いる (`compare-windows.yml` と同じ)。ページを足したら両方の workflow を
直す必要がある。
(3) `dom_heavy.html` は「重いページ」の代理であって実サイトではない。
ネットワーク遅延・サードパーティスクリプト・画像デコードは含まない。
実サイトでの測定は依然として別の課題である (§28.7 の残る課題 3)。

## D112: 休止の対価は「webview の作り直し」であって「ページの再読み込み」ではない (Issue #176 Stage 1) — 予測を実測が否定した

**対象**: D97 Revisit condition (4) の後半 (復帰コスト未計測) を閉じる。
実測は `docs/performance-targets.md` §32、run 34669849332 (`minimal.html`)
と 34670868774 (`dom_heavy.html`)。

### 決定1: 復帰は約 110〜125ms で、**ページの重さで決まる部分ではない**

`tab_resume_ms` は 20 タブで 111.9〜119.4ms、50 タブで 116.1〜124.6ms。
**タブ数でも、ページの重さでも、ほとんど動かない。** 一方 `page_load_ms`
はページの重さで動く (29.4 → 75.5ms)。同一 run 内の比 (再読み込み ÷
再構築) は `minimal.html` で 0.25〜0.28、`dom_heavy.html` で 0.67〜0.75
であり、**99 KB のページでも再読み込みは再構築を超えない。**

これが #176 の実装項目の性格を決める。**VeloX 側で削れるのは「休止の
発生回数」だけであり、「復帰 1 回あたりのコスト」ではない** — 後者は
webview を作り直す時間そのもので、予算や hysteresis の設計では変わら
ない。

### 決定2: 「実サイトでは再読み込みが支配項になる」という予測は誤りだった

**そう書いていたのは D111 の背景である。** D110 Revisit (1) は
「実サイトの復帰は再読み込みのネットワーク/パースを含むため**桁が違い
うる**」までで、どちらが支配項になるかは主張していない。**「支配項が
入れ替わる可能性が高い」と踏み込んだのは D111** であり、訂正の宛先も
そちらである。

根拠は開発環境 (Linux/WebKitGTK、GPU 無しのコンテナ) での観測で、
そこでは同じ比が **121** だった
(`dom_heavy` で `page_load` 355.80ms に対し `tab_resume` 2.95ms)。
Windows/WebView2 の実測は 0.67〜0.75 で、**桁が違う。**

エンジンも環境も違うので「どちらが速い」という話にはならない。
**問題は、開発環境の観測を Windows の予測として使ったことである。**
D46 は「参考環境ですらない場所の数字を実力値として扱わない」と戒めて
きたが、**今回は「実力値」ではなく「予測」という形で同じ誤りをやり
かけた。** 予測として書いたものであっても、根拠が参考環境外の観測なら
同じ扱いを受けるべきである — この点を D110 の記述に対する訂正として
記録する。

### 決定3: メモリ予算はページが重いほど効く — 緩める方向の変更はこれを捨てる

`dom_heavy.html` の 50 タブで、予算 OFF は **4738.1 MiB**、予算 ON は
**648.2 MiB** (削減 86.3%)。`minimal.html` の §31 (61.5%) より効きが
大きい。重いページを多数開いたときこそ、休止が無ければメモリは 4.7 GiB
に達する。

着地点 648.2 MiB は §31 の 527.9 MiB と同様に予算の下であり、**予算は
ページの重さによらず上限として機能している。**

### 決定4: したがって RAM 相対の予算は「緩和」ではなく「休止回数の削減」として検討する

#176 の Stage 1 は「システム搭載 RAM に応じた VeloX Memory Budget の
考え方を設計」を挙げ、D90 Revisit は「RAM 相対の予算は #176 の領分」と
書いていた。本決定はその検討の**動機**を確定させる。

- ❌ 「復帰が遅いから予算を緩める」 — 決定1 により、復帰の遅さは予算を
  緩めても変わらない (発生回数が減るだけ)
- ⭕ 「搭載 RAM に余裕がある機では、休止の発生回数そのものを減らせる」 —
  16 GiB 機で 700 MiB 固定という現状は、**メモリが余っていても 50 タブ中
  49 タブを捨てる**ことを意味する。減らせるのはこの回数である

**つまり RAM 相対の予算の価値は「メモリ削減」ではなく「状態喪失の
回数削減」の側にある。** 評価指標も `rss_total_bytes` ではなく
`suspended_tab_count` を主に見るべきである。

### Revisit condition

(1) **ページ比較が run をまたいでいる** (§32.6 限界 1)。決定1 の比の
比較は「2 つのメトリクスが CPU 速度に同じ比率で効く」ことを暗に仮定して
おり、検証していない。同一 run 内で複数ページを測れるようにする
(Issue #231) まで、決定1 は「同一 run 内の比がページの重さとともに
0.25 → 0.75 へ動いたが 1 を超えなかった」という限定付きの事実である。
(2) **`dom_heavy.html` は実サイトではない。** ネットワーク遅延・
サードパーティスクリプト・画像デコードが入れば、比が 1 を超える可能性は
残る。決定1 を「実サイトでも成り立つ」と読んではいけない。
(3) **状態喪失そのもののコストは依然として数値化されていない。**
決定4 が `suspended_tab_count` を主指標に据えることを提案しているが、
「1 回の状態喪失がユーザにとってどれだけ損か」は測っていない。
(4) 決定2 の訂正は **D111 の背景**の記述に対するものだが、**D111 自体
は書き換えていない** (新しい決定で履歴を残す、`docs/decisions/README.md`
の「重要なルール」4 に従う)。

  **この帰属自体も一度間違えた。** 初版は訂正の宛先を D110 Revisit (1)
  と書いていたが、そこには「桁が違いうる」としか書かれておらず、支配項
  の交代を主張したのは D111 だった (PR #232 のレビューで指摘され修正)。
  **予測の出典を正確に扱うことを主題にした決定の中で、出典を取り違えて
  いた**という、記録に残す価値のある失敗である。

## D113: 搭載 RAM の検出を入れる (Issue #176 / D93 子 Issue 案 B) — 予算の式はまだ変えない

**対象**: D93 が示した子 Issue 分割案の **B (搭載 RAM の検出)**。
実装は `browser::metrics::installed_ram_bytes()`。**製品の挙動は変えて
いない** — 予算の式 (案 C) は別の決定である。

### 決定0: D93 Revisit condition (2) のブロッカーは解消済みである

D93 は「**子 Issue 案 A が済むまで、RAM 相対の式は実装しない**」と定めて
いた。案 A は「Windows でのタブ数スケーリング実測 (1/5/10/20 タブの
`rss_total_bytes`)」である。

**これは D97 (§28) と D109 (§31) で済んでいる。** `tabs_hold_N` で
1/5/10/20/50 タブを同一 run 内で測り、**素の傾きは約 65 MiB/タブで
50 タブまで線形**と確定した (§31.5)。D93 が「未知なのは切片ではなく
傾きである」と書いた箇所は埋まっている。

したがって案 B → 案 C へ進んでよい。**ただし本決定は B だけを行う。**

### 決定1: 検出だけを入れ、予算の式には触れない

D93 が B と C を分けたのは、C が 3 つのパラメータ (下限・比率・上限) の
**それぞれ別の根拠**を要求し、うち比率は「測れない — 製品判断」である
ためである。検出は依存が無く単体で完結し、テストも書ける。

**本決定の後も既定の挙動は 700 MiB の絶対値のまま**であり、
`installed_ram_bytes()` を呼ぶ本体コードはまだ 1 つも無い。

### 決定2: 失敗を `Result` にしない

戻り値は `Option<u64>` で、失敗理由を伝えない。**呼び出し側に判断の余地
が無いから**である — RAM が分からなければ絶対値の既定にフォールバック
するしかなく、「なぜ分からなかったか」で分岐しようがない
(`sample_process_tree_rss` が `RssError` を返すのとは事情が違う: あちらは
呼び出し側が計測を諦めるか再試行するかを決める)。

macOS は `None` を返す。`sysctl hw.memsize` で取れるが、CLAUDE.md の
OS 優先度に従い**必要になるまで足さない。**

### 決定3: 単位を検査する — kB を仮定して黙って 1024 倍しない

`/proc/meminfo` の `MemTotal:` は kernel が常に kB で出すが、パーサは
単位トークンを読み、`kB` 以外なら `None` を返す。**将来 kernel が単位を
変えたときに、黙って 1024 倍ずれた値を返すより `None` の方が安全**である
(呼び出し側はフォールバックするだけで壊れない)。桁あふれも飽和させず
`None` にする。

### 決定4: `velox-bench` の重複パーサを共有実装へ寄せる

`velox-bench` は D104 の機種情報収集で同じ `/proc/meminfo` の同じ行を
読んでおり、**独自のパーサを持っていた。** 同じファイルの同じ行を 2 か所
で解釈していると、片方だけ直したときに**機種情報とメモリ予算が違う値を
見る**ことになる。`browser::metrics::parse_mem_total_bytes` に一本化し、
`velox-bench` 側はそれを呼ぶだけにした。

移譲で挙動が 2 点変わった (単位の検査・桁あふれ時の `None`)。どちらも
結果 JSON の `total_memory_bytes` にとって安全側であり、**委譲が外れて
ローカル実装に戻ったら気付ける**よう、共有パーサ側でしか通らない挙動を
`velox-bench` のテストから確かめている。

### 決定5: Windows は `GlobalMemoryStatusEx` を直接呼ぶ (`unsafe` 1 箇所)

`velox-bench` の機種情報収集は PowerShell を呼ぶ実装で、その doc comment
は「CLAUDE.md が `unsafe` を原則禁止しているため」と理由を書いている。
**本体側では同じ選択をしない。**

- ベンチ実行時に 1 回 PowerShell を起こすのと、**ブラウザ本体が起動時に
  プロセスを 1 つ spawn する**のとでは、コストの意味がまったく違う
  (起動時間は §24 が分解してきた最優先の指標である)。
- `GlobalMemoryStatusEx` は出力構造体を埋めるだけの同期 API で、安全性の
  条件は `dwLength` に構造体の大きさが入っていることだけである。CLAUDE.md
  の「使用する場合は理由をコメントで明記」に従い、`unsafe` ブロックに
  その条件と根拠を書いた。

`ullTotalPhys` が 0 のときは `None` にする (「0 バイトの RAM」と「取れ
なかった」を区別できないため — D105 の「観測できたものだけを書く」と
同じ方向)。

### Revisit condition

(1) **Windows 実機での値は未確認。** `cargo check --target
x86_64-pc-windows-msvc` で型検査は通したが、CLAUDE.md D61 のとおり
**これは実行を保証しない。** 実際の値は案 C で予算の式に使うときに
確認すること。
(2) **まだ誰も呼んでいない。** 本体コードから参照されるのは案 C からで
ある。それまでは `#[allow(dead_code)]` ではなく `pub` で公開している
(`browser` は `velox-bench` からも使うライブラリであり、未使用の警告は
出ない)。
(3) macOS は `None` のまま。案 C で「RAM が分からない機ではどうするか」
を決める必要がある — 絶対値へのフォールバックでよいはずだが、明示的に
決めること。
(4) 物理 RAM (`MemTotal` / `ullTotalPhys`) を見ているが、**コンテナや
cgroup で制限された環境では実際に使える量と食い違う。** この開発環境も
まさにコンテナである。案 C で「ブラウザが使ってよい量」を論じるときに、
cgroup の制限 (`/sys/fs/cgroup/memory.max`) を見るべきかを検討すること。

## D114: 既定のメモリ予算を搭載 RAM 相対にする (Issue #176 / D93 子 Issue 案 C) — ただし「今日より小さくしない」を制約に据える

**対象**: D93 が示した子 Issue 分割案の **C (RAM 相対の予算式)**。実装は
`browser::suspension::memory_budget_for_ram` と
`config::resolve_suspension`。**Issue #176 の本体である。**

D113 (案 B) で搭載 RAM が読めるようになり、D109 (§31) で案 A の実測
(Windows で約 65 MiB/タブ・50 タブまで線形) が済んだ。D93 が置いた
ブロッカーは両方とも解消しており、式を入れる番である。

### 決定1: 式は `clamp(700 MiB, 搭載 RAM / 16, 2048 MiB)` とする

| 搭載 RAM | RAM / 16 | 実際の予算 | D90 (700 MiB) との差 |
| ---: | ---: | ---: | --- |
...
搭載 RAM が読めない環境 (macOS、`/proc` が無い環境) では
`DEFAULT_MEMORY_BUDGET_BYTES` = 700 MiB に落ちる。**分からないときは
今日と同じ挙動**であり、呼び出し側に判断させない (D113 決定2 と同じ形)。

### 決定2: 下限は「今日の既定値」そのものにする — **どのマシンでも予算は小さくならない**

これが本決定で最も強い制約である。理由は目的の側にある:

RAM 相対にする動機は「大容量機で予算が実質 OFF になっている」ことの
解消であり、その先にあるのは **D112 決定4 が確定させた「休止の対価は
避けられない」という事実**である。休止 1 回あたり約 110〜125 ms の
webview 作り直しコストはタブ数にもページの重さにも依らず、**VeloX に
できるのは休止の回数を減らすことだけ**である。ここでどこかのマシンの
予算を小さくすれば休止が増え、目的そのものに反する。

したがって式は**現状からの緩和方向にしか動かさない**。下限を D93 が
検討した「約 397 MiB (休止では届かない下限)」や「500 MiB (応答が飽和し
`max_live_tabs=1` 相当に退化する点)」ではなく 700 MiB に置いたのは、
**700 MiB だけが実測済み (D56 / §12) かつ現行の既定**だからである。
D93 が「裸の比率」を退けた理由 (4 GiB → 178 MiB / 8 GiB → 357 MiB が
どちらも約 397 MiB の下限を割る) は、下限をここに置くことでそのまま
解消される — 小容量機は**1 バイトも変わらない**。

### 決定3: 比率 1/16 と上限 2048 MiB は実測ではなく製品判断である

**これを実測として書かない。** D93 は「比率は測れない — 搭載 RAM を
変えられるマシンが 1 台も無い」と明記しており、本決定でもその状況は
変わっていない (D46 の警告は主張だけでなく**予測**にも及ぶ、という
D112 Revisit の教訓をここでも守る)。

- **比率 1/16 (6.25%)**: 現行の 700 MiB は本プロジェクトの参考環境
  (15.70 GiB) の 4.35% にあたる (D93)。6.25% はその 1.4 倍で、**同じ桁に
  留まる**控えめな値である。16 GiB 機で 1024 MiB、§31.5 の約 65 MiB/タブ
  で割ると発動は**約 10 タブ → 約 15 タブ**に動く。2 の冪なので丸めの
  議論も要らない。
- **上限 2048 MiB**: VeloX の立ち位置は「軽いブラウザ」であり、**2 GiB を
  使うブラウザはその時点で軽くない。** 予算は「使ってよい量の割り当て」
  ではなく**安全網**であって、搭載 RAM が増えた分だけ際限なく使ってよい
  という意味ではない。約 65 MiB/タブで割ると約 31 タブ相当で、現行の
  約 10 タブに対して 3 倍であり、大容量機の利用者には十分な緩和である。

どちらも「これより広げたい利用者には明示指定がある」ことを前提にした
判断である (決定5)。

### 決定4: 搭載 RAM を読む層は `config` だけにし、`Default` は RAM 非依存のまま残す

`browser::suspension` はクロックも `/proc` も自分では読まない純粋ロジック
である (D20)。そこで:

- `memory_budget_for_ram(Option<u64>) -> u64` — 純粋。RAM は引数で受ける。
- `browser::metrics::installed_ram_bytes()` — 環境の探索 (D113)。
- `config::Config::from_env_and_args` がそれを呼び、
  `resolve_suspension(..., installed_ram_bytes)` に渡す。**製品に RAM
  相対の既定値が入る経路はこの 1 箇所だけである。**

**`SuspensionPolicy::default()` と `PerformanceSettings::default()` は
どちらも RAM を見ない** (700 MiB のまま)。実装の途中で
`PerformanceSettings::default()` を RAM 依存にしてみたが、次の 2 つが
壊れたので戻した:

1. `to_settings_on_a_default_config_matches_settings_default` が**走らせた
   マシンによって結果を変える**ようになる。`Config::default()` (純粋な
   `browser` 側の既定を持つ) と `Settings::default()` (RAM 依存) が
   食い違うためで、これは「テストが緑かどうかがどこで走らせたかに依存
   する」という最も避けたい形である。
2. `Default` の意味が「機械の情報が無いときの挙動」から「この機械での
   挙動」にすり替わる。前者は定数であるべきものである。

`settings.json` がまだ無い利用者 (= 既定で動いている大多数) には
`app::run` が `Config::to_settings()` 経由で RAM 相対の値を渡すので、
**実際の既定は起動ごとに RAM 相対になる。** 設定ファイルは利用者が明示的
に保存したときだけ書かれる (`app::apply_updated_settings`) ので、保存せず
に使い続ける限り、機械を買い替えれば予算も追随する。

唯一 700 MiB に落ちるのは「`settings.json` はあるが `memory_budget_mb`
キーだけが無い」場合 (D90 より前に書かれたファイルの移行) である。従来値
に落ちるだけなので退行ではなく、設定画面を開いて保存すればキーが埋まる。

### 決定5: 明示指定には下限も上限もかけない

`VELOX_MEMORY_BUDGET_MB` と設定画面の値は、搭載 RAM にも 700/2048 MiB の
上下限にも一切影響されない。300 MiB も 4096 MiB もそのまま通り、`0` に
よる無効化も従来どおり効く。**搭載 RAM が決めるのは「何も指定しなかった
ときの値」だけ**である。決定3 の上限が製品判断で済むのは、この逃げ道が
あるからである。

### 実測による検証 — 済み (§33)

Windows ランナー (15.99 GiB) は新式で 1023 MiB となり、従来の 700 MiB と
差が出る数少ない実機である。`tabs_hold_resume_20` / `tabs_hold_resume_50`
で新既定 (A) と `VELOX_MEMORY_BUDGET_MB=700` (B) を**同一 run 内で**
A/B 比較した (run 34700469298、§33)。主指標は `suspended_tab_count` と
した — D112 決定4 のとおり、減らせるのは休止の回数だけなので効果はそこに
出る。

| | 20 タブ | 50 タブ |
| --- | ---: | ---: |
| `suspended_tab_count` A (1023 MiB) | **12** | **44** |
| `suspended_tab_count` B (700 MiB) | **16** | **48** |
| 差 | **-4 (25% 減)** | **-4** |

**両方のシナリオで休止が 4 つ減った。** `rss_process_count` も同じく 4 つ
多く、`rss_total_bytes` の差 (273.0 / 258.6 MiB) は §31.5 の 65 MiB/タブ
× 4 タブ = 260 MiB とほぼ一致する — **休止の減少とメモリ差が同じ 1 つの
係数で説明でき**、式が意図どおり効いていることの証拠になっている。
`tab_resume_ms` は A/B で 118〜131 ms と変わらず、D112 決定1 (復帰コスト
は何にも依らない) が予算という軸を足しても成立した。予算の近傍に着地して
いる (A: 1022.6 / 931.4 MiB) ので、上げた分だけ増えて青天井にはならない
という安全網の性質も保たれている。

**検証できていないのは比率 1/16 そのものである。** ランナーの RAM は
機種が変わっても 15.99 GiB のままで、D93 §25.4 の「搭載 RAM を変えられる
マシンが 1 台も無い」は解消していない。本検証は「16 GiB 機で式が効くこと」
までしか言っていない (Revisit condition (1))。

> **計測条件についての注意**: この run は 30 分の `timeout-minutes` に
> 当たって打ち切られた。`trials: 10` は `tabs_hold_resume_N` には過大で
> ある (§32 が完走できたのは `trials: 2`)。集計ステップより前では止まって
> いないので値は有効だが、**50 タブ側は A/B とも 1 反復しか終わっていない**。
> 以後この系のシナリオを回すときは `trials: 2` / `repeats: 2` を上限の
> 目安にすること。

### Revisit condition

(1) **比率 1/16 と上限 2048 MiB は「測れないから決め打った」値である。**
搭載 RAM の異なる実機で `suspended_tab_count` を比較できるようになったら
(D93 §25.4 の「搭載 RAM を変えられるマシンが無い」が解消したら)、両方を
実測で置き直すこと。
(2) **下限 700 MiB は「今日の値」であって「正しい値」ではない。** D93 の
制約 1 (約 397 MiB) が本環境の 1 点のみの実測であることは変わっていない。
Windows での同等の実測が取れたら、下限を実測に基づく値へ動かしてよいかを
検討すること。ただしそのとき決定2 の制約 (小さくしない) を緩めるなら、
休止回数が増える方向の変更であることを明示的に引き受けること。
(3) **物理 RAM を見ており、cgroup / コンテナの制限は見ていない**
(D113 Revisit (4) がそのまま残っている)。VeloX はデスクトップブラウザで
あり、コンテナ内で常用される想定は無いためここでは扱わなかった。CI や
本開発環境のようにコンテナで動かす場面が「製品の挙動を測る」用途に
なったら、`/sys/fs/cgroup/memory.max` を見るべきかを再検討すること。
(4) **`installed_ram_bytes()` は起動時に 1 回だけ読む。** 実行中に搭載
RAM が変わる想定は無いので再読み込みしないが、仮想マシンのホット
アドなど、変わりうる環境を対象にするなら見直すこと。

## D115: 複数ページを同一 run 内で計測できるようにする (Issue #231) — `scenarios` と同じ答えを、ページに対しても出す

**対象**: `perf-windows.yml` の `pages` 入力。**製品コードは 1 行も
変わらない** — 計測基盤の穴埋めである。

### 決定0: これは `scenarios` (Issue #197) と同型の問題である

D96 決定 3 が確定させたのは「`windows-latest` は run ごとに別スペックの
マシンを割り当てるので、**run をまたいだ絶対値の比較は成立しない**」で
ある。複数のタブ数を比べたい要求に対しては、`scenarios` 入力 (複数
シナリオを 1 run で連続実行) がその答えだった。

**ページも同じ要求を持つ。** §32 は `minimal.html` と `dom_heavy.html` を
別々の run で測り、機種が違ったせいで (AMD EPYC 7763 / INTEL XEON
PLATINUM 8573C)、**いちばん知りたかった「ページが重くなると復帰コストは
どう変わるか」に答えられなかった** (§32.6 限界 1)。同一 run 内で読める
比 (`page_load_ms` ÷ `tab_resume_ms`) は取れるが、**2 つのメトリクスが
CPU 速度に同じ比率で効く保証が無い**ため、機種をまたいだ比の比較も厳密
ではない。

したがって答えも同型にする: ページを 1 つの run に収める。

### 決定1: ファイル名にページを入れるのは、入れないと衝突する場合だけ

結果ファイル名は `<scenario>-windows-(baseline|compare)-<n>.json` で、
ページが違っても同じ名前になる。複数ページを回すには区別が要る。

**しかし全 run の名前を変えることはしない。** `cold_startup-windows.json`
のような名前は §21.x / §26.x が本文で参照しており、名前を変えると過去の
記録との対応が取れなくなる。そこで接頭辞は:

- ページが 1 つ (= 従来の指定、`pages` 未指定、外部 URL 指定) →
  **従来どおりシナリオ名のまま。1 バイトも変わらない。**
- ページが 2 つ以上 → `<scenario>@<page-slug>`
  (例: `tabs_hold_20@dom_heavy-windows-baseline-1.json`)

`@` はシナリオ ID に現れない字なので、サマリ側は機械的に分解できる。

### 決定2: 表はページごとに分ける — ページをまたいだ行を並べない

Issue #231 の起票時点で既に指摘されていた懸念をそのまま採る。**同一 run
内ならページ間の比較は成立する**が、それは「同じ表に行として混ぜてよい」
という意味ではない。

`rss_total_bytes` の絶対値はページの重さで動く。タブ数スケーリング表に
ページの違う行が混ざると、**傾きが読めなくなる。** これは D97 が
「`tabs_N` と `tabs_hold_N` を同じ表に並べてはならない」と決めたのと
同じ形の誤読である — **run をまたぐ比較を避けるために作った仕組みが、
今度は表の中で同じ誤読を作る**のでは意味が無い。

そこで:

- A/B 比較表は (シナリオ × ページ) ごと。見出しに ``(ページ名)`` を出す。
- タブ数スケーリング表は**ページごとに 1 つずつ**出す。
- 複数ページのときだけ出る注意書きで、「ページ間の比較はこの run の中
  でだけ成立する」と「表はページごとに分けてある」の**両方**を言う。
  前者だけだと、読み手が別の表の行を目で並べて比べにかかる。

### 決定3: `pages` は自由記述だが、値は検証する

`pages` は複数指定が要るので choice にできない (choice は 1 つしか
選べない)。自由記述の値が URL の組み立てと**結果ファイル名の両方**に
使われるため、`^[A-Za-z0-9_.-]+$` に合致し、かつ
`scripts/bench/pages/` に実在することを確認してから使う。スラッシュや
`..` を通すと配信ディレクトリの外を指しうるし、結果ファイルの置き場所も
ずれる。

入力はシェルへ直接展開せず環境変数で渡す (既存の他ステップと同じ、
script injection 対策)。`Determine benchmark URL` ステップはこれまで
`${{ inputs.url }}` を直接展開していたので、合わせて環境変数経由に直した。

### 決定4: PowerShell の不変条件を Python のテストで守る

サマリ生成は PowerShell で書かれており、**Windows ランナー上でしか
動かない。** 壊れても Linux の CI は緑のままで、気づくのは「高コストな
Windows run を 1 回使い切ったあと、Job Summary に表が出ていないことに
気づいたとき」である。実際にその形の事故が 2 回ある (run 34247895418 で
`tabs_hold_` が並べ替えから落ちた件と、`tabs_hold_resume_` で同じ漏れを
やりかけた件)。

正規表現もシナリオ一覧もページ一覧も**すべて同じ YAML の中にある**ので、
突き合わせは Linux 上でできる。`.github/scripts/test_perf_windows_inputs.py`
を足し、CI の Python テストステップ (D108) で回す:

- `page` の choice が `scripts/bench/pages/` に実在すること
  (決定3 の検証により、実在しないページを選んだ run は起動直後に落ちる)
- 並べ替え用の正規表現が `tabs*` シナリオを**すべて**拾い、キャプチャ 1
  がタブ数になっていること
- スケーリング表の正規表現が `tabs_hold_resume_N` を**意図的に外して
  いる**こと (D97。揃っていないのを漏れと思って直されないよう固定する)

正規表現はテスト側にも定数として持ち、workflow 本体に字面どおり存在する
ことを検査する。片方だけ直せば必ず落ちる。

> **このテストを書いたことで前提の誤りが 1 つ見つかった。** 当初
> 「`scripts/bench/pages/` の全 `.html` が `page` の choice に出ている
> こと」も検査したが、`busy.html` (`background_cpu` 専用) /
> `download.html` / `network_activity.html` が落ちた。これらは**シナリオ
> 専用のフィクスチャであって汎用の計測対象ページではない。** choice に
> 並べると「どのページでも同じように測れる」という誤った示唆を与えるので、
> テストの方を「フィクスチャが choice に入っていないこと」の検査に
> 差し替えた。

### 検証 — 済み (§34)

`pull_request` の自己テスト (この workflow 自身を変更する PR でのみ走る、
D88 のパターン) は**既定入力 = 1 ページ**で走るので、**従来経路が変わって
いないこと**の検証になる。

複数ページ経路は手動 dispatch で確認した (run 34703501485、§34):
`minimal.html,dom_heavy.html` × `tabs_hold_10,tabs_hold_20` が
**ページごとに分かれた 2 つのスケーリング表**として出力され、行は
シナリオ名のみ、並びは (ページの入力順, タブ数) になった。結果ファイルも
衝突せず 2 ページ分揃った。

**この検証 run が、ページ差を同一 run 内で読める初のデータになった。**
§32.5 の「予算はページが重いほど効く」は機種の違う 2 つの run にまたがる
比較だったが (§32.6 限界 1)、同じ傾向が同一 run 内で再現した — 比 (B/A) は
10 タブで 1.043 (minimal) → 1.395 (dom_heavy)、20 タブで 1.854 → 2.903。
10 タブでは `minimal.html` は予算に一度も届かない (休止 0) のに対し
`dom_heavy.html` は 4 タブ休止しており、**「何タブで予算が発動するか」
自体がページの重さで変わる**ことも同一 run 内で示せた。

ただし trials 2 / repeats 1 の最小構成であり、**値を引用してはならない**
(§34.5)。また**これは §33.7 限界 3 の解消ではない** — §33 が比べたのは
「新既定 1023 MiB と従来の 700 MiB」で、本検証が比べたのは「既定と予算
OFF」であり、問いが違う。

### 決定5: `pages` はフィクスチャを弾かず、警告する

レビュー (PR #235) が非対称性を指摘した: `page` (choice) は
シナリオ専用フィクスチャ (`busy.html` / `download.html` /
`network_activity.html`) を**意図的に載せていない**のに、自由記述の
`pages` は同じ制約が効かず素通りする。

**弾く形は採らなかった。** `url` を指定すると loopback 配信自体が止まる
(`serve_fixed_pages=false`) ので、弾くと「`busy.html` を 20 タブ開いた
ときのメモリを測る」のような**正当な計測に逃げ道が無くなる。**

危ないのは能力ではなく**結果の読み違え**である。これらのページはただ
重いのではなく**何かをする** (CPU を回す / ダウンロードを起こす /
ポーリングする) ので、§21〜§34 の「ページの重さ」の系列と同じ表に並ぶと、
**別の現象を重さの違いとして読む**ことになる。そこで:

- ログに `::warning::` を出す
- **その表を読む人の目の前** (Job Summary の A/B 比較表の見出し) にも
  注意書きを出す。ログの警告だけでは、後から表だけを見る人に届かない

フィクスチャ一覧は workflow 側の `$fixturePages` に持ち、テストが字面
一致を検査する (決定4 と同じ形)。**新しいフィクスチャを足したときに
警告だけが漏れる**のを防ぐためである。

合わせて、artifact 名が `pages` を見ていなかった点も直した
(`pages` だけを指定した run が `velox-perf-windows-cold_startup` という
名前で残り、複数ページ run だと読み取れなかった)。`scenarios` と
`pages` はどちらも「1 run に複数の計測を収める」入力なので、**どちらが
指定されていても `multi` にする。**

### Revisit condition

(1) **実行時間の上限。** ページ数 × シナリオ数 × 試行数 × A/B で線形に
増えるのに対し、ジョブは `timeout-minutes: 30` である。§33.1 で実際に
この上限に当たった (打ち切り) ので、組合せを増やすときは `trials` /
`repeats` を下げる運用にしている。ページを 3 つ以上・50 タブ中心の構成が
常用になるなら、timeout の引き上げか、ページごとにジョブを分ける
(matrix) 形を検討すること。**ただし matrix はジョブが分かれる = ランナーが
分かれるので、本決定が解こうとした問題がそのまま戻る。** 引き上げの方が
筋が良い。
(2) **外部 URL は複数指定できない。** `url` は 1 つだけである。loopback
配信の固定ページと違い、外部 URL は比較の前提 (ネットワーク遅延・
キャッシュ・可用性) が揃っているとは限らないため、意図的に用意していない。
必要になったら、まず「何を揃えれば比較が成立するか」から決めること。
(3) **§32 の数値は取り直していない。** `pages` が使えるようになっても、
既に記録した測定結果は当時の条件のまま残す (D46: 記録を後から書き換え
ない)。ページ差を結論にしたいときは測り直すこと。

## D116: 設計判断の索引は手で書かず、`archive.md` から生成して CI で検査する (Issue #227)

**対象**: `docs/decisions/` のカテゴリ別インデックス。実装は
`.github/scripts/decision_index.py` と `test_decision_index.py`。
**製品コードは 1 行も変わらない。**

### 決定0: 起票時の想定より壊れていた — リンクの 64% が切れていた

Issue #227 は「108 件中 55 件が索引から引けない」という数え方で起票され、
対応案の 3 に「**アンカーの表記ゆれに注意。** 既存エントリは
`#d51-windows-release-github-actions-release-windowsyml` のように日本語部分
が落ちた形で書かれており、GitHub が実際に生成するアンカーと一致しているか
確認が要る」と書かれていた。

**確認した結果、一致していなかった。** 既存リンク 22 本のうち **14 本
(64%) がアンカー切れ**だった。`archive.md` の見出しが英語から日本語へ
書き換えられた際に、リンク側が旧見出しのまま取り残されていたものである。

つまり索引は「半分しか載っていない」だけでなく、**載っている分の多くも
機能していなかった。** Markdown としては正しい (リンク先が無いだけ) ので、
何も警告されない類の壊れ方である。

### 決定1: アンカーは推測で書かない — 本家 `github-slugger` と突き合わせる

最初に書いた生成規則は「記号をハイフンに置換」という素朴なもので、
**間違っていた。** GitHub は `、` `・` `「」` `→` `—` を**ハイフンにせず
除去する**:

| 見出しの一部 | 誤 (置換) | 正 (除去) |
| --- | --- | --- |
| `セキュリティ・入力値堅牢性` | `セキュリティ-入力値堅牢性` | `セキュリティ入力値堅牢性` |
| `維持、IPC に` | `維持-ipc-に` | `維持ipc-に` |
| `タブ/プロセス` | `タブ-プロセス` | `タブプロセス` |

この誤りに気づかずに進めていたら、**今回直したのと同じリンク切れを 115 本
作り直す**ところだった。そこで `npm i github-slugger` で本家を入れ、
`archive.md` の**全 115 見出しに対して 1 件の差も無い**ことを確認してから
生成した。`decision_index.py` の `slug()` はその確認を経た移植であり、
`test_slug_removes_punctuation_rather_than_hyphenating_it` が規則そのものを
固定している。

> **教訓の形は D46 と同じである。** 「たぶんこうだろう」で 115 本の
> リンクを量産するのは、実測せずに数値を書くのと同じ種類の誤りである。
> 幸い、ここでは答え合わせができる実装 (`github-slugger`) が存在した。

### 決定2: 索引は手で書かない — 割り当てだけを持ち、あとは生成する

カテゴリ割り当て (`_ASSIGNMENTS`) だけを Python の表として持ち、見出し・
アンカー・並び順は `archive.md` から機械的に作る。**手で書ける余地を
残さない**のが要点である — 55 件の漏れも 14 本のリンク切れも、「手で
維持するもの」だったから起きた。

D1–D70 の割り当ては `README.md` の表が元々示していたものをそのまま引き継ぎ、
丸ごと未索引だった D71 以降を新たに割り当てた。既存の割り当てを動かして
いないので、この変更で「どこを見ればよいか」が変わった Decision は無い。

### 決定3: 主担当は 1 つに限る

README の「重要なルール」3 が既に定めていた方針を、テストで強制する
(`test_every_decision_is_indexed_exactly_once`)。複数カテゴリに同じ
Decision を載せると、**どちらが最新か分からなくなり、片方だけ直される。**
横断的な Decision は主担当だけに載せ、他カテゴリからは本文で参照する。

### 決定4: 「全 Decision を必ず索引する」を CI の強制ルールにする

Issue #227 は対応案 4 で「強制ルールにしてよいかは運用判断なので、この
Issue では提案に留める」としていた。**強制する側を採った。**

理由は、この Issue 自体が「強制が無かったらどうなるか」の実例だからで
ある。PR #220 の分割から Issue 起票までの間に **38 件が丸ごと漏れ**、
リンク切れは誰にも気づかれないまま 14 本溜まった。運用の注意力で防げる
規模を既に超えている。

代償は「Decision を足す PR は索引も直す必要がある」だが、割り当てへの
1 行追加と生成の実行で済み、テストの失敗メッセージがその手順を名指しする。
検査は `ci.yml` の Python テストステップ (D108) にそのまま乗る。

### Revisit condition

(1) **`archive.md` の見出しに切り詰められたものがある。** 例えば D62 は
「スキーム許可リストは維持、IPC に」、D67 は「`browser::settings` に
一元化し、」で文が終わっている。索引はその見出しをそのまま引くので、
一覧の説明文も途中で切れる。**見出しを直せばアンカーが変わる**ため本決定
では触っていない (別 Issue)。直すときは索引の再生成も必要になる。
(2) カテゴリは 6 つのまま増やしていない。`03-performance` が 44 件と
突出しており、計測基盤・メモリ・回帰検知で分割する余地がある。件数が
読みづらさになったら分割を検討すること。ただし分割は既存の割り当てを
動かすので、**「どこを見ればよいか」が変わる**ことを引き受ける判断になる。

## D117: RAM 相対の予算は「重いページほど効かない」 (Issue #176 / D114 の効果測定) — 予算の調整で届く範囲の上限が見えた

**対象**: D114 (RAM 相対の既定メモリ予算) の効果が、ページの重さでどう
変わるか。**本体コードは変更していない** — §33.7 限界 3 が残していた問いを
D115 の `pages` で測っただけである。

### 決定0: 予測が外れた (2 度目)

§33.7 は「§32.5 が『予算はページが重いほど効く』を示しているので、重い
ページでは新既定の効きも大きいと**予想される**」と書いた。実測 (§35) は
**逆**だった:

| ページ | 休止タブ数 A (1023 MiB) | B (700 MiB) | 減少 |
| --- | ---: | ---: | ---: |
| `minimal.html` | 12 | 16 | **-4 (-25%)** |
| `dom_heavy.html` | 16 | 17 | **-1 (-6%)** |

D112 に続いて 2 度目の「自分の予測が実測に否定された」記録である。ただし
**前回と違って、今回は予測を実測として扱う手前で止められている。** §33.7
にも D115 にも「これは予想であって実測ではない」「§34 とは問いが違う」と
明記してあり、**その注意書きが実際に効いた** — §32.5 からこの問いへ外挿
していたら、逆の結論を製品判断の根拠にするところだった。

### 決定1: §32.5 と §35 は「問いが違う」ので矛盾していない

混同しやすいので明示する。

| | 比べているもの | 結論 |
| --- | --- | --- |
| §32.5 / §34 | 予算 **ON vs OFF** | 予算はページが重いほど**効く** |
| **§35** | 予算 **大 (1023) vs 小 (700)** | 予算の**増分**はページが重いほど**効かない** |

どちらも正しい。**「予算があること」の価値と「予算が大きいこと」の価値は
別物**であり、前者から後者を導けない。

### 決定2: 理由は飽和である — 重いページでは既にほぼ全部休止している

`dom_heavy.html` の B は、**19 個の背景タブのうち 17 個を休止してもなお
861.9 MiB** で予算 700 MiB に届いていない (§35.4)。休止できる余地がほぼ
残っていないところに 323 MiB を足しても、使える先が無い。

D93 が記録した「500 MiB 以下では応答が飽和して `max_live_tabs=1` 相当に
退化する」と**同じ現象**であり、そこへ至る経路が「予算を小さくする」から
「ページを重くする」に変わっただけである。

`minimal.html` では両条件とも予算の近傍 (1025.0 / 745.5 MiB) に着地でき、
予算差 323 MiB が §31.5 の約 65 MiB/タブでそのまま 4〜5 タブの差になる。

### 決定3: D114 は否定されていない。否定されたのは「期待」の方である

**D114 の正しさは §33 が示したままである** — 16 GiB 機で休止が 25% 減り、
式は意図どおり動いている。§35 が否定したのは「これで重いページの問題も
軽くなるだろう」という期待である。

言い換えると: **RAM 相対の予算は、助けが最も要らない場面で最もよく効く。**
軽いページ (メモリ圧が小さい) で 25% 減、重いページ (メモリ圧が大きい)
で 6% 減。

### 決定4: Stage 1 の結論として「予算の調整で届く範囲の上限」を記録する

重いページでの状態喪失を減らしたいなら、**予算の値を動かすだけでは届かない。**
残る手段は #176 が Stage 2 以降に置いているものそのものである:

- 1 タブあたりの実コストを下げる (Stage 4: cache / process policy)
- 休止の対象選択を賢くする (Stage 3: tab priority score)
- 休止の代わりに段階的な回収を入れる (Stage 2: memory pressure)

D112 決定4 が「減らせるのは休止の**回数**だけ」と確定させ、D114 がその
回数を実際に減らし、**§35 がその減らし方の上限を示した。** Stage 1
(計測・設計) はここで一区切りである。

### Revisit condition

(1) **20 タブの 1 点のみ。** 50 タブでは `minimal.html` 側も飽和に近づく
はずで (§31.3 は 50 タブで 44〜48 タブ休止)、両ページの差は縮む可能性が
ある。決定0 の表は**「20 タブでの話」**である。タブ数を変えて確かめること。
(2) **搭載 RAM は 15.99 GiB の 1 点のまま** (D114 Revisit (1) がそのまま
残っている)。RAM が大きいマシンでは A 側の予算が 2048 MiB まで上がるので、
重いページでも飽和を脱する可能性がある。**本決定は「16 GiB 機での話」でも
ある。**
(3) 2 反復・1 run である。差の向き (4 タブ vs 1 タブ) は run 内のばらつき
より大きいと考えているが、値そのものを引用するには薄い。

## D118: メモリの内訳は「VeloX 自身 vs エンジン」で割る — プロセス名によるロール分類は Windows では情報を増やさない (Issue #176 Stage 1)

**対象**: Issue #176 Stage 1 のチェック項目のうち 2 つ:

- 「PSS/RSS、WebProcess、NetworkProcess、GPU 関連メモリ等を**可能な範囲で**分類」
- 「Rust heap と WebView/engine 由来メモリを明確に分離して評価」

実装は `browser::metrics::RssSample` の `browser_rss_bytes` /
`engine_rss_bytes`。

### 決定1: 分割の基準はプロセス名ではなく **root pid** にする

`sample_process_tree_rss` は既に「ある pid とその子孫」を歩いている。その
**root がまさに VeloX 自身のプロセス**なので、root だけを別に数えれば
分離できる。

この基準を選んだ理由は、**プロセス名に依存しないこと**である:

- OS・エンジン・バージョンに依らず同じ意味になる
- `browser_rss_bytes + engine_rss_bytes = total_rss_bytes` が**常に**成り立つ
  (同じループで足しているので、取りこぼしや二重計上が原理的に起きない)
- 名前の綴りが変わっても壊れない

### 決定2: ロール別 (WebProcess / Network / GPU) の細分はしない

Issue の文面は「可能な範囲で分類」である。**可能な範囲を調べた結果、
優先 OS では不可能だった。**

| OS | ロールの見分け方 | 名前で割れるか |
| --- | --- | --- |
| Linux / WebKitGTK | `WebKitWebProcess` / `WebKitNetworkProcess` / `WebKitGPUProcess` | **割れる** |
| **Windows / WebView2** | すべて `msedgewebview2.exe`。ロールは**コマンドラインの `--type=`** にしかない | **割れない** |

Windows でコマンドラインを取るには WMI (`Win32_Process.CommandLine`) か
`NtQueryInformationProcess` + PEB 読み出しが要る。前者は依存追加 (D6 の
「依存は必要最小限」に反する)、後者は `unsafe` とバージョン依存の構造体
読みで、**得られるのは内訳の細分 1 つ**である。

`CreateToolhelp32Snapshot` の `PROCESSENTRY32W` は PID/PPID/実行ファイル名を
**追加コストなしで**持っている (だから pass 1 が速い) が、コマンドラインは
持っていない。つまり名前ベースの分類は Windows で**1 ビットも情報を
増やさない。**

Linux だけ細かい内訳が出る状態は、**OS をまたいで比べられない列が増える
だけ**である。D96 が「条件の違う数値を並べさせない」ために払ってきた労力と
逆行するので、採らなかった。

### 決定3: 「VeloX 自身」は「Rust が確保した量」ではない

`browser_rss_bytes` は root プロセスの RSS であり、Rust のヒープに加えて
`tao` のウィンドウやエンジンがプロセス内に置くものを含む。逆に、エンジンの
プロセスが抱えているものの中には VeloX が要求したものもある。

**この数字が答える問いは「フットプリントのうちエンジンのコンテンツ
プロセスの外にある割合はどれくらいか」**であって、「Rust が何バイト
`malloc` したか」ではない。#176 が必要としているのは前者である —
休止はエンジンのプロセスを落とす操作なので、**予算が効いているなら
`engine_rss_bytes` が動き `browser_rss_bytes` はほぼ動かないはず**であり、
そうなっていない A/B は予算とは別の何かが動いていることになる。

アロケータ単位の内訳が要るなら、それは別の道具 (D45 のプロファイリング
方針) の仕事である。

### 決定4: 既存キーの後ろに追記する

perf イベントの JSON には既存キーの**後ろに**足した。D42 が PSS を足した
ときと同じ規則で、古い形を前提に読む消費者が壊れない。

`Option` ではなく素の `u64` にしたのは、**ツリーが歩けた時点で分割は必ず
確定する**からである (PSS のように「読めないことがある」経路が無い)。
一方、**D118 より前に書かれた結果ファイルにはキーが無い** —
`MetricKey::extract` はそれを「0」ではなく「無い」として扱う
(`PssTotalBytes` と同じ規則)。0 として集計すると「VeloX 自身のメモリが
0 バイトだった run」という存在しないデータ点ができる。

### 検証 — 済み (§36)

初計測で決定3 の予想がそのまま成立した (run 34718717835、§36)。

| ページ | 比 (B/A) engine | 比 (B/A) browser |
| --- | ---: | ---: |
| `minimal.html` | **1.885** | 1.057 |
| `dom_heavy.html` | **2.968** | 0.904 |

予算が効いているのは engine 側だけで、VeloX 自身は 4 条件すべてで
**24.1〜26.6 MiB** に収まった (browser 側の A/B 差はどちらも有意差の下限
5 MiB を下回り、符号も逆なのでノイズである)。

**そして Stage 1 の問いに答えが出た**: フットプリントに占める VeloX 自身の
割合は **1.2〜3.7%** である。VeloX の Rust 側のメモリをゼロにしても総量は
その分しか減らない。動かせるレバーは**エンジンのプロセスをいくつ・どれだけ
生かしておくか**しかなく、これは D112 決定4 / D117 決定4 と同じ方向を別の
角度から裏づけている。

### Revisit condition

(1) **Linux だけなら今すぐロール別に割れる。** Linux の測定が製品判断に
使われるようになったら (CLAUDE.md の OS 優先度が変わったら)、
`/proc/<pid>/comm` を読んで `WebKitWebProcess` 等を分ける価値が出る。
その場合も **Windows に無い列であること**を表の側で明示すること。
(2) **WebView2 のロールを安く取る手段が現れたら**細分を再検討する
(`PROCESSENTRY32W` に相当するものでコマンドラインが取れる API、または
WebView2 SDK 自体がプロセス一覧とロールを返す API)。
(3) `browser_rss_bytes` は 1 プロセス分なので、**マルチウィンドウ (D68) でも
1 つのまま**である。ウィンドウごとにプロセスを分ける設計に変えたら、
この分割の意味も変わる。

## D119: タブの「優先度」は既存の `TabState` とは別の軸である — 5 段階は状態機械ではなく再訪likelihood の梯子として定義する (Issue #176 Stage 1)

**対象**: Issue #176 Stage 1 の「Active / Recent / Background / Dormant /
Suspended のタブ状態を定義」。**本決定は定義だけで、コードは追加しない。**
実装は Stage 3 (Tab priority score) の仕事である。

### 決定1: Issue の 5 段階は、2 つの異なる軸が混ざっている

そのまま `TabState` を 5 値に拡張してはならない。既存の
`browser::tab::TabState` (D20) と Issue の列挙は**別のことを言っている**:

| 軸 | 値 | 何を表すか | 誰が決めるか |
| --- | --- | --- | --- |
| **ライフサイクル** (既存 `TabState`) | Active / Background / Suspended / Restoring | **VeloX が webview を確保しているか** | VeloX の実装 |
| **優先度** (Issue の 5 段階) | Active / Recent / Background / Dormant | **利用者が戻ってくる見込み** | 利用者の行動 |

この 2 つは**直交する**。「webview が生きている (Background)」かつ
「1 時間触られていない (Dormant)」タブは普通に存在する。同じ enum に
押し込めると、片方の理由で決まった値をもう片方の意味で読む事故が起きる。

Issue の列挙で `Suspended` だけが浮いているのはこのためである — あれは
**優先度ではなくライフサイクルの値**で、「優先度が最も低い状態」ではなく
「既に回収済み」という結果である。

### 決定2: 優先度は 4 段階、`Suspended` は含めない

したがって Stage 3 が設計する優先度の梯子は次の 4 段階とする。
`Suspended` は `TabState` 側に既にあるので重複させない。

| 段階 | 意味 | 今日のデータで決まるか |
| --- | --- | --- |
| **Active** | 表示中のタブ | **決まる** (`Candidate::active`) |
| **Recent** | 直前まで見ていた。戻る可能性が高い | 決まる (`Candidate::idle` の閾値) |
| **Background** | しばらく触っていないが、まだセッションの一部 | 決まる (同上) |
| **Dormant** | 長時間放置。回収の第一候補 | 決まる (同上) |

`Candidate` が既に持っているのは `active` / `idle` / `loading` /
`protected` (音声再生中) / `process_group` である。**梯子そのものは
`idle` の閾値だけで作れる。**

### 決定3: 閾値は決めない — 決める材料が無い

「何秒で Recent → Background → Dormant か」を**この決定では定めない。**

D46 以来この文書が守っている規則 (実測より先に数字を決め打たない) に
従うなら、閾値には「利用者が実際にどのくらいの間隔でタブに戻るか」の
データが要る。**VeloX にはその計測が無く、本プロジェクトには利用者も
まだ居ない。** ここで 30 秒/5 分/1 時間のような数字を置けば、それは
根拠ではなく雰囲気である。

現行の休止ポリシー (D56/D90) が閾値を持たずに成立していることは、
この保留が実害を生まないことの傍証でもある — 予算シグナルは
「メモリが超えたら LRU 順に落とす」であって、段階を必要としていない。

**Stage 3 が優先度スコアを入れるときに、同時に決めるべきことである。**

### 決定4: 不足しているのは段階ではなく入力である

Issue が Stage 3 で挙げている考慮要素のうち、今日取れないものを明示する:

| 入力 | 今日 | 備考 |
| --- | --- | --- |
| last-used | **ある** | `Candidate::idle` |
| active | **ある** | `Candidate::active` |
| playing-media | **ある** | `Candidate::protected` (音声再生中) |
| 読み込み中 | **ある** | `Candidate::loading` |
| **pinned** | **無い** | ピン留め機能そのものが未実装 |
| **form-input** | **無い** | 入力中かどうかを知る手段が無い。エンジン側の状態で、取るには content webview に JS を常駐させることになる |

**`form-input` は「取れば済む」ものではない。** 休止で失われるものの中核
(D105 が「まだ分からないこと」として残した状態喪失) がまさにこれなので、
Stage 3 で優先度を作るなら、ここが最も価値が高く、同時に最も実装コストが
高い。段階の定義より先に、**この入力を取る手段があるか**を調べるのが
順序として正しい。

### Revisit condition

(1) **閾値は Stage 3 で決める。** そのとき「利用者がタブに戻る間隔」の
計測手段が無いままなら、閾値を置かない設計 (現行の LRU + 予算) を続ける
ことも選択肢である。段階を作ること自体が目的ではない。
(2) ピン留めが実装されたら (未起票)、優先度の入力として最優先で組み込む
...
## D121: `TrySuspend` は採らない — メモリを返さないからで、遅いからではない (Issue #243) — ただし「休止の対価は下げられない」(D112) は機構としては誤りだった

**対象**: Issue #176 Stage 2 / Issue #243。D120 決定3 が「測ってから
決める」と保留した、`TrySuspend` を現行の drop 方式の代わりに採るか。
**実測は `docs/performance-targets.md` §37。**

### 決定1: 既定は `SuspendMechanism::Discard` のまま据え置く

20 タブで `freeze` は `discard` の **1.888 倍**のメモリを使った
(859.6 MiB → 1622.6 MiB、§37.1)。メモリ管理の機構として採れない。

原因ははっきりしている。**`rss_process_count` が 15 → 27。**
`TrySuspend` は名前のとおり**休止であって終了ではない**ので、レンダラの
プロセスが生きたまま残る。プロセスが残れば常駐も残る。

B の 27 プロセス / 1622 MiB は §28 / §31 が測った「休止なしの 20 タブ」と
ほぼ一致する。つまり **`freeze` で休止した 20 タブは、休止していない 20
タブとほとんど同じだけメモリを使っている。**

### 決定2: それでもノブは残す — 消すと次に測るとき同じものを書き直すことになる

`VELOX_SUSPEND_MECHANISM` と `SuspendMechanism` は残す。理由は 3 つ。

1. **決定1 は「この機構では返らない」であって「この API は無価値」では
   ない** (決定4)。次に使うときのために経路を残すほうが安い。
2. 既定が `Discard` である限り、**通常の利用者には一切影響しない。**
   コードパスは環境変数を明示したときだけ通る。
3. 「回収の手段を差し替えられる」という形そのものが、D120 決定5 が
   Linux に経路を見つけたときにも要る (#240)。

ただし**ノブがあることを「いつか既定にする予定」と読んではならない。**
既定にするには §37.1 をひっくり返す実測が要る。

### 決定3: D112 の「休止の対価は下げられない」は、機構としては誤りだった

`tab_resume_ms` は **117.5 ms → 6.25 ms (0.053 倍)** (§37.3)。

D112 は「約 110〜130 ms で、タブ数にもページの重さにも予算の大小にも
依らない。webview の作り直しが支配項」と結論した。**その診断は正しかった
が、結論の射程が広すぎた。** 作り直しをやめれば対価は消える — 今回それを
実際にやってみせた。

D112 を書き換えはしない (当時の実測として正しい)。**本決定が D112 の
「下げられない」に射程の限定を付ける**: 下げる方法は存在し、その代金は
**メモリ 763 MiB** である。VeloX が払えないのは対価ではなく代金のほう
だった。

同じ理由で **D105 の状態喪失も解ける**ことが分かった — `freeze` なら
ページの状態は残る。こちらも「解けない」のではなく「この値段では買えない」
に変わった。

### 決定4: 使いどころがあるとすれば Stage 3 の「Recent」であって、メモリ管理ではない

§37 が示したのは「`TrySuspend` はメモリ機構として使えない」であって、
「`TrySuspend` は使えない」ではない。**6.25 ms で戻れて状態も残る**のは
それ自体が価値である。

D119 決定2 が定義した優先度の梯子を思い出すと、上 2 段は性質が違う。

| 段階 | 利用者が戻る見込み | 欲しいもの |
| --- | --- | --- |
| **Recent** (直前まで見ていた) | **高い** | **速く戻れること。メモリは二の次** |
| Background / Dormant | 低い | メモリが返ること |

**`freeze` は Recent に、`discard` は Dormant に向いている。** 同じ機構を
全タブに一律適用したのが今回の計測で、だから最悪の結果になった —
メモリを返してほしい 19 タブに、メモリを返さない機構を当てたのだから。

ただし**これは仮説であって、まだ何も測っていない。** 「Recent を数タブ
だけ freeze し、残りを discard する」混合方式が、全部 discard より
本当に良いかは別の計測が要る。§37.2 が示したとおり、freeze したタブは
予算の分母に残り続けるので、**混ぜると予算ロジックが誤作動する危険がある**
(休止したのにメモリが減らないタブを、policy は「まだ足りない」と読む)。
Stage 3 で優先度を設計するときに、**予算の勘定から freeze 済みタブを
どう扱うかを決めるのが先**である。

### 決定5: 「休止タブ数が増えた」を成功と読まない

§37.2 の `suspended_tab_count` 12 → 19 は、**効いた印ではなく暴走の印**
である。メモリが下がらないので policy が止まらず、20 タブ中 19 タブまで
休止させてなお予算に届かない。

D105 が `suspended_tab_count` を入れたとき、これは「予算がどれだけ効いたか」
の代理指標のつもりだった。**本計測はその読み方が成立しない条件を初めて
作った** — 「休止しても減らない」機構が入ると、この指標は効き目ではなく
諦めの回数を数える。今後この列を読むときは、必ず `rss_total_bytes` と
`rss_process_count` を並べて見ること。

### Revisit condition

(1) **WebView2 が休止後に遅延してメモリを返すなら、話が変わる。**
§37.5 限界4 のとおり、言えているのは「12 秒では返らない」までである。
長い hold (数分) で再計測して返るようなら決定1 を見直す。
(2) Stage 3 で優先度の段階を実装するとき、決定4 の混合方式を
**予算の勘定を決めてから**測る。
(3) `MemoryUsageTargetLevel(LOW)` (#242) は別物として残っている。あちらは
プロセスを残したまま「節約してよい」と伝える API で、今回の
「プロセスが残るから返らない」という原因がそのまま当てはまるかは
未確認である。**#242 を #243 の結果で閉じてはならない。**

## D122: `MemoryUsageTargetLevel(LOW)` は効く — 56〜60% 返る (Issue #242) — そして D121 が書いた「プロセスが残れば常駐も残る」は誤りだった

**対象**: Issue #176 Stage 2 / Issue #242。D120 決定1 が Stage 2 で見つけた
2 つの API のうち、D121 が扱わなかったほう。**実測は
`docs/performance-targets.md` §38。**

D121 決定5 Revisit condition (3) は「#242 を #243 の結果で閉じてはならない」
と書いた。**その警告は正しかった。** 結果は正反対だった。

### 決定1: 効く。タブを 1 つも休止させずにフットプリントが 56〜60% 減る

20 タブ・予算 OFF (休止ゼロ) で **1596.6 MiB → 700.7 MiB (0.439 倍)**。
`tabs_hold_resume_20` でも 0.407 倍。20 タブすべてが起きたまま、触れば
即座に使える状態でこれである (§38.2)。

切替コストは測れる範囲で変わらず (§38.4)、`cpu_percent` は一致している。
**対価らしい対価が、今のところ見つかっていない。**

### 決定2: D121 決定1 の因果の説明は誤りだった — 結論は変わらないが、理由を訂正する

D121 はこう書いた:

> `rss_process_count` が 15 → 27。`TrySuspend` は休止であって終了では
> ないので、レンダラのプロセスが生きたまま残る。**プロセスが残れば常駐も
> 残る。**

**最後の一文が誤りである。** §38 はプロセス数を 27 のまま 1 つも減らさずに、
常駐を 57〜60% 落としている。プロセスが残ることと常駐が残ることは別の話
だった。

| | §37 `freeze` | §38 `low` |
| --- | ---: | ---: |
| `rss_process_count` | 27 (変わらず) | 27 (変わらず) |
| `rss_engine_bytes` | 増えた | **57〜60% 減った** |

D121 の**結論** (`Freeze` を既定にしない) は実測値そのものに立っているので
揺るがない。訂正するのは**説明**だけである。正しくはこうなる:

- `TrySuspend` は「**止めろ**」と言う。ページの実行を止めるだけで、
  レンダラのヒープを縮めさせない。
- `MemoryUsageTargetLevel` は「**縮めてよい**」と言う。エンジンはそれに従う。

**効き目を分けたのはプロセスの生死ではなく、要求の中身だった。**
D121 は正しい結論に、間違った理屈で辿り着いていた。

この訂正には実務上の意味がある。「プロセスが残る手段は無駄」という一般則を
D121 から読み取ると、**まさに今回効いた手段を最初から捨てることになる。**
D121 はそう読まないよう Revisit condition (3) を置いていたが、置いていな
ければ #242 は着手されなかった可能性が高い。

### 決定3: **既定はまだ変えない。** 測った条件が既定の条件ではないから

`Low` を既定にしたくなる数字だが、**§38 は予算 OFF で測っている**
(§38.1 の分離のため)。既定は予算 ON なので、利用者が実際に置かれる条件
ではない。ここで既定を変えるのは、**既定でない設定で測った結果を既定に
持ち込む**ことになり、この文書が繰り返し戒めてきた誤りそのものである。

必要な追試は 2 つで、どちらも同一 run で測る (D96):

1. **既定の予算 ON で `low` を A/B する** — 休止と組み合わさったときの
   実効値。休止が減るのか、それとも二重に効くのかが分かる。
2. **`low` と `discard` を直接比べる** — §38.5 限界2 のとおり、§37 の
   859.6 MiB との比較は別 run・別機種なので成立しない。「LOW だけで休止
   より小さいのでは」は**魅力的だが、まだ仮説である。**

2 が肯定されると話が大きくなる。**休止 (D9 以来の中核機構) が Windows で
は要らない、あるいは今より遥かに弱くてよい**ことになり、D105 の状態喪失も
D112 の復帰コストも「そもそも払わなくてよい対価」になる。だからこそ、
別 run の数字で先に決めてはならない。

### 決定4: 返ってきた量は測れたが、何を失ったかは測れていない

§38.5 限界4。エンジンが JS ヒープを縮めたのか、描画バッファを捨てたのか、
キャッシュを落としたのかは分からない。`tab_switch_ms` (§38.4) はその代理
でしかなく、**切替が速いことは「失ったものが無い」ことを意味しない** —
例えば描画バッファを捨てていれば、切替後に再描画が要るがその時間は
`tab_switch_ms` に入らない可能性がある。

決定3 の追試を通すときに、**体感に近い指標を 1 つ足せないか**を検討する
こと。少なくとも、この不明点を「対価は無い」と言い換えないこと。

### Revisit condition

(1) 決定3 の追試 2 件で既定を決める。1 だけが済んだ時点で既定を変えない
こと — 「休止と併用して効く」と「休止より効く」は別の主張である。
(2) 決定4 の「何を失ったか」に手がかりが出たら、`Low` の適用範囲を
見直す (例えば Stage 3 の `Recent` 段だけ `Normal` に残す、など)。
(3) `Low` を既定にする日が来たら、**D121 決定2 が `SuspendMechanism` の
ノブを残した理由 (「回収の手段を差し替えられる形」) がこちらにも要る**か
を検討する。2 つのノブが別々に増え続けるのは避けたい。

## D123: Windows では背景タブに `MemoryUsageTargetLevel(LOW)` を既定で伝える (Issue #242) — hint が効くと休止は発火しなくなり、D105 / D112 の対価は日常的には払わなくなる

**対象**: Issue #242。D122 決定3 が「追試 2 件を経てから決める」と保留した、
`BackgroundMemoryTarget` の既定。**実測は
`docs/performance-targets.md` §38・§39。**

### 決定1: 既定を `BackgroundMemoryTarget::Low` にする (Windows のみ)

今日の既定に対して 20 タブで **0.814 倍 / 0.647 倍** (§39.1)。
3 本の独立した run が 700 MiB 前後に再現し (§39.4)、A 同士のばらつきは
A/B 差より 1〜2 桁小さい (§39.3)。

**「既定でない設定で測った結果を既定に持ち込むな」という D122 決定3 の
懸念は解消した** — 追試1 は予算 ON、つまり利用者が実際に置かれる条件で
測っている。

既定を変える判断を後押ししたのは、**今日の既定のほうが乱暴である**という
事実である。現状は 20 タブのうち 12 個を休止させ、そのページの状態を
丸ごと捨て (D105)、戻るたびに 100 ms 前後を払わせている (D112)。
`Low` の未計測リスク (決定5) は、**すでに出荷している挙動の既知の対価より
小さい。**

### 決定2: メモリ予算の既定は変えない — 発火しなくなるのであって、不要になるのではない

§39.1 で観測されたとおり、hint を入れると着地が 700.8 MiB となり予算
(この機体で 1023 MiB) に届かないので、**policy は休止を一度も発火させない。**

ここで「休止が要らない」と結論して予算を切りたくなるが、**切らない。**

- §39.6 限界1・2 のとおり、**50 タブや重いページでは hint だけでは予算に
  届かない可能性がある。** そのとき休止は最後の安全網として要る。
- 「発火しなくなる」は**予算を残したまま自然に起きる**。わざわざ機構を
  外す理由が無い。外せば、効かない条件に入ったときに何も起きなくなる。

つまり **休止は「日常的に起きる回収手段」から「hint で足りないときの
安全網」へ役割が変わる。** これは機構の削除ではなく位置づけの変更であり、
コードの変更を伴わない。

**予算の既定値そのものを見直すかは本決定では扱わない。** hint が入った
あとの適正値は改めて測る必要があり、影響範囲が違う (別 Issue)。

### 決定3: D105 の状態喪失と D112 の復帰コストは、日常的には払わなくなる

Stage 1 の結論 (Issue #176) は 3 点あった。そのうち 2 つの位置づけが変わる。

| | Stage 1 の結論 | D123 後 |
| --- | --- | --- |
| D112: 休止 1 回の対価は下げられない (110〜130 ms) | 避けられない | **20 タブでは休止自体が起きないので払わない** |
| D105: 休止で状態が失われる | 数値化できていない積み残し | **20 タブでは失われない** |
| D118: VeloX 自身は 1.2〜3.7% | — | 変わらず (§39.5) |

**どちらも「解けた」のではなく「発生しなくなった」である。** 予算に届く
条件 (多数のタブ・重いページ) に入れば休止は起き、対価も戻ってくる。
D112 / D105 を「解決済み」として閉じてはならない。

Stage 1 の「動かせるレバーはエンジンのプロセスをいくつ生かしておくか
だけ」という結論も、**誤りだったことになる。** プロセスの数を 27 のまま
にして総量を 4 割落とせた (§39.5)。当時その手段を知らなかっただけで、
レバーはもう 1 本あった。

### 決定4: Windows 限定である

`MemoryUsageTargetLevel` に相当するものが macOS / Linux には無い
(D120 決定5 / Issue #240)。両 OS の挙動は一切変わらず、従来どおり休止で
回収する。CLAUDE.md の「Windows を最優先、他 OS は最低限の整備」に沿う。

**性能値を OS ごとに分ける原則 (Epic #57) がここで効く。** §38 / §39 は
Windows の数字であり、Linux の目標値に流用してはならない。

### 決定5: まだ測っていないことを、決定と一緒に残す

既定にすることと、分かったつもりになることは別である。

1. **何を失ったのか** (D122 決定4 から継続) — エンジンが JS ヒープを
   縮めたのか、描画バッファを捨てたのか、キャッシュを落としたのかは
   不明。`tab_switch_ms` に有意差が無かった (§38.4) ことは
   「失ったものが無い」の証明ではない。
2. **50 タブ / 重いページ** (§39.6 限界1・2) — hint だけで予算に届くかが
   変わりうる。届かなければ決定2 の安全網が働く。
3. **メディア再生中のタブ** (§39.6 限界4) — hint が再生を阻害しないか
   未確認。

1〜3 は **Issue #247** に切り出した。**既定を変えたあとに「実は」が
出たときに、何を測っていなかったかが残っていることが重要である。**

### Revisit condition

(1) 決定5 の 1〜3 のいずれかで問題が出たら、既定を `Normal` に戻すか、
適用範囲を狭める (例: Stage 3 の `Recent` 段だけ `Normal` に残す)。
ノブは残してあるので切り戻しは環境変数 1 つで足りる。
(2) 決定2 の「50 タブでは安全網が要る」が実測で確かめられたら、
予算の適正値を測り直す。hint が入った今の適正値は D114 当時とは違いうる。
(3) macOS / Linux に相当 API が見つかったら (#240)、決定4 の限定を外す。

## D124: hint は「メモリを減らす機構」ではなく「同じ予算で生きたタブを増やす機構」だった (Issue #247) — 既定は維持し、休止は安全網として残す

**対象**: Issue #247 (D123 決定5 の積み残し) のうち、負荷を上げた 3 条件の
計測。**実測は `docs/performance-targets.md` §40。**

### 決定1: `Low` の既定 (D123 決定1) を維持する

3 条件すべてで 1 タブあたり **2.8〜4.3 倍**効率的だった (§40.2)。
D123 を覆す材料は出ていない。

### 決定2: ただし D123 の説明は、負荷が上がると足りない

D123 は 20 タブ・`minimal.html` で「フットプリントが 0.647〜0.814 倍に
なり、休止が発火しなくなる」と書いた。**それは軽い条件でしか成り立たない。**

負荷を上げると**総量の比が逆転する** (§40.1):

| 条件 | 比 (B/A) | 読み |
| --- | ---: | --- |
| 20 / minimal | 1.545 | `low` のほうが小さい |
| 20 / dom_heavy | **0.716** | `normal` のほうが小さい |
| 50 / minimal | **0.763** | `normal` のほうが小さい |

逆転の理由は `low` が悪化したからではなく、**2 つの条件が違う数のタブを
生かしているから**である。`normal` は 1 タブが高いのですぐ予算にぶつかり、
50 タブ中 44 個を休止させて 6 タブしか残さない。`low` は 1 タブが安いので
同じ予算で 34 タブを抱えられる。

したがって正しい言い方はこうなる:

> **`Low` はメモリを減らす機構ではない。同じ予算で生きたままのタブを
> 増やす機構である。**

軽い条件で「メモリが減った」ように見えたのは、**増やせるタブがもう無かった
から** (20 タブ全部が生きていて予算が余っていた) にすぎない。

### 決定3: **総量の列を単独で読んではならない** — 生きタブ数で割ること

D123 決定5 が `suspended_tab_count` について書いた注意と同種で、今度は
`rss_total_bytes` 自体が対象になる。

`suspended_tab_count` が A/B で違う表では、**`rss_total_bytes` の比較は
成立しない。** 2 つの条件が違う仕事をしているからである。必ず

- `生きタブ数 = 総タブ数 − suspended_tab_count`
- `エンジン / 生きタブ`

を出してから比べること。§40.2 の形式を踏襲する。

この読み替えをしないと、**§40.1 の表から「重い条件では旧挙動のほうが
優れている」という正反対の結論が出る。**

### 決定4: 予算を残した D123 決定2 は正しかった

重い条件では A (`low`) が予算の **98〜99%** に張り付き、`suspended_tab_count`
が 4 / 16 と出ている (§40.3)。**安全網は設計どおり働いた。**

D123 決定2 は「発火しなくなるのであって不要になるのではない。50 タブや
重いページでは hint だけでは届かない可能性があり、そのとき安全網として
要る」と書いた。**予想どおりだった。** 予算の既定を変えなかった判断を
ここで確定させる。

### 決定5: 「何を失ったのか」は、まだ一歩も進んでいない

§40.4。`cpu_percent` は A (`low`) の中央値が 0.00、B (`normal`) が
0.39〜0.78 だが、**これを「CPU の対価は無い」と読んではならない。**

- 絶対値がどの条件でも 1 コアの 1% 未満で、分布が 0 に張り付いている。
  中央値 0.00 は「ページが動いていない」のか「サンプラが idle を多く
  拾った」のかを区別できない。
- **`p95` は 4 本とも 1.56 で一致**しており、ピーク時の仕事量は変わって
  いない。

**`cpu_percent` はこの問いに対して指標として粗すぎた。** Issue #247 項目1
(背景タブの JS タイマーが止まっていないか) は未解決のまま残る。

`busy.html` は必要なカウンタ (rAF / `setInterval` / CSS アニメーション) を
既に持ち `<title>` に書き出しているが、**perf 出力に title が載らない**
ため読めない。読むには harness 側の変更が要る。

D122 決定4 が「返ってきた量は測れたが、何を失ったかは測れていない」と
書いてから、**この点は一歩も進んでいない。** 進んだと書かないこと。

### 決定6: 「生きタブが多いほうが良い」は判断であって測定ではない

§40.5 限界4。34 タブが即座に使える 1014.8 MiB と、6 タブの 774.2 MiB の
どちらを選ぶかは**製品判断**である。VeloX は D105 (休止でページの状態が
失われる) と D112 (復帰に約 100 ms) を踏まえて前者を採る。

**数字がそう言っているのではない。** 「メモリが少ないほうが良い」という
基準で読めば、重い条件では `normal` が勝っている。どちらの基準を採るかを
明示せずに §40 の表を引用しないこと。

### Revisit condition

(1) 項目1 (決定5) を解くには `busy.html` のカウンタを perf 出力に載せる
harness 変更が要る。**解くまでは「対価が無い」と言わない。**
(2) 決定6 の判断は、利用者が実際にどれだけタブを開くかが分かったら
見直す。50 タブを常用しないなら、重い条件の挙動は問題にならない。
(3) `dom_heavy.html` 以外の重いページで §40.2 の向きが変わったら、
決定1 (既定の維持) から再検討する。

## D125: 撤回 — beacon は計測タブから 1 本も来ていなかった (Issue #247)。配線を直し、`LOW` の判断は再計測まで保留する

**対象**: Issue #247 / D124 決定5。**この決定の初版は、結論を測って
いないデータの上に置いていた。撤回する。実測は
`docs/performance-targets.md` §41。**

### 決定1: 初版の結論を取り下げる

初版はこう書いていた —「3 run で `low` と `normal` の beacon が同一
だった。背景タブの JS 実行を `LOW` が止めているという証拠は無い」。

**このデータは `background_cpu` のタブから来ていない。**

`beacon` 入力は `bench_url` ステップの `url` 出力にだけクエリを付けて
いたが、実計測ループは `pages` 出力からページ名を取って**素の URL を
組み直していた**。固定ページ配信時は必ずその分岐に入るため、beacon 付き
URL は計測に一度も使われていない。

実際に `url` 出力を使っていたのは、**velox を 8 秒だけ単独起動する診断
ステップ**だけだった (§41.2)。

### 決定2: 「総数が合わない」は、そのまま答えだった

初版の決定3 は「80 本前後出るはずが 13 本しか届かない」を**未説明**と
して残した。**その 13 本の内訳は `hidden` が 1、`visible` が 12 で、
13 = 1 + 12 は診断ステップの全量である。**

違和感は正しかった。**合わない量が「別のものを数えていた」ことを
そのまま指していたのに、サーバ側の取りこぼしを疑ってそちらを潰しに
行った。** 単一スレッド版でも 300/300 通したところで「サーバは原因では
ない」とまでは書いたが、**では何なのかを詰めずに仮説へ飛んだ。**

### 決定3: 「条件を変えたのに値が動かない」ときは、条件が届いていない疑いを先に潰す

診断ステップは `compare_env` による A/B 切り替えより**前**に走る。
3 run は**まったく同じ条件**を測っていたので、一致するのは当然だった。
**`low` と `normal` の違いはそもそも一度も試されていない。**

初版はその一致を「`LOW` は挙動を変えていない証拠」として読んだ。
**変化が無いことを結論の材料にする前に、変化させたものが届いているかを
確かめること。**

### 決定4: `BackgroundMemoryTarget::Low` の既定 (D123 決定1) は据え置く

**積極的な支持が得られたからではなく、覆す材料も無いからである。**
D123 はメモリの実測 (§38–§40) に立っており、その足場は今回の件で
揺らいでいない。動いたのは「背景タブの挙動」側の証拠だけで、そこは
**今や空である。**

D124 決定5 の歯止め (「解くまでは『対価が無い』と言わない」) は
**まだ効いている。** #247 は開いたままにする。

### 決定5: 配線を Linux 上の単体テストで固定する

この壊れ方は **run を 1 回使い切っても気づけない。** 数字は出るし、
ステップは緑になる。出てきた数字が別のものの数字であることは、
workflow を読まないと分からない。

`.github/scripts/test_perf_windows_inputs.py` に 5 件足した
(**修正前の workflow に対して 5 件とも落ちることを確認済み**)。
同ファイルはもともと「Windows でしか動かない PowerShell を Linux 上で
突き合わせる」ために置かれたもので (§26 系列で 2 回同種の失敗があった)、
その枠にそのまま入る。

あわせて**診断ステップを beacon の付かない `url_plain` 出力に切り替えた。**
診断の 8 秒分が計測と同じカウンタに混ざる経路を断つ。

### 決定6: `compare_env` に空文字列を渡しても単独条件にはならない

`workflow_dispatch` の入力は、空文字列を渡すと**宣言時の既定値で
埋められる**。`compare_env` は「空欄 = 比較しない」という意味の入力だが、
`workflow_dispatch` から空欄を指定する手段が無い。**入力設計として
壊れている。** (この観察だけは初版から変わらない。)

### Revisit condition

(1) 修正後の再計測が済んだら、この決定を**実測に基づいて書き直す**。
現状の決定1〜4 は「何も分かっていない」ことの記録であって、結論ではない。
(2) 初版が §41.5 で立てた仮説 (`set_visible(false)` が
`document.visibilityState` に届いていない) は、**その根拠だった
「`hidden` が 1 本だけ」が診断ステップ由来と判明したため足場を失った。**
#251 はこの前提で読み直すこと。仮説として誤りと決まったわけではないが、
**支持する証拠は今のところ無い。**

## D126: 自動クローズの代行はインラインコード内のキーワードを拾ってはならない (Issue #252) — 「閉じません」と書いた PR が Issue を閉じた

**対象**: Issue #252 / D83 (auto-merge がマージ後に `gh issue close` を代行
する仕組み)。**実害は Issue #247。**

### 決定1: インラインコードスパンを除去してから走査する

`.github/scripts/extract_closing_issues.py` はフェンスコードブロックと
引用行を除去していたが、**バッククォート 1 個で囲んだインラインコード
スパンを除去していなかった。**

そのため PR #249 の

> **この PR に (キーワード) は入れていません。**

という**否定文**からキーワードが抽出され、マージ 3 秒後 (13:31:01 →
13:31:04) に #247 が `--reason completed` で閉じられた。

GitHub 本体はコードスパン内の `#123` を **Issue リンクにすらしない。**
D83 は「GitHub 本体の挙動に合わせる」ことを前提に据えているので、
**本体が閉じないものを代行が閉じているのは仕様違反である。**

### 決定2: 未閉じのバッククォートは行をまたいで消さない

フェンス (` ``` `) は閉じ忘れを「そこから末尾まで無視」と安全側に倒して
いるが、**インラインには同じ扱いをしない。**

地の文に混ざる裸のバッククォートは書き損じとして珍しくなく、末尾まで
消すと**下の行にある正当なキーワードまで巻き添えにする**。除去は
「同じ数のバッククォートが同一行内で閉じている場合」に限る。

### 決定3: 2 種類の失敗は等価ではない — 迷ったら「閉じない」に倒す

| 失敗 | 結果 |
| --- | --- |
| 閉じ損ねる | Issue が残る → 人が手で閉じられる |
| **誤って閉じる** | **歯止めが静かに消える** |

#247 は D124 決定5 の「解くまでは『対価が無い』と言わない」という
歯止めを、**開いていること自体で**担保していた。誤クローズはそれを
黙って外す — CI は落ちず、レビューにも出ず、Issue 一覧から消えるだけ
なので**気づく契機が無い。**

閉じ忘れのほうは CLAUDE.md が既に「手動で Issue をクローズしてください」
と案内している範囲に収まる。したがって非対称な扱いが正しい。

### 決定4: 丁寧に書くほど踏む罠だった

「閉じない」と説明するために PR 本文でキーワードに言及するのは、
**むしろ望ましい書き方である。** その書き方が事故を起こしていた。

回帰テストには PR #249 の実際の文面をそのまま入れた。合成した最小例
では、この罠が「実際に書かれる文章」から生まれたことが伝わらない。

### 決定5: 既知のエッジケースは本体に合わせ、安全側へは倒さない

同じ行に**未閉じのバッククォートが先にある**と、それが後続のコードスパンの
開き側とペアになり、囲んだつもりのキーワードが地の文として残る
(`未閉じの ` の後に `Closes #5` と書いた行` → `#5` を拾う)。

**これは CommonMark に忠実な結果である。** 仕様は「最左のバッククォート列と、
それに続く最初の同じ長さの列をペアにする」と定めており、**GitHub 本体も
同じ行を同じように解釈する** — つまり本体もこのキーワードを地の文として
扱う。決定3 の「迷ったら閉じない」より、**D83 の「本体の挙動に合わせる」が
優先する**。ここを本体より安全側へ倒すと、今度は「本体なら閉じるのに
閉じない」というずれが生まれる。

挙動が未定義のまま残らないよう、両方向 (奇数個で漏れる / 偶数個で漏れない)
を単体テストで固定した。PR #250 のレビューで指摘された経路である。

### 決定6: 過去の誤爆を監査した — 実害 1 件、幸運な空振り 2 件

代行が動き始めた PR #170 以降のマージ済み PR を調べた。

| PR | マージ | 危険な文面 | 実害 |
| --- | --- | --- | --- |
| #170 | 自動 | 検証例を多数含む (`#71` / `#73` / `#77` など) | **無し** |
| #174 | 自動 | 無し | 無し |
| #209 | **手動** | `Closes #194` の否定文 | **無し** |
| #244 | 自動 | 無し | 無し |
| #249 | 自動 | `Closes #247` の否定文 | **#247 を誤クローズ** |

**2 件は偶然助かっただけである。**

- #170 は代行を導入した PR 自身で、job の checkout はマージ**前**の
  `main` を取るため、スクリプトがまだ存在せず抽出が走らなかった。
  Phase 4 の Epic **#73 が今も open であること**がその証拠である
  (本文に `Closes #73` を含む形の文字列がある)。
- #209 は本文で手動マージを依頼しており、実際に手動でマージされた。
  **手動マージでは代行そのものが走らない。**

### Revisit condition

(1) GitHub が `AUTO_MERGE_TOKEN` (PAT) でマージするよう変更され、標準の
自動クローズが効くようになったら、代行そのものの要否を D83 ごと見直す。
そのときこの除去規則も本体の挙動と突き合わせ直す。
(2) 「閉じるはずが閉じなかった」という報告が出たら、決定2 の行内限定が
きつすぎないかを疑う。複数行にまたがるコードスパンは現状素通りする。

## D127: 背景タブは `LOW` を受けても JS を止めていない (Issue #247) — ただし `low`/`normal` はまだ分離できておらず、D124 決定5 の問いは開いたまま

**対象**: Issue #247。D125 で撤回した計測の、配線を直したうえでのやり直し。
**実測は `docs/performance-targets.md` §42。**

### 決定1: 経路は本当に繋がった — 13 本 → 2,614 本

§42.1。結果 JSON の `url` にも `?beacon=1` が乗っている。D125 が撤回した
「診断ステップの単発起動だけを数えていた」状態は解消した。

### 決定2: `document.visibilityState` は背景タブに伝わっている

§42.3。`state=hidden` の beacon が 805 本届いた。ページは自分が隠れている
ことを認識している。

**D125 決定4 が「最有力」とした仮説 (`set_visible(false)` が
`visibilityState` に届いていない) は否定された。** Issue #251 を閉じた。

仮説が外れたこと自体より、**その仮説が「壊れた計測から出た数字」を
根拠にしていた**ことのほうが重要である。D125 決定2 の教訓の続きになる。

### 決定3: 背景タブで `setInterval` は動き続けている

`report()` は `setInterval` で 500ms ごとに呼ばれる。止まっていれば beacon は
1 本も来ない。805 本来ているので、止まっていない。

### 決定4: `advanced_frames` / `advanced_ticks` は読んではならない

§42.4。`hidden` の `advanced_frames` が **-5** と負になっている。異常値では
なく、**集計の設計がこの計測に合っていない**ことの表れである。

`serve.py` は `state` ごとに first と last しか持たないが、1 run で velox は
8 回起動し、そのたびにページのカウンタは 0 からやり直す。**別々の起動の
first と last を引き算しても意味がない。**

読めるのは `count` だけである。**負の値が出たことを「おかしいが些細」と
流さないこと** — これは指標が壊れている合図だった。

### 決定5: `low` と `normal` を分離できていないので、問いはまだ開いている

§42.5。`serve.py` は job 全体で 1 つのカウンタしか持たず、A 4 回と B 4 回の
beacon が同じバケットに積み上がっている。

決定3 が言えるのは「**どちらかの条件で**背景タブの JS が動いていた」まで
である。**`low` の背景タブが `normal` と違う振る舞いをするかは、やはり
測れていない。**

D124 決定5 の歯止め (「解くまでは『対価が無い』と言わない」) は**まだ
効いている。** #247 は開いたままにする。

### 決定6: メモリの A/B は、この run が最もきれいに採れている

§42.6。`rss_total_bytes` が A (`low`) 414.1 MB / B (`normal`) 466.3 MB で
**比 1.126**。しかも **`rss_process_count` が 9 で一致し、両腕とも
`suspended_tab_count` が 0**。

D124 決定3 が「`suspended_tab_count` が A/B で違う表では `rss_total_bytes`
の比較は成立しない」と書いた条件に、**この表は当たらない。**

したがって **`low` は 12 タブの `busy.html` で常駐を 52 MB (11%) 減らして
いる — 生きタブ数を変えずに。** §40 の表がプロセス数と休止数の違いに
汚染されていたのに対し、ここは素直に読める。A 同士のばらつきは 1 MB 未満で、
A/B 差より 2 桁小さい (§26.4 の条件も満たす)。

**D123 決定1 の既定 (`Low`) を、初めて積極的に支持する材料が出た。**
D125 決定4 は「覆す材料が無いから据え置く」という消極的な理由だったが、
ここは違う。

### 決定7: beacon を有効にした run の時間系メトリクスは他と並べられない

§42.7。ベンチステップが beacon 無しの約 2 分に対し**約 16 分**。トライアル数
は 2 倍だが時間は 8 倍で、12 タブ × 500ms の HTTP が 2〜4 コアのランナーに
効いている。

メモリ値は A/B とも同じ負荷の下なので比較できるが、`page_load_ms` のような
値を §21〜§34 の系列に並べてはならない。

### Revisit condition

(1) `serve.py` に腕ごと・起動ごとの区切り (例: `/beacon-reset`) を入れたら、
決定5 をやり直す。それが #247 の残りである。
(2) 決定6 の 1.126 は `busy.html` 12 タブの値である。ページや タブ数を変えた
ときに同じ向きに出るかは別途確かめること (§40 で総量の比が条件によって
逆転した前例がある)。

## D128: 自動クローズはコミットメッセージ経由でも起きる (Issue #253) — バッククォートは効かず、閉じているのは GitHub 本体である。D83 の前提は失効した

**対象**: Issue #253 / D126 の続き。**実害は Issue #247 で、同じ日に 2 度目。**

### 決定1: 1 回目の対策は、2 回目の経路を塞いでいなかった

| 回 | 時刻 (UTC) | 経路 | 記録 |
| --- | --- | --- | --- |
| 1 | 13:31:04 | PR 本文のインラインコード → 代行 | D126 |
| **2** | **15:03:21** | **コミットメッセージ → GitHub 本体** | 本決定 |

2 回目は、PR #250 のコミット `d189b19` の**メッセージ本文**に、1 回目の
バグを説明するための文字列をそのまま書いたことによる。**PR 本文のほうは
対策していたが、コミットメッセージは盲点だった。**

### 決定2: 閉じたのは代行ではなく GitHub 本体である

auto-merge のログは `merged: PR #250` と
`skip: Issue #252 は既に closed (PR #250)` の 2 行だけで、**#247 は一度も
扱っていない。** しかも #252 は「マージの 1 秒後にはもう閉じていた」。

| Issue | `closed_by_pull_requests` | 経路 |
| --- | --- | --- |
| #252 | **PR #250** | 本体が PR 本文のキーワードから |
| #247 | **無し** | 本体がコミットメッセージから (PR に紐付かない) |

**`closed_by_pull_requests` の有無が、2 つの経路を見分ける指紋になる。**

### 決定3: D83 の前提は失効した

D83 は「`GITHUB_TOKEN` によるマージは GitHub 側のそれ以上の自動処理を
誘発しない」ことを前提に代行を作った。**しかし現在の auto-merge は
`AUTO_MERGE_TOKEN` (PAT) でマージする** (#194 / D91 で登録、PR #209 で実測)。

PAT による操作は後続処理を誘発するため、**標準の自動クローズが復活して
いる。** つまり:

- **代行は今や概ね不要**である (#252 では実際に空振りした)
- **本体はコミットメッセージも読む。** 代行は PR 本文しか読まないので、
  **本体のほうが守備範囲が広い**
- D102 が「public 化で D55 の前提が失効した」と記録したのと同じ形で、
  **D83 の前提も失効した**

### 決定4: バッククォートはコミットメッセージでは無意味である

D126 決定1 はインラインコードの除去を入れ、CLAUDE.md に「説明したいときは
バッククォートで囲め」と書いた。**その案内はコミットメッセージには効かない。**
GitHub のコミットメッセージ解析は Markdown のコードスパンを解釈しない。

### 決定5: 「気をつけて書く」では防げないことが実証された

1 回目の対策を打った**その PR のコミットメッセージ**で、同じ Issue が
別経路から閉じた。**対策を書いている最中に、対策していない経路を踏んだ。**

D126 決定4 に書いた「丁寧に書くほど踏む罠」がそのまま再現している。バグの
説明にキーワードを書くのは自然な行為で、避け続けることを規律に頼れない。

### 決定6: 止められないものは検知する

GitHub 本体の解析は止められない。したがって**閉じる前に気づく**方向で
対策する — PR のコミットメッセージからキーワードを抽出し、PR 本文の意図した
集合と食い違ったら CI で警告する (#253)。抽出は
`extract_closing_issues.py` をそのまま再利用できる。

**閉じてしまったものを開き直すより、閉じる前に気づく方が安い。**

### Revisit condition

(1) #253 で代行そのものの要否を見直す。本体が動いているなら二重実行でしか
なく、しかも守備範囲が狭い。残すなら「本体が落としたものを拾う」役割に
限定するのが筋である。
(2) `AUTO_MERGE_TOKEN` が外れたら本体の自動クローズも止まる。そのとき
決定3 は反転し、代行が再び本体になる。**どちらの前提で動いているかを
確かめずに、この節を読まないこと。**

## D129: `LOW` は背景タブの実行を何も変えない (Issue #247) — 腕を分離して確定。ただし D124 決定5 の本体はまだ開いている

**対象**: Issue #247。D127 決定5 が「分離できていない」と残した比較を、
`/beacon-reset` を入れて取り直した。**実測は
`docs/performance-targets.md` §43。**

### 決定1: `LOW` と `normal` に差は無い — A 同士のばらつきと同程度である

§43.3。背景タブの beacon は 4 本ともほぼ同じで、**A-A の差が A-B の差と
同程度**だった。§26.4 の判定条件にそのまま当たる。

**したがって `MemoryUsageTargetLevel(LOW)` は、背景タブの JS 実行を
何も変えていない。** D125 が同じ結論を「壊れた計測」で出していたが、
**今回は腕を分離したうえでの直接の観測である。**

### 決定2: rAF はもともと止まっている — `LOW` とは無関係に

§43.4。`advanced_frames` が**両腕とも 0**。隠れたタブに描画フレームを
渡さないのはエンジン自身の既定の抑制で、`normal` でも起きている。

一方 `advanced_ticks` は 103〜113 進む。**タイマーは動き、描画は止まる** —
背景タブの抑制がこの形であることが、初めて数字になった。

Issue #64 が `busy.html` に 3 つのカウンタを置いた目的が、ここで果たされた。

### 決定3: `instances` が集計の正しさの証拠になっている

§43.2。11〜13 は背景タブの数 (12) と一致する。D127 決定4 が記録した
壊れ方 (12 個のタブと 8 回の起動を混ぜて引き算していた) が直ったことが、
この列から読める。`advanced_*` も負になっていない。

**指標が直ったことを、指標自身が示す列を持たせること。** 「直したはず」
ではなく「直っていることが出力から読める」形にする。

### 決定4: メモリ比 1.125 が別機種で再現した

§43.5。§42.6 の Intel Xeon で 1.126、本節の AMD EPYC で 1.125。
**機種が違う run の絶対値は比較できない (D96) が、同一 run 内の A/B 比は
比較できる。** 独立に同じ値が出たことは、§42.6 が機種に依らないことを示す。

D127 決定6 の「D123 の既定を初めて積極的に支持する材料」は、**再現をもって
確定した。**

### 決定5: それでも D124 決定5 の本体は解けていない — #247 は閉じない

答えたのは**派生した問い** (「`LOW` は背景タブの実行を奪うか」→ No) で
ある。**本体 (エンジンが何を縮めたのか) に beacon は原理的に答えられない。**
beacon が運べるのはページから見える値だけで、エンジンの内訳は見えない。

**この決定を「`LOW` に対価は無いと分かった」と要約してはならない。**
分かったのは「**背景タブの実行という観測可能な面では**対価が無い」まで。
D124 決定5 の歯止めは**その範囲で**解除され、それ以外では効いたままである。

### 決定6: 次の一手は `performance.memory` — 3 候補のうち 1 つは潰せる

§43.7。WebView2 は Chromium なので、ページから
`performance.memory.usedJSHeapSize` が読める。beacon に載せれば
**JS ヒープが縮んだかだけは直接測れる。**

残る描画バッファとキャッシュはページから見えないので、D118 決定2 が
「Windows ではロール別に割れない」と書いた壁に戻る。**手段が無いのでは
なく、3 つのうち 1 つだけ手が届く**という状態である。

### Revisit condition

(1) `performance.memory` を beacon に載せたら、決定5 を JS ヒープの分だけ
書き換える。潰せなかった 2 候補は明示して残すこと。
(2) 決定1 は `busy.html` 12 タブでの話である。ページが変われば背景タブの
抑制の掛かり方も変わりうる (§40 で総量の比が条件によって逆転した前例が
ある)。

## D130: `LOW` が縮めたのは JS ヒープではない (Issue #247) — 桁が 3 倍足りない。ページから手の届く範囲は測り切った

**対象**: Issue #247 / D124 決定5 の 3 候補のうち、ページから手の届く 1 つ。
**実測は `docs/performance-targets.md` §44。**

**この決定は D129 Revisit condition (1) を解消する。** D129 は「`performance.
memory` を beacon に載せたら決定5 を JS ヒープの分だけ書き換える」と書いたが、
D129 は既に `main` にあるので**書き換えず、ここに新しい決定として置く**
(既存 `Dxx` を書き換えず新しい Decision を追加する — README の「重要な
ルール」4)。D129 決定5 の 3 候補のうち JS ヒープが否定されたことは、
本決定の決定5 の表が引き継ぐ。

### 決定1: `performance.memory` は WebView2 で読める

§44.1。`heap_count` が `count` と一致し、**すべての beacon が JS ヒープを
運んできた。** D129 決定6 が「読めるはず」と書いた予想が実測で確認された。

### 決定2: JS ヒープに A/B 差は無い

§44.2。A 1.327 MB / B 1.332 MB で、**B 自身のばらつき (0.145 MB) が
A/B 平均差 (0.005 MB) の 29 倍**ある。§26.4 の判定条件に当たる。

### 決定3: 桁が足りない — こちらのほうが強い根拠である

§44.3。**背景タブ 12 個の JS ヒープを全部合わせても約 16 MB**。一方
A/B の `rss_total_bytes` 差は **49.4 MB**。

**全部ゼロにしても差の 32% にしか届かない。**

決定2 は統計的な判断だが、これは**桁の話**である。仮に決定2 が測り損なって
いたとしても、**JS ヒープは 49.4 MB を説明できない。** 2 つの独立した根拠が
同じ結論を指す。

**「差が無い」より「説明できない」のほうが強い。** A/B 比較は測り方を
間違えば差を見落とすが、総量の比較は測り方に依らない。

### 決定4: メモリ比が 3 回、2 機種で再現した

§44.4。1.126 (Intel Xeon) / 1.125 (AMD EPYC) / 1.127 (AMD EPYC)。
3 run とも `rss_process_count` 9 で一致、`suspended_tab_count` 0。

**`low` が 11〜13% 減らすことは、もはや疑う余地がない。** D123 決定1 の
既定は、D127 決定6 で「初めて積極的に支持する材料」、D129 決定4 で
「再現をもって確定」と進み、ここで**3 回目**になった。

### 決定5: ページから手の届く範囲は測り切った — #247 を閉じる

| 候補 | 状態 |
| --- | --- |
| JS ヒープ | **否定** (決定2・3) |
| 描画バッファ | 未測定 — ページから見えない |
| キャッシュ | 未測定 — ページから見えない |

残り 2 つには **D118 決定2 の壁** (WMI 依存追加か PEB の `unsafe`) がある。
**#247 を開いたままにしても、次に打つ手が無い。**

D124 決定5 の歯止め (「解くまでは『対価が無い』と言わない」) は、
**観測できる範囲では解除する** — 背景タブの実行は変わらず (D129 決定1)、
JS ヒープも変わらない (決定2)。**ただし「何を縮めたのか」は分かって
いない。** この 2 つは違う主張である。

### 決定6: 仮説は立てるが Issue にはしない

§44.7。背景タブでは両腕とも rAF が止まっている。描いていないのにエンジンが
最後に合成した面を持ち続けているなら、`LOW` がそれを手放している —
という説明は辻褄が合う。

**しかし測っていない。** D125 で「壊れた計測から出た数字を根拠に仮説を立て、
別 Issue (#251) に切り出す」失敗をしたばかりである。**測る手段が無い仮説を
Issue にすると、開いているだけで進まない Issue が増える。**

入口は D118 決定2 の Revisit condition であって、新しい Issue ではない。

### Revisit condition

(1) D118 決定2 が動いたら (WMI か `unsafe` を入れる判断をしたら)、その
ついでに残り 2 候補を測る。**そのためだけに D118 を動かす価値があるかは、
今のところ無い** — `low` の得 (11〜13%) は既に確定しており、内訳が分かって
も既定は変わらない。
(2) `busy.html` は JS ヒープが 1.33 MB しかない軽いページである。重い
ページ (`dom_heavy` など) ではヒープが総量に占める割合が変わるので、
決定3 の桁の議論はその条件では立て直すこと。

## D131: 誤クローズは避けるのではなく検知する (Issue #253) — そして検知器を PR 本文用の除去規則で作ると、事故そのものを見落とす

**対象**: Issue #253 / D128 の対策。**実装は
`.github/scripts/check_closing_keywords.py`。**

### 決定1: 「気をつけて書く」に頼らない

D128 決定5 のとおり、**1 度目の対策を入れたコミット自身が 2 度目の経路を
踏んだ。** バグの説明にキーワードを書くのは自然な行為で、避け続けることを
規律に頼れない。

GitHub 本体の解析は止められないので、**閉じる前に気づく**ほうへ倒す。
閉じてしまったものを開き直すより安い。

### 決定2: PR 本文が「意図」である

CLAUDE.md 「Issue と PR の紐付け」が**本文に書け**と指示している。
したがって**本文に書かれた集合が意図**であり、コミットメッセージにしか
無い番号は「意図していないのに閉じられる」ものである。

逆向き (本文にあってコミットに無い) は**正常**なので何も言わない。

### 決定3: 検知器を PR 本文用の除去規則で作ると、事故そのものを見落とす

**実装中にこれを踏みかけた。** 検知器は `extract_closing_issues` を
再利用するのが自然だが、**そのままでは事故を検出できない。**

D126 でインラインコードの除去を入れたため、Issue #247 を閉じた
`d189b19` のメッセージ (キーワードがバッククォートに囲まれている) は
**`[]` を返す。**

| | PR 本文 | コミットメッセージ |
| --- | --- | --- |
| GitHub は Markdown として読むか | **読む** | **読まない** |
| バッククォートは効くか | 効く | **効かない** |
| 除去すべきか | **すべき** (D126) | **してはならない** |

`extract_closing_issues` に `markdown` 引数を足し、コミットメッセージは
`markdown=False` で読む。**D126 の修正は正しいが、適用する文脈を間違える
と検知器が盲目になる。**

### 決定4: 落とさず警告にする

意図した close が本文に書き忘れられているだけの場合もありうる。そして
**この検査でマージが止まると、止まった理由の調査コストのほうが事故の修復
コスト (再オープン) を上回る。** 誤クローズは元に戻せる。

### 決定5: 見つけた対象では落とさないが、**配線が壊れていたら落とす**

決定4 は「見つけた対象」の話である。**引数の数が合わないのは PR の内容の
問題ではなく、この検査が動いていないという意味**なので、扱いが違う。

黙って緑を返すと、**検知器が死んだまま誰も気づかない** — それは本決定が
防ごうとしている「静かに歯止めが外れる」形そのものになる。

`ci.yml` の Python テスト探索 (「テストが 1 件も見つからない」で
`::error::` + `exit 1`) と同じ規律に揃えた。**素通りして緑を作らない。**

PR #257 のレビュー指摘。レビューは「`::warning::` で目立たせる (exit 0 の
まま)」を提案したが、**リポジトリに既存の先例があるほうへ揃えた。**

### 決定6: 実データで裏付けてから入れた

`main` の全履歴 201 コミットに掛けた。

| | 件数 |
| --- | ---: |
| キーワード + 番号の出現 | 7 |
| うち意図的 (subject にも同じ番号) | 6 |
| **事故** | **1** (`d189b19`) |

実際の PR 本文を使って確認したところ、**事故 1 件を検出し、誤検知は 0 件**
だった (PR #220 は本文末尾に宣言があるので警告されない)。

**検知案を推測で入れず、過去のデータで誤検知率を確かめてから入れた。**

### 決定7: `fetch-depth: 0` が要る

`pull_request` の checkout は `refs/pull/N/merge` を深さ 1 で取るので、
**既定の shallow clone では `base..head` が引けない。** 検査を足すなら
checkout 側も変える必要がある。リポジトリは 200 コミット程度なので費用は
無視できる。

### 決定8: D83 の代行は残す — ただし前提は D128 決定3 で失効している

#253 が求めた「代行そのものの要否見直し」への答え。

**現状は本体と代行の二重実行**で、しかも本体のほうが守備範囲が広い
(コミットメッセージも読む)。それでも代行を外さない:

1. **本体が動いているのは `AUTO_MERGE_TOKEN` (PAT) があるからである。**
   PAT が失効・削除されれば本体の自動クローズは止まり、代行が再び本体に
   なる。外すとその日に静かに壊れる。
2. **二重実行の害が無い。** 代行は閉じる前に state を確認し、既に closed
   なら何もしない (#252 で実際に空振りしている)。
3. 外す作業そのものにリスクがある一方、得られるのは「1 ステップ減る」だけ。

**ただし D83 の本文にある前提 (`GITHUB_TOKEN` では本体が動かない) は
失効している。** CLAUDE.md には D128 で追記済み。

### Revisit condition

(1) `AUTO_MERGE_TOKEN` を外すことになったら決定8 を見直す。そのとき代行が
唯一の経路に戻るので、**代行が PR 本文しか読まないこと**が効いてくる。
(2) 誤検知の報告が出たら決定6 のデータを取り直す。201 コミットは「今日
までの」データであって、書き方の流行が変われば率も変わる。

## D132: 週次計測を `results/history/` へ自動で取り込む (Issue #211 項目4 の後半) — main へ直接 push する選択肢は最初から無く、`GITHUB_TOKEN` で作った PR は誰にも気付かれない

**対象**: Issue #211 項目4 の後半 / **D106 Revisit condition (1)**。
実装は `.github/workflows/perf-windows.yml` の `ingest-history` ジョブと
`.github/scripts/perf_history_plan.py`。

D106 は schedule を足すところまでを行い、**取り込みの導線は意図的に
未着手のまま残した** — 「誰が commit を作るか」(作者・タイミング・競合・
fork/権限) が独立した意思決定を要するため。ここがその意思決定である。

### 前提: この状態は「貯めているつもりで貯まっていない」だった

D106 で週次実行は始まったが、結果は artifact として 30 日で消えるだけで、
`results/history/` には**リポジトリ全体で 1 件** (committed な Linux の
baseline) しか無い。**項目3「機種を揃えたときの run 間分散を実測する」は、
データが 1 件も貯まらない限り永久に着手できない。**

### 決定1: main へは直接 push しない — できないから

`main` には active な ruleset があり `pull_request` ルールを含む。bypass は
admin ロールのみで、`GITHUB_TOKEN` は該当しない。**「PR にするか直接
push するか」は設計上の選択肢ですらなく、最初から PR しかない。**

これは推測ではなく、実装前に `/repos/noan98/VeloX/rulesets` を読んで
確認した。承認必須数は 0 件なので、PR さえ作れば既存の auto-merge が
そのまま処理できる。

### 決定2: PR は PAT (`AUTO_MERGE_TOKEN`) で作り、push も PAT で行う

**`GITHUB_TOKEN` による PR 作成・push は `on: pull_request` を発火させない**
(GitHub のループ防止仕様)。これを踏むと:

- CI が 1 件も走らない
- auto-merge の起動契機である `workflow_run` も来ない
- schedule (10 分ごと) は D103 が実測したとおり全期間で約 6% しか発火しない

つまり **PR が誰にも気付かれないまま放置される。** D91 が `@claude` メン
ションで実測した「`GITHUB_TOKEN` だと後続の自動処理が動かない」と同じ罠
なので、同じ対処を採る。

**作成だけ PAT にするのでは足りない。** 2 回目以降の更新 push が
`GITHUB_TOKEN` だと `synchronize` が発火せず、**CI が再実行されないまま
auto-merge の判定に入る。** `actions/checkout` の `token` ごと PAT にする。

未設定の場合も PR は作るが、`::warning::` を出して手動対応に落とす
(黙って壊れるより、壊れていることを言う)。

### 決定3: 系列 (`session_id`) は「何を測ったか」で割る

`report.py` は `session_id` と `machine_key` の両方が一致する隣接エントリ
だけを線でつなぐ (D106)。機種は `machine_key` が既に担保しているので、
残る軸は**計測条件**である。

| 軸 | 担保するもの |
| --- | --- |
| 機種 (CPU / コア数 / RAM / OS) | `machine_key` (D106) |
| 計測条件 (A 腕 / B 腕 / 環境変数) | **`session_id` (D132)** |
| シナリオ | 保存先ファイルの分割 |

A 腕と B 腕は必ず別系列にし、**B 条件 (`compare_env`) の文字列そのものを
`session_id` に埋める。** 将来 `compare_env` の既定値が変わったとき、
**同じ系列の中で測っているものが静かに入れ替わる**のを防ぐため — D106 が
機種について採ったのと同じ「変わったら繋がない」方向である。

⚠️ この値は **`inputs.compare_env` から読んではならない。** schedule 実行
では `inputs.*` は常に空文字列で (D106 で実際に踏んだ罠)、既定値の補完は
計測ステップの中で行われる。補完**後**の値をジョブ出力として渡す。

### 決定4: 判定を純粋関数に切り出して Linux で固定する

腕の判定と系列 ID の決定は `.github/scripts/perf_history_plan.py` に置き、
単体テストで固定した。**取り込みが走るのは週 1 回の Windows schedule run
だけで、壊れていても PR の CI は緑のままである** — `test_perf_windows_inputs.py`
の冒頭が書いているのと同じ理由。

壊れ方は 2 通りあり、**後者のほうが有害**である:

1. 取り込みが 0 件 — 「貯めているつもりで何も貯まっていない」
2. 別条件の数値が同じ系列に入る — **比較してはならない数値が繋がる**

1 は気付けば取り返せるが、2 は誤った結論を作る。したがって
**条件が分からない結果は、既定の系列に混ぜずに取り込まない** (警告する)。

### 決定5: 触ってよい範囲を実行時にも検査する

書き込み権限を持つ自動ジョブなので、`results/history/` の外に差分が出たら
**マージさせずに落とす。** 権限を絞る (`permissions` はこのジョブにだけ)
のとは別に、実際の差分でも確かめる。

### 決定6: 冪等にする

`record.py` は単純な追記なので、同じ run を 2 度取り込めば 2 行入る
(`workflow_dispatch` の re-run で普通に起こる)。各エントリの `note` に
`run_id=` を必ず入れ、既に履歴にある run id なら何もしない。

### 実装中に踏んだ罠 (どちらも「初回だけ必ず失敗する」形だった)

**`git diff` は未追跡ファイルを見ない。** 履歴ファイルは初回だけ新規作成
なので、`git diff --quiet` で判定すると**1 回目の取り込みが「変更なし」と
して静かに捨てられる。**

**`git status --porcelain` は未追跡ディレクトリを 1 行に畳む。** 初回は
`results/history/windows/` ごと未追跡なので `?? results/` としか出ず、
それを決定5 の範囲外判定に掛けると**初回が必ず「範囲外の差分あり」として
落ちる。** `--untracked-files=all` が要る。

どちらも手元の git で再現して直した。**週次実行を 1 回空振りさせるまで
気付けない類の失敗**であり、両方ともテストで固定してある。

### やらなかったこと

**ゲート化 (閾値超過で fail) は入れていない。** D96 Revisit condition (2)
のとおり「同一機種なら run を跨いで比較してよい」はまだ言えておらず、
閾値を決める根拠が無い。本決定はその根拠を貯めるところまでである。

**Issue #211 はこの決定では閉じない。** 項目3 (分散の実測) はデータが
貯まってからでないと着手できない。

### Revisit condition

(1) **数週間ぶんが貯まったら項目3 に着手する。** 同一 `machine_key` の
run が複数集まった時点で分散を採り、ゲート化の閾値を決める。
**閾値の下限は既に分かっている** — `docs/performance-targets.md` §45 が
週次と同じ既定条件で同一 run 内のばらつきを ±2.1% と実測した (機種を
跨いだ ±23% とは桁が違う)。run 間のばらつきがこれより小さくなることは
ないので、これより狭い閾値は置けない。
(2) `windows-latest` の機種の引きが偏り、同じ機種が数週間出てこない場合、
週次という頻度そのものを見直す (D106 Revisit (4) と同じ話)。
(3) 取り込み PR が毎週 CI を 1 往復させる。データ 1 行のために Linux +
...
## D135: Stage 3 の優先度の梯子は今は作らない (Issue #176) — 段に割り当てる別々の動作がまだ無く、信用できない入力を必要と分かる前に入れない

**対象**: Issue #176 Stage 3 (Adaptive Memory Policy)。Stage 1 の D119、
Stage 2 の D120 に続く設計判断であり、**実装は含まない。**

> **⚠️ 決定3 と決定4 の前提は誤っている。訂正は D137。**
> この決定は 3 箇所で **Issue #243 を未完了として扱っている**が、#243 は
> 本決定を書く 27 時間前に終わっていた (D121 / §37)。決定4 は D137 決定2 で
> **撤回**されている — 段に割り当てる動作は既に在る。**決定3 と決定5 の**
> **結論自体は D137 で維持されるが、根拠は差し替わっている。**
> 決定1・2 は影響を受けない。

### 前提が Stage 1/2 の時点から変わっている

Stage 3 は Issue 起票時の想定をそのまま作る段のはずだった。ところが
その後の実測で、想定の土台が 3 つ動いている。

| 当時の想定 | 現在 |
| --- | --- |
| 背景タブは段階的に絞っていく | **`LOW` は既に全背景タブへ一律に適用済み** (D123) |
| 段を下げれば実行が抑えられる | **`LOW` は背景タブの実行を何も変えない** (D129) |
| 休止は日常的に起きる | **hint が効くと休止は発火しない** (D123) |

つまり「段を下げる」に対応する**別々の動作が、今は存在しない。**

**⚠️ この一文が誤り (D137 決定2)。** 3 つ目の動作 `freeze` が D121 決定2 で既に実装・保持されており、D121 決定4 が `Recent` 段への割り当てを提案している。

### 決定1: 保護 (絶対) と優先度 (順序) を別のものとして扱う

現行の `Candidate::eligible` は `!active && !loading && !protected` で、
`protected` は「絶対に休止しない」を意味する (今日は音声再生中)。
Stage 3 が足そうとしている入力はこれと性質が違うので、2 つに分ける。

| 種別 | 誰が出した信号か | 効き方 |
| --- | --- | --- |
| **保護** | **VeloX 自身が観測したもの**のみ (active / loading / 再生中) | 休止の対象から外す |
| **優先度** | それ以外 | **順序を動かすだけ。対象からは外れない** |

**ページの申告は、決して保護には入れない。** 入れた瞬間、どのページも
「入力中だ」と申告するだけで自分をメモリに居座らせられる
(`window.rs` の IPC ハンドラが守ってきた「ページ由来の値をそのまま
VeloX の振る舞いの決定に使わない」という規律を破ることになる)。

### 決定2: 信用できない申告は `idle` より下位のキーにする

優先度に入れる場合でも、**`idle` を上回らせない** (同じ `idle` の中での
タイブレークにしか効かせない)。

この形なら、嘘をつくページが得るものは「**同じくらい放置されている
タブより少し後ろに回る**」だけである。1 時間放置されたタブが申告しても、
30 秒前に見ていたタブより先に休止する。そして**全タブが申告すれば順序は
元のまま**で、予算はそのまま守られる。

上限を設ける案・有効期限を設ける案も検討したが、どちらも**新しいパラメータ
を 1 つ増やす**(上限値・失効時間) のに対し、この形はパラメータを増やさずに
同じ性質を得られる。根拠の無い数字を置かない (D46) 方針とも噛み合う。

### 決定3: それでも `form-input` は今は実装しない

**⚠️ 「#243 の結果を待つ」は誤り。理由は D137 決定1 で差し替え。結論は維持。**

決定1・2 で安全に入れる形は決まった。**しかし今は入れない。**

`form-input` を欲しがる理由は「**休止で入力中の状態が失われる**」である。

> 辿るなら **D119 決定4 → D105 の「対象」節** の順で見ること。
> **D105 は見出しだけ読むと繋がらない** — 見出しは「休止タブ数を RSS の
> 隣に出せるようにする」だが、その「対象」節は D97 Revisit condition (4)
> 「**休止による状態喪失のコストが未計測**」への対応だと書いている。
> `form-input` を優先度の入力として挙げたのは D119 決定4 である。

ところが Stage 2 が見つけた `TrySuspend` は**状態を保ったまま**
メモリを返す可能性があり、それを測るのが #243 である。

**もし `TrySuspend` が状態を保つなら、保護の動機はほとんど消える。**
信用できない入力を受け取る仕組みを、必要かどうか分かる前に入れるのは
順序が逆である。#243 の数字を見てから決める。

### 決定4: 4 段階の梯子 (Recent / Background / Dormant …) は今は作らない

**⚠️ この決定は撤回されている (D137 決定2)。段に割り当てる動作は既に在る。**

梯子が意味を持つのは、**段ごとに違う動作を割り当てられるとき**である。
今ある動作は 2 つだけで、しかも片方は既に一律適用されている:

- `MemoryUsageTargetLevel(LOW)` — **全背景タブへ既定で適用済み** (D123)
- 休止 — `idle` の降順で選ぶ (現行の実装)

**段に割り当てる先が無い。** この状態で梯子を作ると、状態遷移と閾値だけが
増えて振る舞いは何も変わらない — 動いているように見えて何もしていない
コードになる。

現行の「`idle` 降順」は「利用者が戻る見込みの順」の最も素直な近似であり、
段に切る利得は、段ごとの動作が増えるまで発生しない。

### 決定5: 閾値は引き続き決めない

**⚠️ 根拠のうち「決定4 により」の部分は失効 (D137 決定2)。結論は維持。**

D119 決定3 のとおり「何秒で Recent → Background → Dormant か」を決める
根拠が無い (利用者が居らず、戻る間隔の計測も無い)。決定4 により、
**そもそも決める必要が今は無い。**

### Stage 3 で実際にやるべきこと (順序)

**⚠️ 項目1 (「#243 の結果を待つ」) は誤り。#243 は本決定より前に完了している (D137)。**

1. **#243 の結果を待つ** — `TrySuspend` が状態を保つか。保つなら D105 /
   D112 が同時に消え、Stage 3 の動機の大半が変わる
2. `pinned` は**ピン留め機能そのものの実装**が先 (UI + 状態)。優先度の
   話ではなく機能追加であり、別 Issue が要る
3. 段ごとの動作が 2 つ以上になった時点で、初めて梯子を検討する

### Revisit condition

**⚠️ (1) は誤り。** #243 は本決定より前に完了しており、答えは「状態は保つが**メモリを返さない**」だった (D121)。差し替えは D137 決定1。

(1) **#243 で `TrySuspend` が状態を保たないと分かったら決定3 を見直す。**
その場合 `form-input` の価値が戻るので、決定1・2 の形で入れる。
(2) 段ごとに割り当てられる動作が増えたら決定4 を見直す。候補は Stage 4
(キャッシュ方針) で、段に応じてキャッシュを削る動作が作れるなら梯子に
意味が出る。
...
## D137: D135 決定3・決定4 の前提は誤っていた (Issue #176 Stage 3) — #243 は D135 より前に終わっており、段に割り当てる動作は既に在る

**対象**: D135 の訂正。**D135 は書き換えない** — D122 決定2 が D121 に対して
行ったのと同じ形で、誤りを本決定に記録する。ただし D122 のときと違い、
**D135 は「当時の実測として正しかった」のではなく、書いた時点で既に
誤っていた。**

### 何が誤っていたか

D135 は 3 箇所 (決定3、「Stage 3 で実際にやるべきこと」項目1、Revisit
condition (1)) で **#243 を未完了として扱い、「#243 の数字を見てから
決める」と書いた。**

| | |
| --- | --- |
| D121 (#243 の結論) のコミット | `d3934cc` **2026-09-13 16:08 UTC** |
| Issue #243 のクローズ | **2026-09-13 16:43 UTC** |
| D135 のコミット | `83e2231` **2026-09-14 19:42 UTC** |

**#243 は D135 を書く 27 時間前に終わっていた。** 結果は D121 と
`docs/performance-targets.md` §37 に記録済みで、コードにも入っている
(`SuspendMechanism::Freeze`)。

### 決定1: D135 決定3 の結論は維持する。ただし理由を差し替える

D135 決定3 はこう分岐させた。

> **もし `TrySuspend` が状態を保つなら、保護の動機はほとんど消える。**

**この分岐は、どちらの枝にも入らなかった。** D121 の答えは 2 つに割れて
いたからである。

| D135 が立てた問い | D121 の答え |
| --- | --- |
| `TrySuspend` は状態を保つか | **保つ** |
| (D135 が問わなかった) メモリは返るか | **返らない** — 20 タブで `discard` の **1.888 倍** (§37.1) |

返らないので **`Freeze` は採用されず、既定は `Discard` のまま**である
(D121 決定1)。つまり **VeloX が実際に休止するとき、今も状態は失われる。**
D135 が想定した「保護の動機が消える」枝には入っていない。

**よって `form-input` の価値は消えていない。** それでも今は実装しない
という結論は維持できるが、根拠は D135 が書いたものではなく D123 である:

- D123 決定1 で背景タブに `LOW` を既定適用した結果、20 タブでは予算に
  届かず **policy が休止を一度も発火させない** (D123 決定2)
- つまり休止は「日常的な回収手段」から「**hint で足りないときの安全網**」
  へ役割が変わった (同決定2)

失われる状態の総量そのものが小さくなったのであって、**失われなくなった
のではない。** 50 タブや重いページでは hint だけでは予算に届かず (D123
決定2 が安全網を残した理由)、そこでは今も状態が飛ぶ。`form-input` の
価値はゼロではなく、**その重い使い方のところに集中している。**

決定1・2 (保護と優先度の分離、信用できない申告は `idle` より下位) は
本訂正の影響を受けない。**入れる形は決まっているので、必要になった時点で
入れられる。**

### 決定2: D135 決定4 は撤回する — 段に割り当てる動作は既に在る

D135 決定4 はこう書いた。

> 今ある動作は 2 つだけで、しかも片方は既に一律適用されている
> (`MemoryUsageTargetLevel(LOW)` / 休止)。**段に割り当てる先が無い。**

**3 つ目が抜けている。** D121 決定2 は `SuspendMechanism` のノブを意図的に
残し、決定4 で**段ごとの割り当てそのものを提案している**:

| 段階 | 戻る見込み | 向く機構 | 根拠 |
| --- | --- | --- | --- |
| **Recent** | 高い | **`freeze`** — 6.25 ms で戻り、状態も残る | §37.3 |
| Background / Dormant | 低い | `discard` — プロセスごと返る | §37.1 |

これはコードにも実在する。`src/browser/suspension.rs` の
`SuspendMechanism::Freeze` の doc コメントは、残してある理由として
**「Stage 3 が作る優先度の梯子の `Recent` 段に向くかもしれないから」**と
明記している。

**選択的に適用できる動作が 1 つ (`discard`) から 2 つ (`freeze` /
`discard`) になった時点で、梯子は意味を持ちうる。** D135 決定4 が
「梯子を作らない」根拠とした事実が、そもそも成立していなかった。

**ただし「だから今すぐ梯子を作る」ではない。** D121 決定4 自身が
「**これは仮説であって、まだ何も測っていない**」と書いている。撤回するのは
結論ではなく**根拠**であり、正しい現状は「作らない」でも「作る」でもなく
決定3 のとおりである。

D135 決定5 (閾値は決めない) は「決定4 により、そもそも決める必要が今は
無い」を根拠にしていたので、**その根拠も同時に失効する。** ただし D119
決定3 の本体 (利用者が居らず「戻る間隔」の計測が無いので数字を置けない) は
健在なので、**結論は維持される。**

### 決定3: Stage 3 が次に片付けるのは「予算の勘定から freeze 済みタブをどう扱うか」

D121 Revisit condition (2) が既に指示している。

> Stage 3 で優先度の段階を実装するとき、決定4 の混合方式を
> **予算の勘定を決めてから**測る。

これは体裁上の順序ではなく、**混ぜると予算ロジックが誤作動する**ことが
実測で分かっているからである (§37.2)。

`freeze` したタブはメモリを返さないので予算の分母に残り続ける。policy は
それを「まだ足りない」と読み、**20 タブ中 19 タブまで休止させてなお
止まらなかった。** D121 決定5 はこれを「効いた印ではなく暴走の印」と
書いている。

つまり混合方式には、policy 側に「**この休止はメモリを返さないので、
予算の達成には数えない**」という区別が要る。その区別をどう置くかが
Stage 3 の最初の設計判断であり、**本決定ではまだ決めない** — 梯子の段
そのものの設計と一体で決めるべきで、片方だけ先に決めると噛み合わない。

### なぜこの誤りが起きたか

D135 は `TrySuspend` を **D120 (Stage 2 の調査)** の文脈で辿り、その先に
ある **D121 (Stage 2 の実測)** に行き着かないまま書かれた。D120 は
「測ってから決める」で終わっているので、そこで止まると #243 は未完了に
見える。

**Decision 番号を辿るだけでは足りない。** 「この Issue はまだ開いているか」
を書く前に確かめること。アーカイブを `TrySuspend` で検索すれば D121 は
一度で出た。

### 検出器は足さない (測ったうえで)

「未完了として参照している Issue が実は閉じている」を CI で検査すべきかを
考え、実際に数えた。アーカイブ全体で `#N の結果を待つ` 系の記述は **2 件**
しかなく、うち 1 件 (`#30 待ち`) は**かつての制約を引用して「この Issue で
実際に解消した」と書いている正しい用法**である。実際のバグは D135 の 1 件
だけだった。

さらに、Issue の open/closed を CI で照合する検査は**壊れた検査になる**。
決定記録は歴史的記録なので、**執筆時点では正しかった「#N 待ち」が、
その Issue が閉じるたびに後から赤くなる。** 正しい記録を赤くする検査は、
ci.yml が「クロージングキーワードの意図確認」で避けたのと同じ形
(調査コストが事故の修復コストを上回る) に落ちる。

**足さない。** 規律として決定2 の末尾に書いた確認手順を残す。

### Revisit condition

(1) 決定3 の勘定を決めたら、D121 決定4 の混合方式を測る。結果次第で
D135 決定4 / 決定5 の結論そのもの (今は作らない / 閾値を決めない) が
動く。
(2) `LOW` が効かない条件 (50 タブ・重いページ、D123 決定2) の実測が
入ったら決定1 を見直す。そこで状態喪失が実際にどれだけ起きるかが分かれば、
`form-input` を入れるかの判断材料になる。

## D138: メモリ予算は「メモリを返す休止」しか命じない (Issue #176 Stage 3) — 見込み解放量を機構に聞く。機構は設定値ではなく実際に使えるものを使う

**対象**: Issue #176 Stage 3 の最初の設計判断。D137 決定3 が「Stage 3 が
次に片付けるのはこれ」と名指しした、**予算の勘定から `freeze` 済みタブを
どう扱うか。** 梯子そのものはまだ作らない (D137 決定2 のとおり、混合方式は
仮説で未計測)。

### 暴走はコードから確かめた — 実測の再現ではなく、原因の特定

§37.2 は `freeze` で休止させると `suspended_tab_count` が 12 → 19 まで
伸び、それでも予算に届かなかったと記録している。D121 決定5 はこれを
「効いた印ではなく暴走の印」と書いたが、**どこがそうさせたかは書いて
いない。** 今回コードで特定した。

1. `Tabs::suspension_candidates` は `TabState::Suspended` のタブを候補から
   外す。`freeze` したタブも VeloX の帳簿上は休止済みなので、ここで落ちる
2. ところが `freeze` はメモリを返さないので、**次のサンプルも同じ値**である
3. `plan` の `tabs_to_free` は `超過 / ESTIMATED_BYTES_PER_TAB` を計算する。
   この式は**どの休止も 64 MiB 返す**ことを前提にしている
4. サンプルが下がらないので超過も減らず、次のスイープがまた要求する

**この系には不動点が無い。** 手元で 20 タブ / 予算 700 MiB / §37.1 の
`freeze` 側の着地点 1622 MiB を入れると、**1 回のスイープで 15 個**
(922 MiB ÷ 64 MiB) を命じ、次のスイープで残りも取った。§37.2 の 19 個は
この算術の帰結であって、偶然ではない。

### 決定1: 見込み解放量を機構に聞く。0 なら要求は 0 個であって、全部ではない

`SuspendMechanism::expected_reclaim_bytes_per_tab()` を足し、`plan` に
機構を渡すようにした。

| 機構 | 見込み解放量 | 根拠 |
| --- | ---: | --- |
| `Discard` | `ESTIMATED_BYTES_PER_TAB` (64 MiB) | D54 / §10.2 |
| **`Freeze`** | **0** | **§37.1 の実測** — 20 タブで 1622.6 MiB、`discard` の 859.6 MiB に対し 1.888 倍で、これは「休止なしの 20 タブ」とほぼ同じ |

`tabs_to_free` は `per_tab == 0` のとき**割り算をせずに 0 を返す。**
ここが決定の全部である — 0 で割れないから特別扱いするのではなく、
**「休止しても減らない」に対する正しい答えが「0 個休止する」だから**である。

この形なら、`freeze` を指定した利用者が得るのは「予算を理由とする休止が
起きない」であって、「全タブが無意味に休止させられる」ではない。
**予算に届かないという事実は変わらないが、届かないまま状態を捨て続ける
のをやめる。**

`idle_after` と `max_live_tabs` は**そのまま効かせる。** これらはメモリの
話ではない。ここも一緒に止めると、「状態を保ったまま実行だけ止める」という
`freeze` 本来の使い道 (D121 決定4) が丸ごと死ぬ。

### 決定2: `freeze` はメモリ機構ではなく、CPU と状態の機構である

決定1 を「`freeze` は使えないから無効化する」と読まないこと。§37.3 が
示したのは **`tab_resume_ms` 117.5 ms → 6.25 ms (0.053 倍)** で、状態も
残る。捨てるには惜しい性質である。
...
| 機構 | メモリ | 実行 | 状態 | 復帰 |
| --- | --- | --- | --- | ---: |
| `MemoryUsageTargetLevel(LOW)` | **56〜60% 返る** (§38) | 変わらない (D129) | 残る | — |
| `Freeze` | 返らない (§37.1) | **止まる** | **残る** | **6.25 ms** |
| `Discard` | **プロセスごと返る** | 止まる | **失われる** | 117.5 ms |

3 つは別々の軸を動かしている。`LOW` はメモリだけ、`Discard` はメモリと
引き換えに状態、`Freeze` は実行と復帰速度。**メモリ予算という 1 本の
信号で 3 つを駆動しようとしたのが誤りだった。**

なお D129 が「`LOW` は背景タブの実行を何も変えない」と確定させている
ので、**背景タブの実行を止める手段は今のところ `Freeze` しか無い。**
Issue #64 (Background Tab CPU) の側から見ると、これは使える札が 1 枚
見つかったことになる。**ただし本決定では何も約束しない** — `freeze` の
CPU 効果は測っていない (D46)。

### 決定3: 機構は「設定値」ではなく「このウィンドウが実際に使えるもの」を渡す

決定1 を素朴に実装すると**新しいバグを作る。**
`VELOX_SUSPEND_MECHANISM=freeze` は Windows 以外でも指定でき、その環境で
`freeze_webview` は `false` を返して**必ず discard にフォールバックする。**
つまり Linux / macOS では `freeze` 指定でもメモリは普通に返る。ここで
機構名だけを見て予算信号を切ると、**返るのに切る**ことになり、決定1 が
直そうとしたのと逆向きの誤りになる。

そこで `ui::window` に `effective_suspend_mechanism` を置き、**ウィンドウ
生成時に 1 回だけ**解決する。

- Windows 以外 → 常に `Discard` (freeze 経路が存在しない)
- Windows で `freeze` 指定 → **D120 決定1 の probe をそのまま使い**、
  `ICoreWebView2_3` が無ければ `Discard` へ落とす

probe は副作用が無く (`QueryInterface` を試すだけ)、答えはプロセス内で
変わらない (全 webview が同じ Runtime を共有する)。**調査ログに出すのと
同じ値**を使うため、`webview2_suspend::support` に `OnceLock` を置いて
`log_support_once` と共有した — ログと挙動が食い違うと、ログを見て挙動を
説明できなくなる。

解決した値をウィンドウに持たせる副次効果として、**不可能と分かっている
dispatch を毎回試してエラーログを出すのもやめる。**

残る取りこぼしは 1 つある: Windows で probe は通るが**個々のタブについて
エンジンが freeze を断った**場合、そのタブは遅れて discard され実際には
メモリが返る。このとき予算信号は「返らない」と見なしたままなので、
**返せるのに休止を命じない**方向に外れる。取り込み不足であって暴走では
ないので、安全側である。`freeze` は既定ではなく計測用の opt-in なので、
影響範囲も明示的に指定した利用者に限られる。

### 検証

- **暴走を先に再現した** — 修正前のコードで、20 タブ・予算 700 MiB・
  §37.1 の `freeze` 側の着地点を入れると 1 スイープで 15 個を命じることを
  確認 (assert で 15 が出た)
- `the_memory_signal_orders_nothing_when_suspending_returns_nothing` —
  同じ入力で `Freeze` なら 0 個
- `the_memory_signal_still_orders_suspensions_that_do_return_memory` —
  `Discard` なら 15 個。**止めすぎていないことの確認**
- `the_other_two_signals_are_unaffected_by_the_mechanism` — idle /
  tab-count は機構に依らず同じ結果
- `a_zero_estimate_never_divides` — `tabs_to_free` の水準で直接固定
  (上の 3 つは `plan` 越しなので、ここが素通りしても気付きにくい)
- 見込み解放量を `Freeze => ESTIMATED_BYTES_PER_TAB` に戻すと
  **2 件が落ちることを確認済み** (テストが実際に効いていることの検証)
- `#[cfg(windows)]` の新コードが Windows ターゲットの検査に**本当に
  入っているか**を、わざと綴りを壊して確認した (D134 の教訓)。Linux は
  0 エラー、Windows だけが捕まえた

### Revisit condition

(1) **梯子 (D121 決定4 の混合方式) を作るときは、見込み解放量が機構ごと
ではなく段ごとになる。** そのとき `tabs_to_free` の「一律 `per_tab`」は
「候補を順に足していって超過を埋める」形へ変える必要がある。本決定は
機構がプロセス全体で 1 つである今日の形に対する最小の修正である。
(2) Windows で個々のタブの freeze 失敗が実際に起きると分かったら、決定3
の取りこぼしを見直す。今は probe が通れば成功する前提で組んでいる。
(3) `freeze` の CPU 効果を測ったら決定2 に数字を入れる。Issue #64 と
合わせて評価する。

## D139: コンテキストメニューに載せたのは「印刷」と「ページを保存」だけ (Issue #161) — リンク/画像の保存は #46 の関数では作れない

**対象**: Issue #161 (#39 の積み残し)。D78 が用意した拡張点にテーブル行を
足す作業のはずだった。**印刷とページ保存は足せた。リンク/画像の保存は
足せなかった** ので、その理由を残す。

### Issue の前提が 1 つ間違っていた

#161 はこう書いている。

> ページ/リンク/画像の「名前を付けて保存」— #46 (名前を付けて保存) が
> マージされてから、`MenuAction::SaveLinkAs(String)` /
> `SaveImageAs(String)` / `SavePageAs` を追加し、**#46 が用意する保存
> ダイアログ相当の関数を呼ぶ。**

#46 と #40 はどちらもマージ済み (それぞれ PR #159 / PR #155) で、そこは
前提どおりだった。**間違っていたのは「#46 の関数を呼べばリンクも保存
できる」のほうである。**

`BrowserWindow::request_save_page` は `tab_id` を受け取り、**その webview
が今表示している内容をシリアライズする** (Windows は MHTML、他は HTML —
D76)。つまり引数の `url` は保存対象の指定ではなく、ファイル名の推測と
記録用のラベルである。**任意の URL を取ってくる関数ではない。**

リンクや画像の保存に必要なのは「この URL の中身をこのパスに落とす」で、
これは別の機構である。

### wry 0.56 にその機構は無い

`wry::WebView` の公開メソッドを全部見た。`load_url` / `print` /
`evaluate_script` / `zoom` / cookie 操作などはあるが、**ダウンロードを
プログラムから開始する API は無い。** #103 が「進捗コールバックも
キャンセルハンドルも公開されていない」と記録したのと同じ根である
(あちらは開始済みダウンロードの制御、こちらは開始そのもの)。

### JS で `<a download>` を踏ませる案は採らない

`evaluate_script` でページ内に `<a href=… download>` を作ってクリック
させれば、VeloX が既に登録しているダウンロードハンドラ (#16) が拾う。
同一オリジンならこれで動く。

**採らない理由は、クロスオリジンで壊れ方が悪いからである。** Chromium は
クロスオリジンの `download` 属性を無視して**ただの遷移にする。** 画像 URL
なら画像が表示されるだけなので、利用者から見ると
**「画像を保存」を押したらページが移動した**ことになる。保存できない
だけならまだしも、見ていたページを失わせるのは明確な退行である。

同一オリジンかどうかで項目の有無を変える案も考えたが、利用者にとって
「保存できる画像とできない画像がある」理由が説明できない。

### 決定: 2 つだけ足す。リンク/画像の保存は #161 に残す

| 項目 | 状態 |
| --- | --- |
| `MenuAction::Print` | **足した** — Ctrl/Cmd+P と同じ `print_active_tab` を呼ぶ |
| `MenuAction::SavePage` | **足した** — Ctrl/Cmd+S と同じ `request_save_page` を呼ぶ |
| `SaveLinkAs` / `SaveImageAs` | **足していない** — 上記の理由 |

**どちらも既存のヘルパーをそのまま呼ぶ。** 2 つ目の実装を書かないので、
コンテキストメニュー経由とショートカット経由で保存形式・保存先・エラー
表示が食い違うことが起こりえない。

**ページ全体に効く項目なので、どのクリックでも出す。** リンクの上でも
画像の上でも「このページ」を意味するのは実ブラウザと同じである。並びは
再読み込みの直後 (移動 → ページ操作 → クリック対象の操作) で固定した。

`enabled` は常に `true`。メニューが開けたということは、そのタブに生きた
webview があるということである — 背景タブの webview は非表示なので
右クリックを受け取れない。

### なぜ #161 を閉じないか

受け入れ条件の 1 つ目が「**リンク/画像**/ページの保存がコンテキスト
メニューから実行できる」なので、満たしていない。閉じずに残す。

着手するなら順序は #103 と同じになる — wry への upstream 提案か、
プラットフォーム別のネイティブ実装か、自前 HTTP クライアント (Cookie /
セッションを失うので #16 が却下済み) の三択で、いずれも
「テーブルに 1 行足す」(D78) の規模ではない。**#161 の cost:low は、
残った半分については実態と合っていない。**

### Revisit condition

(1) wry がダウンロード開始 API を公開したら、`SaveLinkAs` /
`SaveImageAs` を D78 の手順どおりに足す。#103 と同時に解ける見込みが高い。
(2) それまでに利用者から要望が来たら、同一オリジン限定で `<a download>`
を使う案を再検討する。ただしクロスオリジンで遷移させない手当てが先。

## D140: ダウンロードの `success` を鵜呑みにしない (Issue #128) — 1 件失敗するとセッションの以後全部が失敗扱いになるのを実測で確認した

**対象**: Issue #128 の 2 つの疑い。#127 / D53 がソース読みの段階で見つけ、
実測で確認できなかったもの。**両方とも実機で決着した。片方は当たり、
もう片方は外れていた。**

### 疑い(1) は事実だった — ソースと実機の両方で確認

wry 0.56.1 (`src/webkitgtk/web_context.rs:315`) は
`failed: Rc<RefCell<bool>>` を **`connect_download_started` の外側で 1 つ**
作る。各ダウンロードの `connect_failed` がこれを `true` にし、**どこでも
`false` に戻さない。** そして `connect_finished` はこう呼ぶ:
...
つまり一度 `failed` が立つと、**以後そのレジストレーション経由の完了通知は
すべて `path = None` / `success = false` になる。** D53 が Linux の登録を
共有 `WebContext` の 1 回に集約したので、影響範囲はセッション全体である。

**実機で再現した** (Xvfb + WebKitGTK 2.52.6)。失敗 1 件のあとに成功 2 件:

| 順 | ダウンロード | 実際 | 報告 |
| --- | --- | --- | ---: |
| 1 | truncated.bin | **失敗** | `success=false` ✅ |
| 2 | velox-test.txt | **成功** (20 バイト、ディスク上に正常) | `success=false` ❌ |
| 3 | velox-test (1).txt | **成功** (同上) | `success=false` ❌ |

利用者から見ると「ファイルは保存されているのにパネルは失敗と言い続ける」。

### 疑い(2) は再現しなかった — そして最初の実験は私の取り違えだった

D53 は「途中で切断するサーバからのダウンロードでは `DownloadCompleted` が
一切届かなかった」と記録している。**届いた。**

ただしそこに辿り着く前に、**自分の実験器具のほうが間違っていた**ので
記録しておく。

| 実験 | サーバ | 結果 |
| --- | --- | --- |
| 1 | **HTTP/1.0** + `Content-Length: 10000000`、10 バイトで切断 | `success=true`。10 バイトのファイルが「完了」として残る |
| 2 | **HTTP/1.1** で同じ切断 | `success=false` / `path=None`。**部分ファイルは削除される** |
| 3 | 接続拒否 (`127.0.0.1:9`) への `download` リンク | **`DownloadStarted` すら発生しない。**何も記録されない |
| 4 | 実験2 の失敗 → `data:` ダウンロード 2 件 | 上の表のとおり全部 `false` |

実験1 を見た時点では「**不完全なファイルが成功として報告される**」という、
Issue より深刻な発見に見えた。**違った。** HTTP/1.0 では接続切断が本文の
正当な終端なので、WebKitGTK が完了と見なすのは筋が通っている
(`Content-Length` との矛盾を咎めないのは寛容すぎるが、HTTP/1.0 +
`Content-Length` + 早期切断という壊れたサーバ相手の話であり、追っていない)。

**Python の `http.server` は既定が HTTP/1.0 である。** D53 の検証も同じ形の
スクリプトだった可能性があり、「届かなかった」という観測がこの取り違えに
由来するかは断定できないが、**再現を試みる人は HTTP/1.1 を明示すること。**
統合テストの自前サーバがそうしている理由でもある。

さらに `Connection: close` を送るとまた `success=true` に戻る (libsoup が
早期切断を正当な終端として受け入れる)。**このヘッダ 1 本でテストが無言で
空振りになる**ので、テスト側にも理由を書いた。

### 決定1: `success` を鵜呑みにせず、記録済みの destination の存在で覆す

`browser::downloads::completion_succeeded(reported_success, destination_exists)`
を足した。

| 報告された `success` | destination | 記録 |
| --- | --- | --- |
| `true` | 見ない | 完了 |
| `false` | **ある** | **完了** — 汚染されたフラグ |
| `false` | ない | 失敗 |

**「ある」で覆してよい根拠は実験2 である** — 本当に失敗したとき
WebKitGTK は部分ファイルを消す。だから本物の失敗がここで成功に化けること
はない。**推測ではなく測った** (D46)。

`destination` は `DownloadStarted` の時点で `DownloadEntry` に記録済み
なので、`path = None` で返ってきても VeloX は何を見ればよいか知っている。

### 決定2: 大きさも内容も見ない

サイズ比較をしたくなるが、**比較する相手が無い。** wry は進捗
コールバックを一切公開しておらず (#103)、期待される長さが VeloX に届く
経路が無い。存在だけが使える信号であり、実験2 がそれで足りると示した。

### 決定3: 接続拒否で何も記録されないのは、変えない

実験3 のとおり、応答が返る前に接続が失敗すると `download-started` 自体が
起きない。**ダウンロードになっていないものを、失敗したダウンロードとして
記録はしない。** 実ブラウザも同じで、ナビゲーションの失敗として扱われる。

タイムアウトやサイズ監視で `Failed` に落とす案 (#128 のやること 3 番目) は
**要らない**と判断する。監視が必要なのは「開始したのに終わらない」場合
だが、実験2・4 でどちらも終端イベントは届いている。届かない実験3 は
そもそも開始していない。

### 決定4: 本筋の修正は upstream にある

決定1 は VeloX 側の緩和であって、原因を直してはいない。**wry 側の修正は
2 行で済む** — `failed` を `connect_download_started` のクロージャの
*内側*で作れば、ダウンロードごとに独立する。

VeloX 側の緩和はそれが入っても無害である (`success = true` で返るように
なれば決定1 の表の 1 行目を通るだけ)。

### 検証

- **汚染を先に再現した** — 修正前のコードで実験4 を実行し、成功 2 件が
  `success=false` で届くことを確認
- `tests/integration.rs` に `downloads_stay_correct_after_one_fails`。
  自前の HTTP/1.1 切断サーバ (std のみ、依存追加なし) で失敗を作り、
  そのあとの成功 2 件が完了として記録されることを Xvfb 上で確認
- **修正を外すと `["failure", "failure", "failure"]` になり落ちることを
  確認済み** (テストが実際に効いていることの検証)
- `completion_succeeded` の真理値表を単体テスト 3 件で固定
- 統合テストは**記録された結果**を見ており、wry が報告を直しても
  落ちない形にした

### 決定5: 緩和は WebKitGTK だけに効かせる — 最初は全 OS に効かせてしまっていた

**この節はレビューでの指摘を受けた訂正である。** 決定1 の実装は当初
`reported_success || destination_exists` を**プラットフォーム分岐なしで**
適用していた。原因は Linux 固有と分かっていたのに、である。

wry 0.56.1 の 3 バックエンドを全部読んで確かめた。

| バックエンド | `success` の決まり方 | 汚染 |
| --- | --- | --- |
| **WebKitGTK** (`webkitgtk/web_context.rs:315`) | 登録単位で共有の `failed` フラグ | **ある** |
| **WebView2** (`webview2/mod.rs:896`) | `state == COREWEBVIEW2_DOWNLOAD_STATE_COMPLETED` を**そのダウンロードの操作から**読む | 無い |
| **WKWebView** (`wkwebview/download.rs`) | `did_finish` が `true`、`did_fail` が `false` を**ダウンロードごとに**渡す | 無い |

**WebView2 と WKWebView の `false` はエンジンの本当の判定である。**
それを「部分ファイルが残っているから」で覆すと、**本物の失敗を成功として
記録する。** しかも「本当の失敗で部分ファイルが消えるか」は
**WebKitGTK でしか測っていない** (実験2)。つまり未計測の挙動を、
CLAUDE.md が最優先とする Windows に当てていた — この決定自身が
「推測で置かない」(D46) と書いた隣で、である。

`ui::window::DOWNLOAD_SUCCESS_FLAG_IS_SHARED` を足して、緩和をその
バックエンドに限定した。`false` のときは destination を見にすら行かない。

**この定数は `DOWNLOAD_HANDLERS_PER_CONTEXT` の再利用にしていない。**
問うていることが違う (登録場所 / フラグの共有) ので、片方を変えたときに
もう片方が黙って動くのは望ましくない。今日たまたま答えが一致している
だけである。

**統合テストは Linux でしか走らない** (Windows CI は `--lib` のみ、D61)
ので、この分岐が Windows 上で自動検証されることは今後も無い。だから
影響範囲を原因側と一致させておくことに意味がある。

### Revisit condition

(1) wry が `failed` を per-download にしたら、決定1 は無害なまま残る。
外すかは、そのとき `path` が正しく返るかを測ってから決める。
(2) **部分ファイルを残す失敗モードが見つかったら決定1 を見直す。**
測ったのは切断 1 種類だけである。残す失敗モードがあると、決定1 は
そこで誤って成功と記録する。
(3) **Windows / macOS で「本物の失敗時に部分ファイルが残るか」を測ったら
決定5 を見直す。** 残らないと分かれば緩和を全 OS に広げてもよいが、
そもそもあちらにフラグ汚染が無いので広げる利得は無い。
(4) macOS は `path` が常に `None` (D28) なので、そちらで
`destination_exists` がどう振る舞うかは別途確認が要る。今日は Linux の
挙動だけを根拠にしている。

## D141: セッションはウィンドウの配列で保存する (Issue #149) — 旧スキーマは `sanitize` で 1 ウィンドウに畳んで受ける。2 枚目以降はイベントループの中で開く

**対象**: Issue #149。#29 / D68 が「スコープ外」として残し、D68 の
「見送ったもの・既知の制約」1 番に記録されていた積み残し。Ctrl/Cmd+N で
開いた 2 枚目以降のウィンドウのタブが、再起動で失われていた。

### 決定1: `SessionSnapshot` はウィンドウの配列を持つ

```
{"windows": [{"tabs": [...], "active_index": 0}, ...]}
```

1 ウィンドウ分は `SavedWindow` に切り出した。`from_tabs` も
`sanitize` の per-window 部分もそちらへ移り、`SessionSnapshot` は
「ウィンドウをどう束ねるか」だけを見る。

**どのウィンドウを保存するかは `browser::session` が決めない。**
`app::persist_session` が非プライベートなものだけを渡す。この層は
プライバシーという概念を持たない (D20) ので、privacy の判定がここに
降りてくると層が壊れる。

### 決定2: 旧スキーマは `sanitize` で畳む — 専用の移行ステップを作らない

`tabs` / `active_index` はフィールドとして残し、**読むためだけに**使う。
`sanitize` が「`windows` が空で `tabs` が空でないなら、それが 1 枚目」と
畳む。

**`sanitize` を選んだのは、そこが既に「このファイルを信用しない」唯一の
入口だからである。** 別の移行関数を作ると、それを呼ばない経路が生まれうる。
`load_session` → `sanitize` は #25 以来の唯一の経路で、増やしていない。

畳んだあと旧フィールドは空にする。**残すと次の書き込みでディスクに戻り、
同じタブが 2 つの形で並ぶ。** シリアライズ側も `skip_serializing_if` で
落としてあるので、新しく書かれるファイルに旧フィールドは現れない。

両方の形が同時にあるファイル (手で編集した場合) は `windows` を採り、
旧フィールドは**捨てる。足さない** — どちらを意図したか当てに行くと、
利用者のタブが黙って倍になる。

### 決定3: 2 枚目以降はイベントループの中で開く

`BrowserWindow` の構築には `EventLoopWindowTarget` が要り、それは
`event_loop.run` が始まるまで存在しない。そこで `run` は 2 枚目以降を
`UserEvent::RestoreWindows` として proxy に積み、**ループの最初のパスで**
開く。

この形には副次的な利点が 2 つある。

1. **Ctrl/Cmd+N と同じ経路を通る。** `open_new_window` と共通の
   `attach_browser_window` に寄せたので、復元したウィンドウがテーマや
   ブックマークバーの初期状態で新規ウィンドウと食い違うことがない。
   2 つ目の構築箇所を作らない。
2. **起動計測が 1 ウィンドウのままである。** #182 の
   `process_start` → `window_created` の分解は**最初の 1 枚について**の
   ものであり、§21〜§26 の数字はすべてその条件で採られている。2 枚目を
   `run` の中で建てると、この区間の意味が静かに変わる。

復元したウィンドウのタブ一覧は、そのツールバーが `ready` を送った時点で
`sync_tab_strip` が描く。ここに追加の配線は要らなかった。

### 決定4: `AppState::primary_window` を消した

このフィールドの唯一の読み手が `persist_session` の
「`window_id != state.primary_window` なら書かない」だった。決定1 で
全ウィンドウを書くようになり、読み手がゼロになった。

自動化のターゲットは `run` の `automation_window` という別の可変変数で
あり (フィールドの doc コメント自身がそう書いていた)、こちらは無関係。
**使われない状態を残さない** (CLAUDE.md の compiler warning ゼロ)。

### 検証

- **両方の半分を、それぞれ壊して落ちることを確認した。**
  - 保存側を `#149` 前 (`.take(1)`) に戻す → 「both windows should be
    saved」で落ちる
  - `RestoreWindows` を送らないようにする → 「second never came back」で
    落ちる
  - **2 つの assert が別々の壊れ方を捕まえる**ので、片方だけ直っている
    状態で緑にならない
- 統合テスト `restoring_the_previous_session_reopens_every_window` —
  実バイナリを 2 回起動し、launch 2 が**自分の `Windows` から書き直した**
  `session.json` が 2 ウィンドウのままであることを見る。復元できなかった
  ウィンドウは `state.windows` に居ないので書かれない、という形で
  「本当に開いた」ことを証明している
- 既存の `restoring_the_previous_session_reopens_its_tabs_across_a_real_relaunch`
  は新スキーマ経由でそのまま通る (1 ウィンドウの経路は変わっていない)
- 旧スキーマの JSON 文字列からの読み込み、旧フィールドが round-trip
  しないこと、両方の形が同時にある場合を単体テストで固定
- `cargo fmt` / `clippy` (Linux + Windows ターゲット) / `cargo test`
  (unit 1079 + 統合 16、Xvfb 実走) / Python 270 件

### Revisit condition

(1) 旧スキーマのフィールドは、**利用者のディスクから旧 `session.json` が
無くなったと言える時点**で消せる。バージョンの下限を決める仕組みが無い
ので、今は消さない。消すときは決定2 の畳み込みも一緒に落とす。
(2) ウィンドウの**位置とサイズ**は保存していない。復元されたウィンドウは
既定の位置に開く。要望が出たら `SavedWindow` に足す — スキーマは既に
ウィンドウ単位なので、足す場所はある。

## D142: フォーム入力の検知手段は「ある」 (#176 Stage 3 の前提調査) — ただし iframe からは必ず親フレーム経由で中継する。直接送信は Windows で黙って消える

**対象**: Issue #176 Stage 3 の前提。D119 決定4 が「優先度スコアの入力の
うち `pinned` と `form-input` は今日取れない」と記録し、#176 本文が
**「`form-input` は休止で失われるものの中核 (D105 の積み残し) なので、
Stage 3 で段階を作るより先に取得手段の有無を調べるのが順序として正しい」**
と着手順を明示していた。本項はその調査であり、実装は含まない (Stage 2 が
D120 で調査を終えて実装を #242 / #243 に切り出したのと同じ形)。

`src/` への変更は無い。

### 結論: 取れる。ただし経路は 1 本しかない

|  | スクリプトは動くか | `window.ipc` は届くか |
| --- | --- | --- |
| メインフレーム | 動く | **届く** |
| サブフレーム (Linux/WebKitGTK) | 動く | 届かない (`window.ipc` が無い) |
| サブフレーム (Windows/WebView2) | 動く | **存在するが黙って捨てられる** |

### 決定1: メインフレームは既存の仕組みでそのまま取れる — 新しい機構を足さない

D18 (DevTools ショートカット) / D23 (タブ操作ショートカット) / D78
(コンテキストメニュー) が既に確立している「`with_initialization_script` で
注入したスクリプトが、固定の sentinel 文字列だけを untrusted IPC チャネルに
流す」形に、4 本目として素直に乗る。

しかも**このチャネルに流れるものとしては最も狭い**。D78 のコンテキスト
メニューは「ページ由来の値を上限付きでパースする」ところまで広げているが、
フォーム入力の信号は **bool 1 つ**で、ページ由来のデータを一切運ばない。

**ページが嘘をつけることは新しい問題ではない。** 「入力中だ」と偽り続ける
ページは自分が休止されなくなるだけで、それは**今日すでに無音の音声を再生
すれば得られる権能**である (`Candidate::protected` は
`is_playing_audio` で埋まる)。新しい濫用の種類は増えない。

### 決定2: サブフレームからは**常に** `window.parent.postMessage` で中継する — `window.ipc` を直接試さない

素直に書けば「まず `window.ipc` を試し、無ければ親に中継する」になる。
**これは Windows で壊れる。** そして Linux では動くので、Linux だけで
検証すると気付けない。

- **Linux/WebKitGTK**: wry が ipc shim (`Object.defineProperty(window, 'ipc', …)`)
  を `w.init(…, true)` — **メインフレーム限定**でハードコードしている
  (wry 0.56.1 `src/webkitgtk/mod.rs:348`)。サブフレームに `window.ipc` は
  存在しないので、フォールバックが正しく発動する。
- **Windows/WebView2**: wry 自身のドキュメントが
  「**Windows: scripts are always added to subframes regardless of the
  `for_main_frame_only` option**」と書いている (`src/lib.rs:1029`)。
  ipc shim も同じ経路で注入されるので、**サブフレームにも `window.ipc` は
  存在する**。ところが wry は top-level の `add_WebMessageReceived` しか
  登録しておらず (`src/webview2/mod.rs:955`)、`ICoreWebView2Frame` を
  一切取得していない。Microsoft のリファレンスは 2 つのイベントを明確に
  分けている — `CoreWebView2.WebMessageReceived` は「**top-level document**
  が `window.chrome.webview.postMessage` を実行したとき」、
  `CoreWebView2Frame.WebMessageReceived` は「**iframe** が実行したとき」。
  つまり iframe からの送信は**例外も出さず、戻り値の区別も無く、ただ
  消える**。

`window.parent.postMessage` はクロスオリジンでも届く (それが
`postMessage` の存在理由である) ため、中継だけに一本化しても失うものは
無い。**「両 OS で動く経路」と「片方で黙って壊れる経路」があるとき、
分岐させる理由が無いなら 1 本に寄せる。**

CLAUDE.md の OS 優先度 (Windows 最優先) がそのまま効いた例である。D140
決定5 は「Linux 固有の不具合への対処を全 OS に適用しない」だったが、
ここは向きが逆で、**Linux で素直に動く設計が Windows で静かに壊れる**。
どちらも「片方の OS だけを見て決めない」という同じ規律の適用にあたる。

### 決定3: 効くかどうかはここでは決めない

D120 決定3 と同じ立場を取る。「取れる」と「休止ポリシーに入れる価値が
ある」は別である。

- 何割のタブが「入力あり」と判定されるか、その判定がどれだけ保護しすぎる
  (= 休止回数を減らして D114 / §35 の効果を削る) かは**未計測**である。
- 入力フラグをいつクリアするか (送信時・遷移時・そもそもクリアしない) の
  正解も、計測しないと決められない。

数字が無いまま閾値を置けば根拠ではなく雰囲気になる (D46)。実装と計測は
別 Issue に切り出す。

### 実測 (Linux/WebKitGTK、Xvfb + dbus-run-session)

使い捨ての wry プログラム 1 本で、3 つの結果を区別できるようにして測った。

- `sub-direct:*` が来る → サブフレームでスクリプトが動き、`window.ipc` も届いた
- `sub-relay:*` が来る → スクリプトは動いたが `window.ipc` が無い
- 何も来ない → スクリプトがそもそも動いていない

結果は `main-direct:loaded` / `main-direct:input` / `sub-relay:loaded` /
`sub-relay:input` の 4 つが届き、`sub-direct:*` は**来なかった**。
サブフレームでも `input` イベントは見えている。

**実験器具を疑う (D140)。** 「クロスオリジンで中継できた」と言うには、
その iframe が本当に別オリジンでなければならない。カスタムプロトコルで
`probe://main` と `probe://other` の 2 オリジンを立て、子フレーム自身に
`document.location.origin` と「親の `document` が読めるか」を報告させて
確かめた — 子は `probe://other` で、`window.parent.document` の参照は
例外になった (= 真にクロスオリジン)。その状態で中継は届いている。

Windows 側は実機が無く (#197 と同じ制約)、wry と Microsoft の
ドキュメント・バインディングの読みに留まる。**ただし決定2 の設計は
「iframe からの直接送信が届くかどうか」に依存しない** — 届いても届かなく
ても中継経路だけを使うので、この未検証部分が設計の正しさを左右しない。

### 見送ったもの・既知の制約

(1) **`pinned` は依然として取れない。** ピン留め機能そのものが未実装で、
これは検知手段の問題ではなく機能の不在である (D119 決定4)。本項は
`form-input` のみを扱った。
(2) **「入力あり」の定義はまだ決めていない。** 実測では
`document.addEventListener('input', …, true)` が主フレーム・サブフレーム
とも発火することだけを確認した。`contenteditable` や、値を戻して実質空に
した場合の扱いは実装時に決める。
(3) **中継はメインフレームのスクリプトに依存する。** メインフレームの
スクリプトが (ページの細工などで) 動かなければ、サブフレームの信号も
届かない。これは D18/D23/D78 が既に負っているのと同じ依存で、新しく
増える種類のものではない。

**Revisit condition**: (1) 実機の Windows が使えるようになったら、iframe
からの直接送信が本当に消えるかを確かめる (消えないなら決定2 の「常に中継」
は保険として残るだけで、害は無い)。(2) wry が `ICoreWebView2Frame` の
WebMessage を扱うようになったら、決定2 の前提が変わる。(3) 決定3 の計測が
「保護しすぎて休止が減らない」と出たら、フォーム入力を `protected` と同格に
するのをやめ、優先度スコアの 1 入力に格下げする。

## D143: 入力中のフォームを持つタブは休止しない (Issue #272) — フラグは `Tab` に置き、政策の切り替えは信号の出口ではなく入口で行う

**対象**: Issue #272。D142 が「取得手段はある」と結論づけた調査の実装側。
D105 が積み残しとして記録していた「休止すると入力中の内容が失われる」を
塞ぐ。

### 決定1: フラグは `browser::tab::Tab` が持ち、ナビゲーションだけがクリアする

`has_form_input: bool` を `Tab` に足した。`Tabs` でも `app.rs` 側の
`HashMap` でもなく `Tab` にしたのは、これが**そのタブが表示している
ドキュメントの性質**だからである。ドキュメントが入れ替われば無効になる
という寿命が `title`/`favicon` と完全に同じで、クリアする場所も同じ
(`Tab::on_navigation_started`) で済む。

**ドキュメントが生きている間は一方向にしか動かない。** 利用者が入力して
から空にしたフィールドも「ここには入力があった」と数え続ける。間違えた
ときの代償が非対称だからである — 入力中のタブを捨てれば書いたものが
消える (D105) 一方、生かしすぎて払うのはメモリで、その量は D114/§35 が
既に測っている。D142 決定3 がクリア方針を計測待ちとしたので、**その保守的
な端**を採った。

### 決定2: `Candidate::has_form_input` を `protected` に畳まない。政策の切り替えは `app.rs` に置く

`protected` (音声再生) と同じ「休止しない」効果を持つが、別のフィールドに
した。両者は違うことを言っている — `protected` は「呼び出し側が決めた」、
こちらは「ページが報告した」である。

**そのうえで、保護の on/off は `suspension::plan` ではなく
`app.rs` の `FormInputDetected` ハンドラに置いた。** 切るときは
「フラグを立てない」だけで、`browser::` 側は政策を知らないままでいられる
(D20)。副産物として**計測の 2 腕でスクリプトの挙動が完全に同じになる**:
どちらの腕でも注入され、どちらでも `input` を拾い、どちらでも IPC を
1 本流す。違うのは受け側がフラグを立てるかどうかだけなので、腕の差に
スクリプトのコストが混ざらない。

### 決定3: 既定は「保護する」。`VELOX_PROTECT_FORM_INPUT=0` で切る

未計測なのに既定を決めるのは D46 に反するように見えるが、ここで決めて
いるのは**効果の大きさではなく、間違える向き**である。保護しすぎたときに
失うのはメモリで、しなさすぎたときに失うのは利用者が書いた文章である。

認識できない綴りは既定 (保護する) を保つ — `VELOX_SUSPEND_MECHANISM` /
`VELOX_BACKGROUND_MEMORY_TARGET` と同じ規則。**打ち間違いが、入力を失う
側の腕を黙って選んではいけない。**

### 実測で分かったこと: WebKitGTK では `file://` ページに IPC が届かない

統合テストが最初に落ちた。原因を追ったところ、**`file://` のページには
wry の `window.ipc` shim がそもそも入らない**ことが分かった。

**器具を疑ってから結論した (D140)。** 使い捨ての wry プログラム 1 本で、
同じページを `file://` とカスタムプロトコルの 2 通りで読み込み、
`with_on_page_load_handler` でページが実際に読めていることを確認した
うえで比べた:

| スキーム | ページ読み込み | IPC |
| --- | --- | --- |
| `file://` | 成功 | **何も届かない** |
| `probe://` (カスタムプロトコル) | 成功 | 届く |

これは #272 が持ち込んだ問題ではない。**D18 の DevTools ショートカット・
D23 のタブ操作ショートカット・D78 のコンテキストメニューも、Linux では
ローカルファイルを開いている間すべて効いていない。** 影響範囲が本 Issue を
超えるので #275 に切り出した (Windows/WebView2 で同じかは未確認)。

本 PR での対処は統合テストに閉じている。フィクスチャを `file://` から
ループバックの HTTP サーバ 2 台に替えた — ポートが違うので iframe は
**本当にクロスオリジン**になり、D142 決定2 の中継経路をそのまま踏む。
...
| わざと壊した箇所 | 落ちるもの |
| --- | --- |
| `Candidate::eligible` から `has_form_input` を外す | 単体 2 件 |
| スクリプトを「まず `window.ipc`、駄目なら中継」に戻す | 形状テスト 1 件 |
| 中継の受け口 (`message` リスナ) を外す | 統合テスト 1 件 |

2 番目が要点である。**これは Windows でしか起きず、しかも無言で壊れる**
(D142 決定2) ので、挙動ではなくスクリプトの**形**を検査している —
`window.ipc` の呼び出しが 1 箇所だけで、それが `isMain` 分岐の内側に
あり、中継がその else 側にあること。Linux で動かすテストでは原理的に
捕まえられない種類の退行を、静的な検査で押さえている。

### レビューで見つかった抜け: 中継は 1 段しか登れていなかった

初版は中継の受け口 (`message` リスナ) を `isMain` の内側に置いていた。
トップ直下の iframe なら `window.parent` がトップそのものなので届くが、
**iframe の中の iframe** では孫の中継が中間フレームに届き、そこには
リスナが無いので**そこで黙って消えていた** — 本項が塞ごうとしている
「休止で入力が消える」を、ネストしたケースだけ再発させていたことになる。
決済ウィジェットが別の埋め込みの中にある形は実際にある。

**直し方は「リスナを全フレームで登録する」だけで済んだ。** `report` は
既に「自分がどのフレームか」で振る舞いを分けているので、中間フレームは
受け取った token でそのまま `report` を呼び、自分の親へ中継する。
深さに制限が無くなる。

再発防止はスクリプトの**形**で押さえた — 残ってよい `if (isMain)` は
`report` の中の 1 つだけなので、その出現数を 1 に固定している。統合
テストも 3 オリジン・2 段ネストのものを足した (修正前に落ちることを
確認済み)。

### 見送ったもの・既知の制約

(1) **効果は測っていない。** 何割のタブが保護対象になるか、それが
D114/§35 のメモリ効果をどれだけ削るかは #272 に残っている。
`VELOX_PROTECT_FORM_INPUT` はそのためにある。
(2) **`pinned` は相変わらず無い。** ピン留め機能そのものが未実装
(D119 決定4 / D142 見送り (1))。
(3) **中継はメインフレームのスクリプトに依存する。** そこが動かなければ
サブフレームの信号も届かない。D18/D23/D78 が既に負っているのと同じ依存。
(4) **Windows 実機での確認は未了** (#197 と同じ制約)。形状テストは
「Windows で壊れる書き方になっていないこと」を保証するが、Windows で
実際に動くことの保証ではない。

**Revisit condition**: (1) #272 の計測が「保護しすぎ」と出たら、
`protected` と同格をやめて優先度スコアの 1 入力へ格下げする
(D142 Revisit (3))。(2) #275 が解けて `file://` でも IPC が届くように
なったら、統合テストのループバックサーバは `file://` フィクスチャに
戻してよい。(3) ピン留めが入ったら、同じ `Candidate` の隣に `pinned` が
並ぶ。


## D144: タブのピン留めを実装し、休止から保護する (Issue #277) — `pinned` は `has_form_input` と違い、ページではなくトラステッドな UI が出す信号なので絶対保護に入れる

**対象**: Issue #277。D119 Revisit (2)・D135 決定1・D143 Revisit (3) が
積み残していた「`pinned` は相変わらず無い」を塞ぐ。D143 (`has_form_input`)
の実装パターンをそのまま踏襲する — `Tab` にフラグを足し、`Candidate` に
別フィールドとして追加し、`eligible()` の入口で弾く。

### 決定1: `pinned` は `Tab` の属性であり、ナビゲーションでクリアしない

`has_form_input` はドキュメントの性質 (ナビゲーションで無効になる) だが、
`pinned` は**タブそのものの性質**である。ページが入れ替わってもピン留めは
外れない — ブックマークバーのタブや、常時開いておきたいタブ (メール・
チャットなど) を想定した機能なので、ここが外れたら機能として成立しない。
`Tab::on_navigation_started`/`on_load_finished` はどちらも触らず、
`Tabs::toggle_pinned` (トグルボタン経由) だけが変更する。

### 決定2: `Candidate::pinned` は `protected`/`has_form_input` とは別のフィールドにし、`eligible()` で絶対保護にする

D135 決定1 は「保護 (絶対) に入れてよいのは VeloX 自身が観測したものだけ、
ページの申告は決して保護に入れない」と定めた。`has_form_input` (ページが
`window.ipc` 越しに自己申告する、信用できない入力) はこの規律の対象で
あり、D143 が例外的に `eligible()` へ入れたのは、D142 決定1 (無音の音声
再生で今日すでに得られる権能なので、新しい濫用は増えない) と D143 決定3
(間違える向きを「入力を失う」側にしない) という 2 つの議論を経たうえでの
判断で、D142 Revisit (3) がその格下げ条件を残している。

`pinned` はこの規律に触れない。**申告するのはページではなく、利用者が
トラステッドなツールバー UI (自前 HTML、D11) のピン留めボタンを押した
ことそのもの**である。コンテンツの IPC を経由しないので、
`window.rs` が守ってきた「ページ由来の値をそのまま振る舞いの決定に
使わない」規律とは無関係の話であり、D119 決定4/Revisit (2) が
「唯一、推測が要らない入力」と呼んだ理由もここにある。したがって
`protected` と同格の絶対保護として `eligible()` に加える
(`!self.active && !self.loading && !self.protected && !self.has_form_input && !self.pinned`)。

`has_form_input` に畳まなかったのと同じ理由で `protected` にも畳まない —
「誰が保護を決めたか」を後から追えるようにするため、フィールドとしては
別に保つ (D143 決定2 と同じ判断)。

### 決定3: タブ順序は「ピン留めタブは常に先頭に連続する」という不変条件を持たせる

`Tabs` に新しい不変条件を追加した: ピン留めタブは表示順で必ず先頭に
連続する (`self.tabs[..k]` がちょうどピン留めタブ、`k` はその数)。

これを成立させる箇所は `toggle_pinned` 1 か所で足りる。`open`/`open_at`/
`reopen_closed` は常に末尾に新しい (未ピンの) タブを追加するだけなので、
不変条件が既に成り立っていれば新しいタブがピン留めタブより前に来ることは
構造的にありえない — 個別に対処するコードは要らない。

`toggle_pinned` の移動先は次の 1 行で決まる: 対象タブを配列から取り除いた
「後」の残りのタブのうち先頭からピン留めタブが何個あるか。これは
ピン留めする場合の「最後のピン留めタブの直後」にも、ピン留め解除する
場合の「最初の未ピンの位置」にも一致する — 呼び出し前に不変条件が
保たれていれば、対象タブを抜いた時点で残りのピン留めタブは全部まだ
先頭に固まっているので、「残りのピン留め数」がどちら向きでも同じ境界に
なる。ドラッグ&ドロップによる並べ替えは対象外 (Issue の要求にも無い)。

### 決定4: ツールバー UI ではピン留めタブを縮小表示にし、閉じるボタンを外す

タブ 1 個の横幅は限られているので (D21)、ピン留めタブはファビコン
(無ければ頭文字のフォールバック) とピン留めボタンだけの縮小表示にした。
閉じるボタンは出さない — ピン留めの意図が「誤操作で閉じたくない」こと
そのものなので、外すのが素直。**Ctrl/Cmd+W やコンテンツ側のショートカット
経由の close は塞がない** — ストリップの閉じるボタンだけを引っ込める
UI 上の配慮であり、`Tabs::close` 自体はピン留めを見ない。キーボードで
明示的に閉じる操作まで防ぐのはやり過ぎで、意図しない全タブロックに
繋がりかねない。

新しいツールバーコマンドは既存の `SuspendTab`/`CloseTab` と同じ命名・IPC
形状 (`{"cmd":"toggle_pin_tab","id":N}`) に揃えた `ToggleTab` を足した:
`TogglePinTab { id }`。`app.rs` 側は `Tabs::toggle_pinned` を呼んで
`Some` なら `sync_tab_strip` で再描画するだけで、`SuspendTab` の
ハンドラと同じ形をしている。

### 決定5: セッション永続化は `has_form_input` と対称的に「保存する・復元する」

`has_form_input` はエンジン側の一時状態なので永続化しない
(`SavedTab` に無い) が、`pinned` は利用者の意思そのものなので次回起動でも
保つ必要がある。`SavedTab::pinned: bool` を `#[serde(default)]` 付きで
追加した — D141 が `windows` で通した「フィールドが無い旧スキーマの
...
| わざと壊した箇所 | 落ちるもの |
| --- | --- |
| `Candidate::eligible` から `!self.pinned` を外す | 単体 2 件
(`a_pinned_tab_survives_every_signal`,
`pinning_pins_its_process_group_like_a_protected_tab`) |

`Tabs` 側にはピン留めタブが常に先頭に連続する不変条件の単体テストを
足した (`pinning_moves_a_tab_to_just_after_the_last_pinned_tab`,
`unpinning_moves_a_tab_to_the_first_unpinned_position`,
`a_newly_opened_tab_never_lands_before_a_pinned_tab`,
`reopen_closed_never_lands_before_a_pinned_tab`) ほか、アクティブタブが
移動に巻き込まれないこと (`toggle_pinned_never_disturbs_which_tab_is_active`)、
未知の id が `None` を返すことも確認した。`session.rs` には
...
(1) **ドラッグ&ドロップによる並べ替えは実装していない。** タブ順序の
入れ替えはピン留め操作のみが行う。
(2) **優先度の梯子 (D119/D135) はまだ実装しない。** `pinned` は今回
「絶対保護」としてのみ効き、D119 決定4 が構想していた「優先度スコアの
最優先入力」としての使い方はまだ無い — 梯子自体が D135 決定3 のとおり
未着手なので、繋ぎ込み先が無い。
(3) **Windows 実機での確認は未了** (#197/D143 決定4 と同じ制約)。

**Revisit condition**: (1) 優先度スコア (D119/D135) を実装するときは、
D119 決定4/Revisit (2) のとおり `pinned` を最優先の入力として組み込む
...
## D148: 休止スイープの時系列は Job Summary に出す (Issue #176 Stage 3 / D147 Revisit condition (2)) — 生ログを読む最初の形は「判定ごとの要求タブ数 vs 実際の休止数」で、予算は RAM から Rust と同じ式で再現する

**対象**: D147 が「最初の実測を見てから決める」と見送っていた、生ログ
(`results/perf-logs/*.jsonl`) を読む解析の形。D147 のマージ直後に
決めることにした理由と、その形を記録する。

### なぜ「最初の実測を見てから」を待たなかったか

D147 (3) は、先に解析を作ると §46.5 の仮説 2 に都合のよい切り方だけを
用意しかねない (D46) として見送った。しかし D147 を検証した run
(35555473203) で、**artifact の zip をこの作業環境から取得できない**
ことが分かった — GitHub の artifact は Azure Blob Storage から配信され、
Claude Code on the web のネットワークポリシーはそのホストへの接続を
拒否する。つまり生ログを残しても、**run ページで読める形にしなければ
次の一手 (D145 Revisit (1)) に進めない。**

そこで「何を読むか」を最小限に絞って先に固定し、Job Summary に出す。
絞り方の基準は、§46.5 の**両方の仮説を同じ表で区別できること**
(片方に都合のよい切り方にならないこと) とした。

### 決定1: 表の 1 行は「メモリ判定 1 回」

`tab_suspend` は `app::sweep_tabs` が 1 タブ休止するたびに 1 行書くので、
1 回の判定で 4 タブ休止すれば `ts_ms` が数 ms しか違わない 4 行になる。
`.github/scripts/perf_log_timeline.py` はこれを**スイープ**にまとめる
(間隔 200 ms 以内の連続した `tab_suspend` を 1 行に。判定周期は既定
5 秒、Issue #197 の詰めた周期でも 500 ms なので、その半分より十分小さい)。
D145 の `tab_resuspend_delay_ms` が「4 タブずつ」を分布の形でしか
出せなかったのは、この単位を持っていなかったからである。

各行には、スイープ直前の `rss` サンプル (`total_rss_bytes`、Windows の
判定が実際に見る量 — `app.rs` の `total_pss_bytes.unwrap_or(total_rss_bytes)`)
と、その古さ (perf の RSS サンプラとメモリ判定のサンプラは別物なので、
「直前」は近似であることを表に出す)、直後の `rss` を並べる。

### 決定2: 「要求タブ数」を Rust と同じ式で出し、実際の休止数と並べる

§46.5 の 2 つの仮説は、この 2 列の関係で分かれる:

| 仮説 | 表に現れる形 |
| --- | --- |
| 2. 見込み解放量 (64 MiB) が過大で、要求が小さく出て収束に回数が要る | **要求 = 実際**が続き、超過量が毎回小さくしか減らない |
| 1. 疑似プロセスグループの丸ごと回収 (§46.5 で B 腕により否定済み) | **実際 > 要求** (⚠️ 印) |

要求タブ数は `suspension::tabs_to_free` (`ceil(超過 / 見込み解放量)`、
超過があれば必ず 1 以上、見込み 0 なら 0) の複製で、予算は perf ログに
無いので `memory_budget_for_ram` (`clamp(RAM / 16, 700 MiB, 2048 MiB)`、
D114) を複製して `env_info` ステップの RAM から求める (丸めた `ram_gib`
ではなく `TotalVisibleMemorySize` の生の値を `ram_bytes` として出力に足した
— 1 MiB 単位で合わせるため)。**複製である以上ずれうる**ので、
`test_perf_log_timeline.py` が §46 の実測 (15.99 GiB → 1023 MiB) と
`tabs_to_free` の Rust 側テストが固定する性質をそのまま固定する。Rust 側
を変えるときはここも変える。

### 決定3: 置き場は `.github/scripts/`、`velox-bench` のサブコマンドにはしない

D147 Revisit (2) は両方を候補に挙げていた。Python にしたのは、(a) 読み手が
Job Summary であり、Windows ランナーでは `python` が既に固定ページの配信に
使われている、(b) `velox-bench` に入れると Rust 側の定数を直接参照できて
複製が消える利点はあるが、集計の本体 (`benchmark.rs`) に「表示のための
時系列整形」を持ち込むことになり、`velox-bench` は「集計の IO 層」という
役割 (D44) からはみ出す、(c) 何を読むかがまだ動く段階で、Windows ジョブの
ビルドを伴わずに直せる、の 3 点による。読む形が安定して Rust の定数との
複製が負担になったら、そのとき `velox-bench` 側へ移す (Revisit (1))。

ステップは `continue-on-error` で、`--only-with-suspends` により休止が
起きないシナリオでは表が膨らまない。`results/perf-logs` が無い run
(bench が 1 度も走らなかった) では何もせず終わる。

### 見送ったもの・既知の制約

(1) この時点では**まだ 1 本も実測を読んでいない**。表の形が §46.5 に
答えられるかは、次の `tabs_hold_bounce_50` の run で分かる。
(2) 判定が見たメモリ量そのものは記録していない。`sweep_tabs` に
「判定時の `total_bytes` と要求タブ数」を perf レコードとして書かせれば
近似が消えるが、それは perf ログのスキーマ変更 (D42 の追記規則) を伴う
ので、近似で足りるかを先に見る。
(3) Linux の perf-gate では判定が PSS を見るため、この表の超過量は過大に
出る。perf-windows にしか組み込んでいない (D147 (1) と同じ Windows 優先)。

### 最初の実測 (2026-09-21、run 35562270535、§47)

この表を `tabs_hold_bounce_50` (A = 既定 / B = `VELOX_MAX_TABS_PER_PROCESS=1`)
で初めて読んだ。決定2 の表が予告した 2 つの形は**両方とも出て、しかも
腕で分かれた**: A は要求 1〜2 に対して毎回ちょうど 4 (疑似プロセス
グループの上限。§46.5 が否定した仮説 1 は A の「4」の説明としては正し
かった)、B は要求 3 に対して 3 (`tabs_to_free` は設計どおり)。1 タブ
あたりの返りは 14〜26 MiB で見込み 64 MiB の 25〜40% (仮説 2 の前提は
成立)。

しかし本題は別にあった。**スイープで返した分が次の判定 (5 秒後) までに
戻る** — B は 1 サイクルで約 41 MiB 返して約 41 MiB 戻る純増減ゼロの
定常状態、A は予算のまわりを ±50 MiB で往復する。「5 秒に 4 タブ」は
収束の途中ではなく定常状態であり、見込み解放量を直しても止まらない。
`ESTIMATED_BYTES_PER_TAB` の OS 別化 (Revisit (3)) は**戻りの正体が
分かるまで行わない** (D46)。
...
どのプロセスがなぜ膨らむかは総量では決められず、perf の RSS サンプル
間隔を 1 秒に詰めて増え方の形を読んだ (§47.6)。戻りは定常的な漏れでは
なく、復帰ラウンドの 5〜20 秒後に集中する山だった。さらに予算 OFF の腕
...
解析を `velox-bench` のサブコマンドに移して定数の複製を消す。(2) 次の
...
値を返し、追加のカーネル往復も無い。`trials=4` で取り直した run
35614426718 (別機種、Xeon 8573C) も B の 8 試行が ±3 MiB で同じ形。
Revisit (1) の後半 (判定の入力を私的コミットに替える) に進んだ (D151)。
...
## D152: Windows の予算の判定は私的コミットを既定にする (Issue #176 Stage 3 / §47.10) — `VELOX_MEMORY_BUDGET_INPUT` の既定を `private` に替え、`resident` はオプトアウトとして残す

**対象**: D151 のノブで回した A/B (§47.10、run 35618824317、
`tabs_hold_bounce_50`、切り替えなしの 60 秒保持、各 8 試行)。D151
Revisit (1) は「B の休止が初回のスイープで収束し、その後の揺り戻しが
消えるなら既定を替える」としていた。

### 実測 (§47.10)

| | A (`resident`) | B (`private`) |
| --- | --- | --- |
| mark 後のスイープ | 4 回 (+3.3 / +13.3 / +18.3 / +23.3)、+48 にもう 1 回 | 0 回 (6 試行)、+3.3 に 1 回 (2 試行) |
| 休止の合計 | 28 タブ | 20 タブ (6 試行) / 24 タブ (2 試行) |
| mark 後のプロセス数 | 45 → 29 | 37 のまま |
| 私的コミット +0 → +60 | 1222 → 775 MiB | 1024 → 971 MiB (予算 1024 の下で横ばい) |
| ワーキングセット +0 → +60 | 1082 → 1013 MiB (±50 で往復) | 964 → 1225 MiB (2 段の階段、休止なし) |

B はタブを開いている最中の 4 スイープで予算の下に入り、以降 60 秒間
判定は何も命じない。ワーキングセットの 2 段の階段 (§47.8 / §47.9) は
そのまま出るが、私的コミットは −3 / +5 しか動かないので休止に変換され
ない。A は mark 後も 4〜5 回休止し続ける (§46 以来の「5 秒に 4 タブ」)。
条件は満たされた。

### 決定1: 既定を `PrivateCommit` に替える

`MemoryBudgetInput::default()` と `Config::default().memory_budget_input`
を `PrivateCommit` にする。`resident` は `VELOX_MEMORY_BUDGET_INPUT=
resident` で選べるオプトアウトとして残す (`BackgroundMemoryTarget::Normal`
と同じ位置づけ、D123)。Linux では両方 PSS なので挙動は変わらない —
perf-gate (Linux) の数値にも影響しない。

`perf_log_timeline.py --budget-input` と perf-windows の `Get-BudgetInput`
の既定も `private` に揃える。**D152 より前の run (§46〜§47.10 の A 腕) を
読み直すときは `--budget-input resident` を渡す** — 当時の判定はワーキング
セットを見ていたので、既定のまま読むと「超過 0 / 要求 0 ⚠️」の行が並ぶ
(テスト `test_markdown_shows_demand_next_to_actual` が §46 の run 2 をその
形で読んでいる)。

### 決定2: 予算の式と見込み解放量は据え置く

`memory_budget_for_ram` (RAM / 16、D114) と `ESTIMATED_BYTES_PER_TAB`
(64 MiB、D54) は変えない。B は同じ予算 1024 MiB で 50 タブを 20 休止で
収めており、私的コミット基準で予算を引き直す必要は今回の数字からは出て
こない。見込み解放量は実測 (1 タブ約 24 MiB、§47.9) より大きいが、要求
1→4、6→8、4→4、2→4 と「実際 ≥ 要求」のまま収束しており、疑似プロセス
グループ単位の丸め (§47.1) が差を吸収している。D145 Revisit (1) 後半は
このまま閉じない — 次にグループ上限や 1 タブあたりの回収量を触るときに
一緒に見る。

### 見送ったもの・既知の制約

(1) `results/history/windows/` の既存の系列 (D152 より前) は判定の入力が
違う。`session_id` は同じ `perf-windows-schedule-A` のままなので、週次の
ダッシュボードで D152 をまたぐ比較は**同じ系列に見えて条件が違う**。
D96 Revisit (2) の分散計測はこの commit を境に読み分けること (系列を
分ける仕組みは足していない — 週次 1 点なので注釈で足りる)。(2) 記録済み
の値 (§21〜§47.10) はすべて `resident` で測ったもので、書き換えない
(D96 / D111)。(3) 本作業環境では `cargo test` を回せない (D146 (2))。
`cargo clippy --target x86_64-pc-windows-msvc --all-targets` と CI で
担保する。

**Revisit condition**: (1) 次の週次 run (`tabs_hold_50` /
`tabs_hold_resume_50` / `tabs_hold_bounce_50`、既定入力) で
`suspended_tab_count` と `tab_resuspend_count` が §47.10 の B と同じ形
(mark 後の休止ほぼゼロ) にならなければ、上書き入力 (60 秒窓・切り替え
なし) の外では違う挙動をしているということなので、§47.10 の条件を
