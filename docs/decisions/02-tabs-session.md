...
主担当は **D8–D15, D20, D22–D40, D74, D119, D136, D141** の 32 件 (網羅的な一覧は下の「詳細」節。ここは読み進める手がかりとしての要約)。
...
**このカテゴリが主担当の Decision: D8–D15, D20, D22–D40, D74, D119, D136, D141** (32 件)
...
- [D136](./archive.md#d136-プライベートウィンドウの-omnibox-は履歴入力履歴を読まない-issue-157--ブックマークは残すパネルは変えない) — プライベートウィンドウの Omnibox は履歴・入力履歴を読まない (Issue #157) — ブックマークは残す。パネルは変えない
- [D141](./archive.md#d141-セッションはウィンドウの配列で保存する-issue-149--旧スキーマは-sanitize-で-1-ウィンドウに畳んで受ける2-枚目以降はイベントループの中で開く) — セッションはウィンドウの配列で保存する (Issue #149) — 旧スキーマは `sanitize` で 1 ウィンドウに畳んで受ける。2 枚目以降はイベントループの中で開く
