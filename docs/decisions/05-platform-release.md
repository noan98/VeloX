...
主担当は **D50–D55, D68, D70, D73, D83, D91, D98, D100, D103, D107–D108, D126, D128, D131, D133–D134, D153** の 22 件 (網羅的な一覧は下の「詳細」節。ここは読み進める手がかりとしての要約)。
...
**このカテゴリが主担当の Decision: D50–D55, D68, D70, D73, D83, D91, D98, D100, D103, D107–D108, D126, D128, D131, D133–D134, D153** (22 件)
...
- [D128](./archive.md#d128-自動クローズはコミットメッセージ経由でも起きる-issue-253--バッククォートは効かず閉じているのは-github-本体であるd83-の前提は失効した) — 自動クローズはコミットメッセージ経由でも起きる (Issue #253) — バッククォートは効かず、閉じているのは GitHub 本体である。D83 の前提は失効した
- [D131](./archive.md#d131-誤クローズは避けるのではなく検知する-issue-253--そして検知器を-pr-本文用の除去規則で作ると事故そのものを見落とす) — 誤クローズは避けるのではなく検知する (Issue #253) — そして検知器を PR 本文用の除去規則で作ると、事故そのものを見落とす
- [D133](./archive.md#d133-codex-が沈黙しても永久ブロックしない-issue-207--緩和の根拠が特定の文言だけだと文言が来なければ待ち続ける) — Codex が沈黙しても永久ブロックしない (Issue #207) — 緩和の根拠が「特定の文言」だけだと、文言が来なければ待ち続ける
- [D134](./archive.md#d134-最優先-os-のコードが-lint-の網から外れていた-issue-158--cfgwindows-は-linux-の-clippy-から見えない) — 最優先 OS のコードが lint の網から外れていた (Issue #158) — `#[cfg(windows)]` は Linux の clippy から見えない
