...
- [D62](./archive.md#d62-セキュリティ入力値堅牢性-35--スキーム許可リストは維持ipc-にサイズ上限js-埋め込みに追加エスケープブックマークの壊れた-folder_id-をロード時に自己修復) — セキュリティ・入力値堅牢性 (#35) — スキーム許可リストは維持、IPC にサイズ上限、JS 埋め込みに追加エスケープ、ブックマークの壊れた `folder_id` をロード時に自己修復
