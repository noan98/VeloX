...
def _windows_nodes() -> dict[int, tuple[int, _ProcMemory | None]]:
    ...
    def private_working_set_bytes(handle) -> int | None:
        """`QueryWorkingSet` の各ブロックの `Shared` ビットを見て私有ページを数える。

        バッファ長は事前に分からないので、`ERROR_BAD_LENGTH` で返ってきた
        `NumberOfEntries` を見て採り直す。**採り直しの間にもページは増減する**ので
        余裕を持たせ、それでも足りなければ諦めて `None` を返す (過小評価した値を
        黙って返すより、取れなかったと言う方がよい)。
        """
        entries = 4096
        for _ in range(4):
            ...
            if psapi.QueryWorkingSet(handle, ctypes.byref(buf), size):
                ...
                for i in range(1, actual + 1):
                    # PSAPI_WORKING_SET_BLOCK: Protection:5, ShareCount:3,
                    # Shared:1, ... 共有ページは Shared ビットが立つ。
                    if not ((int(buf[i]) >> 8) & 1):
                        private_pages += 1
